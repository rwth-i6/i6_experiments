...
class ComputeTSEJob(Job):
    ...
    def _compute_word_boundaries(
        ...
        seq_allophones = [alignments.allophones[item[1]] for item in align_seq]
    ...
    def run(self) -> None:
        ...
        for idx, seq_tag in enumerate(file_list, start=1):
            word_starts, word_ends = self._compute_word_boundaries(
                alignments, seq_tag, self.silence_phone, self.upsample_factor
            )
            ...
            ref_word_starts, ref_word_ends = self._compute_word_boundaries(
                ref_alignments, ref_seq_tag, self.ref_silence_phone, self.ref_upsample_factor
            )
        ...
        self.out_word_start_frame_differences = {
            key: start_differences[key] for key in sorted(start_differences.keys())
        }
        self.out_word_end_frame_differences = {key: end_differences[key] for key in sorted(end_differences.keys())}
        self.out_boundary_frame_differences = {key: differences[key] for key in sorted(differences.keys())}
        self.out_tse_frames = statistics.mean(abs(diff) for diff in differences)
