...
from i6_core import corpus, recognition
from sisyphus import tk

from i6_experiments.users.berger import helpers as helpers
from i6_experiments.users.berger.helpers.scorer import ScoreJobType, ScorerInfo
...
from .types import ConfigType, TrainJobType
...
class BaseSystem(ABC, Generic[TrainJobType, ConfigType]):
    def __init__(
        ...
        self._data_inputs: Dict[str, dataclasses.RasrDataInput] = {}
    ...
    def get_exp_names(self) -> List[str]:
        return list(self._returnn_configs.keys())

    def get_recog_exp_names(self) -> Dict[str, List[str]]:
        return {key: list(returnn_config.recog_configs.keys()) for key, returnn_config in self._returnn_configs.items()}
    ...
    def init_corpora(
        ...
        self._data_inputs.update(corpus_data)

    @staticmethod
    def _get_scorer(
        corpus_file: tk.Path,
        stm_kwargs: Dict,
        scorer_type: ScoreJobType,
        score_kwargs: Dict,
        ...
        return ScorerInfo(ref_file=stm_path, job_type=scorer_type, score_kwargs=score_kwargs_mod)

    def setup_scoring(
        self,
        scoring_corpora: Optional[Dict[str, tk.Path]] = None,
        scorer_type: ScoreJobType = recognition.ScliteJob,
        ...
        for key, corpus_file in scoring_corpora.items():
            ...
            scorer = self._get_scorer(
                ...
                stm_path=self._data_inputs[key].corpus_object.stm,
                glm_path=self._data_inputs[key].corpus_object.glm,
            )
            self._data_inputs[key].scorer = scorer
        ...
        **kwargs,
        ...
        for exp_name in exp_names:
            ...
            for c_key in corpora:
                named_corpus = dataclasses.NamedRasrDataInput(c_key, self._data_inputs[c_key])
                for recog_exp_name in recog_exp_names.get(exp_name, returnn_configs.recog_configs.keys()):
                    assert recog_exp_name in returnn_configs.recog_configs
                    recog_config = returnn_configs.recog_configs[recog_exp_name]
    ...
    def run_dev_recog_step(self, **kwargs) -> None:
        self.run_recog_step_for_corpora(corpora=self._dev_corpora, **kwargs)

    def run_test_recog_step(self, **kwargs) -> None:
        self.run_recog_step_for_corpora(corpora=self._test_corpora, **kwargs)
        ...
        **kwargs,
        ...
        for exp_name in exp_names:
            ...
            if align_descriptor is not None:
                align_exp_name = f"{exp_name}_{align_descriptor}"
                alias_prefix = f"align_{align_descriptor}"
            else:
                align_exp_name = exp_name
                alias_prefix = "align"
            exp_results = {}
            for c_key in corpora:
                named_corpus = dataclasses.NamedRasrDataInput(c_key, self._data_inputs[c_key])
            ...
            results[align_exp_name] = exp_results
        return results
