...
def forward_step(*, model: torch.nn.Module, extern_data: TensorDict, **kwargs):
    ...
    audio_features_prim_len = (
        extern_data["features_primary"].dims[1].dyn_size_ext.raw_tensor
    )
    audio_features_sec = extern_data["features_secondary"].raw_tensor
    audio_features_sec_len = (
        extern_data["features_secondary"].dims[1].dyn_size_ext.raw_tensor
    )
    assert audio_features_prim_len is not None

    print("Audio features_prim shape", audio_features_prim.shape)
    print("Audio features_sec shape", audio_features_sec.shape)
    print("Audio features_prim len", audio_features_prim_len)
    print("Audio features_sec len", audio_features_sec_len)
