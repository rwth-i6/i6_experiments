...
def train_step(*, model: torch.nn.Module, extern_data: TensorDict, **kwargs):
    ...
    classes_masked, _ = torch.nn.utils.rnn.pad_packed_sequence(
        classes_packed, batch_first=True, padding_value=-100
    )

    loss = torch.nn.functional.cross_entropy(
        input=log_probs, target=classes_masked, ignore_index=-100, reduction="sum"
    )
