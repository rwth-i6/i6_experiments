...
class SpecaugmentModuleV1(torch.nn.Module):
    def __init__(self, step: int, config: SpecaugmentConfigV1):
        super().__init__()
        self.max_time_num = config.max_time_mask_num
        self.max_feature_num = config.max_feature_mask_size

        self.max_time = config.max_time_mask_size
        self.max_feature = config.max_feature_mask_size

        if config.increase_steps is None:
            self.warmup_factor = 1
        else:
            self.warmup_factor = 1 + len(
                [inc_step for inc_step in config.increase_steps if inc_step > step]
            )
    ...
    def forward(self, audio_features: torch.Tensor):
        ...
        for _ in range(random_time_num):
            x = torchaudio.functional.mask_along_axis(
                x, mask_param=self.max_time, mask_value=0.0, axis=1
            )

        max_feature_num = self.max_feature_num // self.warmup_factor
        random_feature_num = int(
            torch.randint(low=0, high=max_feature_num, size=(1,)).item()
        )

        for _ in range(random_feature_num):
            x = torchaudio.functional.mask_along_axis(
                x, mask_param=self.max_feature, mask_value=0.0, axis=2
            )
