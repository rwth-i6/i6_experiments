...
from i6_core.features import GammatoneJob, basic_cache_flow
...
from i6_experiments.users.berger.args.jobs.rasr_init_args import get_feature_extraction_args_16kHz
...
from i6_experiments.users.berger.helpers import ArpaLMData
from i6_experiments.users.berger.network.helpers import smbr
...
from i6_experiments.users.berger.systems.functors.rasr_base import LatticeProcessingType, dataclasses
...
def returnn_config_generator(
    ...
    **kwargs,
    ...
    if variant == ConfigVariant.TRAIN or variant == ConfigVariant.PRIOR:
        ...
        net_dict["output"]["loss_scale"] = 0.1

        lm_data = ArpaLMData(
            scale=10.0,
            filename=tk.Path(
                "/work/asr3/irie/data/librispeech/lm/seq_train/960hr/kn2.no_pruning.gz", hash_overwrite="ls_2gram"
            ),
        )
        ...
        acoustic_mixture_path = mm.CreateDummyMixturesJob(num_outputs, 1).out_mixtures  # type: ignore

        feature_scorer = rasr.PrecomputedHybridFeatureScorer(
            prior_mixtures=acoustic_mixture_path,
            priori_scale=0.3,
            prior_file=tk.Path(
                "/work/asr4/vieting/setups/converse/work_libricss/i6_core/returnn/extract_prior"
                "/ReturnnComputePriorJobV2.CjEjmdbJ9yT5/output/prior.xml"
            ),
        )

        gt_job = GammatoneJob(crp=train_corpus.crp, **get_feature_extraction_args_16kHz()["gt"])
        feature_path = rasr.FlagDependentFlowAttribute(
            "cache_mode",
            {
                "bundle": gt_job.out_feature_bundle["gt"],
                "task_dependent": gt_job.out_feature_path["gt"],
            },
        )
        feature_flow = basic_cache_flow(cache_files=feature_path)
        feature_flow.flags["cache_mode"] = "bundle"

        lattice_flow = copy.deepcopy(feature_flow)
        lattice_flow.nodes["cache"]["path"].value.alternatives = {
            "bundle": gt_job.out_feature_bundle["gt"],
            "task_dependent": gt_job.out_feature_path["gt"],
        }
        lattice_flow.flags["cache_mode"] = "bundle"

        smbr.add_rasr_smbr_output_layer(
            network=net_dict,
            from_list="output",
            ...
            feature_flow_lattice_generation=...,
            feature_flow_smbr_training=feature_flow,
            loss_corpus_path=train_corpus.crp.corpus_config.file,  # type: ignore
            loss_lexicon_path=train_lexicon,
            lm=lm_data,
            am_args=am_args,
        )
...
def run_exp() -> SummaryReport:
    ...
    train_args = exp_args.get_hybrid_train_step_args(num_epochs=300, gpu_mem_rqmt=24)
    dev_recog_args = [
        exp_args.get_hybrid_recog_step_args(
            ...
            lm_scales=[10.0],
            feature_type=FeatureType.CONCAT_SEC_MIX_GAMMATONE_16K,
