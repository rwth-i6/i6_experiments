...
def py():
    ...
    transform_func = "tf.math.floordiv(tf.where(tf.math.greater(source(0), 135), source(0)-126, source(0)) + 2, 3)"
    ...
    nn_args = hybrid_args.get_nn_args(
        ...
        recog_keys=[dev_key],
        recog_args={
            "lm_scales": [2.8, 3.0, 3.2],
            "prior_scales": [0.2, 0.25, 0.3],
    ...
    system = hybrid_system.HybridSystem()
    system.init_system(
        hybrid_init_args=init_args,
        train_data=nn_data_inputs["train"],
        cv_data=nn_data_inputs["cv"],
        devtrain_data=nn_data_inputs["devtrain"],
