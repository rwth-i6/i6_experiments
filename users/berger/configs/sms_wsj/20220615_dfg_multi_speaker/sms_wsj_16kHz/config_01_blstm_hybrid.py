...
def run_exp(
    ...
    **kwargs,
    ...
    return (
        summary_report,
        sms_wsj_hybrid_system.tf_nn_checkpoints[
            f"{train_key}.train_{train_key}.cv_{name}"
        ][num_subepochs],
    )
...
def py(
    ...
    report, model = run_exp_partial(
        name_suffix="newbob", schedule=LearningRateSchedules.Newbob
    )
    summary_report.merge_report(report)
