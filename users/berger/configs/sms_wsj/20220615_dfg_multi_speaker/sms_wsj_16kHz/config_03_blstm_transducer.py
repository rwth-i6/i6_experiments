...
from ..wsj_8kHz.config_02_blstm_ctc import py as run_ctc
...
def run_exp(alignments: Dict[str, Any], **kwargs) -> SummaryReport:
    ...
    train_blstm_net, train_python_code = make_context_1_blstm_transducer_blank(
        ...
        output_args={
            "label_smoothing": 0.2,
            "focal_loss_factor": kwargs.get("focal_loss", 0.0),
        },
    )
...
def py(alignments: Optional[Dict[str, Any]] = None) -> SummaryReport:
    ...
    for time_num, max_time in [(1, 15), (3, 10), (2, 15), (3, 15)]:
        name_suffix = f"oclr-6e-04_pool-4_specaug-tn-{time_num}_mt-{max_time}"
    ...
    tk.register_report(f"{dir_handle}/{filename_handle}/summary.report", summary_report)
