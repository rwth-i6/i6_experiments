__all__ = ["recipe_imports", "get_model_serializers"]
...
from ...common_deps.serialization import build_config_constructor_serializers_v2
...
def get_model_serializers(model_class: Type[ModuleType], model_config: ModelConfiguration) -> Collection:
    constructor_call, model_imports = build_config_constructor_serializers_v2(
        cfg=model_config,
        variable_name="cfg",
    )

    model_serializers: List[SerializerObject] = [
        Import(
            f"{model_class.__module__}.{model_class.__name__}",
        ),
    ]
    model_serializers.extend(model_imports)
    ...
    return Collection(model_serializers)
