...
def add_initial_conv(
    ...
    linear_size: int = 512,
    conv_outputs: List[int] = [32, 32, 64, 64],
    conv_filters: List[int] = [3, 3, 3, 3],
    conv_strides: List[int] = [1, 1, 1, 1],
    max_pool: List[Optional[Tuple[int, int]]] = [None, (2, 2), None, (2, 1)],
    activations: List[Optional[str]] = [None, "swish", None, "swish"],
    reuse_from_name: Optional[str] = None,
    **kwargs,
) -> str:
    ...
    for idx, (n_out, filter, stride) in enumerate(zip(conv_outputs, conv_filters, conv_strides), start=1):
        ...
        if len(activations) >= idx and activations[idx - 1] is not None:
        ...
        if len(max_pool) >= idx and max_pool[idx - 1] is not None:
