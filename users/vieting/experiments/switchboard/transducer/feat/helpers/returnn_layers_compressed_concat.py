...
class CompressedConcatLayer(_ConcatInputLayer):
    ...
    def __init__(self, sources, **kwargs):
        ...
        def body(b, out):
            enc_padded = tf.pad(
                enc_placeholder[b, : enc_lens[b]],
                [(0, 0), (0, F2)],
                mode="CONSTANT",
                constant_values=0,
            )  # (T, F1+F2)
            dec_padded = tf.pad(
                dec_placeholder[b, : dec_lens[b]],
                [(0, 0), (F1, 0)],
                mode="CONSTANT",
                constant_values=0,
            )  # (U, F1+F2)
            ...
            enc_dec_reshaped = tf.reshape(
                enc_dec_concat, [enc_lens[b] * dec_lens[b], F1 + F2]
            )  # (T*U, F1+F2)
        ...
        out_size = {
            0: tf.stack([tf.tensordot(enc_lens, dec_lens, axes=1), F1 + F2], axis=0)
        }
