...
class AEDLabelScorer(LabelScorerIntf):
    ...
    def score_and_update_state(
        ...
        if prev_state is not None:
            lstm_state_1, lstm_state_2, att_context, accum_att_weights, lm_h, lm_c, ilm_lstm_state_1, ilm_lstm_state_2, ilm_att = tuple([state.squeeze(0) for state in prev_state])
            # pack lstm state
        ...
        zero_ilm_logits, ilm_states = self.model.decoder(
            torch.mean(encoder_outputs, dim=1, keepdim=True), single_labels, audio_features_len, shift_embeddings=prev_state is None, state=ilm_in_state
        )  # [B,1,Vocab] and state of [B, *]
        ...
        states = states[0][0], states[0][1], states[1], states[2], lm_out_state[0], lm_out_state[1], ilm_states[0][0], ilm_states[0][1], ilm_states[2]
