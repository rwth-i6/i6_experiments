...
class I6EnergyCapture():
    ...
    def start(self):
        self.client.connect("137.226.223.2", 1883, 60)
    ...
    def _on_message(self, client, userdata, msg):
        ...
        self.last_power_amd = d["line_in"][0]["voltage"] * d["line_in"][0]["current"]  # in watt = voltage * current
        self.last_power_intel = d["line_in"][1]["voltage"] * d["line_in"][1]["current"]  # in watt = voltage * current
        print(f"current W AMD: {self.last_power_amd} INTEL: {self.last_power_intel}")
        current_time = time.time()
        self.ws_amd += self.last_power_amd * (current_time - self.step_time)  # in watt seconds ( = Joule)
        self.ws_intel += self.last_power_intel * (current_time - self.step_time)  # in watt seconds (= Joule)
    ...
    def finish(self):
        ...
        return total_time, self.ws_amd, self.ws_intel
...
def forward_init_hook(run_ctx, **kwargs):
    ...
    run_ctx.energy_capture = I6EnergyCapture()
...
def forward_finish_hook(run_ctx, **kwargs):
    ...
    if run_ctx.print_rtf:
        print("Total-AM-Time: %.2fs, AM-RTF: %.4f" %
              (run_ctx.total_am_time, run_ctx.total_am_time / run_ctx.running_audio_len_s))
        print("Total-Search-Time: %.2fs, Search-RTF: %.4f" %
        ...
        print("Total-time: %.2f, Total-recog-time: %.2f, Batch-RTF: %.4f" % (time.time() - run_ctx.start_time, total_proc_time, total_proc_time / run_ctx.running_audio_len_s))


    total_time, amd_ws, intel_ws = run_ctx.energy_capture.finish()
    print(f"AMD Energy: {amd_ws} ws")
    print(f"INTEL Energy: {intel_ws} ws")
