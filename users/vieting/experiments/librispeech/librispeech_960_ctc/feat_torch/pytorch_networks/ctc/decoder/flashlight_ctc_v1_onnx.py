...
def forward_init_hook(run_ctx, **kwargs):
    ...
    dummy_data = torch.randn(3, 16000, device="cpu")
    # dummy_data_len, _ = torch.sort(torch.randint(low=1000, high=16000, size=(3,), device="cpu", dtype=torch.int64), descending=True)
    dummy_data_len = torch.ones((3,))*16000
    # scripted_model = torch.jit.trace(model.eval(), example_inputs=(dummy_data, dummy_data_len))
    onnx_program = torch.onnx.dynamo_export(model, dummy_data, dummy_data_len)
