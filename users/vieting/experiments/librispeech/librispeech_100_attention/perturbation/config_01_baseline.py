...
from i6_experiments.users.vieting.experiments.librispeech.librispeech_100_attention.perturbation.\
  net_perturbations import add_center_freq_perturb_const, utterance_level_filterbanks
...
def conformer_tf_features():
  ...
  for level, scale, prob in [
    ("batch", 10.0, 0.5), ("utterance", 10.0, 0.5),  # probability 1.0 does not converge
  ]:
    name_tmp = exp_prefix + "/" + f"raw_log10_lvl{level}_cf_const{scale}({prob})"
    ...
    report_list.append(run_exp_v2(
      ...
      train_args=args_tmp, report_args={"center_freq": f"const{scale}({prob})", "level": level}))
