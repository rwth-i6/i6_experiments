...
@dataclass
class SpecaugConfigV5(ModelConfiguration):
    start_epoch: int
    ...
    mask_sorting_strategy: Optional[str]
...
@dataclass
class SupervisedConvolutionalFeatureExtractionV5Config(FeatureExtractionConfig):
    """
    Attributes:
    ...
    specaug_config: SpecaugConfigV5
    ...
    def from_dict(cls, d):
        d = d.copy()
        d["specaug_config"] = SpecaugConfigV5(**d["specaug_config"])
        return cls(**d)


class SupervisedConvolutionalFeatureExtractionV5(nn.Module):
    """
    Like V2, but this includes SpecAugment where the masks are based on sorted filters.
    """

    def __init__(self, cfg: SupervisedConvolutionalFeatureExtractionV5Config):
        super().__init__()
    ...
    def forward(self, raw_audio: torch.Tensor, length: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.training and run_ctx.epoch >= self.cfg.specaug_config.start_epoch:
            if self.cfg.specaug_config.mask_sorting_strategy is None:
                filter_idcs_env = None
            elif self.cfg.specaug_config.mask_sorting_strategy == "center_frequency":
