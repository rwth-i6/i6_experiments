...
def get_nn_args(
    ...
    num_epochs: int = 250,
    use_rasr_returnn_training=True,
    **net_kwargs
):
...
def get_rc_returnn_configs(
    ...
    blstm_base_config.update(
        {
            ...
            "gradient_noise": 0.3,
            "learning_rates": list(np.linspace(2.5e-5, 3e-4, 50))
            + list(np.linspace(3e-4, 2.5e-5, 50)),
    ...
    rc_extern_data = ExternData(
        extern_data=get_default_data_init_args(
    ...
    rc_construction_code = Import(
        rc_package + ".default_hybrid_v2.construct_hybrid_network"
    )
    ...
    return {
        ...
        "blstm_oclr_v1_focal_loss": construct_from_net_kwargs(
            blstm_base_config, net_kwargs_focal_loss
        ),
    }
