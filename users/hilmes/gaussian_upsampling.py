...
class GaussianUpsampling(nn.Module):
  ...
  @nn.scoped
  def __call__(self, inp: nn.Tensor, durations: nn.Tensor, variances: nn.Tensor, time_dim: nn.Dim) -> nn.Tensor:
    """
    ...
    w_t = w_t / nn.combine(nn.math_norm(w_t, p=1, axis=time_dim), 10 ** -16, kind="maximum")
