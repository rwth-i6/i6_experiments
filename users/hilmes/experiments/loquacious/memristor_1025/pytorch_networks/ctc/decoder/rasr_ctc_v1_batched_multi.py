...
def _search_one_lm(args):
    ...
    print("lm_scale %s: searched %d segments x %d prior scales in %.2fs"
          % (lm_scale, len(tags), len(prior_scales), search_time))
    return lm_scale, search_time  # return kept for the serial/other callers; ignored by Process
...
def forward_finish_hook(run_ctx, **kwargs):
    ...
    search_start = perf_counter()
    # RASR's Flf::Lexicon is a process-global singleton (ctor asserts !us_), so a
    # process may build at most ONE SearchAlgorithm in its lifetime. Run each search
    # task in its own fresh spawned process (spawn: fork-after-CUDA can hang, the
    # forward used the GPU), capping concurrency at num_workers via waves.
    #
    # We deliberately do NOT use multiprocessing.Pool: under spawn it passes internal
    # semaphores to children via SemLock._rebuild, which can fail on some nodes
    # (/dev/shm race with the resource_tracker); Pool then respawns the dying workers
    # forever (observed: 967k-line log, 0 progress). Plain Process objects carrying
    # only picklable args (scales, file paths, prior ndarray) need no named semaphore,
    # and an explicit exitcode check makes a startup failure error out immediately.
    ctx = multiprocessing.get_context("spawn")
    for wave_start in range(0, len(tasks), num_workers):
        wave = tasks[wave_start:wave_start + num_workers]
        procs = []
        for task in wave:
            p = ctx.Process(target=_search_one_lm, args=(task,))
            p.start()
            procs.append((task[0], p))
        for lm_scale, p in procs:
            p.join()
            if p.exitcode != 0:
                raise RuntimeError(
                    "search worker for lm_scale %s exited with code %s" % (lm_scale, p.exitcode)
                )
