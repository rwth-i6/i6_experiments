...
from .memristor_v10_modules_keep_encs import LinearQuant, ActivationQuantizer, QuantizedMultiheadAttention, Conv1dQuant
    ...
    for name, param in run_ctx.engine._model.state_dict().items():
        ...
        if "weight" in name and not "memristor" in name:
            ...
            if any(x in name for x in ["linear", "conv"]):
                if not any(x in name for x in ["frontend", "final", "linear_pos"]):
