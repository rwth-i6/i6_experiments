...
def bpe_loq_small_memristor_width_1125():
        ...
        for activation_bit in [8]:
            for weight_bit in [4]:
                for dim in [384, 512, 768, 1024]:
                    frontend_config_dim = copy.deepcopy(frontend_config_sub4)
                    frontend_config_dim.out_features = dim
                    model_config_base = MemristorModelTrainConfigV8(
                    ...
                    for seed in range(3):
                        ...
                        training_name = prefix_name + "/" + network_module_mem_v9 + f"_{epochs//5}eps_{dim}dim_{weight_bit}_{activation_bit}_seed_{seed}"
                        ...
                        for num_cycles in range(1, 11):
                            ...
                            recog_name = prefix_name + "/" + network_module_mem_v9 + f"_{weight_bit}_{activation_bit}_seed_{seed}/cycle_{num_cycles // 11}"
                            res_conv = eval_model(
    ...
    tk.register_report("reports/loquacious/small_width", partial(build_base_report, full_results, False), required=full_results, update_frequency=600)
