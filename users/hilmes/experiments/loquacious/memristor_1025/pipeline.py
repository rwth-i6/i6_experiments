...
from .data.common import get_devtest_all_stms
...
def search(
    ...
    for key, (test_dataset, test_dataset_reference) in test_dataset_tuples.items():
        ...
        if return_ctm:
            wers[search_name], search_job, ctm_file = search_single(
                search_name,
    ...
    if return_ctm:
        return search_jobs, wers, ctm_file
...
def compute_prior(
    ...
    if "loquacious" in prefix_name and "medium" in prefix_name:
        search_job.rqmt["time"] += 2
...
def prepare_memristor(
    ...
    # (since the 2026-08-10 default-pin switch to 601680e the default
    # SynaptogenML content supports it; older pins ignore the env harmlessly).
    # The workers pin their BLAS to a single thread themselves. 8 workers /
    # 8 CPUs: larger requests starve at QOSMaxCpuPerUserLimit with no backfill
    # reservation. set_env and rqmt are not hashed -> no job re-runs from this.
...
def prepare_asr_model(
    ...
    asr_model = ASRModel(
        checkpoint=checkpoint,
        network_module=train_args["network_module"] if not split_preparation else train_args["network_module"],
        net_args=train_args["net_args"],
...
def evaluate_all(prefix_name: str, dev_ctms: Dict[str, tk.Path], test_ctms: Optional[dict[str, tk.Path]]):
    """
    ...
    if len(test_ctms) > 0:
        ...
        test_wer = test_sclite_job.out_wer
    return dev_sclite_job.out_wer, test_wer
