...
class Model(torch.nn.Module):
    ...
    def __init__(self, epoch, step, **kwargs):
        ...
        dropout_broadcast_axes = kwargs.pop("dropout_broadcast_axes", None)

        mhsa_with_bias = kwargs.pop("mhsa_with_bias", True)
        pos_emb_config = ConformerPosEmbConfig(**kwargs.pop("pos_emb_config"))
        ...
        mhsa_cfg = ConformerMHSARelPosV1Config(
            ...
            with_bias=mhsa_with_bias,
            dropout=0.2,
            dropout_broadcast_axes=dropout_broadcast_axes,
            learnable_pos_emb=pos_emb_config.learnable_pos_emb,
            rel_pos_clip=pos_emb_config.rel_pos_clip,
            with_linear_pos=pos_emb_config.with_linear_pos,
            with_pos_bias=pos_emb_config.with_pos_bias,
            separate_pos_emb_per_head=pos_emb_config.separate_pos_emb_per_head,
            pos_emb_dropout=pos_emb_config.pos_emb_dropout,
        ...
        block_cfg = ConformerRelPosBlockV1Config(ff_cfg=ff_cfg, mhsa_cfg=mhsa_cfg, conv_cfg=conv_cfg)
        ...
        conformer_cfg = ConformerRelPosEncoderV1Config(num_layers=12, frontend=frontend, block_cfg=block_cfg)
    ...
    def forward(
        ...
        conformer_out, _ = self.conformer(audio_features_masked_2, mask, return_layers=[11])

        upsampled = self.upsample_conv(conformer_out[0].transpose(1, 2)).transpose(1, 2)  # final upsampled [B, T, F]
...
def export2(*, model: Model, model_filename: str):
