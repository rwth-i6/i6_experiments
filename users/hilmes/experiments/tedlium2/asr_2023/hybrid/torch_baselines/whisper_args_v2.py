...
def get_pytorch_returnn_configs(
    ...
    hubert_chunk = copy.deepcopy(whisper_jj)
    hubert_chunk["chunking"] = "400:200"
    ...
    def construct_from_net_kwargs(
        ...
        if any(x is not None for x in [max_seqs, grad_acc, learning_rate, torch_amp]):
            ...
            if torch_amp is not None and not recognition:
    ...
    # ) for x in
        #[0, 2, 4, 6, 8, 10], # 9.2
        #[0, 1, 2, 3, 4, 5, 6, 7, 8, 12, 16, 20, 23], # 8.5
        #[0, 1, 2, 8, 16, 21, 22, 23]]}, # 9.6
    ...
    # ) for x in [9]}, # 10.3
    # **{f"torch_whisper_v3_medium_en_keepuntil_{x}_jjlr_bfloat16_test2": construct_from_net_kwargs(
    #         whisper_jj,
    #         {
    #             "model_type": "whisper_tune_v3",
    #             "just_encoder": True,
    ...
    #         torch_amp={"dtype": "bfloat16"}
    # ) for x in [9]},  # 9.4
    "torch_whisper_v3_medium_en_tune_all_jjlr_split_10_acc_50": construct_from_net_kwargs(
        whisper_jj,
        {
            "model_type": "whisper_tune_v3",
            "just_encoder": True,
    ...
    **{f"torch_hubert_test_large_whisp": construct_from_net_kwargs(  # does not converge properly
        hubert_chunk,
        {
            "model_type": "hubert_tune",
            "config_dict":
                {
                    "model_name": "large-ls960-ft",
