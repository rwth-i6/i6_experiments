...
def get_pytorch_returnn_configs(
    ...
    base_config = {
        ...
        "save_ignore_params_prefixes": ["whisper"],
    ...
    return {
    **{f"torch_distill_whisper_large_test_{x}": construct_from_net_kwargs(
        whisper_config,
        {
            "model_type": "distill_whisper_v1",
            "whisper_dict": {
                "model_name": "base.en",
                "distill_scale": x,
                "just_encoder": True,
        ...
        max_seqs=10,
        grad_acc=2,
    ) for x in [0.0, 0.1, 0.5, 1.0, 10, 100]}  # 7.8, 7.7, 7.7, 7.7, 8.0, 8.2 without chunking, now running with some updates
    }
