...
class Model(nn.Module):
    ...
    def forward(self, raw_audio: torch.Tensor, raw_audio_len: torch.Tensor):
        ...
        audio_features_size = self.hubert._get_feat_extract_output_lengths(raw_audio_len)
        ...
        upsampled = upsampled[:, 0: torch.max(audio_features_size)*2, :]
...
def train_step(*, model: Model, extern_data, **_kwargs):
    ...
    assert torch.round(phonemes_len - 1.5).to(dtype=torch.int32) == features_len, (torch.round(phonemes_len - 1.5).to(dtype=torch.int32), features_len)
