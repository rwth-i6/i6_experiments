...
def forward_step(*, model: torch.nn.Module, extern_data: TensorDict, **kwargs):
    audio_features: torch.Tensor = extern_data["data"].raw_tensor
    audio_features_len: torch.Tensor = extern_data["data"].dims[1].dyn_size_ext.raw_tensor

    log_probs, logits_ce_order, features_len = model(
        features=audio_features,
        feature_len=audio_features_len.to(audio_features.device),
    )  # [B, T, F]
