...
def add_linear_layer(
    ...
    **kwargs
    ...
    network[name] = {"class": "linear", "n_out": size, "from": fromList, "activation": activation}
    if l2 is not None:
        network[name]["L2"] = l2
...
def add_activation_layer(network, name, fromList, activation, **kwargs):
    network[name] = {"class": "activation", "from": fromList, "activation": activation}
    if kwargs.get("loss", None) is not None:
        network = add_loss_to_layer(network, name, **kwargs)
    return network, name
...
def add_lstm_layer(
    network, name, fromList, size, l2=0.01, dropout=0.1, bidirectional=True, unit="nativelstm2", **kwargs
):
    if bidirectional:
...
def add_constant_layer(network, name, value, dtype="int32", with_batch_dim=True, **kwargs):
    network[name] = {"class": "constant", "value": value, "dtype": dtype, "with_batch_dim": with_batch_dim}
...
def add_cast_layer(network, name, fromList, dtype="float32"):
    network[name] = {"class": "cast", "from": fromList, "dtype": dtype}
    return network, name


def add_expand_dim_layer(network, name, fromList, axis, out_type=None):
    network[name] = {"class": "expand_dims", "from": fromList, "axis": 2}  # if int, then automatically batch major
...
def add_copy_layer(network, name, fromList, initial=None, loss=None, **kwargs):
    network[name] = {"class": "copy", "from": fromList}
...
def add_compare_layer(network, name, fromList, value=None, kind="not_equal", initial=None):
    network[name] = {"class": "compare", "from": fromList, "kind": kind}
...
def make_subnet(fromList, net):
    subnet = {"class": "subnetwork", "from": fromList, "subnetwork": net}
    return subnet


# masked computation
def add_mask_layer(
    network: object, name: object, fromList: object, mask: object, unit: object = {"class": "copy"}, **kwargs: object
) -> object:
    network[name] = {
...
def add_unmask_layer(network, name, fromList, mask, **kwargs):
    network[name] = {"class": "unmask", "from": fromList, "mask": mask}
...
def add_padding_layer(
    network, name, fromList, axes="T", padding=(0, 1), value=0, mode="constant", n_out=None, **kwargs
):
    network[name] = {"class": "pad", "from": fromList, "axes": axes, "padding": padding, "value": value, "mode": mode}
...
def add_time_postfix_layer(network, name, fromList, postfix, repeat=1):
    network[name] = {"class": "postfix_in_time", "from": fromList, "postfix": postfix, "repeat": repeat}
    return network, name


def add_axis_range_layer(network, name, fromList, axis="T", unbroadcast=True):
    network[name] = {"class": "range_in_axis", "from": fromList, "axis": axis, "unbroadcast": unbroadcast}
    return network, name


def add_shift_layer(network, name, fromList, axis="T", amount=1, pad=True, **kwargs):
    network[name] = {"class": "shift_axis", "from": fromList, "axis": axis, "amount": amount, "pad": pad}
...
def add_seq_len_mask_layer(network, name, fromList, axis="T", mask_value=0):
    network[name] = {"class": "seq_len_mask", "from": fromList, "axis": axis, "mask_value": mask_value}
    return network, name
...
def add_merge_dim_layer(network, name, fromList, axes="except_time", **kwargs):
    network[name] = {"class": "merge_dims", "from": fromList, "axes": axes}
    return network, name


def add_split_dim_layer(network, name, fromList, axis, dims, **kwargs):
    network[name] = {"class": "split_dims", "from": fromList, "axis": axis, "dims": dims}
    return network, name
...
def add_squeeze_layer(network, name, fromList, axis, enforce_batch_dim_axis=None):
    network[name] = {"class": "squeeze", "from": fromList, "axis": axis}
...
def add_layer_norm_layer(network, name, fromList):
    network[name] = {"class": "layer_norm", "from": fromList}
    return network, name


def add_batch_norm_layer(network, name, fromList, **kwargs):
    network[name] = {"class": "batch_norm", "from": fromList}
...
def add_combine_layer(network, name, fromList, kind="add", **kwargs):
    network[name] = {"class": "combine", "from": fromList, "kind": kind}
...
def add_eval_layer(network, name, fromList, eval_str, **kwargs):
    network[name] = {"class": "eval", "from": fromList, "eval": eval_str}
    if kwargs.get("loss", None) is not None:
        network = add_loss_to_layer(network, name, **kwargs)
    if kwargs.get("initial", None) is not None:
        network[name]["initial_output"] = kwargs.get("initial", None)
...
def add_variable_layer(network, name, shape, **kwargs):
    network[name] = {"class": "variable", "shape": shape}
    return network, name


# generic attention
def add_attention_layer(network, name, base, weights, **kwargs):
    network[name] = {"class": "generic_attention", "base": base, "weights": weights}
    return network, name


def add_spatial_softmax_layer(network, name, fromList, **kwargs):
    network[name] = {"class": "softmax_over_spatial", "from": fromList}
    return network, name


def add_rel_pos_encoding_layer(network, name, fromList, n_out, clipping=64, **kwargs):
    network[name] = {"class": "relative_positional_encoding", "from": fromList, "n_out": n_out, "clipping": clipping}
    return network, name


def add_self_attention_layer(
    network, name, fromList, n_out, num_heads, total_key_dim, key_shift=None, attention_dropout=None, **kwargs
):
    network[name] = {
...
def add_conv_layer(
    network, name, fromList, n_out, filter_size, padding="VALID", l2=0.01, bias=True, activation=None, **kwargs
):
    network[name] = {
        "class": "conv",
        "from": fromList,
...
def add_gating_layer(network, name, fromList, activation=None, gate_activation="sigmoid", **kwargs):
    network[name] = {"class": "gating", "from": fromList, "activation": activation, "gate_activation": gate_activation}
    return network, name


def add_reduce_layer(network, name, fromList, mode="mean", axes="T", keep_dims=False, **kwargs):
    network[name] = {"class": "reduce", "from": fromList, "mode": mode, "axes": axes, "keep_dims": keep_dims}
    return network, name


def add_reduce_out_layer(network, name, fromList, mode="max", num_pieces=2, **kwargs):
    network[name] = {"class": "reduce_out", "from": fromList, "mode": mode, "num_pieces": num_pieces}
    return network, name


# Convolution block
def add_conv_block(
    network, fromList, conv_layers, conv_filter, conv_size, pool_size=None, name_prefix="conv", **kwargs
):
...
def build_encoder_network(num_layers=6, size=512, max_pool=[], **kwargs):
    ...
    if kwargs.pop("initial_convolution", False):
        ...
        network, fromList = add_conv_block(
            network, fromList, conv_layers, conv_filter, conv_size, pool_size=pool, **kwargs
        )
    # BLSTM layers
...
def add_conformer_block(network, name, fromList, size, dropout, l2, **kwargs):
    ...
    def add_mhsa_module(net, n, fin, heads, posEncSize, posEncClip, posEnc=True):
        ...
        net, fout = add_self_attention_layer(
            net, n + "_self_attention", fout, size, heads, size, key_shift=fpos, attention_dropout=dropout
        )
    ...
    def add_conv_module(net, n, fin, filterSize, bnFix, bnFreeze, bn2ln):
        ...
        net, fout = add_conv_layer(
            net, n + "_depthwise_conv", fout, size, filterSize, padding="same", l2=l2, groups=size
        )
        if bn2ln:
...
def build_conformer_encoder(num_blocks=12, size=512, dropout=0.1, l2=0.0001, max_pool=[], **kwargs):
    ...
    if kwargs.get("initial_convolution", True):
        ...
        if kwargs.get("vgg_conv", True):
            network, fromList = add_conv_block(
                network, fromList, 1, (3, 3), 32, pool_size=(1, 2), activation="swish", **kwargs
            )
            stride1, stride2 = kwargs.get("vgg_conv_strides", (2, 2))
            network, fList = add_conv_layer(
                network,
                "conv_2",
                network[fromList]["from"],
                64,
                (3, 3),
                padding="same",
                strides=(stride1, 1),
            ...
            network, fList = add_conv_layer(
                network, "conv_3", fList, 64, (3, 3), padding="same", strides=(stride2, 1), activation="swish", **kwargs
            )
            ...
            network, fromList = add_conv_block(
                network, fromList, conv_layers, conv_filter, conv_size, strides=strides, **kwargs
            )
        ...
        network, fromList = build_encoder_network(
            num_layers=layers, size=uniSize, max_pool=pool, dropout=dropout, l2=l2, **kwargs
        )
        assert not max_pool
...
def add_loss_layer(network, name, fromList, loss="ce", **kwargs):
    network[name] = {"class": "loss", "from": fromList, "loss_": loss}
...
def add_output_layer(network, fromList, name="output", loss="ce", loss_opts=None, cls="softmax", **kwargs):
    network[name] = {"class": cls, "from": fromList}
    if loss is not None:
...
def add_sMBR_output(inNetwork, name="output_ac", output="output", ce_smooth=0.1, **kwargs):
    ...
    network[name] = {
        ...
        "loss_opts": {
            "sprint_opts": crnn.CustomCRNNSprintTrainingJob.create_sprint_loss_opts(
                loss_mode="sMBR", num_sprint_instance=1
            )
        },
    }
    return network
...
def add_full_sum_output_layer(network, fromList, num_classes, loss="fast_bw", name="output", **kwargs):
    output_args = {
        ...
        "loss": loss,
        "loss_opts": {
            "sprint_opts": crnn.CustomCRNNSprintTrainingJob.create_sprint_loss_opts(**kwargs),
            "tdp_scale": kwargs.get("tdp_scale", 0.0),
        },
        "extra_args": {"target": None, "n_out": num_classes},  # no target to infer output size
    }
...
def make_internal_LM_rec_network(recUnit, name, scale, lm_output, num_classes, blankIndex=0, posterior="output"):
    ...
    recUnit[posterior].update({"class": "linear", "activation": "log_softmax"})
    # TODO exclude bias ?
    ...
    recUnit, fList = add_padding_layer(
        recUnit, "intLM_prior", fList, axes="F", value=0, padding=(1, 0), n_out=num_classes
    )
...
## ----------------------- extra python code ----------------------- ##
# SpecAugment #
def get_spec_augment_mask_python(
    codeFile=None,
    max_time_num=6,
    max_time=5,
    max_feature_num=4,
