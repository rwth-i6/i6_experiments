...
from .self_distill_conformer_auxloss_v1_cfg import ModelConfig, DistillConfig
...
def train_step(*, model: Model, data, run_ctx, **kwargs):
    ...
    for teacher_logit, student_logit in zip(teacher_logits, student_logits):
        ...
        soft_targets_loss = torch.sum(soft_targets * (soft_targets.log() - soft_prob)) / soft_prob.size()[0] * (
                    T ** 2)
        num_phonemes = torch.sum(labels_len)
        run_ctx.mark_as_loss(name="KL", loss=soft_targets_loss, scale=model.distill_config.distill_scale,
