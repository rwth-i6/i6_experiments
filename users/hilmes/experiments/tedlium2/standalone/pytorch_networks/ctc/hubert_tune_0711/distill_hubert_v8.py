...
from .distill_hubert_v8_cfg import ModelConfig, DistillConfig
...
def train_step(*, model: Model, data, run_ctx, **kwargs):
    ...
    if model.distill_config.warmup_loss is not None and run_ctx.epoch < model.distill_config.warmup_loss:
    ...
    elif model.distill_config.mix_nonblank_blank is not None:
        ...
        if model.distill_config.mix_nonblank_blank > 0:
            run_ctx.mark_as_loss(
                name=f"KL_non_blank",
                loss=soft_nonblank_loss,
                scale=model.distill_config.distill_scale * model.distill_config.mix_nonblank_blank,
                inv_norm_factor=num_phonemes,
            )
        if model.distill_config.mix_nonblank_blank < 1:
            run_ctx.mark_as_loss(
                name=f"KL_blank",
                loss=soft_blank_loss,
                scale=model.distill_config.distill_scale * (1 - model.distill_config.mix_nonblank_blank),
