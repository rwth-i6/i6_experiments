...
from .search.documented_ctc_beam_search_v5 import CTCBeamSearch
...
def forward_finish_hook(run_ctx, **kwargs):
    ...
    print(
        "Total-LM-Time: %.2fs, LM-RTF: %.4f" % (run_ctx.total_lm_time,
            run_ctx.total_lm_time / run_ctx.running_audio_len_s)
    )
    run_ctx.rtf_file.write(
        "Total-LM-Time: %.2fs, LM-RTF: %.4f \n" % (run_ctx.total_lm_time,
            run_ctx.total_lm_time / run_ctx.running_audio_len_s)
    )
...
def forward_step(*, model, data, run_ctx, **kwargs):
    ...
    if run_ctx.print_rtf:
        ...
        lm_time = run_ctx.ctc_decoder.lm_time
        search_time = search_time - lm_time
