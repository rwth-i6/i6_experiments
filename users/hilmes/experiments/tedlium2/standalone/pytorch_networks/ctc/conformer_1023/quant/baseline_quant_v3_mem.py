...
from .baseline_quant_v1_cfg import (
...
from .baseline_quant_v3_modules_mem import LinearQuant, QuantizedMultiheadAttention, Conv1dQuant
...
class ConformerPositionwiseFeedForwardQuant(nn.Module):
    ...
    def __init__(self, cfg: ConformerPositionwiseFeedForwardQuantV1Config):
        ...
        self.linear_ff = LinearQuant(
            ...
            bias=True,
            quant_output=cfg.linear_quant_output,
        )
...
class ConformerConvolutionQuant(nn.Module):
    ...
    def __init__(self, model_cfg: ConformerConvolutionQuantV1Config):
        ...
        self.pointwise_conv1 = LinearQuant(
            ...
            activation_bit_prec=model_cfg.activation_bit_prec,
            ...
            bias=True,
            quant_output=model_cfg.linear_quant_output,
        )
        # TODO: Quantize this
            ...
            conformer_config = ConformerEncoderQuantV1Config(
                ...
                block_cfg=ConformerBlockQuantV1Config(
                    ff_cfg=ConformerPositionwiseFeedForwardQuantV1Config(
                        ...
                        weight_bit_prec=self.cfg.weight_bit_prec,
                        activation_bit_prec=self.cfg.activation_bit_prec,
                        linear_quant_output=self.cfg.linear_quant_output,
                    ),
                    mhsa_cfg=QuantizedMultiheadAttentionV1Config(
                        ...
                        bit_prec_W_q=self.cfg.weight_bit_prec,
                        bit_prec_W_k=self.cfg.weight_bit_prec,
                        bit_prec_W_v=self.cfg.weight_bit_prec,
                        bit_prec_dot=self.cfg.weight_bit_prec,
                        bit_prec_A_v=self.cfg.weight_bit_prec,
                        bit_prec_W_o=self.cfg.weight_bit_prec,
                        moving_average=self.cfg.moving_average,
                        linear_quant_output=self.cfg.linear_quant_output,
                    ),
...
def static_quant_finish_hook(run_ctx, **kwargs):
    ...
    torch.save(
        {"model": run_ctx.engine._model.state_dict(), "epoch": 250, "step": run_ctx.engine._train_step}, "model.pt"
    )
