...
def eow_phon_ted_1023_base(quant=False):
    ...
    tk.register_report("reports/size_report", partial(build_report, report))
    ...
    tk.register_report("reports/distill_report", partial(build_distill_report, distill_report))
    tk.register_report("reports/distill_no_drop_stud_report", partial(build_distill_report, no_drop_stud_report))
    tk.register_report("reports/distill_larger_report", partial(build_distill_report, larger_distill_report))
    train_config = {
        "optimizer": {"class": "adamw", "epsilon": 1e-16, "weight_decay": 1e-3},
        "learning_rates": list(np.linspace(7e-6, 5e-4, 110))
                          + list(np.linspace(5e-4, 5e-5, 110))
