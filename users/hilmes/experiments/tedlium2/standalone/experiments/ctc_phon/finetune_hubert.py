...
def eow_phon_ted_tune_pos_enc_w_hubert():
    ...
    for name, (chkpt, prior_file) in checkpoints.items():
        ...
        for epochs in train_epochs:
            ...
            tk.register_report(
                f"reports/distill_pos_enc_w_hubert_report_last_old_{epochs}",
...
def eow_phon_ted_tune_pos_enc_w_hubert_new4():
    prefix_name = "experiments/tedlium2/ctc_rnnt_standalone_2024/ctc_eow_phon/distill_pos_enc_w_hubert_new4"
    ...
    for name, (chkpt, prior_file) in checkpoints.items():
        ...
        for epochs in train_epochs:
                    ...
                    for T in [2]:
                        ...
                        for keep in [1, 2, 3, 4, 5]:
                            ...
                            if not os.path.exists(
                                f"{train_job._sis_path()}/finished.run.1"
                            ):  # sync back was successful
                                train_job.hold()
                                train_job.move_to_hpc = True
                            train_job.rqmt["gpu_mem"] = 48
                            results = eval_model(
            ...
            tmp_rep["baselines"] = distill_report["baselines"]
            tk.register_report(
                f"reports/distill_pos_enc_w_hubert_report_lastv4_{epochs}",
                partial(build_hubert_distill_report, tmp_rep),
                required=distill_report,
                update_frequency=900,
            )
...
def eow_phon_ted_tune_pos_enc_w_hubert_amp2():
    prefix_name = "experiments/tedlium2/ctc_rnnt_standalone_2024/ctc_eow_phon/distill_pos_enc_w_hubert_amp2"
    ...
    for name, (chkpt, prior_file) in checkpoints.items():
        ...
        for epochs in train_epochs:
                ...
                for distill_scale in [0.25, 0.9, 1.0]:
                    for T in [2]:
                        ...
                        train_config_distill = {
                            ...
                            + list(np.linspace(5e-5, 1e-7, 30)),
                            #############
                            "batch_size": 90 * 16000,  # TODO: test if this is the issue
                            "max_seq_length": {"audio_features": 35 * 16000},
                            "accum_grad_multiple_step": 2,
                            "torch_amp_options": {"dtype": "bfloat16"},
                        }
            ...
            tmp_rep["baselines"] = distill_report["baselines"]
            tk.register_report(
                f"reports/distill_pos_enc_w_hubert_report_last_amp2_{epochs}",
                partial(build_hubert_distill_report, tmp_rep),
                required=distill_report,
                update_frequency=900,
            )
...
def eow_phon_ted_tune_pos_enc_w_hubert_rerun11():
    prefix_name = "experiments/tedlium2/ctc_rnnt_standalone_2024/ctc_eow_phon/distill_pos_enc_w_hubert_rerun11"
    ...
    for name, (chkpt, prior_file) in checkpoints.items():
        ...
        for epochs in train_epochs:
                ...
                for distill_scale in [0.25, 0.9, 1.0]:
                    for T in [2]:
                        ...
                        train_config_distill = {
                            ...
                            "max_seq_length": {"audio_features": 35 * 16000},
                            "accum_grad_multiple_step": 2,
                            "asdf": True # random flag for new hash
                        }
                        train_args_distill = {
                            "config": train_config_distill,
                            "network_module": distill_module_v3,
            ...
            tmp_rep["baselines"] = distill_report["baselines"]
            tk.register_report(
                f"reports/distill_pos_enc_w_hubert_report_last_rerun11_{epochs}",
                partial(build_hubert_distill_report, tmp_rep),
                required=distill_report,
                update_frequency=900,
            )
