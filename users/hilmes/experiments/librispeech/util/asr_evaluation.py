...
from i6_core.returnn.search import SearchBPEtoWordsJob, ReturnnSearchFromFileJob, ReturnnComputeWERJob
from i6_core.text import PipelineJob


def asr_evaluation(config_file, corpus, returnn_root, returnn_python_exe, output_path, additional_parameters=None,
                   segment_file=None):
  """
  ...
  bliss_corpus = CorpusReplaceOrthFromTxtJob(bliss_corpus=corpus, text_file=text, segment_file=segment_file).out_corpus
  text_dict = CorpusToTextDictJob(bliss_corpus, segment_file=segment_file).out_dictionary
  ...
  wer = ReturnnComputeWERJob(
    ...
    reference=text_dict, returnn_root=returnn_root, returnn_python_exe=returnn_python_exe
  )
