...
from i6_experiments.users.hilmes.experiments.librispeech.nar_tts_2022.data import TTSForwardData, get_inference_dataset
...
def gmm_duration_cheat():
  ...
  train_job = tts_training(
    config=train_config, returnn_exe=returnn_exe, returnn_root=returnn_root, prefix=name, num_epochs=200
  )
  # synthesis

  extraction_config = get_extraction_config(speaker_embedding_size=256, returnn_common_root=returnn_common_root, training_datasets=training_datasets)
  extraction_job = tts_forward(checkpoint=train_job.out_checkpoints[200], config=extraction_config, returnn_exe=returnn_exe, returnn_root=returnn_root, prefix=name)
