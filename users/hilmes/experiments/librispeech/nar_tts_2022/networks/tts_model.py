...
class Conv1DBlock(nn.Module):
  ...
  def __init__(
    ...
    self.bn = nn.BatchNorm(epsilon=bn_epsilon, use_mask=False)
...
class VariationalAutoEncoder(nn.Module):
  ...
  def __init__(
    ...
    for _ in range(num_mixed_blocks):
      ...
      self.mixed_stack.append(
        Conv1DBlock(dim=linear_size, filter_size=conv_filter_size, dropout=dropout)
      )
    self.latent_dim = nn.FeatureDim("VAE_lat_dim", latent_size)
    self.lin_mu = nn.Linear(self.latent_dim)
    self.lin_log_var = nn.Linear(self.latent_dim)
