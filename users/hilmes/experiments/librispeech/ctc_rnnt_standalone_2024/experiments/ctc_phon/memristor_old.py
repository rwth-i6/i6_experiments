...
from ...pytorch_networks.ctc.qat_0711.memristor_v8_cfg import QuantModelTrainConfigV8
...
def eow_phon_ls960_0426_memristor_noise():
    prefix_name = "experiments/librispeech/ctc_rnnt_standalone_2024/ls960_ctc_eow_phon_memristor/noise"
        ...
        for dim in dims:
            for weight_bit in weight_bits:
                ...
                for seed in range(seeds):
                    ...
                    training_name = prefix_name + "/" + network_module_mem_v9 + f"_{1000//10}eps_{dim}dim_w{weight_bit}_a{activation_bit}_seed_{seed}"
                    train_job = training(training_name, train_data, train_args, num_epochs=1000, **default_returnn)
                    if not os.path.exists(f"{train_job._sis_path()}/finished.run.1"):  # sync back was successful
                        train_job.rqmt['cpu'] = 12
                        train_job.hold()
                        ...
                        for dev in [0.05]:
                            ...
                            for seed in range(seeds):
                                ...
                                results, best_params_job = eval_model(
                                    ...
                                    train_job=train_job,
                                    train_args=prior_args, # this effectively turns off noise
                                    train_data=train_data,
                                    decoder_config=as_training_rasr_config,
                                    dev_dataset_tuples={"dev-other": dev_dataset_tuples["dev-other"]},
                                    result_dict=results,
