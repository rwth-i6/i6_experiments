...
def eow_phon_ls960_distill_base():
    ...
    train_data = build_eow_phon_training_datasets(
        prefix=prefix_name, librispeech_key="train-other-960", settings=train_settings,
    )
                ...
                for spec in [8, 16]:
                    for num_heads in [4, 8, 12]:
                        ...
                        specaug_config_test = SpecaugConfig(
                            repeat_per_n_frames=25, max_dim_time=20, max_dim_feat=spec, num_repeat_feat=5,
                        )
                        ...
                        results = eval_model(
                            ...
                            lm_scales=[1.8, 2.0, 2.2, 2.4, 2.6, 2.8, 3.0, 3.2, 3.4],
            ...
            for epochs in [500, 1000]:
                for spec in [16]:
                    for num_heads in [8, 12]:
                        ...
                        report[training_name] = results
                        del results
                        if dim == 512 and epochs == 500 and num_heads == 8:
                            train_config = {
                                "optimizer": {"class": "adamw", "epsilon": 1e-16, "weight_decay": 1e-2},
                                "learning_rates": list(np.linspace(7e-6, 5e-4, (epochs - 40) // 2))  # try higher start
                                + list(np.linspace(5e-4, 5e-5, (epochs - 40) // 2))
                            ...
                            training_name = (
                                ...
                                + "/"
                                + network_module_pos_enc
                                + f"_better_params_{epochs}_{dim}_{num_heads}_{spec}_{spec_start}_11gb"
                            )
                            train_job = training(
                                training_name, train_data, train_args, num_epochs=epochs, **default_returnn
                            )
...
def eow_phon_ls960_distill_hubert():
    ...
    for testset in ["train-other-960"]:
        train_dataset_tuples[testset] = build_test_dataset(dataset_key=testset, settings=train_settings,)
    ...
    for model in [
        ...
        for factor in [1, 2]:
            ...
            train_config_24gbgpu_amp = {
                ...
                # "cleanup_old_models": {"keep_last_n": 4, "keep_best_n": 4, "keep": keep_epochs,},
                "torch_amp_options": {"dtype": "bfloat16"},
            }
                ...
                for checkpoint in ["best4"]:
                    for n_best in [10]:
                        ...
                        kd_hyp, prior_file, model_checkpoint = generate_kd_hypothesis(
                            ...
                            prior_scale=0.5,
                            lm_scale=2.0,
                            train_referece=train_dataset_tuples["train-other-960"][1],
                            debug=True,
                        )
    ...
    base_train_config_distill = {
        "optimizer": {"class": "adamw", "epsilon": 1e-16, "weight_decay": 1e-2,},
        "learning_rates": list(np.linspace(7e-6, 5e-4, (epochs - 40) // 2))
        + list(np.linspace(5e-4, 5e-5, (epochs - 40) // 2))
        + list(np.linspace(5e-5, 1e-7, 40)),
        #############
        "batch_size": 150 * 16000,
        "max_seq_length": {"audio_features": 35 * 16000},
        "accum_grad_multiple_step": 2,  # TODO: wait for baseline here
        "gradient_clip_norm": 1.0,
    }
    for name, (chkpt, kd_hyps, prior_file, prior_scale) in checkpoints.items():
        name, num, n_best = name.split("_")
        for epochs, dim, spec_start, spec, num_heads, layer_count, teacher_ep in []:
            for distill_scale in [0.25, 1.0]:
                for T in [2]:
                    ...
                    distill_report["baselines"][
                        baseline_prefix + "/" + baseline_module + f"_{teacher_ep}_{dim}_{num_heads}_{spec}_{spec_start}"
                    ] = baselines[
                        baseline_prefix + "/" + baseline_module + f"_{teacher_ep}_{dim}_{num_heads}_{spec}_{spec_start}"
                    ]
                    ...
                    training_name = (
                        ...
                        + distill_module_v2
                        + f"_chkpt_{num}"
                        + f"_{layer_count}_{dim}_{heads}_{spec_start}_{distill_scale}_{T}_elim_blank"
                    )
                    train_job = training(
                        training_name, train_data, train_args_distill, num_epochs=epochs, **default_returnn
                    )
                    ...
                    if dim == 512 and distill_scale == 0.25:
                        for start_epoch in [5, 10, 25, 50, -5, -10, -25, -50, -100, -200]:
                            ...
                            training_name = (
                                ...
                                + distill_module_v2
                                + f"_chkpt_{num}"
                                + f"_{layer_count}_{dim}_{heads}_{spec_start}_{distill_scale}_{T}_elim_blank_{start_epoch}"
                            )
                            train_job = training(
                                training_name, train_data, train_args_distill, num_epochs=epochs, **default_returnn
                            )
                        ...
                        training_name = (
                            ...
                            + distill_module_v2
                            + f"_chkpt_{num}"
                            + f"_{layer_count}_{dim}_{heads}_{spec_start}_{distill_scale}_{T}_trim_blanks"
                        )
                        train_job = training(
                            training_name, train_data, train_args_distill, num_epochs=epochs, **default_returnn
                        )
                    ...
                    if dim == 384:
                        ...
                        training_name = (
                            ...
                            + distill_module_v2
                            + f"_chkpt_{num}"
                            + f"_{layer_count}_{dim}_{heads}_{spec_start}_{distill_scale}_{T}_elim_blank_prior"
                        )
                        train_job = training(
                            training_name, train_data, train_args_distill, num_epochs=epochs, **default_returnn
                        )
                        ...
                        training_name = (
                            ...
                            + f"_{layer_count}_{dim}_{heads}_{spec_start}_{distill_scale}_{T}_kdhyps"
                        )
                        train_job = training(
                            training_name, train_data, train_args_distill, num_epochs=epochs, **default_returnn
                        )
                        ...
                        for warmup in [10, 25, 50, 100]:
                            ...
                            training_name = (
                                ...
                                + distill_module_v2
                                + f"_chkpt_{num}"
                                + f"_{layer_count}_{dim}_{heads}_{spec_start}_{distill_scale}_{T}_warm{warmup}_elim_blanks"
                            )
                            train_job = training(
                                training_name, train_data, train_args_distill, num_epochs=epochs, **default_returnn
                            )
                            ...
                            training_name = (
                                ...
                                + distill_module_v2
                                + f"_chkpt_{num}"
                                + f"_{layer_count}_{dim}_{heads}_{spec_start}_{distill_scale}_{T}_warm{warmup}_long"
                            )
                            train_job = training(
                                training_name,
                                train_data,
                        ...
                        for keep in [1, 2, 3, 4, 5, 10, 100]:
                            ...
                            training_name = (
                                ...
                                + distill_module_v2
                                + f"_chkpt_{num}"
                                + f"_{layer_count}_{dim}_{heads}_{spec_start}_{distill_scale}_{T}_keepsome{keep}"
                            )
                            train_job = training(
                                training_name, train_data, train_args_distill, num_epochs=epochs, **default_returnn
                            )
                        ...
                        for keep in [1, 5]:
                            ...
                            training_name = (
                                ...
                                + distill_module_v2
                                + f"_chkpt_{num}"
                                + f"_{layer_count}_{dim}_{heads}_{spec_start}_{distill_scale}_{T}_warm25_keepsome{keep}"
                            )
                            train_job = training(
                                training_name, train_data, train_args_distill, num_epochs=epochs, **default_returnn
                            )
                            ...
                            del results
    tk.register_report(
        "reports/ls_distill_pos_enc_w_hubert_report",
