...
def bpe_ls960_1225_memristor_train_ablation():
            ...
            for dim in [128, 256, 384, 512, 768, 1024]:
                for weight_bit in [3, 4, 5, 6, 7, 8]:
                    if weight_bit in [6, 7]:
                        if not dim == dim or not epochs == 1000:
                            continue
    ...
    tk.register_report("reports/lbs/v2/memristor_bpe_train_ablation", partial(build_qat_report_v2, memristor_report),
                       required=memristor_report, update_frequency=400)
    return FINETUNE_MODELS, memristor_report


def bpe_ls960_1225_memristor_width(seed: int = 0):
    # data_prefix is kept identical to bpe_ls960_1225_memristor_train_ablation so the datasets
    # (and therefore overlapping training jobs) hash-match and dedup. The aliases/report use a
    # dedicated "/width" subfolder so the new results are easy to find; aliases are not hashed.
    data_prefix = "experiments/librispeech/ctc_rnnt_standalone_2024/bpe_ls960_memristor"
    prefix_name = data_prefix + "/width"
    ...
    # Prior/LM ranges kept identical to bpe_ls960_1225_memristor_train_ablation for comparability.
    rasr_prior_scales = [0.2, 0.3, 0.4, 0.5]
    rasr_lm_scales = [0.6, 0.7, 0.8, 0.9, 1.0, 1.1, 1.2]
    ...
    # Sweep definition. Activation bit loop is kept for future extension but only runs at 8 for now.
    # Each config is (dim, num_layers, epochs, weight_bit); epochs are full epoch counts
    # (train_partition_epoch=10 -> named as epochs//10 "eps").
    activation_bits = [8]
    width_dims = [128, 256, 384, 512, 768, 1024, 1536, 2048]
    epoch_list = [250, 500, 1000, 2000, 3000]
    layer_list = [10, 12, 14, 16]
    weight_bits = [4, 8]

    sweep_configs = {}
    # Width sweep: 12 layers, 100 eps (=1000 epochs)
    for dim in width_dims:
        for weight_bit in weight_bits:
            sweep_configs[(dim, 12, 1000, weight_bit)] = (dim, 12, 1000, weight_bit)
    # Epoch sweep: dim 512, 12 layers
    for epochs in epoch_list:
        for weight_bit in weight_bits:
            sweep_configs[(512, 12, epochs, weight_bit)] = (512, 12, epochs, weight_bit)
    # Layer sweep: dim 512, 100 eps (=1000 epochs)
    for num_layers in layer_list:
        for weight_bit in weight_bits:
            sweep_configs[(512, num_layers, 1000, weight_bit)] = (512, num_layers, 1000, weight_bit)

    for activation_bit in activation_bits:
        for dim, num_layers, epochs, weight_bit in sorted(sweep_configs.values()):
            frontend_config_dim = _make_frontend_config(dim)
            prior_train_dac_settings = DacAdcHardwareSettings(
                input_bits=0,
                output_precision_bits=0,
            ...
            model_config_kwargs = _make_model_config_kwargs(
                frontend_config_dim, dim, activation_bit, prior_train_dac_settings
            )
            model_config_kwargs["num_layers"] = num_layers  # default is 12 -> identical dict for 12-lay
            model_config = MemristorModelTrainConfigV8(
                **model_config_kwargs,
                weight_bit_prec=weight_bit,
            )
            ...
            training_name = prefix_name + "/" + network_module_mem_v10 + f"_{epochs // 10}eps_{num_layers}lay_{dim}dim_w{weight_bit}_a{activation_bit}_seed_{seed}"
            train_job = training(training_name, train_data_bpe128, train_args, num_epochs=epochs, **default_returnn)
            FINETUNE_MODELS[training_name] = train_job.out_checkpoints[epochs]
            if not os.path.exists(
                f"{train_job._sis_path()}/finished.run.1"):  # sync back was successful
            ...
            # QAT (non-memristor) recognition -- active
            _ = run_non_memristor_eval(
                training_name=training_name,
                train_job=train_job,
                train_args=train_args,
            ...
            # Memristor (hardware cycle) recognition -- set up but disabled for now.
            # Guarded by "if False" until the recog setup is finalized, plus the usual
            # training-finished check (as in the HPC case).
            if False and os.path.exists(f"{train_job._sis_path()}/finished.run.1"):
                max_runs = 10 if dim <= 512 else 3
                run_memristor_cycle_eval(
                    train_job=train_job,
                    train_data=train_data_bpe128,
                    train_config=train_config_24gbgpu,
                    model_config=model_config,
                    recog_name_prefix=prefix_name + "/" + network_module_mem_v11 + f"_{epochs // 10}eps_{num_layers}lay_{dim}dim_w{weight_bit}_a{activation_bit}_seed_{seed}",
                    rasr_config=rasr_config_memristor,
                    greedy_config=greedy_decoder_memristor,
                    dev_dataset_tuples={"dev-other": dev_dataset_tuples["dev-other"]},
                    prior_scales=[0.5],
    ...
    tk.register_report("reports/lbs/v2/memristor_bpe_width", partial(build_qat_report_v2, memristor_report),
                       required=memristor_report, update_frequency=400)
    return FINETUNE_MODELS, memristor_report