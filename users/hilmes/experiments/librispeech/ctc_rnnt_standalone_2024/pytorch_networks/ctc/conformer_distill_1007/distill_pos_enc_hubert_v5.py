...
def train_step(*, model: Model, data, run_ctx, **kwargs):
    ...
    if model.distill_config.warmup_loss is not None and run_ctx.epoch < model.distill_config.warmup_loss:
        ...
        run_ctx.mark_as_loss(name=f"KL", loss=soft_targets_loss, scale=model.distill_config.distill_scale), # norm not needed here as torch does it
        ...
        run_ctx.mark_as_loss(name=f"KL", loss=soft_targets_loss, scale=model.distill_config.distill_scale, inv_norm_factor=torch.tensor(teacher_tmp.size(0)))
