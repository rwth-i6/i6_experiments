...
no_outq variant: identical to memristor_v11_mem_inited, but the activation output
quantizers after the memristor layers (*_out_quant) are nn.Identity() as well, like
the input quantizers already are. Recognition-only module: the mem-init/conversion
job still runs with memristor_v11 and its converted_model.pt is loaded here; the
out-quant observer keys in that checkpoint are tolerated as unexpected keys
(checkpoint load is non-strict).

Superseded: new work uses the generic shim ctc.qat_0711.no_outq.memristor_v11 (see
no_outq/_strip.py), which needs no per-version file copy. This file is kept because
the first 5-cycle no_outq run was hashed against it.
"""
...
from .memristor_v11_no_outq_modules_mem_inited import ActivationQuantizer, QuantizedMultiheadAttention
...
class Model(torch.nn.Module):
    def __init__(self, model_config_dict, **kwargs):
        ...
        if self.train_config.quantize_output is True:
            ...
            self.lin_out_out_quant = nn.Identity()
