...
v16 adds the option to prune before quant
"""
...
from .memristor_v16_modules import LinearQuant, ActivationQuantizer, QuantizedMultiheadAttention, Conv1dQuant
...
class ConformerPositionwiseFeedForwardQuant(nn.Module):
    ...
    def prep_quant(self):
        ...
        if self.linear_ff.pruning_config is not None:
            with torch.no_grad():
                self.linear_ff.weight.data = self.linear_ff.prune_for_memristor_init(
        ...
        if self.linear_out.pruning_config is not None:
            with torch.no_grad():
                self.linear_out.weight.data = self.linear_out.prune_for_memristor_init(
...
class ConformerConvolutionQuant(nn.Module):
    ...
    def prep_quant(self):
        ...
        if self.pointwise_conv1.pruning_config is not None:
            with torch.no_grad():
                self.pointwise_conv1.weight.data = self.pointwise_conv1.prune_for_memristor_init(
        ...
        if self.pointwise_conv2.pruning_config is not None:
            with torch.no_grad():
                self.pointwise_conv2.weight.data = self.pointwise_conv2.prune_for_memristor_init(
...
class Model(torch.nn.Module):
    ...
    def prep_quant(self):
        ...
        if self.train_config.quantize_output is True:
            ...
            if self.lin_out[-1].pruning_config is not None:
                with torch.no_grad():
                    self.lin_out[-1].weight.data = self.lin_out[-1].prune_for_memristor_init(
                        self.lin_out[-1].weight.data, training=False
                    )
            mem_lin = TiledMemristorLinear(
