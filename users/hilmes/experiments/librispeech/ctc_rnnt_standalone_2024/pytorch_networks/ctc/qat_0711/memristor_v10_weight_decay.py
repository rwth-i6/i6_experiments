...
from .memristor_v10_weight_decay_cfg import (
...
def train_step(*, model: Model, data, run_ctx, **kwargs):
    ...
    pos_enc_norm_loss = 0
    for block in model.conformer.module_list:
        for module in block.module_list:
            if isinstance(module, ConformerMHSAQuant):
                pos_enc_norm_loss += torch.norm(module.mhsa.linear_pos.weight, p=2) ** 2
    run_ctx.mark_as_loss(name=f"pos_enc_l2", loss=pos_enc_norm_loss, scale=model.train_config.l2_norm)
...
def adc_init_hook(run_ctx, **kwargs):
    run_ctx.engine._model.prep_perfect_quant()

def adc_finish_hook(run_ctx, **kwargs):
        ...
        if "weight" in name and not "memristor" in name:
            ...
            if any(x in name for x in ["linear", "conv"]):
                if not any(x in name for x in ["frontend", "final"]):
                    assert False, (name, "There should not be a non memristor weight here")
    print("Save model under converted_model.pt")
    torch.save({"model": run_ctx.engine._model.state_dict(), "epoch": 500, "step": run_ctx.engine._train_step},
        "converted_model.pt")

def adc_step(*, model: Model, data, run_ctx, **kwargs):
    raw_audio = data["raw_audio"]  # [B, T', F]
    raw_audio_len = data["raw_audio:size1"]  # [B]
