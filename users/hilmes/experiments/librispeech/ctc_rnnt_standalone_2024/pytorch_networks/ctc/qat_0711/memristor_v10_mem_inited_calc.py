...
from .memristor_v10_modules_mem_inited_calc import ActivationQuantizer, QuantizedMultiheadAttention
...
def compute_stats_finish_hook(run_ctx, **kwargs):
    ...
    for i in range(len(run_ctx.engine._model.conformer.module_list)):
        for layer in run_ctx.engine._model.conformer.module_list[i].module_list:
            if isinstance(layer, ConformerMHSAQuant):
                dc["number_positions"][i] = layer.mhsa.linear_pos.converter.number_positions
                dc["number_clipped"][i] = layer.mhsa.linear_pos.converter.number_clipped
                dc["number_clipped_larger"][i] = layer.mhsa.linear_pos.converter.number_clipped_larger
    import pickle as pkl
    with open("num_clipped.pkl", "wb") as f:
        pkl.dump(dc, f)
