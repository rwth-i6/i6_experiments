...
fast variant: identical to memristor_v11_mem_inited, but enables the SynaptogenML
opt-in fast inference path (synaptogen_ml.set_fast_inference) at import time.

seeded variant: additionally fixes the RNG seed at import time so the standard and
fast paths draw identical noise streams and can be compared directly.
"""

# Enable the SynaptogenML opt-in fast inference path for this forward run.
# Imported name mirrors the try/except used by the memristor layers below
# (torch_memristor if installed, otherwise the synaptogen_ml checkout on sys.path).
try:
...
# Fixed seed so the standard and fast paths draw identical noise streams.
import numpy as _np
import torch as _torch

_np.random.seed(42)
_torch.manual_seed(42)  # also seeds all CUDA devices

# RETURNN does not log GPU memory in forward mode (torch_log_memory_usage is a
# training-loop feature), so report the peak ourselves at process exit; the
# monitoring loop parses this token from the job log.
import atexit as _atexit


def _report_peak_cuda_mem():
    try:
        if _torch.cuda.is_available() and _torch.cuda.is_initialized():
            peak_gb = _torch.cuda.max_memory_allocated() / 1024**3
            print(f"SYN_PEAK_CUDA_MEM_GB: {peak_gb:.2f}", flush=True)
    except Exception:
        pass


_atexit.register(_report_peak_cuda_mem)
...
from .memristor_v11_modules_mem_inited import ActivationQuantizer, QuantizedMultiheadAttention
    ...
    for i in range(len(run_ctx.engine._model.conformer.module_list)):
        for layer in run_ctx.engine._model.conformer.module_list[i].module_list:
            if isinstance(layer, ConformerMHSAQuant):
                ...
                # print(f"Quantile 0.25, 0.75, 0.9, 0.99", torch.quantile(stack, torch.tensor([0.25, 0.75, 0.9, 0.99])))
