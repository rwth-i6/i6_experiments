...
no_outq variant: identical to memristor_v11_modules_mem_inited, but the activation
output quantizers after the memristor layers (*_out_quant) are nn.Identity() as well,
like the input quantizers already are. Recognition-only: loads the converted_model.pt
produced with memristor_v11; the out-quant observer keys in that checkpoint are
tolerated as unexpected keys (checkpoint load is non-strict).
...
from .memristor_v11_cfg import QuantizedConformerMHSARelPosV1Config
import math
