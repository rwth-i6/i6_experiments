...
from .memristor_v7_modules_mem_inited import LinearQuant, ActivationQuantizer, QuantizedMultiheadAttention, Conv1dQuant
...
class Model(torch.nn.Module):
    ...
    def forward(
        ...
        """
        assert self.conformer.module_list[0].conv.pointwise_conv1.bias is not None
        assert self.conformer.module_list[0].conv.depthwise_conv.bias is not None
    ...
    def prep_quant(self):
        ...
        if self.train_config.quantize_output is True:
            self.final_linear.initialized = True
