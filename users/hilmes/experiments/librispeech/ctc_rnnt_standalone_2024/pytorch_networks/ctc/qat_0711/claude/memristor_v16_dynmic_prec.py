...
from .memristor_v16_dynmic_prec_modules import (
    LinearQuant,
    ActivationQuantizer,
    QuantizedMultiheadAttention,
    Conv1dQuant,
    TiledWeightQuantizer,
    convert_linear_to_memristor,
)
...
class ConformerPositionwiseFeedForwardQuant(nn.Module):
    ...
    def __init__(self, cfg: ConformerPositionwiseFeedForwardQuantV4Config):
        ...
        lin_1_prec, lin_2_prec = split_ff_spec(cfg.weight_bit_prec)
        self.linear_ff = LinearQuant(
            in_features=cfg.input_dim,
            out_features=cfg.hidden_dim,
            weight_bit_prec=lin_1_prec,
    ...
    def prep_quant(self):
        self.linear_ff = convert_linear_to_memristor(
        ...
        self.linear_out = convert_linear_to_memristor(
    ...
    def prep_torch_quant(self):
        for lin in [self.linear_ff, self.linear_out]:
            if isinstance(lin.weight_quantizer, TiledWeightQuantizer):
                raise NotImplementedError("prep_torch_quant does not support per-tile mixed precision")
...
class ConformerConvolutionQuant(nn.Module):
    ...
    def __init__(self, model_cfg: ConformerConvolutionQuantV4Config):
        ...
        pconv_1_prec, pconv_2_prec, dconv_prec = split_conv_spec(model_cfg.weight_bit_prec)
        self.pointwise_conv1 = LinearQuant(
            in_features=model_cfg.channels,
            out_features=2 * model_cfg.channels,
            weight_bit_prec=pconv_1_prec,
    ...
    def prep_quant(self):
        ...
        self.pointwise_conv1 = convert_linear_to_memristor(
        ...
        self.pointwise_conv2 = convert_linear_to_memristor(
    ...
    def prep_torch_quant(self):
        for lin in [self.pointwise_conv1, self.pointwise_conv2]:
            if isinstance(lin.weight_quantizer, TiledWeightQuantizer):
                raise NotImplementedError("prep_torch_quant does not support per-tile mixed precision")
...
class Model(torch.nn.Module):
    def __init__(self, model_config_dict, **kwargs):
        ...
        def _make_block_cfg(weight_bit_prec):
            ...
            w_i_prec, w_o_prec, learn_emb_prec = split_mhsa_spec(
                mhsa_prec, with_linear_pos=self.train_config.pos_emb_config.with_linear_pos
            )
            ...
            return ConformerBlockQuantV1Config(
                ...
                mhsa_cfg=QuantizedConformerMHSARelPosV1Config(
                    ...
                    bit_prec_W_i=w_i_prec,
                    bit_prec_W_o=w_o_prec,
                    moving_average=self.train_config.moving_average,
                    ...
                    correction_settings=self.train_config.correction_settings,
                    with_bias=True,
                    bit_prec_learn_emb=learn_emb_prec,
        ...
        if self.train_config.quantize_output is True:
            assert isinstance(self.train_config.weight_bit_prec, (int, float)), (
            ...
            self.lin_out = [
                LinearQuant(
                    ...
                    weight_quant_method=self.train_config.weight_quant_method,
                    bias=True,
                    noise_config=self.train_config.weight_noise,
                    pruning_config=self.train_config.weight_pruning,
    ...
    def prep_quant(self):
        ...
        if self.train_config.quantize_output is True:
            mem_lin = convert_linear_to_memristor(
    ...
    def prep_torch_quant(self):
        ...
        for module in self.modules():
            if isinstance(module, TiledWeightQuantizer):
                raise NotImplementedError("prep_torch_quant does not support per-tile mixed precision")
        if self.train_config.quantize_output is True:
            print(self.lin_out)
...
def prune_init_hook(run_ctx, **kwargs):
    model = run_ctx.engine._model
    with torch.no_grad():
        for module in model.modules():
            if hasattr(module, "pruning_config") and module.pruning_config is not None:
                module.weight.data = module.pruning_config.apply(module.weight.data, training=False)
    torch.save(
        {"model": model.state_dict(), "epoch": 500, "step": run_ctx.engine._train_step},
        "pruned_model.pt",
    )


def prune_finish_hook(run_ctx, **kwargs):
    pass


def prune_step(*, model: Model, data, run_ctx, **kwargs):
    pass
