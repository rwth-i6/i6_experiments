...
def get_librispeech_bpe(corpus_key, bpe_size, unk_label="<unk>", output_prefix=""):
  ...
  subword_nmt_repo = get_returnn_subword_nmt(
    commit_hash=subword_nmt_commit_hash, output_prefix=output_prefix
  )
...
def get_mixed_cv_segments(output_prefix="datasets"):
  ...
  dev_cv_segments = PipelineJob(
    [dev_clean_subset, dev_other_subset], [], mini_task=True
  ).out
...
def get_librispeech_tts_segments():
  ...
  ls100_segments = SegmentCorpusJob(
    bliss_corpus_dict["train-clean-100"], 1
  ).out_single_segment_files[1]
  generate_tts_segments_job = GenerateBalancedSpeakerDevSegmentFileJob(
    segment_file=ls100_segments, dev_segments_per_speaker=4
  )
  return (
    generate_tts_segments_job.out_train_segments,
    generate_tts_segments_job.out_dev_segments,
  )
...
def get_ls_train_clean_500_tts_silencepreprocessed(alias_path=""):
  """
  This returns the silence-preprocessed version of LibriSpeech train-clean-100 with
  FFmpeg silence preprocessing using a threshold of -50dB for silence
  :return:
  ...
  train_360_corpus = corpus_object_dict["train-other-500"]
