...
class DistributeSpeakerEmbeddings(Job):
  ...
  def __init__(self,  speaker_embedding_hdf, bliss_corpus=None, text_file=None, options=None, use_full_seq_name=True):
    """
    ...
    assert self.bliss_corpus or self.text_file and not (self.bliss_corpus and self.text_file)
  ...
  def run(self):
    ...
    if self.bliss_corpus:
      ...
      pipe = subprocess.Popen(["zcat", "-f", self.text_file.get_path()], stdout=subprocess.PIPE)
...
class SpeakerLabelHDFFromBliss(Job):
  ...
  def run(self):
    ...
    hdf_writer = SimpleHDFWriter(tk.uncached_path(self.out), dim=num_speakers, ndim=1)
