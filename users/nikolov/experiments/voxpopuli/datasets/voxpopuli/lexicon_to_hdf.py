...
from i6_experiments.users.nikolov.experiments.voxpopuli.datasets.voxpopuli.lexicon import get_bliss_lexicon, get_bliss_lang_lexicons
...
def vocab_check(hdf_path):
    ...
    vocabs = ['all.vocab',  'bpe_256.vocab',  'bpe_4989.vocab',  'bpe_512.vocab',  'bpe_5158.vocab', 'corp_mc_5019.vocab',  'corp_mc_5021.vocab',  'en_es.vocab',  'mc_4943.vocab']
    for v in vocabs:
        vocab = Vocabulary.create_vocab(vocab_file=f"/work/asr3/jxu/hiwis/nikolov/multilang_0325/vocab/{v}", unknown_label=None)
...
def lexicon_to_hdf(
    ...
    if len(splits)<3:
        ...
        for lang in langs:
            for split in splits:
                if not corpus_set:
                    corpus_path = tk.Path(f"/work/asr3/jxu/hiwis/nikolov/multilang_0325/corpus/asr_{lang}.{split}.corpus.xml.gz")
...
def lexicon_to_hdf_miami(
    ...
    for dataset in sets:
        for split in splits:
            j = BlissCorpusToTargetHdfJob(
                            bliss_corpus=ReplaceUnknownWordsJob(tk.Path(corpus_path + f"miami.{dataset}.corpus.xml.gz"), lexicon_file=lexicon_path).out_corpus_file,
		    				bliss_lexicon=lexicon_path,
		    				returnn_root=MINI_RETURNN_ROOT,
		    )
            tk.register_output(
