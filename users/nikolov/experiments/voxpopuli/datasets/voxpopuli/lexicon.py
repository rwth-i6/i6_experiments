...
def get_bliss_lexicon(
    ...
    bpe_train_job = ReturnnTrainBpeJob(text_file=full_text_path, bpe_size=bpe_size, prefix=False, subword_nmt_repo=SUBWORD_NMT_REPO)
    ...
    tk.register_output(
                            f"voxpopuli_asr_legacy_{bpe_size}/bpe",
    ...
    tk.register_output(
                            f"voxpopuli_asr_legacy_{bpe_size}/lexicon",
...
def get_bliss_lexicons_from_bpe(
    ...
    mapped_raw_lexicon_file = tk.Path(raw_lexicon_path + f"/lex.txt")
    mapped_text_file = tk.Path(full_text_path + f"/all.txt")
    bliss_lexicon = LexiconFromTextFileJob(mapped_raw_lexicon_file, compressed=True)
    bliss_lexicon.add_alias(os.path.join(subdir_prefix, f"create_lexicon_qll_job"))
    bpe_lexicon = CreateBPELexiconJob(
        base_lexicon_path=bliss_lexicon.out_bliss_lexicon,
        bpe_codes=bpe_codes,
        bpe_vocab=bpe_vocab,
    ...
    bpe_lexicon.add_alias(os.path.join(subdir_prefix, f"create_bpe_qll_lexicon_job"))
...
def get_bliss_lang_lexicons(
    ...
    for lang in langs:
        ...
        bpe_train_job = ReturnnTrainBpeJob(text_file=mapped_text_file, bpe_size=bpe_size['base'], prefix=prefix, subword_nmt_repo=SUBWORD_NMT_REPO)
...
def get_unique_bpe_bliss_lexicons(
    ...
    for lang in langs:
        ...
        bpe_train_job = ReturnnTrainBpeJob(text_file=mapped_text_file, bpe_size=bpe_size[lang], prefix=prefix, subword_nmt_repo=SUBWORD_NMT_REPO)
...
def get_text_lexicon(prefix: str, bpe_size: dict[str, int], add_prefix: bool, lexicon_path: str = None) -> tk.Path:
    ...
    elif 'concat' in bpe_size :
        bliss_lex = get_bliss_lexicons_from_bpe(
                subdir_prefix = prefix,
                raw_lexicon_path = "/work/asr3/jxu/hiwis/nikolov/multilang_0325/vocab",
                full_text_path = "/work/asr3/jxu/hiwis/nikolov/multilang_0325/vocab",
                bpe_size={'base': bpe_size['concat']},
                bpe_codes=tk.Path("/work/asr3/jxu/hiwis/nikolov/multilang_0325/work/i6_core/bpe/train/ReturnnTrainBpeJob.dpPG5jgFCRfM/output/bpe.codes"),
                bpe_vocab=tk.Path("/work/asr3/jxu/hiwis/nikolov/multilang_0325/vocab/bpe_5158.vocab"),
                add_prefix=add_prefix)
    else:
