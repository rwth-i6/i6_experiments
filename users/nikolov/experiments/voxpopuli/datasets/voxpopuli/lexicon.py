...
def get_bliss_lexicon(
    ...
    bpe_train_job.add_alias(os.path.join(subdir_prefix, "train_bpe_job"))
    tk.register_output(
                            f"voxpopuli_asr_{bpe_size}/bpe",
    ...
    tk.register_output(
                            f"voxpopuli_asr_{bpe_size}/lexicon",
...
def get_bliss_lang_lexicons(
    ...
    for lang in langs:
        ...
        bliss_lexicon = LexiconFromTextFileJob(mapped_raw_lexicon_file, compressed=True)
        bliss_lexicon.add_alias(os.path.join(subdir_prefix, f"create_lexicon_prefix_{lang}_job"))
        ...
        bpe_train_job = ReturnnTrainBpeJob(text_file=mapped_text_file, bpe_size=bpe_size, prefix=prefix, subword_nmt_repo=SUBWORD_NMT_REPO)

        bpe_train_job.add_alias(os.path.join(subdir_prefix, f"train_bpe_prefix_{lang}_lexicon_job"))
        ...
        bpe_lexicons[lang].add_alias(os.path.join(subdir_prefix, f"create_bpe_prefix_{lang}_lexicon_job"))
    ...
    merge_lexicon_job.add_alias(os.path.join(subdir_prefix, "merge_prefix_lexicon_job"))
    tk.register_output(
                            f"voxpopuli_asr/lexicon_prefix_ordered_{bpe_size}",
                            merge_lexicon_job.out_bliss_lexicon,
    ) 
...
def get_text_lexicon(prefix: str, bpe_size: int, add_prefix: bool) -> tk.Path:
    """
