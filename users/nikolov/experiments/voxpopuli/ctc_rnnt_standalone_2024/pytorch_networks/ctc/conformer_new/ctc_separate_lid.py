...
class Model(torch.nn.Module):
    def __init__(self, model_config_dict, **kwargs):
        ...
        self.num_langs = 16  # number of language heads
        out_dim = self.cfg.label_target_size + 1  # + CTC blank∏

        self.final_linears = nn.ModuleList([
            nn.Linear(conformer_size, out_dim) for _ in range(self.num_langs)
        ])
    ...
    def forward(
            ...
            language_id: torch.Tensor,  # [B] long
    ):
        """
        ...
        language_id = language_id.to(device=conformer_out.device, dtype=torch.long)
        
        B, T, H = conformer_out.shape
        out_dim = self.cfg.label_target_size + 1
        
        # Stack head params: W [L, out_dim, H], b [L, out_dim]
        W = torch.stack([lin.weight for lin in self.final_linears], dim=0)
        b = torch.stack([lin.bias   for lin in self.final_linears], dim=0)
        
        # Select params per sample: W_sel [B, out_dim, H], b_sel [B, out_dim]
        W_sel = W.index_select(0, language_id)
        b_sel = b.index_select(0, language_id)
        
        # Batched linear: logits [B, T, out_dim]
        # (B,T,H) x (B,out_dim,H) -> (B,T,out_dim)
        logits = torch.einsum("bth,boh->bto", conformer_out, W_sel) + b_sel.unsqueeze(1)
