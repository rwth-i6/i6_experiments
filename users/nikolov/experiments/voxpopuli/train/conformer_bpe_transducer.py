...
def conformer_rnnt_baseline(
    ...
    #import sys
    #sys.path.append('/u/kaloyan.nikolov/git/returnn')
...
def conformer_ctc_noreturnn(
    ...
    # ---------------------------------------------------------------------------------------------------------------- #
    def run_exp(ft_name, datasets, train_args, search_args=None, recog_args=None, num_epochs=600,
                decoder="ctc.decoder.flashlight_ctc_v1_onnx_v2", with_prior=False, evaluate_epoch=None, recog_mem:int = 10, vocab_name:str=None):
        #training_name = "/".join(ft_name.split("/")[:-1])
        search_args = search_args if search_args is not None else {}
    ...
    for peak_lr in learning_rates:
        ...
        if len(lang_list) < 16:
            ...
            datasets=train_data, train_args=train_args, search_args=search_args, recog_args=recog_args, with_prior=False, evaluate_epoch=eval_epoch, vocab_name=vocab_name)
            
            train_job.rqmt["gpu_mem"] = gpu_mem
            tk.register_output(f"output/{vocab_name}/{'_'.join(lang_list)}/ctc/{batch_size}_{bpe_size}_lr{peak_lr}/learning_rates", train_job.out_learning_rates)
            tk.register_output(f"output/{vocab_name}/{'_'.join(lang_list)}/ctc/{batch_size}_{bpe_size}_lr{peak_lr}/out_model_dir", train_job.out_model_dir)
            ...
            datasets=train_data, train_args=train_args, search_args=search_args, recog_args=recog_args, with_prior=False, evaluate_epoch=eval_epoch, recog_mem=recog_mem, vocab_name=vocab_name)
            
            train_job.rqmt["gpu_mem"] = gpu_mem
            tk.register_output(f"output/ctc/{vocab_name}/{batch_size}_{bpe_size}_lr{peak_lr}/learning_rates", train_job.out_learning_rates)
            tk.register_output(f"output/ctc/{vocab_name}/{batch_size}_{bpe_size}_lr{peak_lr}/out_model_dir", train_job.out_model_dir)
