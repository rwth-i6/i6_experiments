...
def eow_phon_ls960_1023_base():
    prefix_name = "example_setups/librispeech/ctc_rnnt_standalone_2024/ls960_ctc_eow_phon/baseline_no_center"
    ...
    from ...pytorch_networks.ctc.decoder.flashlight_ctc_v1_onnx import DecoderConfig
    ...
    from ...pytorch_networks.ctc.conformer_new.i6modelsV1_VGG4LayerActFrontendV1_v6_cfg import (
        SpecaugConfig,
    ...
    model_custom_config = ModelConfig(
        ...
        module_list=["ff", "conv", "custom-mha", "ff"],
        module_scales=[0.5, 1.0, 1.0, 0.5],
    )

    model_torch_config = ModelConfig(
        feature_extraction_config=fe_config,
        frontend_config=frontend_config,
        specaug_config=specaug_config,
        label_target_size=vocab_size_without_blank,
    ...
    network_module = "ctc.conformer_new.i6modelsV1_VGG4LayerActFrontendV1_v6"
    train_args = {
        "config": train_config_24gbgpu_amp,
        "network_module": network_module,
        "net_args": {"model_config_dict": asdict(model_torch_config)},
        "debug": False,
    }

    training_name = prefix_name + "/" + network_module + ".torch_mha"  + ".512dim_sub4_24gbgpu_50eps"
    ...
    tune_and_evaluate_helper(
        training_name+"/onnx_export", asr_model_onnx, default_decoder_config, lm_scales=[2.3, 2.5, 2.7], prior_scales=[0.2, 0.3, 0.4], decoder_module='ctc.decoder.flashlight_ctc_v1_onnx_v2'
    )
    ...
    train_args = {
        "config": train_config_24gbgpu_amp,
        "network_module": network_module,
        "net_args": {"model_config_dict": asdict(model_custom_config)},
        "debug": False,
    }

    training_name = prefix_name + "/" + network_module + ".custom_mha" + ".512dim_sub4_24gbgpu_50eps"
    train_job = training(training_name, train_data, train_args, num_epochs=500, **default_returnn)
    train_job.rqmt["gpu_mem"] = 24
    asr_model = prepare_asr_model(
        training_name, train_job, train_args, with_prior=True, datasets=train_data, get_specific_checkpoint=500
