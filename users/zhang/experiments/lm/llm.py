...
class HuggingFaceLmPerplexityJob(Job):
    ...
    def __init__(self, *, model_dir: tk.Path, text_file: List[tk.Path], batch_size: int = None,
                 llm_name: str, lower_case:bool = True, eos_symbol: str = "", version:int = 3):
        super().__init__()
...
class HuggingFaceLmRescoringJob(Job):
    ...
    def run(self):
        ...
        for seq_tag, n_best in d_rec.items():
            ...
            hyps_buffer = []
            scores_buffer = []
            original_score_buffer = []
            for original_score, hyp in n_best:
                if self.prompt: #TODO: Now only for a same fixed shared context
                ...
                if len(batch_lines) == self.batch_size:
                    ...
                    if self.prompt:
                        ...
                        input_ids = torch.cat([enc_prompt["input_ids"], enc_hyp["input_ids"]], dim=1).to(device)  # [B,M+N]
                    ...
                    with torch.no_grad():
                        ...
                        if self.prompt:
                            ...
                            logits = logits[:, -hyp_input_ids.shape[1] - 1: -1, :]  # [B,N,V]
                        # shift to consider the correct probs seq:
                        # with P(... | prompt) and without P(... | ... last_word)
                        log_probs = torch.log_softmax(logits, dim=-1)

                        llm_scores = torch.gather(
                            log_probs, dim=-1, index=gather_ids
                        ).squeeze()
                        # attention_mask is 1 for real tokens, 0 for pads
                        llm_scores = llm_scores * enc_hyp["attention_mask"][:,1:].to(device)  # [B, T]
                        llm_scores = llm_scores.sum(dim=1)  # [B]

                        # Not sure if normalization over lengths is needed
                        #lengths = enc_hyp["attention_mask"].sum(dim=1)  # number of real tokens per sample
                        #llm_scores = total_logprob_per_example / lengths

                    scores_buffer += llm_scores.tolist()
                    batch_tok_count = int(attention_mask.sum().item())
                    total_tokens += batch_tok_count
                    ...
                    batch_lines = []  # clear for next batch
                hyps_buffer.append(hyp)
                original_score_buffer.append(original_score)


            if batch_lines: # Process any leftover lines (if total lines % batch_size != 0)
                batch_count += 1
                enc_hyp = tokenizer(
                    batch_lines,
                    return_tensors="pt",
                ...
                if self.prompt:
                    ...
                    input_ids = torch.cat([enc_prompt["input_ids"], enc_hyp["input_ids"]], dim=1).to(device)  # [B,M+N]
                    attention_mask = torch.cat([enc_prompt["attention_mask"], enc_hyp["attention_mask"]], dim=1).to(
                        device)
                ...
                with torch.no_grad():
                    ...
                    # attention_mask is 1 for real tokens, 0 for pads
                    llm_scores = llm_scores * enc_hyp["attention_mask"].to(device)  # [B, T]
                    llm_scores = llm_scores.sum(dim=1)  # [B]
                ...
                batch_tok_count = int(attention_mask.sum().item())
                total_tokens += batch_tok_count
            reorder = [*zip(scores_buffer, original_score_buffer, hyps_buffer)]
            reorder.sort(key=lambda x:x[1]+self.scale*x[0],reverse=True)
            # Only add top1 hyp
            out_file.write(f"  ({reorder[0][1]!r}, {reorder[0][2]!r}),\n")
            out_file.write("],\n")

        # Explicit cleanup to avoid stuck in CG state
        del model
        del tokenizer
        torch.cuda.empty_cache()
        import gc
...
def py():
    ...
    for llm_name in ["Llama-3.2-1B",
        ...
        resco_job.add_alias("lm/" + llm_name + "/rescoring_"  + f"w{llm_scale}".replace(".","") + ("low" if lower_case else "") + "period")
        tk.register_output(
