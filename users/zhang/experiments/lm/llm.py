...
LLM_rqmt = {"meta-llama/Llama-3.2-1B": {"time": 2, "cpu": 1, "mem": 16, "gpu": 1, "gpu_mem": 48},
            "meta-llama/Llama-3.1-8B": {"time": 2, "cpu": 1, "mem": 40, "gpu": 1, "gpu_mem": 80},
            ...
            "microsoft/phi-4":{"time": 2, "cpu": 1, "mem": 65, "gpu": 1, "gpu_mem": 141},
            }




def get_raw_text_func_ES_spm(seq:str)-> str:
    ...
    seq = seq.split()
    assert "<blank>" not in seq, f"blank should not be here!"
    if len(seq) == 0:
        return " ".join(seq)
...
def get_raw_text_func_bpe(seq:str)-> str:
    seq = seq.split()
    assert "<blank>" not in seq, f"blank should not be here!"
    if len(seq) == 0:
        return " ".join(seq)
...
def get_prompt(LLM_FXIED_CTX, LLM_FXIED_CTX_SIZE, task_name:str = "LBS"):
    prompt = None
    if LLM_FXIED_CTX:
        prompt = get_fix_context_file(LLM_FXIED_CTX_SIZE, task_name=task_name)
...
class HuggingFaceLmPerplexityJobV3(Job):
    """Compute perplexity for a HuggingFace Transformer LLM model on given text dict corpus.
    Also output sentence wise score as text dict
    """

    def __init__(self, *, model_dir: tk.Path, prompt: [List[str] | tk.Path] = None, text_file: List[tk.Path], batch_size: int = None,
                 score_mode: str = "nll", lower_case:bool = False, context_len_limit: int = None, eos_symbol: str = "", word_ppl: bool = False,
                 add_eos_to_completion: bool = True, use_prev_context: bool = False, version:int = 10):
        super().__init__()
        self.model_dir = model_dir
        self.text_file = text_file
        ...
        self.word_ppl = word_ppl
        self.out_ppl = self.output_path("ppl")
        self.out_scores_dict = self.output_path("scores_dict.py.gz")
        self.rqmt = {"time": 4, "cpu": 1, "mem": 8 + self.batch_size//2, "gpu": 1}
    ...
    def get_raw_text(self, text:str):
        """Not totally safe:
        Will Return as is If not BPE or SPM sequence"""
        assert isinstance(text,str)
        if "@@" in text:
            return raw_text_from_bpe_seq(text)
        elif "▁" in text:
            return get_raw_text_func_ES_spm(text)
        else:
            return text

    def _score_batch(self, batch_lines, batch_prompt, tokenizer, model, device):
        """A simpler version, not used for now:
         1. will pad between prompt and completion
         2. will skip scoring of first token if there is no BOS added by default tokenizer behaviour"""
    ...
    def run(self):
        ...
        def _score_batch(batch_lines, batch_prompt, tokenizer, model, device):
            ...
            """
            Manual log-softmax + gather scorer that:
              - Use EOS as BOS if there is no one exist (as set during initialisation),
              - When no prompt given, Add BOS anyway even if tokenizer does not add it.
              - concatenates prompt+hyp per example (no interior PADs),
              #- Optionally provide position id for each real token, #(Seems give minor impact so commented out for now in following)
              - pads once across the batch,
              - indexes the correct time steps for each hypothesis token,
              - sums exact token log-probs and returns (nll, token_count, total_bpe_tokens).
            """
            ...
            enc_hyp = tokenizer(
                ...
                padding=False,
                truncation=True,
                add_special_tokens=False,  # never add bos here, handle the BOS in prompt configuration below
                max_length=tokenizer.model_max_length,
            )
            enc_prm = tokenizer(
                batch_prompt,
            ...
            # 10) Per-sample sums and counts
            per_sample_sum_logp = tok_logp.to(torch.float64).sum(dim=1).cpu()  # [B]
            per_sample_tok_cnt = hyp_mask.sum(dim=1).to(torch.int64).cpu()  # [B]

            nll_list = [float(v) for v in per_sample_sum_logp.tolist()]  # sum(log p) per sample
            tok_list = [int(v) for v in per_sample_tok_cnt.tolist()]  # token count per sample

            # Diagnostics: per-sample raw BPE token lengths for hypotheses
            bpe_tok_list = [len(x) for x in enc_hyp["input_ids"]]

            # Batch totals
            nll_sum = float(per_sample_sum_logp.sum().item())
            tok_sum = int(per_sample_tok_cnt.sum().item())
            bpe_tok_sum = int(sum(bpe_tok_list))

            # Sanity guard
            assert logits_hyp.shape[:2] == hyp_ids.shape[:2], (logits_hyp.shape, hyp_ids.shape)

            return nll_list, tok_list, nll_sum, tok_sum

        def safe_score_batch(batch_lines, batch_prompt, tokenizer, model, device):
            """
            Try to handle CUDA OOM by reducing batch size:
            Returns (total_nll: torch.Tensor on device, total_tok_count: int).
            Falls back by halving the batch on CUDA OOM until batch size = 1.
            """
            import gc
            ...
            def _helper(lines, prompts):
                try:
                    ...
                    if _is_oom(e):
                        ...
                        print(f"\nOOM: Split to {mid}/{self.batch_size}")
                        _report_dev_memory_stats()
                        nll_list1, tok_list1, nll1, tok1 = _helper(lines[:mid], prompts[:mid])
                        print(f"\nOOM: Second half {mid}/{self.batch_size}")
                        torch.cuda.empty_cache()
                        gc.collect()
                        nll_list2, tok_list2, nll2, tok2 = _helper(lines[mid:], prompts[mid:])
                        # Sum results; nll is a scalar tensor, tok_count is int or scalar tensor
                        total_nll = nll1 + nll2
                        total_nll_list = nll_list1 + nll_list2
                        total_tok = tok1 + tok2
                        total_tok_list = tok_list1 + tok_list2
                        return total_nll_list, total_tok_list, total_nll, total_tok
                    else:
                        # Not an OOM; bubble up
                        raise
        ...
        # { seq_tag: (score_value, normalized_text) }
        record_scores = {}
        ...
        for seq_tag, raw_line in d_rec.items():
            ...
            if not line:
                continue
            current_record = extract_record_id(seq_tag)
            norm_text = self.get_raw_text(line)  # ### NEW: normalized for output dict
            line = norm_text
            total_word_tokens += len(line.split()) + (1 if self.add_eos_to_completion else 0)

            # Push line + eos to scoring batch; keep tag and normalized text in parallel
            batch_lines.append(line + eos_symbol)
            batch_tags.append((seq_tag, norm_text))  # ### NEW
            ...
            if self.use_prev_context and current_record != last_record:
                ...
                if len(batch_lines) == 1:
                    # single item: get detailed per-line to fill record_scores
                    batch_count += 1
                    nll_list, tok_list, nll_sum, tok_sum = safe_score_batch(
                        batch_lines, batch_prompt, tokenizer, model, device
                    )
                    total_logprob += nll_sum
                    total_tokens += tok_sum

                    # write per-sentence records
                    for (tag, norm), nll_i, tok_i in zip(batch_tags, nll_list, tok_list):
                        if score_mode == "ppl":
                            score_val = float(math.exp(nll_i / max(tok_i, 1)))
                        elif score_mode == "nll":
                            score_val = float(nll_i)
                        else:
                            # In case you also want logprob, expose another mode: logprob = -nll
                            score_val = float(nll_i)
                        record_scores[tag] = (score_val, norm)

                else:
                    # multi: score all except boundary last, then score last alone
                    batch_count += 2
                    core_lines = batch_lines[:-1]
                    core_tags = batch_tags[:-1]
                    last_line = [batch_lines[-1]]
                    last_tag = [batch_tags[-1]]

                    # core batch detailed
                    nll_list, tok_list, nll_sum, tok_sum = safe_score_batch(
                        core_lines, batch_prompt, tokenizer, model, device
                    )
                    total_logprob += nll_sum
                    total_tokens += tok_sum
                    for (tag, norm), nll_i, tok_i in zip(core_tags, nll_list, tok_list):
                        score_val = float(math.exp(nll_i / max(tok_i, 1))) if score_mode == "ppl" else float(nll_i)
                        record_scores[tag] = (score_val, norm)

                    # boundary item alone (no prompt)
                    *_, nll_b, tok_b = _score_batch(last_line, [], tokenizer, model, device)
                    total_logprob += nll_b
                    total_tokens += tok_b
                    tag, norm = last_tag[0]
                    score_val = float(math.exp(nll_b / max(tok_b, 1))) if score_mode == "ppl" else float(nll_b)
                    record_scores[tag] = (score_val, norm)

                batch_lines, batch_prompt, batch_tags = [], [], []  # clear for next batch
                print(f"Clear context for record {last_record}")

            if self.prompt and not boundary:
                batch_prompt.append(self.prompt.lower() if self.lower_case else self.prompt)
            ...
            if len(batch_lines) == self.batch_size:
                batch_count += 1
                # get detailed to fill record_scores
                nll_list, tok_list, nll_sum, tok_sum = safe_score_batch(
                    batch_lines, batch_prompt, tokenizer, model, device
                )
                total_logprob += nll_sum
                ...
                batch_lines, batch_prompt, batch_tags = [], [], []

            if self.use_prev_context:
                if ctx_rec is not None:
                    ctx = self.get_raw_text(ctx_rec[seq_tag])
                    print(f"Ctx for {seq_tag}: {ctx}")
                else:
                    ctx = line
                    print(f"Transcription for {seq_tag}: {ctx}")
        ...
        with open(self.out_scores_dict, "w") as out:
            json.dump(record_scores, out, indent=2)
...
def raw_text_from_bpe_seq(seq:str):
    return seq.replace("@@ ","").replace(" <s>", "")
...
def gt_ppl_check(task_name: str, search_error_job_path: str, model_id: str, recog_out_path:str):
    w_gt_recog_out = search_error_job_path + "/output/search_errors_seq_dict.py"
    missing_gt_dict = tk.Path(search_error_job_path + "/output/missing_gt_dict.json")
    from i6_experiments.users.zhang._i6_core.returnn.search import SearchTakeBestJob
    from i6_experiments.users.zhang.utils.search_error import SearchErrorDiagnosticsJob, DiagnosticsConfig
    recog_out = SearchTakeBestJob(search_py_output=tk.Path(recog_out_path)).out_best_search_results
    name = os.path.basename(model_id)
    batch_size = 60
    model = DownloadHuggingFaceRepoJob(model_id=model_id)
    hyp_score_job = HuggingFaceLmPerplexityJobV3(
                    model_dir=model.out_hub_cache_dir,
                    text_file=[recog_out], # get_test_corpus_text(keys=[ds_name])
                    ...
                    context_len_limit=100,
                    add_eos_to_completion=True
                )
    w_gt_score_job = HuggingFaceLmPerplexityJobV3(
                    model_dir=model.out_hub_cache_dir,
                    text_file=[tk.Path(w_gt_recog_out)], # get_test_corpus_text(keys=[ds_name])
                    batch_size=batch_size,#max(batch_size//2,1) + 2,
                    lower_case=USE_LOWER_CASE,
                    word_ppl=True,
                    prompt=recog_out,
    ...
    w_gt_score_job.rqmt.update(LLM_rqmt[model_id])
    hyp_score_job.rqmt.update(LLM_rqmt[model_id])
    tk.register_output(f"test/{model_id}_{task_name}_hyp_ppl", hyp_score_job.out_ppl)
    tk.register_output(f"test/{model_id}_{task_name}_w_gt_ppl", w_gt_score_job.out_ppl)
    hyp_llm_scores = hyp_score_job.out_scores_dict
    w_gt_llm_scores = w_gt_score_job.out_scores_dict
    diag_job = SearchErrorDiagnosticsJob(gt_json=w_gt_llm_scores, hyp_json=hyp_llm_scores,
                                     missing_gt_json=missing_gt_dict, config=DiagnosticsConfig(dataset_hint="librispeech"))
    tk.register_output(f"test/{model_id}/{task_name}_SE_diagnotstic_stats", diag_job.out_json)
...
def ppl_stats():
    ...
    #ds_names = ["test-clean","test-other", "dev-clean", "dev-other"]
    ...
    ctx_order = [0,
                 ...
                 ] if dev else [0,50,80,100,200]
    ctx_order = [100]#[0,50,80,200]
    batch_sizes = [1, 10, 40]
...
def py():
    ...
    # task_name = "LBS"
    # search_error_job_path = "/nas/models/asr/hzhang/setups/2025-07-20--combined/work/i6_experiments/users/zhang/experiments/decoding/rescoring/RescoreSearchErrorJob.0z8W02YlOEsj"
    # recog_out_path = "/nas/models/asr/hzhang/setups/2025-07-20--combined/work/i6_experiments/users/zhang/experiments/decoding/lm_rescoring/LmRescoringJob.Q1P4HYSoCElF/output/output.py.gz"

    model_id = "microsoft/phi-4"
    task_name = "LBS"
    search_error_job_path = "/nas/models/asr/hzhang/setups/2025-07-20--combined/work/i6_experiments/users/zhang/experiments/decoding/rescoring/RescoreSearchErrorJob.OBjxUEwTqrxq"
    recog_out_path = "work/i6_experiments/users/zhang/experiments/decoding/lm_rescoring/LmRescoringJob.ZtYxoJdLVbXq/output/output.py.gz"
    gt_ppl_check(task_name=task_name,search_error_job_path=search_error_job_path,recog_out_path=recog_out_path,model_id=model_id)