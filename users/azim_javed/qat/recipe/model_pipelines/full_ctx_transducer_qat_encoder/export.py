__all__ = ["export_encoder", "export_scorer", "export_state_initializer", "export_state_updater"]

from i6_core.returnn import PtCheckpoint
from i6_core.returnn.config import CodeWrapper
from returnn.tensor.tensor_dict import TensorDict
from sisyphus import tk

from i6_experiments.common.setups.serialization import Import

from ..common.onnx_export import export_model as _export_model
from ..common.serializers import get_model_serializers
from .pytorch_modules import (
    LstmTransducerQATEncoderConfig,
    LstmTransducerQATEncoderEncoder,
    LstmTransducerQATEncoderRecogConfig,
    LstmTransducerQATEncoderScorer,
    LstmTransducerQATEncoderStateInitializer,
    LstmTransducerQATEncoderStateUpdater,
)

# -----------------------
# --- Export routines ---
# -----------------------


def export_scorer(model_config: LstmTransducerQATEncoderRecogConfig, checkpoint: PtCheckpoint) -> tk.Path:
    model_serializers = get_model_serializers(model_class=LstmTransducerQATEncoderScorer, model_config=model_config)

    return _export_model(
        model_serializers=model_serializers,
        forward_step_import=Import(
            f"{_scorer_forward_step.__module__}.{_scorer_forward_step.__name__}", import_as="forward_step"
        ),
        checkpoint=checkpoint,
        returnn_config_dict={
                "dim_enc_batch": CodeWrapper('Dim(dimension=1, name="enc_batch")'),
                "dim_enc_feature": CodeWrapper(f'Dim(dimension={model_config.enc_dim}, name="enc_feature")'),
                "dim_pred": CodeWrapper(f'Dim(dimension={model_config.pred_dim}, name="pred")'),
                "dim_target": CodeWrapper(f'Dim(dimension={model_config.target_size}, name="target")'),
                "extern_data": {
                    "encoder_state": {
                        "dim_tags": CodeWrapper("(dim_enc_batch, dim_enc_feature)"),
                        "dtype": "float32",
                    },
                    "lstm_out": {
                        "dim_tags": CodeWrapper(f'(batch_dim, dim_pred)'),
                        "dtype": "float32",
                    },
                },
                "model_outputs": {
                    "scores": {
                        "dim_tags": CodeWrapper(f'(batch_dim, dim_target)'),
                        "dtype": "float32",
                    },
                },
        },
        input_names=["encoder_state", "lstm_out"],
        output_names=["scores"],
        metadata={
            "lstm_out": "lstm_out",
        },
    )


def export_state_initializer(model_config: LstmTransducerQATEncoderConfig, checkpoint: PtCheckpoint) -> tk.Path:
    model_serializers = get_model_serializers(model_class=LstmTransducerQATEncoderStateInitializer, model_config=model_config)

    return _export_model(
        model_serializers=model_serializers,
        forward_step_import=Import(
            f"{_state_initializer_forward_step.__module__}.{_state_initializer_forward_step.__name__}",
            import_as="forward_step",
        ),
        checkpoint=checkpoint,
        returnn_config_dict={
            "dim_state_batch": CodeWrapper('Dim(dimension=1, name="state_batch")'),
            "dim_pred_layers": CodeWrapper(f'Dim(dimension={model_config.pred_num_layers}, name="pred_layers")'),
            "dim_pred": CodeWrapper(f'Dim(dimension={model_config.pred_dim}, name="pred")'),
            "extern_data": {},
            "model_outputs": {
                "lstm_c": {
                    "dim_tags": CodeWrapper("(dim_state_batch, dim_pred_layers, dim_pred)"),
                    "dtype": "float32",
                },
                "lstm_h": {
                    "dim_tags": CodeWrapper("(dim_state_batch, dim_pred_layers, dim_pred)"),
                    "dtype": "float32",
                },
                "lstm_out": {
                    "dim_tags": CodeWrapper("(dim_state_batch, dim_pred)"),
                    "dtype": "float32",
                },
            },
        },
        input_names=[],
        output_names=["lstm_c", "lstm_h", "lstm_out"],
        metadata={
            "lstm_c": "lstm_c",
            "lstm_h": "lstm_h",
            "lstm_out": "lstm_out",
        },
    )


def export_state_updater(model_config: LstmTransducerQATEncoderConfig, checkpoint: PtCheckpoint) -> tk.Path:
    model_serializers = get_model_serializers(model_class=LstmTransducerQATEncoderStateUpdater, model_config=model_config)

    return _export_model(
        model_serializers=model_serializers,
        forward_step_import=Import(
            f"{_state_updater_forward_step.__module__}.{_state_updater_forward_step.__name__}",
            import_as="forward_step",
        ),
        checkpoint=checkpoint,
        returnn_config_dict={
            "dim_pred_layers": CodeWrapper(f'Dim(dimension={model_config.pred_num_layers}, name="pred_layers")'),
            "dim_pred": CodeWrapper(f'Dim(dimension={model_config.pred_dim}, name="pred")'),
            "dim_target": CodeWrapper(f'Dim(dimension={model_config.target_size}, name="target")'),
            "extern_data": {
                "lstm_c_in": {
                    "dim_tags": CodeWrapper("(batch_dim, dim_pred_layers, dim_pred)"),
                    "dtype": "float32",
                },
                "lstm_h_in": {
                    "dim_tags": CodeWrapper("(batch_dim, dim_pred_layers, dim_pred)"),
                    "dtype": "float32",
                },
                "token": {
                    "dim_tags": CodeWrapper("(batch_dim,)"),
                    "sparse_dim": CodeWrapper("dim_target"),
                    "dtype": "int32",
                },
            },
            "model_outputs": {
                "lstm_c_out": {
                    "dim_tags": CodeWrapper("(batch_dim, dim_pred_layers, dim_pred)"),
                    "dtype": "float32",
                },
                "lstm_h_out": {
                    "dim_tags": CodeWrapper("(batch_dim, dim_pred_layers, dim_pred)"),
                    "dtype": "float32",
                },
                "lstm_out": {
                    "dim_tags": CodeWrapper("(batch_dim, dim_pred)"),
                    "dtype": "float32",
                },
            },
        },
        input_names=["lstm_c_in", "lstm_h_in", "token"],
        output_names=["lstm_c_out", "lstm_h_out", "lstm_out"],
        metadata={
            "lstm_c_in": "lstm_c",
            "lstm_h_in": "lstm_h",
            "lstm_c_out": "lstm_c",
            "lstm_h_out": "lstm_h",
            "lstm_out": "lstm_out",
        },
    )


# -----------------------
# --- Forward steps -----
# -----------------------


def _scorer_forward_step(*, model: LstmTransducerQATEncoderScorer, extern_data: TensorDict, **_):
    import returnn.frontend as rf

    run_ctx = rf.get_run_ctx()

    encoder_state = extern_data["encoder_state"].raw_tensor  # [1, E]
    assert encoder_state is not None

    lstm_out = extern_data["lstm_out"].raw_tensor  # [B, P]
    assert lstm_out is not None

    scores = model(
        encoder_state=encoder_state,
        lstm_out=lstm_out,
    )

    run_ctx.mark_as_output(name="scores", tensor=scores)


def _state_initializer_forward_step(*, model: LstmTransducerQATEncoderStateInitializer, **_):
    import returnn.frontend as rf

    run_ctx = rf.get_run_ctx()

    lstm_out, lstm_h, lstm_c = model()

    run_ctx.mark_as_output(name="lstm_c", tensor=lstm_c)
    run_ctx.mark_as_output(name="lstm_h", tensor=lstm_h)
    run_ctx.mark_as_output(name="lstm_out", tensor=lstm_out)


def _state_updater_forward_step(*, model: LstmTransducerQATEncoderStateUpdater, extern_data: TensorDict, **_):
    import returnn.frontend as rf

    run_ctx = rf.get_run_ctx()

    lstm_c_in = extern_data["lstm_c_in"].raw_tensor
    assert lstm_c_in is not None

    lstm_h_in = extern_data["lstm_h_in"].raw_tensor
    assert lstm_h_in is not None

    token = extern_data["token"].raw_tensor
    assert token is not None

    lstm_out, lstm_h_out, lstm_c_out = model(token=token, lstm_h=lstm_h_in, lstm_c=lstm_c_in)

    run_ctx.mark_as_output(name="lstm_c_out", tensor=lstm_c_out)
    run_ctx.mark_as_output(name="lstm_h_out", tensor=lstm_h_out)
    run_ctx.mark_as_output(name="lstm_out", tensor=lstm_out)
