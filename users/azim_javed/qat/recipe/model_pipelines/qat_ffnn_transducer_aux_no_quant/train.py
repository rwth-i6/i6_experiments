__all__ = [
    "QATFFNNTransducerAuxNoQuantTrainOptions",
    "QATFFNNTransducerAuxNoQuantPrunedTrainOptions",
...
from .pytorch_modules import QATFFNNTransducerAuxNoQuantModel


@dataclass
class QATFFNNTransducerAuxNoQuantTrainOptions(TrainOptions):
    enc_loss_scale: float
    pred_loss_scale: float


@dataclass
class QATFFNNTransducerAuxNoQuantPrunedTrainOptions(QATFFNNTransducerAuxNoQuantTrainOptions):
    delay_penalty: float
...
def get_train_step_import(
    options: QATFFNNTransducerAuxNoQuantTrainOptions,
) -> PartialImport:
...
def get_pruned_train_step_import(
    options: QATFFNNTransducerAuxNoQuantPrunedTrainOptions,
) -> PartialImport:
    return PartialImport(
...
def _do_rnnt_pruning(am: torch.Tensor, lm: torch.Tensor, ranges: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    Args:
      ...
      ranges:
        A tensor containing the symbol indices for each frame that we want to
...
def _train_step(
    *,
    model: QATFFNNTransducerAuxNoQuantModel,
    data: dict,
    run_ctx: RunCtx,
    enc_loss_scale: float,
    pred_loss_scale: float,
...
def _train_step_pruned(
    ...
    **_,
    ...
    if not has_mismatch:
        ...
        boundary = torch.stack([begin, begin, targets_size, encoder_states_size.to(dtype=torch.long)], dim=1)
