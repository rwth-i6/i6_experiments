...
class ConformerPositionwiseFeedForwardQuantV4Config(ModelConfiguration):
    ...
    def from_dict(cls, d):
        d = d.copy()
        for name in ["weight_quant_dtype", "activation_quant_dtype"]:
            if d[name] == "qint8":
            ...
            d[name] = weight_dtype
        return ConformerPositionwiseFeedForwardQuantV4Config(**d)
...
class QuantizedConformerMHSARelPosV1Config(ModelConfiguration):
    ...
    activation_quant_dtype: Union[str, torch.dtype]
    activation_quant_method: str
    dot_quant_dtype: Union[str, Union[str, torch.dtype]]
    dot_quant_method: str
    Av_quant_dtype: Union[str, torch.dtype]
    Av_quant_method: str
