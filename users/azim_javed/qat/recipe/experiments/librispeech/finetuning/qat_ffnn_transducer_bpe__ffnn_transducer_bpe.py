...
from ....model_pipelines.qat_ffnn_transducer.train import (
    QATFFNNTransducerTrainOptions,
...
def map_params(num_layers=12, pred_num_layers=2):
    excluded = []
    mapped = {}
    ...
    excluded.extend(
        [
            ...
            "encoder_output.1",
            "encoder_output.3",
            "encoder_output.2.weight_quantizer",
            "token_embedding.weight_quantizer",
            "prediction_output_in_q",
            "prediction_output_out_q",
            "prediction_output.1",
            "prediction_output.3",
            "prediction_output.2.weight_quantizer",
            "joint_net_q1_in",
            "joint_net_q1_out",
            "joint_net_q2_in",
            "joint_net_q2_out",
            ...
            "joint_net.6",
            "joint_net.8",
            "joint_net.2.weight_quantizer",
            "joint_net.7.weight_quantizer",
        ]
    )
    for i in range(pred_num_layers):
        ...
        excluded.extend(
            [
                f"prediction_net.{5 * i + 1}",
                f"prediction_net.{5 * i + 3}",
                f"prediction_net.{new_lin}.weight_quantizer",
            ]
        )
        mapped[f"prediction_net.{new_lin}.weight"] = f"prediction_net.{3 * i + 1}.weight"
        mapped[f"prediction_net.{new_lin}.bias"] = f"prediction_net.{3 * i + 1}.bias"

    mapped.update(
        {
            "encoder_output.2.weight": "encoder_output.1.weight",
            "encoder_output.2.bias": "encoder_output.1.bias",
            "prediction_output.2.weight": "prediction_output.1.weight",
            "prediction_output.2.bias": "prediction_output.1.bias",
            "joint_net.2.weight": "joint_net.1.weight",
            "joint_net.2.bias": "joint_net.1.bias",
            "joint_net.7.weight": "joint_net.4.weight",
            "joint_net.7.bias": "joint_net.4.bias",
        }
    )
    # token_embedding.weight keeps the same name in both models -> no mapping.
    # joint_net is quantized in the full-QAT model: LinearQuants sit at idx 2 and 7
    # (base Linears were at 1 and 4), with ActivationQuantizer aliases at 1,3,6,8.
