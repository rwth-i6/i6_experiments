...
def map_params(num_layers=12, pred_num_layers=2):
    ...
    # --- prediction network + output heads are now quantized (LinearQuant/EmbeddingQuant
    #     with inserted ActivationQuantizers), so the Linear weights shift index and new
    #     quantizer/observer params appear that have no counterpart in the base checkpoint. ---
    excluded.extend(
        [
            "enc_output_in_q",
            "enc_output_out_q",
    ...
    # token_embedding.weight and joint_net.* keep the same names in both models -> no mapping.
