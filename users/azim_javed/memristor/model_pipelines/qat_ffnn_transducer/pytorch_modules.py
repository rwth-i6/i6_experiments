...
class QATFFNNTransducerModel(torch.nn.Module):
    def __init__(self, cfg: QATFFNNTransducerConfig, **_):
        ...
        self.token_embedding = EmbeddingQuant(
            num_embeddings=self.target_size, embedding_dim=cfg.context_embedding_dim, padding_idx=cfg.target_size - 1, weight_bit_prec=cfg.weight_bit_prec, weight_quant_dtype=cfg.weight_quant_dtype, weight_quant_method=cfg.weight_quant_method, bias=True)
...
class QATFFNNTransducerEncoder(QATFFNNTransducerModel):
    def __init__(self, cfg: QATFFNNTransducerConfig, **_):
