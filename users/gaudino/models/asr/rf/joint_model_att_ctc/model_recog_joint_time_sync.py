...
def model_recog_time_sync(
    *,
    ...
    enc_args_ctc, enc_spatial_dim_ctc = model_ctc.encode(
    ...
    initial_target = rf.constant(
        model.bos_idx, dims=batch_dims_, sparse_dim=model_ctc.target_dim_w_blank
    )
    ...
    ctc_out_raw = enc_ctc.copy_transpose(
        (batch_size_dim, enc_spatial_dim, model_ctc.target_dim_w_blank)
    ).raw_tensor  # [B,T,V+1]
    ...
    if search_args.get("prior_scale", 0.0) > 0.0:
        ...
        ctc_out_log_raw = ctc_out_log_raw - (
            prior * search_args.get("prior_scale", 0.3)
        )
    ...
    for i in range(torch.max(max_seq_len.raw_tensor)):
        ...
        if i == 0:
            input_embed = rf.zeros(
                batch_dims_ + [model_aed.target_embed.out_dim],
                feature_dim=model_aed.target_embed.out_dim,
            )
        else:
        ...
        label_log_prob = rf.Tensor(
            name="label_log_prob",
            dims=(batch_size_dim, beam_dim, model_ctc.target_dim_w_blank),
            dtype="float32",
        ...
        label_log_prob = rf.where(
            ...
            rf.sparse_to_dense(
                model.eos_idx,
                axis=model_ctc.target_dim_w_blank,
                label_value=0.0,
                other_value=-1.0e30,
            ),
            label_log_prob,
        ...
        seq_log_prob, (backrefs, target), beam_dim = rf.top_k(
            ...
            axis=[beam_dim, model_ctc.target_dim_w_blank],
