...
class ATTDecoder(AbsDecoder):
    def __init__(self, model, batch_dims, enc_spatial_dim):
        super().__init__()
        self.model = model
    ...
    def init_state(self, x):
        return self.model.decoder_default_initial_state(
            batch_dims=self.batch_dims, enc_spatial_dim=self.enc_spatial_dim
        )
    ...
    def score(self, yseq, state, enc_args):
        ...
        step_out, decoder_state = self.model.loop_step(
            **enc_args,
            enc_spatial_dim=self.enc_spatial_dim,
            input_embed=input_embed,
            state=state,
        )
        step_out.pop("att_weights", None).raw_tensor
        logits = self.model.decode_logits(input_embed=input_embed, **step_out)
        label_log_prob = rf.log_softmax(
            logits, axis=self.model.target_dim
        )

        return label_log_prob.raw_tensor[0], decoder_state
