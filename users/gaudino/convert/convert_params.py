...
def convert_ff(old_w_ff_re: numpy.ndarray, old_parts_order, new_parts_order) -> numpy.ndarray:
    # in: (in_dim+dim, 4*dim)
    # out: (4*dim, in_dim)
    # i = input_gate, j = new_input, f = forget_gate, o = output_gate
    assert old_w_ff_re.ndim == 2
    ...
    parts = {k: v for k, v in zip(old_parts_order, numpy.split(old_w_ff, 4, axis=1))}
    new_w_ff = numpy.concatenate([parts[k] for k in new_parts_order], axis=1)  # (in_dim,4*dim)
    new_w_ff = new_w_ff.transpose()  # (4*dim,in_dim)
    return new_w_ff

def convert_rec(old_w_ff_re: numpy.ndarray, old_parts_order, new_parts_order) -> numpy.ndarray:
    # in: (in_dim+dim, 4*dim)
    # out: (4*dim, in_dim)
    # i = input_gate, j = new_input, f = forget_gate, o = output_gate
    assert old_w_ff_re.ndim == 2
    ...
    parts = {k: v for k, v in zip(old_parts_order, numpy.split(old_w_rec, 4, axis=1))}
    new_w_rec = numpy.concatenate([parts[k] for k in new_parts_order], axis=1)  # (dim,4*dim)
    new_w_rec = new_w_rec.transpose()  # (4*dim,dim)
    return new_w_rec

def convert_bias(old_bias: numpy.ndarray, old_parts_order, new_parts_order, forget_gate_bias: float = 1.0) -> numpy.ndarray:
    # in: (4*dim,)
    # out: (4*dim,)
    assert old_bias.ndim == 1
    assert old_bias.shape[0] % 4 == 0

    parts = {k: v for k, v in zip(old_parts_order, numpy.split(old_bias, 4, axis=0))}
    parts["f"] += forget_gate_bias
    new_bias = numpy.concatenate([parts[k] for k in new_parts_order], axis=0)  # (4*dim,)
    return new_bias