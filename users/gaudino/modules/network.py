...
class ReturnnNetwork:
    ...
    def add_conv_block(
        ...
        for idx, hwpc in enumerate(hwpc_sizes):
            ...
            extra_conv_opts_1 = extra_conv_opts
            if spatial_dims:
                extra_conv_opts_1["in_spatial_dims"] = spatial_dims
            if spatial_dims or merge_out_fixed:
                ...
                extra_conv_opts_1["out_spatial_dims"] = spatial_dims
            if merge_out_fixed:
                from returnn.tensor import Dim

                out_dim = Dim(n_out, name=f"{prefix_name}conv{idx}.out")
                extra_conv_opts_1["out_dim"] = out_dim
