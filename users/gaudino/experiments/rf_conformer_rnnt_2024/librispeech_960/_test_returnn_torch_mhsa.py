...
rf_mhsa.qkv.weight._raw_backend.set_parameter_initial_value(rf_mhsa.qkv.weight, qkv_weight)
rf_mhsa.qkv.bias._raw_backend.set_parameter_initial_value(rf_mhsa.qkv.bias, qkv_bias)

rf_mhsa.proj.weight._raw_backend.set_parameter_initial_value(rf_mhsa.proj.weight, proj_weight.raw_tensor)
...
torch_mhsa = torch.nn.MultiheadAttention(
            512, 1, dropout=0.1, batch_first=True,
        )

state_dict = torch_mhsa.state_dict()
state_dict['in_proj_weight'] = qkv_weight.raw_tensor.T
...
torch_output, _ = torch_mhsa(torch_input, torch_input, torch_input, key_padding_mask=None, need_weights=False)
...
print("Same: ", torch.allclose(rf_output.raw_tensor, torch_output, atol=1e-6))
