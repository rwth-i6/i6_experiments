...
# )
from i6_experiments.users.gaudino.experiments.rf_conformer_att_2023.librispeech_960.model_recogs.model_forward_ctc_max import model_forward_ctc_max
...
def sis_run_with_prefix(prefix_name: str = None):
    ...
    for scales, beam_size in product(
        [(0.8, 0.2, 0.05, 0.68, 0.4)],
        [32]
        # lm + ilm 0.8, 0.2, 0.05, 0.68, 0.4 best -> 3.9 test-other
        # - only lm 0.7, 0.3, 0.0, 0.55 best
        # - accidentally optimized on test (0.7, 0.3, 0.05, 0.68, 0.4), -> 3.82 test-other but can't use
    ):
