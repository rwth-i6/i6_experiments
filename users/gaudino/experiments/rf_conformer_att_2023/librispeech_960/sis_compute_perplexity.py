...
from i6_experiments.users.gaudino.experiments.rf_conformer_att_2023.perplexity import (
    ComputePerplexityJob,
)
...
def sis_run_with_prefix(prefix_name: str = None):
    ...
    task_lib960 = get_librispeech_task_bpe10k_raw(with_eos_postfix=True)

    extern_data_dict = task_lib960.train_dataset.get_extern_data()
    default_target_key = task_lib960.train_dataset.get_default_target()
    targets = Tensor(name=default_target_key, **extern_data_dict[default_target_key])
    target_dim = targets.feature_dim_or_sparse_dim

    # load baseline model w trafo lm
    new_chkpt_path = tk.Path(
        "/work/asr3/zeineldeen/hiwis/luca.gaudino/setups-data/2023-08-10--rf-librispeech/work/i6_experiments/users/gaudino/returnn/convert_ckpt_rf/librispeech/trafo_lm_only_24_02_06/network.023.pt",
    ...
    # compute perplexity
    name = prefix_name + "/lib960_trafo_lm"
