...
from returnn_import.github_com.rwth_i6.returnn_experiments.v20210324123103_75f78096518b.common.datasets.asr.librispeech import \
    oggzip, vocabs, lm
from returnn_import.github_com.rwth_i6.returnn_experiments.v20210324123103_75f78096518b.common.data import \
...
def rnnt_loss(source, **kwargs):
    ...
    costs = rnnt_loss(log_probs.get_placeholder_as_batch_major(), targets.get_placeholder_as_batch_major(), enc_lens,
                      dec_lens,
...
def eval_lm_fusion(source, **kwargs):
    if _lm_fusion == "log-linear":
        ...
        print("Using un-normalized log-linear LM combination with EOS, with LM-scale %.2f and eta %.2f." % (
        _lm_scale, _lm_eta))
        ...
        print("Using density ratio (interp) approach, with LM-scale %.2f and internal scale %.2f" % (
        _lm_scale, _lm_scale_internal))
...
def _get_network_full_sum(epoch0: int):
    ...
    net_dict = _get_network(full_sum_loss=True, grow_encoder=False, pretrain_frac=pretrain_frac,
                            target=_target if _task == "train" else "targetb")
...
def _get_network(target: str, full_sum_loss: bool = False, full_sum_alignment: bool = False, ce_loss: bool = False,
    ...
    net_dict.update({
        ...
        "enc_ctx0": {"class": "linear", "from": "encoder", "activation": None, "with_bias": False,
    ...
    net_dict["encoder_mean"] = {"class": "reduce", "from": "encoder0", "mode": "mean", "keep_dims": False,
                                "axes": ["t"]}  # [B, enc-dim]
    ...
    def get_output_dict(train, search, target, beam_size=beam_size):
        return {
            ...
            "unit": {
                ...
                "prev_out_non_blank": {"class": "reinterpret_data", "from": "prev:output_", "set_sparse": True,
                                       "set_sparse_dim": _target_num_labels},
                "external_lm": {"class": "masked_computation",
                                "mask": "prev:output_emit",
                                "from": "prev_out_non_blank",
                                "unit": {"class": "subnetwork", "from": "data", "n_out": _target_num_labels,
                                         "load_on_init": _lm_model,
                                         "subnetwork": {
                                             "input": {"class": "linear", "n_out": 128, "activation": "identity",
                                                       "from": "data"},
                                             "lstm0": {"class": "rec", "unit": "nativelstm2", "n_out": 2048,
                                                       "dropout": 0.0, "L2": 0.0, "direction": 1, "from": ["input"]},
                                             "lstm1": {"class": "rec", "unit": "nativelstm2", "n_out": 2048,
                                                       "dropout": 0.0, "L2": 0.0, "direction": 1, "from": ["lstm0"]},
                                             "lstm2": {"class": "rec", "unit": "nativelstm2", "n_out": 2048,
                                                       "dropout": 0.0, "L2": 0.0, "direction": 1, "from": ["lstm1"]},
                                             "lstm3": {"class": "rec", "unit": "nativelstm2", "n_out": 2048,
                                                       "dropout": 0.0, "L2": 0.0, "direction": 1, "from": ["lstm2"]},
                                             "output": {"class": "linear", "activation": "log_softmax",
                                                        "use_transposed_weights": True, "n_out": _target_num_labels,
                                                        "from": ["lstm3"]}
                                         }}},
                ...
                "lm_masked": {"class": "masked_computation",
                              ...
                              "masked_from": "base:lm_input" if _task == "train" else None,
                              # enables optimization if used
                              "unit": {
                                  "class": "subnetwork", "from": "data",
                                  "subnetwork": {
                                      "input_embed": {"class": "linear", "activation": None, "with_bias": False,
                                                      "from": "data", "n_out": 256},
                                      ...
                                      "lstm0_zoneout": {"class": "rnn_cell", "unit": "ZoneoutLSTM",
                                                        "unit_opts": {"zoneout_factor_cell": 0.15,
                ...
                "readout_in": {"class": "linear", "from": ["am", "lm"], "activation": None, "n_out": 1000, "L2": l2,
                               "dropout": 0.2,
                               "out_type": {"batch_dim_axis": 2 if _task == "train" else 0,
                                            "shape": (None, None, 1000) if _task == "train" else (1000,),
                                            "time_dim_axis": 0 if _task == "train" else None}},  # (T, U+1, B, 1000
                "readout": {"class": "reduce_out", "mode": "max", "num_pieces": 2, "from": "readout_in"},

                "readout_in_0am": {"class": "linear", "from": ["base:encoder_mean", "lm"], "activation": None,
                                   "n_out": 1000, "L2": l2, "dropout": 0.2, "reuse_params": "readout_in"},
                "readout_0am": {"class": "reduce_out", "mode": "max", "num_pieces": 2, "from": "readout_in_0am"},
                # [T, U+1, B, 2*V']
                "label_log_prob_0am": {"class": "linear", "from": "readout_0am", "activation": "log_softmax",
                                       "n_out": _target_num_labels, "reuse_params": "label_log_prob"},
                # (B, T, U+1, 1030)

                "label_log_prob": {
                    "class": "linear", "from": "readout", "activation": "log_softmax", "dropout": 0.3,
                    "n_out": _target_num_labels},  # (B, T, U+1, 1030)

                "fused_label_log_prob": {"class": "eval",
                                         "from": ["external_lm_unmask", "label_log_prob", "prev:u", "prev:out_str"] + (
                                             ["label_log_prob_0am"] if _lm_fusion.startswith(
                                                 "divide-by-prior") else []), "eval": eval_lm_fusion},
                ...
                # (B, T, U+1, 1)
                "blank_log_prob0": {"class": "eval", "from": "emit_prob0",
                                    "eval": "tf.compat.v1.log_sigmoid(-source(0))"},  # (B, T, U+1, 1)

                # apply the EOS-symbol prob on the last blank-frame ([B, 1] -> [B] + (lm_scale)*[B]) -> [B,1]
                "blank_log_prob_w_eos": {"class": "eval", "from": ["blank_log_prob0", "external_lm_unmask"],
                                         "eval": "tf.expand_dims((0.5)*source(0)[:, 0] + 0.5*source(1)[:, eos_symbol], axis=-1)",
                                         "eval_locals": {"lm_scale": _lm_scale, "eos_symbol": 0},
                                         "out_type": {"batch_dim_axis": 0, "dim": 1, "shape": (1,)}},  # [B, 1]
                "enc_seq_len_minus1": {"class": "eval", "from": "base:enc_seq_len",
                                       "eval": "tf.maximum(source(0) - 1, 0)",
                                       "out_type": {"shape": (), "batch_dim_axis": 0, "dim": None}},  # [B, 1]
                "last_frame": {"class": "eval", "from": ["prev:t", "enc_seq_len_minus1"],
                               "eval": "tf.greater(source(0), source(1))",  # [B] (bool)
                               "out_type": {"dtype": "bool"}},  # [B] (bool)
                "blank_log_prob": {"class": "switch", "condition": "last_frame", "true_from": "blank_log_prob_w_eos",
                                   "false_from": "blank_log_prob0"},  # [B, 1]

                "label_emit_log_prob": {"class": "combine", "kind": "add",
                                        "from": ["fused_label_log_prob", "emit_log_prob"]},
                ...
                "output_emit": {"class": "copy", "from": "output_is_not_blank", "is_output_layer": True,
                                "initial_output": True},
                ...
                "dt": {"class": "switch", "condition": "output_is_not_blank", "true_from": "const0",
                       "false_from": "const1"},
    ...
    net_dict["output"] = get_output_dict(train=(_task == "train"), search=(_task != "train"), target=target,
                                         beam_size=beam_size)
