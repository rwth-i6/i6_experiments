...
def conformer_baseline():
  ...
  for peak_lr in [5e-4, 6e-4, 8e-4]:
    for reps in [3, 4, 5]:
      for accum_grad in [1, 2]:
        args = copy.deepcopy(oclr_args)
        args['batch_size'] = 15_000 * 80
        args['max_seq_length'] = 75
        args['accum_grad'] = accum_grad
        args['pretrain_reps'] = reps
        args['oclr_opts']['peak_lr'] = peak_lr
        run_exp(
          f"base_conf_12l_lstm_1l_conv6_OCLR_sqrdReLU_peak{peak_lr}_bs15k_bpe500_pretrainReps-{reps}_accum{accum_grad}_currV1", train_args=args,
