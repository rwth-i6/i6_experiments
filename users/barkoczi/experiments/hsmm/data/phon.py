...
def build_eow_phon_training_datasets_with_hdf(
    ...
    *,
    ...
    """
    Like ``build_eow_phon_training_datasets`` but adds an extra HDF-backed stream
    ...
    return build_training_datasets_with_hdf(
        train_ogg=train_ogg,
        dev_clean_ogg=dev_clean_ogg,
        dev_other_ogg=dev_other_ogg,
        settings=settings,
...
# --- Append this to your existing data/phon.py ---

def build_eow_phon_training_datasets_custom_split(
    prefix: str,
    librispeech_key: str,
    settings: DatasetSettings,
    *,
    ...
    """
    Build phoneme datasets with flexible subsetting for fast debugging.
    The remaining unused fraction is safely discarded into an 'unused' bucket.
    """
    total_fraction = train_fraction + devtrain_fraction
    if total_fraction > 1.0:
        raise ValueError("train_fraction + devtrain_fraction cannot exceed 1.0")
    ...
    split_dict = {"train": train_fraction, "devtrain": devtrain_fraction}
    if total_fraction < 1.0:
        split_dict["unused"] = 1.0 - total_fraction
    ...
    split_job.add_alias(os.path.join(prefix, f"{librispeech_key}", "train_devtrain_custom_split"))
