...
def rasr_search_wav2vec2_large_pca_gaussian():
    root_prefix = "example_setups/librispeech/feature_dump/ls960_wav2vec2_large_pca_gaussian_rasr_search"
    stats_prefix = "example_setups/librispeech/feature_dump/ls960_wav2vec2_large_pca_label_stats_sweep_new_alignments"
    ...
    stats_outputs, stats_job = compute_pca_label_stats(
    ...
    # Keep the corpus dependency visible in the graph. The helper above returns
    # the same Bliss paths, but this makes the intent explicit for recognition.
    ...
    for lm_scale in lm_scales:
        recog_config = build_librasr_phmm_recognition_config(
            ...
            logfile_suffix=f"pca_gaussian_layer{target_layer}_dim{pca_dim}_lm{lm_scale:g}",
        )
        search(
            prefix_name=f"{root_prefix}/layer_{target_layer}/pca_{pca_dim}/search_lm{lm_scale:g}",
