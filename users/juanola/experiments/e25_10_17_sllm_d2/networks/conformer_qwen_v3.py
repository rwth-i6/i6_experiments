...
class SllmV3(
    ...
    def __init__(
            ...
            *,
            ...
            # LORA
            encoder_lora_opts: Optional[Dict[str, Any]] = None,
            decoder_lora_opts: Optional[Dict[str, Any]] = None,
        ...
        # Layers
        assert encoder_lora_opts is None or using_encoder, "Encoder lora opts specified but not using encoder!"
        assert freeze_encoder_from_the_start is None or using_encoder, "Encoder freeze specified but not using encoder!"
        assert decoder_lora_opts is None or using_decoder, "Decoder lora opts specified but not using decoder!"
        assert freeze_decoder_from_the_start is None or using_decoder, "Decoder freeze specified but not using decoder!"
        assert freeze_adapter_from_the_start is None or (using_encoder and using_decoder), "Adapter freeze specified but not using (decoder and adapter)!"
        ...
        # LORA
        if encoder_lora_opts is not None:
            assert False, "Not implemented yet!"
            # freeze_encoder_from_the_start = True
            # encoder_lora_config = LoraConfig(**encoder_lora_opts)
            # self.encoder.apply_lora(encoder_lora_config) # should be changed if used!!
            # print(" +++ Using LoRA to encoder +++ ")

        if decoder_lora_opts is not None:
            freeze_decoder_from_the_start = True
            decoder_lora_config = LoraConfig(task_type="CAUSAL_LM", **decoder_lora_opts)
            self.apply_lora_to_decoder(decoder_lora_config)
            print(" +++ Using LoRA to decoder +++ ")
    ...
    def apply_lora_to_decoder(self, lora_config: LoraConfig):
    ...
    def get_qwen_input_embeds(
        ...
        input_target_embeddings = self.decoder.get_input_embeddings()(text_tokens)  # [B, L] -> [B, L, F]
