...
from i6_experiments.users.juanola.experiments.e25_10_17_sllm_d2.configurations.pipeline.search_config import (
    ...
    search_baseline_v2_multiple_beams, search_v4_ctc_sllm_multiple_beams,
    ...
    V4_slides_combs_without_prior, search_baseline_v2_multiple_beams_v5, )
...
def exp_v4() -> ExperimentConfig:
    return ExperimentConfig(
        ...
        search=[*base_searches()], #search_baseline(),
    )


def filter_v4_no_ctc_recogs(configs: List[SearchConfig]) -> List[SearchConfig]: # TODO: remove!!!!
    new_configs = []
    for config in configs:
        if config.forward_method == "forward_step_ctc_decoding_v2" and config.ctc_scales == [None]:
    ...
    return replace(
        ...
        search=[
            ...
            *V4_autoscaling_good_combs_1_reduced(
            ...
            # *V4_autoscaling_good_combs_1( # TODO: run
            #     ext_encoder=PretrainedExternalModules.CTC_STANDALONE_3_LAYERS.value,
            #     ext_decoder=PretrainedExternalModules.LLM_BASE_COMBINED_V2.value
            # )
            ...
            V4_autoscaling_64_ctc_prior_sllm_lm(
                ...
                use_prior=False,
                auto_scaling_use_ctc_sum_scores=True,
                prior_relative_to=None,
            ),

            *V4_slides_combs_without_prior(
                ext_encoder=PretrainedExternalModules.CTC_STANDALONE_3_LAYERS.value,
                ext_decoder=PretrainedExternalModules.LLM_BASE_COMBINED_V2.value,
            ),
...
def exp_v7_with_beam_ln() -> ExperimentConfig:
    return replace(exp_v7(), search=[
        ...
        dataclasses.replace(search_baseline_v2_multiple_beams_v4(), gpu_memory=24),
        replace(search_baseline_v2_multiple_beams_v4(), length_norm_exponent=0.0, gpu_memory=24),

        dataclasses.replace(search_baseline_v2_multiple_beams_v5(), gpu_memory=24),
        replace(search_baseline_v2_multiple_beams_v5(), length_norm_exponent=0.0, gpu_memory=24),
    ])
...
def exp_v13() -> ExperimentConfig:
    ...
    return replace(
        exp_v7(),
        network=network_linear_adapter(),
    )
...
def bv2_pre_ed_s_c_4gpu() -> ExperimentConfig:
    return replace(
        model_v2_small_baseline(),
        pretrained=enc_dec_small_combined(),
...
def bv2_ds_pre_d_b_c_f2() -> ExperimentConfig:
    return replace(
        bv2_ds_pre_d_b_c(), network=network_with_frozen_layers(network_baseline_v2_td(), decoder_partial_epochs=2)
    )
...
def bv2_ds_pre_ed_b_c_f1() -> ExperimentConfig:
    return replace(
        ...
        network=network_with_frozen_layers(
            network_baseline_v2_td(), encoder_partial_epochs=1, decoder_partial_epochs=1
        ),
    )
...
def bv2_pre_ed_s_c_f1() -> ExperimentConfig:
    return replace(
        ...
        training=i6_4gpu_setup_v4(),
        network=network_with_frozen_layers(
            network_baseline_v2_td_linear_small(), encoder_partial_epochs=1, decoder_partial_epochs=1
        ),
    )
...
def exp_v14_3ctc_b_pre_ed() -> ExperimentConfig:
    return replace(exp_v7(), network=network_base_v2_3ctc(), training=itc_v2_80k(), pretrained=enc_dec_base_combined())


def exp_v14_3ctc_s_pre_ed() -> ExperimentConfig:
    return replace(
        exp_v7(), network=network_small_v2_3ctc(), training=i6_4gpu_setup_v4(), pretrained=enc_dec_small_combined()
    )


def exp_v14_3ctc_b_pre_ed_f20() -> ExperimentConfig:
    return replace(
...
def exp_v14_3ctc_s_pre_ed_f10() -> ExperimentConfig:
    return replace(
        exp_v14_3ctc_s_pre_ed(),
        network=network_with_frozen_layers(
            network_small_v2_3ctc(), encoder_partial_epochs=10, decoder_partial_epochs=10
        ),
    )
...
def exp_v7_s2v2()  -> ExperimentConfig:
    return replace(exp_v7_s2(), training=replace(itc_batch_size_80k(), random_seed=1234))
...
def exp_v5_s2v2()  -> ExperimentConfig:
    return replace(exp_v5_s2(), training=replace(itc_batch_size_80k(), random_seed=1234))
...
def n2_test() -> ExperimentConfig:
    return replace(
        ...
        search=[V3_search_baseline_ctc_decoding_11gb()],
    )
