...
class ReturnnDecodingExperiment(DecodingExperiment):
  ...
  def base_alias(self) -> str:
    return "%s/returnn_no-recomb_length-scale-1.0_beam-12/epoch_%d" % (self.variant_name, self.epoch)
  ...
  def get_ctm_path(self) -> Path:
    if self.dump_best_traces:
      ...
      search_job = ReturnnSearchJobV2(
        ...
        model_checkpoint=self.checkpoint,
        returnn_config=self.returnn_config,
        returnn_python_exe=Path("/u/rossenbach/bin/returnn_tf2.3_launcher.sh"),
        returnn_root=Path("/u/schmitt/src/returnn"),
        device="gpu",
        mem_rqmt=4,
        time_rqmt=1)
      search_job.add_alias("%s/search_%s" % (self.base_alias, self.corpus_key))
      tk.register_output(search_job.get_one_alias(), search_job.out_search_file)
    ...
    return SearchWordsToCTMJob(
      ...
      SWBCorpora.corpus_paths[SWBCorpora.corpus_mapping[self.corpus_key]]).out_ctm_file
...
class RasrDecodingExperiment(DecodingExperiment):
  ...
  @property
  def base_alias(self):
    return "%s/rasr_decoding_limit12_pruning12.0_vit-recomb_neural-length_max-seg-len-20_length-scale-1.0/epoch_%d" % (self.variant_name, self.epoch)
