...
def run_exps():
  ...
  data.analyze_gradients_jobs["baseline_v3_full-sum"] = {}
        ...
        for full_sum_train_alias, full_sum_checkpoint in train.train_center_window_att(
          ...
          for epoch, chckpt in full_sum_checkpoint["checkpoints"].items():
            if epoch in keep_epochs_full_sum:
              ...
              data.analyze_gradients_jobs["baseline_v3_full-sum"][alias][f"epoch-{epoch}"] = pipeline.decoding_exps[0].analyze_gradients_job
