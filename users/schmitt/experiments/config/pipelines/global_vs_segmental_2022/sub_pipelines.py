...
from recipe.i6_core.corpus.segments import SplitSegmentFileJob
...
from recipe.i6_private.users.schmitt.returnn.tools import RASRLatticeToCTMJob, RASRDecodingJob, CompileTFGraphJob, \
  ...
  WordsToCTMJob, DumpAlignmentFromTxtJobNew, RASRRealignmentParallelJob, SwitchLabelInAlignmentJob, \
  RASRDecodingJobParallel
...
from recipe.i6_experiments.users.schmitt.experiments.config.pipelines.global_vs_segmental_2022.build_rasr_configs import build_lattice_to_ctm_config, build_decoding_config, build_realignment_config, \
  build_phon_align_extraction_config, build_iterate_corpus_config, build_extract_alignment_config
...
def run_search_from_file(
        config_file_path, parameter_dict, model_dir, load_epoch, default_model_name, name, bliss_corpus, alias_suffix, time_rqmt=1, mem_rqmt=4):
  assert type(parameter_dict) == dict
  ...
  ctm_job = SearchWordsToCTMJob(bpe_to_words_job.out_word_search_results, bliss_corpus)
...
def run_bpe_returnn_decoding(
  returnn_config: ReturnnConfig, checkpoint, bliss_corpus, name, dataset_key, num_epochs, device="gpu", alias_addon="",
...
def run_rasr_decoding_parallel(
  corpus_path, segment_path, lexicon_path, feature_cache_path, label_file_path, concurrent,
  simple_beam_search, blank_label_index, reduction_factors, start_label_index, blank_update_history,
  allow_label_recombination, allow_word_end_recombination, full_sum_decoding, label_pruning, label_pruning_limit,
  use_lm_score, word_end_pruning, word_end_pruning_limit, label_recombination_limit, label_unit,
  skip_silence, lm_type, lm_scale, lm_file, lm_image, lm_lookahead, max_seg_len,
  ...
  model_checkpoint, name, num_epochs, lm_lookahead_cache_size_high=None, label_scorer_type="tf-rnn-transducer",
  lm_lookahead_cache_size_low=None, lm_lookahead_history_limit=None, lm_lookahead_scale=None,
  time_rqmt=10, mem_rqmt=4, alias_addon="", debug=False, max_batch_size=256):
  ...
  split_segments_job = SplitSegmentFileJob(
    segment_file=segment_path, concurrent=concurrent)

  decoding_crp.corpus_config.segments.file = None
  decoding_crp.segment_path = split_segments_job.out_segment_path
  decoding_crp.concurrent = concurrent

  rasr_decoding_job = RASRDecodingJobParallel(
    rasr_exe_path=rasr_exe_path, flf_lattice_tool_config=decoding_config, crp=decoding_crp,
    model_checkpoint=model_checkpoint, dump_best_trace=False,
    mem_rqmt=mem_rqmt, time_rqmt=time_rqmt, use_gpu=True)
  rasr_decoding_job.add_alias(name + ("/%s/rasr-decoding-epoch-%d" % (alias_addon, num_epochs)))
  alias = rasr_decoding_job.get_one_alias()
  tk.register_output(alias + "/results", rasr_decoding_job.out_lattice_bundle)
  ...
  lattice_to_ctm_job = RASRLatticeToCTMJob(
    rasr_exe_path=rasr_exe_path, lattice_path=rasr_decoding_job.out_lattice_bundle,
    crp=lattice_to_ctm_crp, flf_lattice_tool_config=lattice_to_ctm_config)
  # lattice_to_ctm_job.add_alias(name + ("/lattice-to-ctm-epoch-%d" % num_epochs) + alias_addon)
  alias = name + ("/lattice-to-ctm-epoch-%d" % num_epochs) + alias_addon
  tk.register_output(alias, lattice_to_ctm_job.out_ctm)
...
def calc_rasr_search_errors(
  ...
  model_checkpoint, name, num_epochs, lm_lookahead_cache_size_high=None, label_scorer_type="tf-rnn-transducer",
  lm_lookahead_cache_size_low=None, lm_lookahead_history_limit=None, lm_lookahead_scale=None,
  time_rqmt=10, mem_rqmt=4, gpu_rqmt=1, alias_addon="", debug=False, max_batch_size=256, orig_idx=None, new_idx=None):
  ...
  dump_align_from_txt_job = DumpAlignmentFromTxtJob(
    ...
    segment_file=segment_path if new_idx is None else Path("/u/schmitt/experiments/transducer/config/dependencies/seg_cv_head3000"),
    num_classes=num_classes
  )
  dump_align_from_txt_job.add_alias(name + ("/%s/rasr-decoding-dump-traces-best-traces-epoch-%d" % (alias_addon, num_epochs)))
  out_hdf_align = dump_align_from_txt_job.out_hdf_align
  if new_idx is not None and orig_idx is not None:
    assert type(new_idx) == int and type(orig_idx) == int
    switch_label_job = SwitchLabelInAlignmentJob(
      alignment=out_hdf_align, new_idx=new_idx, orig_idx=orig_idx
    )
    out_hdf_align = switch_label_job.out_alignment


  calc_search_err_job = CalcSearchErrorJob(
    returnn_config=train_config, rasr_config=extern_sprint_rasr_config,
    rasr_nn_trainer_exe=rasr_nn_trainer_exe,
    segment_file=segment_path if new_idx is None else Path("/u/schmitt/experiments/transducer/config/dependencies/seg_cv_head3000"),
    blank_idx=blank_idx,
    search_targets=out_hdf_align,
  ...
  lattice_to_ctm_job = RASRLatticeToCTMJob(rasr_exe_path=rasr_exe_path, lattice_path=rasr_decoding_job.out_lattice,
    crp=lattice_to_ctm_crp, flf_lattice_tool_config=lattice_to_ctm_config)
  # lattice_to_ctm_job.add_alias(name + ("/dump-traces-lattice-to-ctm-epoch-%d" % num_epochs) + alias_addon)
  alias = name + ("/lattice-to-ctm-epoch-%d" % num_epochs) + alias_addon
  tk.register_output(alias, lattice_to_ctm_job.out_ctm)
  ...
  return out_hdf_align, bpe_ctm_to_words_job.out_ctm_file


def calc_rasr_search_errors_parallel(
  corpus_path, segment_path, lexicon_path, feature_cache_path, label_file_path, length_norm, concurrent,
  simple_beam_search, blank_label_index, reduction_factors, start_label_index, blank_update_history,
  allow_label_recombination, allow_word_end_recombination, full_sum_decoding, label_pruning, label_pruning_limit,
  use_lm_score, word_end_pruning, word_end_pruning_limit, label_recombination_limit, label_unit,
  skip_silence, lm_type, lm_scale, lm_file, lm_image, lm_lookahead, max_seg_len, num_classes,
  ...
  model_checkpoint, name, num_epochs, lm_lookahead_cache_size_high=None, label_scorer_type="tf-rnn-transducer",
  lm_lookahead_cache_size_low=None, lm_lookahead_history_limit=None, lm_lookahead_scale=None,
  time_rqmt=10, mem_rqmt=4, alias_addon="", debug=False, max_batch_size=256, orig_idx=None, new_idx=None):
  ...
  lattice_to_ctm_crp, lattice_to_ctm_config = build_lattice_to_ctm_config(corpus_path=corpus_path,
    segment_path=segment_path, lexicon_path=lexicon_path, lattice_path=rasr_decoding_job.out_lattice_bundle)

  lattice_to_ctm_job = RASRLatticeToCTMJob(rasr_exe_path=rasr_exe_path, lattice_path=rasr_decoding_job.out_lattice_bundle,
    crp=lattice_to_ctm_crp, flf_lattice_tool_config=lattice_to_ctm_config)
  # lattice_to_ctm_job.add_alias(name + ("/dump-traces-lattice-to-ctm-epoch-%d" % num_epochs) + alias_addon)
  alias = name + ("/lattice-to-ctm-epoch-%d" % num_epochs) + alias_addon
  tk.register_output(alias, lattice_to_ctm_job.out_ctm)
...
def run_rasr_realignment(
  ...
  extraction_config, extraction_post_config = build_extract_alignment_config(
    allophone_path=allophone_path, state_tying_path=state_tying_path, lexicon_path=lexicon_path,
    corpus_path=corpus_path, segment_path=segment_path, feature_cache_path=feature_cache_path,
  ...
  dump_align_from_txt_job = DumpAlignmentFromTxtJobNew(
    ...
    num_classes=num_classes)
  dump_align_from_txt_job.add_alias(name + ("/%s/rasr-realignment-epoch-%d-%s-best-traces" % (alias_addon, num_epochs, data_key)))
...
def run_rasr_realignment_parallel(
  compile_config, name, corpus_path, segment_path, lexicon_path, feature_cache_path, allophone_path, label_pruning,
  label_pruning_limit, rasr_am_trainer_exe_path, rasr_nn_trainer_exe_path, model_checkpoint, num_epochs,
  label_recombination_limit, blank_label_index, context_size, label_file, reduction_factors, max_segment_len,
  start_label_index, state_tying_path, num_classes, time_rqmt, blank_allophone_state_idx, length_norm,
  blank_update_history, loop_update_history, mem_rqmt, data_key, concurrent, use_gpu,
  alias_addon=""
):
  ...
  realignment_crp.concurrent = concurrent

  realignment_job = RASRRealignmentParallelJob(
    rasr_exe_path=rasr_am_trainer_exe_path, crp=realignment_crp, model_checkpoint=model_checkpoint,
    mem_rqmt=mem_rqmt, time_rqtm=time_rqmt, am_model_trainer_config=realignment_config, use_gpu=use_gpu,
    blank_allophone_state_idx=blank_allophone_state_idx)
  realignment_job.add_alias(name + ("/%s/rasr-realignment-epoch-%d-%s" % (alias_addon, num_epochs, data_key)))
  alias = realignment_job.get_one_alias()
  tk.register_output(alias + "/results", realignment_job.out_alignment_bundle)
...
def run_eval(ctm_file, reference, name, dataset_key, num_epochs, alias_addon=""):
  ...
  score_job = Hub5ScoreJob(
    reference, Path("/work/asr2/oberdorfer/kaldi-stable/egs/swbd/s5/data/eval2000/glm"), ctm_file
  )
...
def calculate_search_errors(
  ...
  name, epoch, ref_targets, blank_idx, dataset_key, bliss_corpus, length_norm, concat_seq_tags_file=None, alias_addon="",
  concat_seqs=False, stm_path=None):
  assert not concat_seqs or stm_path
  ...
  search_job.add_alias(name + "/%s/search_%s_%d" % (alias_addon, dataset_key, epoch))
