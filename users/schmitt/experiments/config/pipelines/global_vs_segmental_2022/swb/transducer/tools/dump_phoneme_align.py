...
def hdf_dump_from_dataset(dataset, phon_to_id, id_to_phon, id_to_multiphone, id_to_state, hdf_dataset, parser_args):
  ...
  print("ID TO MULTIPHONE: ", id_to_multiphone)
...
def init(rasr_config_path, time_red, rasr_exe, state_tying_file):
  """
  ...
  with open(state_tying_file, "r") as state_tying_file:
    relevant_lines = []
...
def main(argv):
  ...
  parser.add_argument('--state_tying_file', type=str)
  ...
  dataset, phon_to_id, id_to_phon, id_to_multiphone, id_to_state = init(
    rasr_config_path=args.rasr_config, time_red=args.time_red, rasr_exe=args.rasr_exe, state_tying_file=args.state_tying_file)
