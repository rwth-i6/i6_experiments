...
from recipe.i6_experiments.users.schmitt.experiments.config.pipelines.global_vs_segmental_2022.swb.transducer import conformer
...
def get_extended_net_dict(
  ...
  if behavior_version is not None:
    net_dict ["source"]["from"] = "data:data"
  ...
  if task == "train":
    ...
    if ctc_aux_loss:
      ...
      if use_eos and task == "train":
        net_dict["ctc"]["target"] = "label_ground_truth_w_eos"
  elif task == "search":
    net_dict.update({
      "output_non_sil": {
  ...
  if behavior_version is not None and enc_type != "lstm":
    net_dict["encoder0"] = net_dict["encoder"].copy()
    net_dict["encoder"] = {"class": "swap_axes", "from": ["encoder0"], "axis1": "f", "axis2": "t"}
  ...
  def get_fast_network_dict(rec, rec_full):
    if not rec:
      ...
      if not rec_full:
        ...
        return {
          "class": "rec", "unit": "zoneoutlstm" if use_zoneout else "nativelstm2",
          "from": length_model_inputs, "n_out": 128, "L2": l2,
          "dropout": 0.3,
          "unit_opts": {
            "zoneout_factor_cell": 0.15, "zoneout_factor_output": 0.05
          } if use_zoneout else {"rec_weight_dropout": 0.3}}
  ...
  def get_label_model(net_dict):
    ...
    if hybrid_hmm_like_label_model:
      ...
      lm_dict = {
        ...
        "input_embed": {
          "class": "merge_dims", "from": "input_embed0",
          "axes": "except_time" if behavior_version is None else ["f", "stag:input_embed0:window"]
        },
        "lm": {
          "class": "rec", "unit": "zoneoutlstm" if use_zoneout else "nativelstm2", "n_out": lstm_dim,
    ...
    if task == "train":
      rec_unit_dict.update({
        ...
        "label_prob": {
          ...
          "target": "label_ground_truth" if not use_eos else "label_ground_truth_w_eos",
        ...
        "output": {
          'class': 'choice', 'target': "label_ground_truth" if not use_eos else "label_ground_truth_w_eos",
          'beam_size': beam_size, 'from': "data",
          "initial_output": sos_idx, "cheating": "exclusive" if task == "train" else None}
      })
      ...
      net_dict.update({
        "label_model": {
          ...
          "name_scope": "output/rec", "is_output_layer": True, "unit": {}}})
      net_dict["label_model"]["unit"].update(rec_unit_dict)
      # in this case, change the target of the label model loop
      if use_eos:
        net_dict.update({
          "target_with_eos": {
            ...
            "postfix": sos_idx,
            "register_as_extern_data": "label_ground_truth_w_eos",
            "repeat": 1,
          },
        })
  ...
  def get_length_model(net_dict):
    ...
    if length_model_type.startswith("seg"):
      ...
      net_dict["output"]["unit"].update({
        ...
        "emit_prob0": {
          "class": "linear", "from": "s", "activation": None, "n_out": 1,
    ...
    if task == "train" and length_model_type in ["frame", "seg-static"]:
      net_dict["output"]["unit"].update({
        ...
        "emit_blank_prob": {
          ...
          "loss": "ce" if use_time_sync_loop else None,
          "loss_opts": {"focal_loss_factor": length_model_focal_loss} if use_time_sync_loop else None}
      })
  ...
  def get_output_dict():
    ...
    if force_eos:
      # create a mask where all label log probs are set to a very low value except for the eos log prob, which is set
      # to 0. add this mask to the output log prob to "force" the search to output eos in the last frame
      assert length_model_type == "frame" and task != "train"
      output_dict["unit"]["output_log_prob0"] = output_dict["unit"]["output_log_prob"].copy()
      output_log_prob_mask = [-1000000] * (target_num_labels + 1)
      # output_log_prob_mask[sos_idx] = 0
      output_dict["unit"].update({
        "const0.0": {"axis": "F", "class": "expand_dims", "from": "const0.0_0"},
        "const0.0_0": {"class": "constant", "value": 0.0, "with_batch_dim": True, "dtype": "float32"},
        "eos_log_prob": {
          "class": "slice", "axis": "f", "slice_start": 0, "slice_end": 1, "slice_step": 1, "from": "output_log_prob0"},
        "output_log_prob_labels": {
          "class": "slice", "axis": "f", "slice_start": 1, "slice_end": 1031, "slice_step": 1,
          "from": "output_log_prob0"},
        ...
        "output_log_prob_mask": {
          "class": "constant", "value": CodeWrapper("np.array(%s)" % output_log_prob_mask), "dtype": "float32"
        },
        "output_log_prob1": {
          "class": "copy", "from": ["output_log_prob_mask", "eos_log_prob"]
        },
        "output_log_prob2": {
          "class": "copy", "from": ["const_neg_inf", "output_log_prob_labels", "const_neg_inf"]
        },
        "output_log_prob": {
          "class": "switch", "condition": "is_last_frame", "true_from": "output_log_prob1",
          "false_from": "output_log_prob2"
        }
      })
...
def get_global_import_net_dict(task, targetb_blank_index, target_num_labels, targetb_num_labels, sos_idx, sil_idx, weight_feedback=False, feature_stddev=None, prev_att_in_state=True, segment_center_window_size=None):
  if task == "eval":
    net_dict = {
      "#info": {
