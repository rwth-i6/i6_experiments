...
def calc_segment_stats_with_sil(blank_idx, sil_idx):
  ...
  map_init_non_sil_seg_len_to_count = Counter()
  map_final_non_sil_seg_len_to_count = Counter()
      ...
      try:
        ...
        for i, idx in enumerate(non_blank_idxs):
          ...
          if idx in sil_idxs:
            ...
            if i == 0:
              map_init_non_sil_seg_len_to_count.update([seg_len])
            elif i == len(non_blank_idxs) - 1:
              map_final_non_sil_seg_len_to_count.update([seg_len])
            else:
  ...
  ax = plt.gca()
  quantiles = [np.quantile(hist_data, q) for q in [.8, .85, .90, .95, .99]]
  ...
  # plot histograms for initial non-sil segment lens
  hist_data = [item for seg_len, count in map_init_non_sil_seg_len_to_count.items() for item in [seg_len] * count]
  plt.hist(hist_data, bins=30, range=(0, 100))
  ax = plt.gca()
  quantiles = [np.quantile(hist_data, q) for q in [.8, .85, .90, .95, .99]]
  for n, q in zip([90, 95, 99], quantiles):
    # write quantiles to file
    with open("init_non_sil_percentile_%s" % n, "w+") as f:
      f.write(str(q))
    ax.axvline(q, color="r")
  plt.savefig("init_non_sil_histogram.pdf")
  plt.close()

  # plot histograms for final non-sil segment lens
  hist_data = [item for seg_len, count in map_final_non_sil_seg_len_to_count.items() for item in [seg_len] * count]
  plt.hist(hist_data, bins=30, range=(0, 100))
  ax = plt.gca()
  quantiles = [np.quantile(hist_data, q) for q in [.8, .85, .90, .95, .99]]
  for n, q in zip([90, 95, 99], quantiles):
    # write quantiles to file
    with open("final_non_sil_percentile_%s" % n, "w+") as f:
      f.write(str(q))
    ax.axvline(q, color="r")
  plt.savefig("final_non_sil_histogram.pdf")
  plt.close()
