...
from i6_experiments.users.schmitt.experiments.config.pipelines.global_vs_segmental_2022_23.dependencies.general.returnn.exes import \
  RETURNN_EXE, RETURNN_ROOT, RETURNN_CURRENT_ROOT
...
def build_alias(**kwargs):
  ...
  if kwargs["model_type"] == "seg":
    ...
    if kwargs["segment-center-window-size"] is not None:
      alias += ".win-size%s" % (
        int(kwargs["segment-center-window-size"] * 2) if not kwargs["use-glob-win"] else "-glob")
    ...
    if kwargs["seg-boundary-random-shift"] is not None:
      alias += ".seg-bound-shift-%d" % kwargs["seg-boundary-random-shift"]
    ...
    if "slow-rnn-inputs" in kwargs:
      alias += ".slow-rnn-in_" + "+".join([inp.replace("_", "-") for inp in kwargs["slow-rnn-inputs"]]).replace("base:",
                                                                                                                "").replace(
        "prev:", "prev_")
...
default_variant_transducer = dict(
  ...
  segment_center_window_size=None, pretrain=True, seg_boundary_random_shift=None,
  ...
  lr_measure="dev_error_output/label_prob", conf_use_blstm=False, conf_batch_norm=True, max_seqs=200,
  conf_num_blocks=12,
...
default_variant_enc_dec = dict(
  ...
  set_dim_tag_correctly=True, features="gammatone",
  dynamic_lr=False,
)
...
models = {
  "segmental": {
    "seg.rna-bpe.lr-meas-lab.full-ctx.time-red6.fast-rec.fast-rec-full.seg.mlp-att.am2048.prev-att-in-state.frame-length-model-in_am+prev-out-embed.no-ctx-reg": {
      "config": dict(default_variant_transducer, use_attention=True, label_type="rna-bpe", lstm_dim=1024,
                     ctx_size="inf",
                     att_type="mlp", att_area="seg", time_red=[3, 2], prev_att_in_state=True,
                     length_model_focal_loss=0.0,
  ...
  "segmental_align_augment": {
    "seg.rna-bpe.lr-meas-lab.seg-bound-shift-1.full-ctx.time-red6.fast-rec.fast-rec-full.seg.mlp-att.am2048.prev-att-in-state.frame-length-model-in_am+prev-out-embed.no-ctx-reg": {
      "config": dict(default_variant_transducer, use_attention=True, label_type="rna-bpe", lstm_dim=1024,
                     ctx_size="inf",
                     att_type="mlp", att_area="seg", time_red=[3, 2], prev_att_in_state=True,
                     length_model_focal_loss=0.0,
                     length_model_inputs=["am", "prev_out_embed"], fast_rec=True, fast_rec_full=True,
                     label_model_focal_loss=0.0, att_ctx_reg=False, lr_measure="dev_error_label_model/label_prob",
                     set_dim_tag_correctly=False, seg_boundary_random_shift=1),
      "returnn_python_exe": RETURNN_EXE,
      "returnn_root": RETURNN_ROOT,
    },
  },
  "segmental_kl_div": {
    "seg.rna-bpe.lr-meas-lab.full-ctx.time-red6.fast-rec.fast-rec-full.seg.mlp-att.kl-div-1.00.am2048.prev-att-in-state.frame-length-model-in_am+prev-out-embed.no-ctx-reg": {
      "config": dict(default_variant_transducer, use_attention=True, label_type="rna-bpe", lstm_dim=1024,
                     ...
                     length_model_inputs=["am", "prev_out_embed"], fast_rec=True, fast_rec_full=True,
                     label_model_focal_loss=0.0, att_ctx_reg=False, lr_measure="dev_error_label_model/label_prob",
                     set_dim_tag_correctly=False, att_weights_kl_div_scale=1.),
      "returnn_python_exe": RETURNN_EXE,
      "returnn_root": RETURNN_ROOT,
    },
    "seg.rna-bpe.lr-meas-lab.full-ctx.time-red6.fast-rec.fast-rec-full.seg.mlp-att.kl-div-2.00.am2048.prev-att-in-state.frame-length-model-in_am+prev-out-embed.no-ctx-reg": {
      "config": dict(default_variant_transducer, use_attention=True, label_type="rna-bpe", lstm_dim=1024,
                     ctx_size="inf",
                     att_type="mlp", att_area="seg", time_red=[3, 2], prev_att_in_state=True,
                     length_model_focal_loss=0.0,
                     length_model_inputs=["am", "prev_out_embed"], fast_rec=True, fast_rec_full=True,
                     label_model_focal_loss=0.0, att_ctx_reg=False, lr_measure="dev_error_label_model/label_prob",
                     set_dim_tag_correctly=False, att_weights_kl_div_scale=2.),
      "returnn_python_exe": RETURNN_EXE,
      "returnn_root": RETURNN_ROOT,
    },
    "seg.rna-bpe.lr-meas-lab.full-ctx.time-red6.fast-rec.fast-rec-full.seg.mlp-att.kl-div-0.50.am2048.prev-att-in-state.frame-length-model-in_am+prev-out-embed.no-ctx-reg": {
      "config": dict(default_variant_transducer, use_attention=True, label_type="rna-bpe", lstm_dim=1024,
                     ctx_size="inf",
                     att_type="mlp", att_area="seg", time_red=[3, 2], prev_att_in_state=True,
                     length_model_focal_loss=0.0,
                     length_model_inputs=["am", "prev_out_embed"], fast_rec=True, fast_rec_full=True,
                     label_model_focal_loss=0.0, att_ctx_reg=False, lr_measure="dev_error_label_model/label_prob",
                     set_dim_tag_correctly=False, att_weights_kl_div_scale=.5),
      "returnn_python_exe": RETURNN_EXE,
      "returnn_root": RETURNN_ROOT,
    },
  },
  "segmental_conformer": {
    ...
    #                      length_model_inputs=["am", "prev_out_embed"], fast_rec=True, fast_rec_full=True,
    #                      label_model_focal_loss=0.0, att_ctx_reg=False, lr_measure="dev_error_label_model/label_prob",
    #                      enc_type="conf-mohammad-11-7", set_dim_tag_correctly=True,
    #                      features="raw"),
    # "returnn_python_exe": RETURNN_EXE,
  ...
  "segmental_w_split_silence_sep_sil_model": {
  },
  "center-window_diff_sizes": {
    "seg.win-size0.rna-bpe.lr-meas-lab.full-ctx.time-red6.fast-rec.fast-rec-full.seg.mlp-att.am2048.prev-att-in-state.frame-length-model-in_am+prev-out-embed.no-ctx-reg": {
      "config": dict(default_variant_transducer, use_attention=True, label_type="rna-bpe", lstm_dim=1024,
                     ctx_size="inf",
                     att_type="mlp", att_area="seg", time_red=[3, 2], prev_att_in_state=True,
                     length_model_focal_loss=0.0,
                     length_model_inputs=["am", "prev_out_embed"], fast_rec=True, fast_rec_full=True,
                     label_model_focal_loss=0.0, att_ctx_reg=False, segment_center_window_size=0,
                     lr_measure="dev_error_label_model/label_prob", set_dim_tag_correctly=False),
      "returnn_python_exe": RETURNN_EXE,
      "returnn_root": RETURNN_ROOT,
    },
    "seg.win-size2.rna-bpe.lr-meas-lab.full-ctx.time-red6.fast-rec.fast-rec-full.seg.mlp-att.am2048.prev-att-in-state.frame-length-model-in_am+prev-out-embed.no-ctx-reg": {
      "config": dict(default_variant_transducer, use_attention=True, label_type="rna-bpe", lstm_dim=1024,
                     ctx_size="inf",
                     att_type="mlp", att_area="seg", time_red=[3, 2], prev_att_in_state=True,
                     length_model_focal_loss=0.0,
                     length_model_inputs=["am", "prev_out_embed"], fast_rec=True, fast_rec_full=True,
                     label_model_focal_loss=0.0, att_ctx_reg=False, segment_center_window_size=1,
                     lr_measure="dev_error_label_model/label_prob", set_dim_tag_correctly=False),
      "returnn_python_exe": RETURNN_EXE,
      "returnn_root": RETURNN_ROOT,
    },
    "seg.win-size4.rna-bpe.lr-meas-lab.full-ctx.time-red6.fast-rec.fast-rec-full.seg.mlp-att.am2048.prev-att-in-state.frame-length-model-in_am+prev-out-embed.no-ctx-reg": {
      "config": dict(default_variant_transducer, use_attention=True, label_type="rna-bpe", lstm_dim=1024,
                     ctx_size="inf",
                     att_type="mlp", att_area="seg", time_red=[3, 2], prev_att_in_state=True,
                     length_model_focal_loss=0.0,
                     length_model_inputs=["am", "prev_out_embed"], fast_rec=True, fast_rec_full=True,
                     label_model_focal_loss=0.0, att_ctx_reg=False, segment_center_window_size=2,
                     lr_measure="dev_error_label_model/label_prob", set_dim_tag_correctly=False),
      "returnn_python_exe": RETURNN_EXE,
      "returnn_root": RETURNN_ROOT,
    },
    "seg.win-size16.rna-bpe.lr-meas-lab.full-ctx.time-red6.fast-rec.fast-rec-full.seg.mlp-att.am2048.prev-att-in-state.frame-length-model-in_am+prev-out-embed.no-ctx-reg": {
      "config": dict(default_variant_transducer, use_attention=True, label_type="rna-bpe", lstm_dim=1024,
                     ctx_size="inf",
                     att_type="mlp", att_area="seg", time_red=[3, 2], prev_att_in_state=True,
                     length_model_focal_loss=0.0,
                     length_model_inputs=["am", "prev_out_embed"], fast_rec=True, fast_rec_full=True,
                     label_model_focal_loss=0.0, att_ctx_reg=False, segment_center_window_size=8,
                     lr_measure="dev_error_label_model/label_prob", set_dim_tag_correctly=False),
      "returnn_python_exe": RETURNN_EXE,
      "returnn_root": RETURNN_ROOT,
    },
  },
  "center-window_diff_sizes_wo_chunking": {
    "seg.win-size0.rna-bpe.lr-meas-lab.full-ctx.time-red6.fast-rec.fast-rec-full.seg.mlp-att.am2048.prev-att-in-state.frame-length-model-in_am+prev-out-embed.no-ctx-reg.chunk-size-None": {
      "config": dict(default_variant_transducer, use_attention=True, label_type="rna-bpe", lstm_dim=1024,
                     ctx_size="inf",
                     att_type="mlp", att_area="seg", time_red=[3, 2], prev_att_in_state=True,
                     length_model_focal_loss=0.0,
                     length_model_inputs=["am", "prev_out_embed"], fast_rec=True, fast_rec_full=True,
                     label_model_focal_loss=0.0, att_ctx_reg=False, segment_center_window_size=0, chunk_size=None,
                     lr_measure="dev_error_label_model/label_prob", set_dim_tag_correctly=False),
      "returnn_python_exe": RETURNN_EXE,
      "returnn_root": RETURNN_ROOT,
    },
    "seg.win-size2.rna-bpe.lr-meas-lab.full-ctx.time-red6.fast-rec.fast-rec-full.seg.mlp-att.am2048.prev-att-in-state.frame-length-model-in_am+prev-out-embed.no-ctx-reg.chunk-size-None": {
      "config": dict(default_variant_transducer, use_attention=True, label_type="rna-bpe", lstm_dim=1024,
                     ctx_size="inf",
                     att_type="mlp", att_area="seg", time_red=[3, 2], prev_att_in_state=True,
                     length_model_focal_loss=0.0,
                     length_model_inputs=["am", "prev_out_embed"], fast_rec=True, fast_rec_full=True,
                     label_model_focal_loss=0.0, att_ctx_reg=False, segment_center_window_size=1, chunk_size=None,
                     lr_measure="dev_error_label_model/label_prob", set_dim_tag_correctly=False),
      "returnn_python_exe": RETURNN_EXE,
      "returnn_root": RETURNN_ROOT,
    },
    "seg.win-size4.rna-bpe.lr-meas-lab.full-ctx.time-red6.fast-rec.fast-rec-full.seg.mlp-att.am2048.prev-att-in-state.frame-length-model-in_am+prev-out-embed.no-ctx-reg.chunk-size-None": {
      "config": dict(default_variant_transducer, use_attention=True, label_type="rna-bpe", lstm_dim=1024,
                     ctx_size="inf",
                     att_type="mlp", att_area="seg", time_red=[3, 2], prev_att_in_state=True,
                     length_model_focal_loss=0.0,
                     length_model_inputs=["am", "prev_out_embed"], fast_rec=True, fast_rec_full=True,
                     label_model_focal_loss=0.0, att_ctx_reg=False, segment_center_window_size=2, chunk_size=None,
                     lr_measure="dev_error_label_model/label_prob", set_dim_tag_correctly=False),
      "returnn_python_exe": RETURNN_EXE,
      "returnn_root": RETURNN_ROOT,
    },
    "seg.win-size8.rna-bpe.lr-meas-lab.full-ctx.time-red6.fast-rec.fast-rec-full.seg.mlp-att.am2048.prev-att-in-state.frame-length-model-in_am+prev-out-embed.no-ctx-reg.chunk-size-None": {
      "config": dict(default_variant_transducer, use_attention=True, label_type="rna-bpe", lstm_dim=1024,
                     ctx_size="inf",
                     att_type="mlp", att_area="seg", time_red=[3, 2], prev_att_in_state=True,
                     length_model_focal_loss=0.0,
                     length_model_inputs=["am", "prev_out_embed"], fast_rec=True, fast_rec_full=True,
                     label_model_focal_loss=0.0, att_ctx_reg=False, segment_center_window_size=4, chunk_size=None,
                     lr_measure="dev_error_label_model/label_prob", set_dim_tag_correctly=False),
      "returnn_python_exe": RETURNN_EXE,
      "returnn_root": RETURNN_ROOT,
    },
    "seg.win-size16.rna-bpe.lr-meas-lab.full-ctx.time-red6.fast-rec.fast-rec-full.seg.mlp-att.am2048.prev-att-in-state.frame-length-model-in_am+prev-out-embed.no-ctx-reg.chunk-size-None": {
      "config": dict(default_variant_transducer, use_attention=True, label_type="rna-bpe", lstm_dim=1024,
                     ctx_size="inf",
                     att_type="mlp", att_area="seg", time_red=[3, 2], prev_att_in_state=True,
                     length_model_focal_loss=0.0,
                     length_model_inputs=["am", "prev_out_embed"], fast_rec=True, fast_rec_full=True,
                     label_model_focal_loss=0.0, att_ctx_reg=False, segment_center_window_size=8, chunk_size=None,
                     lr_measure="dev_error_label_model/label_prob", set_dim_tag_correctly=False),
      "returnn_python_exe": RETURNN_EXE,
      "returnn_root": RETURNN_ROOT,
    },
  ...
  "center-window_w_silence": {
    "seg.win-size8.rna-bpe-sil-time-red-6.lr-meas-lab.full-ctx.time-red6.fast-rec.fast-rec-full.seg.mlp-att.am2048.prev-att-in-state.frame-length-model-in_am+prev-out-embed.no-ctx-reg": {
      "config": dict(default_variant_transducer, use_attention=True, label_type="rna-bpe-sil-time-red-6", lstm_dim=1024,
                     ctx_size="inf",
                     att_type="mlp", att_area="seg", time_red=[3, 2], prev_att_in_state=True,
                     length_model_focal_loss=0.0,
                     length_model_inputs=["am", "prev_out_embed"], fast_rec=True, fast_rec_full=True,
                     label_model_focal_loss=0.0, att_ctx_reg=False, segment_center_window_size=4,
                     lr_measure="dev_error_label_model/label_prob", set_dim_tag_correctly=True),
      "returnn_python_exe": RETURNN_EXE,
      "returnn_root": RETURNN_ROOT,
    },
    "seg.win-size8.rna-bpe-split-sil-time-red-6.lr-meas-lab.full-ctx.time-red6.fast-rec.fast-rec-full.seg.mlp-att.am2048.prev-att-in-state.frame-length-model-in_am+prev-out-embed.no-ctx-reg": {
      "config": dict(default_variant_transducer, use_attention=True, label_type="rna-bpe-split-sil-time-red-6",
  ...
  "global_conformer": {
    "glob.best-model.raw-features.bpe534.time-red6.am2048.1pretrain-reps.conf-mohammad-11.4.d-lr.no-weight-feedback.no-prev-target-in-readout.pretrain-like-seg": {
      "config": dict(
        default_variant_enc_dec, glob_model_type="best", pretrain_reps=1, label_type="bpe534",
        with_weight_feedback=False, pretrain_type="like-seg", att_ctx_with_bias=False, use_l2=True,
        prev_target_in_readout=False, enc_type="conf-mohammad-11.4", set_dim_tag_correctly=True,
        features="raw", pretrain=False,
        dynamic_lr=True
      ),
      "returnn_python_exe": RETURNN_EXE,
      "returnn_root": RETURNN_CURRENT_ROOT,
...
for model_type in models:
  if model_type in [
    ...
    "segmental_w_split_silence_sep_sil_model", "center-window_w_silence"]:
