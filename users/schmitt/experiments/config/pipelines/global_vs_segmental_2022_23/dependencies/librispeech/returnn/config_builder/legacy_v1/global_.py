...
class GlobalEncoderDecoderConfig:
  def __init__(
    ...
    if self.task == "train" and enc_type == "conf-mohammad-11.4":
      self.function_prolog += [speed_pert]

    if dynamic_lr:
      self.function_prolog += [dynamic_learning_rate]

    if not enc_type.startswith("conf-mohammad"):
      ...
      """
      In this case, we use a `get_network` function to get different networks for pretraining
      """
      from i6_experiments.users.schmitt.experiments.config.pipelines.global_vs_segmental_2022_23.dependencies.swb.returnn.network_builder import mohammad_conformer
      conformer_version = re.findall("conf-mohammad-(.+)", enc_type)
      assert len(conformer_version) == 1
      conformer_version = conformer_version[0]

      if conformer_version == "11.4":
      ...
      self.function_prolog += [get_network]
  ...
  def get_config(self):
    ...
    return ReturnnConfig(
      ...
      staged_network_dict=self.networks_dict
    )
