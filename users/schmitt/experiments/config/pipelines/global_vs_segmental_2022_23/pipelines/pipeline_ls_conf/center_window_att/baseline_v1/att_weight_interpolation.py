...
def center_window_att_import_global_global_ctc_align_gaussian_att_weight_interpolation_length_model_only_non_blank_ctx_use_label_model_state(
        win_size_list: Tuple[int, ...] = (128,),
        n_epochs_list: Tuple[int, ...] = (10,),
        const_lr_list: Tuple[float, ...] = (1e-4,),
        std_list: Tuple[float, ...] = (1.,),
    ...
    for n_epochs in n_epochs_list:
      for const_lr in const_lr_list:
        for std in std_list:
          for gauss_scale in gauss_scale_list:
            for dist_type in dist_type_list:
              alias = f"{base_alias}/att_weight_interpolation_length_model_only_non_blank_ctx_use_label_model_state/win-size-%d_%d-epochs_%f-const-lr/%s/std-%f_scale-%f" % (
                win_size, n_epochs, const_lr, dist_type, std, gauss_scale
              )
