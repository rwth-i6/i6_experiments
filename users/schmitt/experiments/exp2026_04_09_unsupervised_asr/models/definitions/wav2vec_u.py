"""
wav2vec-U (wav2vec Unsupervised) GAN model — the *first* training stage of the fairseq
``examples/wav2vec/unsupervised`` pipeline (generative adversarial training that matches
segmented/PCA-reduced speech representations to unpaired phoneme text).

This is a self-contained reimplementation of
``fairseq/examples/wav2vec/unsupervised/models/wav2vec_u.py`` (commit on ``main``). We do **not**
import from fairseq; the few fairseq building blocks it relies on (``SamePad``, ``TransposeLast``)
are re-implemented below and the fairseq ``target_dictionary`` is replaced by a handful of explicit
index arguments.

The companion train step (``train_steps.wav2vec_u``) plays the role of fairseq's ``ModelCriterion``
+ the composite-optimizer alternation: it drives the generator/discriminator updates on alternating
global steps and marks the losses this module returns.

Entry point for RETURNN: ``Model(epoch, step, *, ...)`` (wired via
``__network_module = "definitions.wav2vec_u.Model"``).
"""

from __future__ import annotations

__all__ = ["Model", "Wav2vec_UConfig", "SegmentationConfig", "SegmentationType"]
...
# ---------------------------------------------------------------------------------------------
# fairseq building blocks (re-implemented, see fairseq/modules/{same_pad,transpose_last}.py)
# ---------------------------------------------------------------------------------------------


class SamePad(nn.Module):
    """Trim conv output on the right to emulate 'same' padding (causal or symmetric)."""
...
class TransposeLast(nn.Module):
    """Transpose the last two dimensions of the input."""
...
# ---------------------------------------------------------------------------------------------
# Top-level GAN model (fairseq ``Wav2vec_U``)
# ---------------------------------------------------------------------------------------------


class Model(nn.Module):
    """
    wav2vec-U GAN model. Equivalent to fairseq's ``Wav2vec_U`` (``@register_model("wav2vec_u")``).

    Instead of a fairseq ``target_dictionary`` the label-vocabulary specifics are passed explicitly:

    :param output_dim: size of the phoneme output vocabulary (``len(target_dictionary)``), i.e. the
        number of generator/discriminator output classes (includes the special symbols below).
    :param pad_idx: index of the padding symbol (fairseq ``dictionary.pad()``); used to build the
        token (real-text) padding mask.
    :param eos_idx: index of the eos symbol (fairseq ``dictionary.eos()``).
    :param sil_idx: index of ``<SIL>`` in the vocabulary if present (fairseq
        ``dictionary.index("<SIL>")``), else None. Used for ``blank_is_sil`` / ``zero_index``.
    :param unk_idx: index of the unk symbol; only used to assert the blank index differs from it.
    """
    ...
    def __init__(
        ...
        *,
        ...
        eos_idx: int = 2,
        unk_idx: int = 3,
        sil_idx: Optional[int] = None,
        segmentation: Optional[Dict] = None,
        **cfg_kwargs,
    ):
        super().__init__()

        seg_cfg = self._build_segmentation_config(segmentation)
        cfg = Wav2vec_UConfig.from_kwargs(segmentation=seg_cfg, **cfg_kwargs)
        self.cfg = cfg

        self.zero_index = sil_idx if sil_idx is not None else 0
        ...
        self.blank_index = sil_idx if (cfg.blank_is_sil and sil_idx is not None) else 0
        assert self.blank_index != unk_idx
    ...
    @staticmethod
    def _build_segmentation_config(segmentation: Optional[Dict]) -> SegmentationConfig:
        if segmentation is None:
            return SegmentationConfig()
        segmentation = dict(segmentation)
        seg_type = segmentation.get("type", SegmentationType.NONE)
        if isinstance(seg_type, str):
            seg_type = SegmentationType[seg_type]
        segmentation["type"] = seg_type
        return SegmentationConfig(**segmentation)

    # --- param-group freezing (composite-optimizer alternation is emulated by the train step) ---

    def _set_group_requires_grad(self, group: str, flag: bool):
        for p in self.parameters():
            if getattr(p, "param_group", None) == group:
                p.requires_grad = flag

    def freeze_generator(self):
        self._set_group_requires_grad("generator", False)

    def unfreeze_generator(self):
        self._set_group_requires_grad("generator", True)

    def freeze_discriminator(self):
        self._set_group_requires_grad("discriminator", False)

    def unfreeze_discriminator(self):
        self._set_group_requires_grad("discriminator", True)

    # --- adversarial training bookkeeping ---
    ...
    def normalize(self, dense_x):
        ...
        code_perplexity = torch.exp(-torch.sum(hard_probs * torch.log(hard_probs + 1e-7), dim=-1))
