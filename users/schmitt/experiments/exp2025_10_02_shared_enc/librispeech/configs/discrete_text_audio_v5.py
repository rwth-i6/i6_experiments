"""
Same as v4 but using a shared decoder with separate embeddings and output layers for text and audio
"""
...
def py():
    ...
    for config, train_name in [
        *[(dict_update_deep(
            ...
            f"baseline_bs-{batch_size}_mal-{min_audio_len}_mpl-{min_phon_len}"
        ) for batch_size, min_audio_len, min_phon_len in (
                (30_000, 600, 100),
        )]
    ]:
        run_experiment(
    ...
    for config, train_name in [
        *[(dict_update_deep(
            copy.deepcopy(base_config),
            {
                ...
                "train_args.text_aux_loss_scales": (0.5,),
            },
        ),
            f"baseline_bs-{batch_size}_mal-{min_audio_len}_mpl-{min_phon_len}_w-aux-loss-0.5"
        ) for batch_size, min_audio_len, min_phon_len in (
                (30_000, 600, 100),
        )]
    ]:
