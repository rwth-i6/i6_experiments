...
def py():
    ...
    train_data_v2 = get_train_data(
        ...
        phoneme_partition_epoch=1,
        audio_partition_epoch=1,
    )
    ...
    for config, train_name in [
        *[(dict_update_deep(
            copy.deepcopy(base_config),
            {
                ...
                "train_args.audio_masking_opts.min_span": min_span,
                "train_args.audio_masking_opts.max_span": max_span,
                "train_args.text_aux_loss_scales": (1.0,),
                "train_args.pseudo_audio_text_ce_loss_scale": 1.0,
                "train_args.pseudo_text_audio_ce_loss_scale": text_audio_loss_scale,
                "model_args.text_out_dim": train_data.datastreams["labels"].vocab_size,
                "model_args.audio_out_dim": train_data.datastreams["features"].vocab_size,
                "model_args.text_aux_loss_layers": text_aux_loss_layers,
                "model_args.audio_aux_loss_layers": (),
            },
            [
                "model_args.aux_loss_layers"
            ]
        ),
            f"baseline_bs-{batch_size}_text-aux-loss-{'-'.join(str(l) for l in text_aux_loss_layers)}_text-audio-loss-scale-{text_audio_loss_scale}"
        ) for batch_size, mask_prob, min_span, max_span, text_aux_loss_layers, text_audio_loss_scale in (
                (30_000, 0.3, 1, 3, (6,), 0.0),
                (30_000, 0.3, 1, 3, (6,), 1.0),
        )]
    ]:
        run_experiment(
            training_name=f"{prefix_name}/{train_name}",
    ...
    for config, train_name in [
        *[(dict_update_deep(
        ...
        run_experiment(
            training_name=f"{prefix_name}/{train_name}_data-v2",
            config=copy.deepcopy(config),
