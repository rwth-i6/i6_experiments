__all__ = ["State", "LabelScorer", "beam_search_v1"]
...
from i6_models.assemblies.transformer.transformer_decoder_v1 import TransformerDecoderV1
...
class LabelScorer(Protocol, Generic[State]):
    ...
    @abstractmethod
    def step_decoder(self, labels: Tensor, state: State, decoder: TransformerDecoderV1) -> Tuple[Tensor, State]:
        """
...
def beam_search_v1(
    *,
    ...
    with torch.no_grad():
        ...
        initial_beam = 1
        target = torch.full([batch_size, initial_beam], bos_idx, dtype=torch.int32, device=device)  # Batch, Beam
        ...
        ended_default = F.one_hot(torch.tensor(eos_idx, device=device), num_classes=out_dim)
