...
from i6_experiments.users.schmitt.util.serialization import get_import_py_code

from i6_experiments.users.schmitt import tools_paths
from i6_experiments.users.schmitt.datasets.task import Task
from i6_experiments.users.schmitt.datasets.score_results import RecogOutput, ScoreResultCollection
from i6_experiments.users.schmitt.model_interfaces import ModelDef, ModelDefWithCfg, RecogDef, serialize_model_def
from i6_experiments.users.schmitt.model_interfaces.model_with_checkpoints import ModelWithCheckpoint, \
  ModelWithCheckpoints
from i6_experiments.users.schmitt.returnn.training import get_relevant_epochs_from_training_learning_rate_scores
...
from i6_experiments.users.schmitt.util.util import PartialImportCustom, ReturnnConfigCustom
from i6_experiments.users.schmitt.experiments.exp2025_03_10_ctc_usr.ctc import model_recog_lm, model_recog_flashlight, \
  model_recog_lm_albert
from i6_experiments.users.schmitt.experiments.exp2025_03_10_ctc_usr.lm.n_gram import get_kenlm_n_gram, get_binary_lm
from i6_experiments.users.schmitt.datasets.librispeech import get_bpe_lexicon, LibrispeechOggZip
from i6_experiments.users.schmitt.recognition.scoring import ComputeWERJob, _score_recog
...
def recog_training_exp(
  ...
  if save_pseudo_labels is not None:
    ...
    # TODO: here, we can use a custom recog_input, which uses epoch 0 for recognition
    # TODO: maybe better, we add an entry to the summary json, which contains the result for random init
    # TODO: add epoch 0 to fixed_kept_epochs of ModelWithCheckpoints
...
class _RecogAndScoreFunc:
  ...
  def __call__(self, epoch_or_ckpt: Union[int, PtCheckpoint]) -> tuple[ScoreResultCollection, tk.Path]:
    ...
    res, label_paths = recog_model(
      ...
      cache_manager=self.cache_manager,
      # TODO: just for debug use dev-other
      dev_sets=["dev-other"]
    )
...
def recog_model(
  ...
  return task.collect_score_results_func(
    outputs) if calculate_scores else None, recog_paths if save_pseudo_labels else None
...
def search_config_v2(
  ...
  returnn_recog_config_dict["recog_hyperparams"] = args
    ...
    python_epilog=[
      serialization.Collection(
        [
          ...
          *serialize_model_def(model_def),
          serialization.Import(_returnn_v2_get_model, import_as="get_model"),
          serialization.Import(recog_def, import_as="_recog_def"),
          # PartialImportCustom(
...
def _returnn_v2_forward_step(*, model, extern_data: TensorDict, **_kwargs_unused):
  ...
  recog_def = config.typed_value("_recog_def")
  extra = {}
  recog_hyperparams = config.typed_value("recog_hyperparams", {})
  extra.update(recog_hyperparams)
...
class GetBestRecogTrainExp(sisyphus.Job):
  ...
  def hash(cls, parsed_args: Dict[str, Any]) -> str:
    ...
    assert isinstance(res, ScoreResultCollection), f"unexpected {res!r}"
