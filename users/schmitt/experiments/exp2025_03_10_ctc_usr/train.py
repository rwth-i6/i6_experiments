...
import copy
from i6_experiments.users.schmitt.model_interfaces import ModelT, ModelDef, ModelDefWithCfg, TrainDef, \
  serialize_model_def
...
if TYPE_CHECKING:
  ...
  from i6_experiments.users.schmitt.datasets.task import Task, DatasetConfig
  from i6_experiments.users.schmitt.model_with_checkpoints import ModelWithCheckpoints, Checkpoint
...
def train(
  ...
  from i6_experiments.users.schmitt.datasets.utils import multi_proc as mp_ds_utils
  from i6_experiments.users.schmitt.model_interfaces.model_with_checkpoints import ModelWithCheckpoints
  from i6_experiments.users.schmitt.experiments.exp2025_03_10_ctc_usr.recog import SharedPostConfig
  from i6_experiments.users.schmitt.util.sis_setup import get_base_module
  ...
  if init_params:
    returnn_train_config_dict["preload_from_files"] = {
      "pretrained_params": {
...
def _returnn_v2_train_step(*, model, extern_data: TensorDict, **_kwargs_unused):
  ...
  elif train_def.__name__ == "lm_train_def":
    train_def(
      model=model,
      data=data,
      data_spatial_dim=data_spatial_dim,
