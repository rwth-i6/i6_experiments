...
def calc_segment_stats_with_sil(blank_idx, sil_idx, vocab):
  ...
  while dataset.is_less_than_num_seqs(seq_idx):
      ...
      try:
        ...
        for i, idx in enumerate(non_blank_idxs):
          ...
          if seg_len > max_seg_len:
            max_seg_len = seg_len
          if seg_len > max_seg_len_current_seq:
            max_seg_len_current_seq = seg_len
          ...
          if idx in sil_idxs:
            ...
            if seg_len > 20:
              print(f"Seq with segment length {seg_len} for segment {i} ({label_str}): {seq_tag} ({seq_str})")
              print("\n")
    ...
    if max_seg_len_current_seq < 20:
      print(f"Seq with max segment length below 20: {seq_tag}")
      print("\n")
