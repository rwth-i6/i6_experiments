...
def dump(hdf_dataset_in, hdf_dataset_out, sil_idx, blank_idx, max_len):
  """
  :type dataset: Dataset.Dataset
  :param options: argparse.Namespace
  """
  seq_idx = 0
  max_len = np.floor(max_len)
  ...
  while hdf_dataset_in.is_less_than_num_seqs(seq_idx) and seq_idx <= float("inf"):
    ...
    sil_seg_bounds = np.where(data == sil_idx)[0]
    seg_bounds = np.where(data != blank_idx)[0]
    # pretend that there is another label before the 0th position
    seg_bounds = np.insert(seg_bounds, 0, -1)

    print("DATA: ", data)

    for i, bound in enumerate(seg_bounds):
      if bound in sil_seg_bounds:
        # calculate the size of the silence segment
        size = seg_bounds[i] - seg_bounds[i-1]
        # divide each silence segment into segments with max size max_len
        new_seg_bounds = [int(np.floor(seg_bounds[i-1] + max_len * (j + 1))) for j in range(int(size/max_len))]
        data[new_seg_bounds] = sil_idx


    print("NEW DATA: ", data)
    print("SIL SEG BOUNDS: ", sil_seg_bounds)
...
def main(argv):
  ...
  arg_parser.add_argument('--max_len', type=float)
  ...
  try:
    dump(hdf_dataset_in, hdf_dataset_out, sil_idx=args.sil_idx, blank_idx=args.blank_idx, max_len=args.max_len)
