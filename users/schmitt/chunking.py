...
custom_chunkin_w_reduction_func_str = """
def chunking(dataset, seq_idx_start, **_kwargs):
    from returnn.util.basic import NumbersDict
    seq_idx = seq_idx_start
    while dataset.is_less_than_num_seqs(seq_idx):
        ...
        chunk_size["targets"] = int((chunk_size["data"] - {red_subtrahend} + {red_factor} - 1) / {red_factor})
        chunk_step["targets"] = int((chunk_step["data"] - {red_subtrahend} + {red_factor} - 1) / {red_factor})
