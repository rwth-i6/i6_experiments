...
class ReturnnDumpSearchJob(Job):
  """
  Given a model checkpoint, run search task with RETURNN
  """

  def __init__(self, search_data, model_checkpoint, returnn_config, *, output_mode="py", log_verbosity=3, device="gpu",
          time_rqmt=4, mem_rqmt=4, cpu_rqmt=2, returnn_python_exe=None, returnn_root=None, ):
    """
    ...
    self.out_search_seqs_file = self.output_path("search_out_seqs")

    self.returnn_config = ReturnnDumpSearchJob.create_returnn_config(**kwargs)
    self.returnn_config.post_config["search_output_file"] = self.out_search_file

    self.rqmt = {
      "gpu": 1 if device == "gpu" else 0, "cpu": cpu_rqmt, "mem": mem_rqmt, "time": time_rqmt, }
  ...
  def create_files(self):
    ...
    cmd = [tk.uncached_path(self.returnn_python_exe), os.path.join(tk.uncached_path(self.returnn_root), "rnn.py"),
    ...
    assert os.path.exists(
      tk.uncached_path(self.model_checkpoint.index_path)), "Provided model does not exists: %s" % str(
      self.model_checkpoint)

  def run(self):
    call = [tk.uncached_path(self.returnn_python_exe), os.path.join(tk.uncached_path(self.returnn_root), "rnn.py"),
      self.out_returnn_config_file.get_path(), ]
    sp.check_call(call)
    shutil.move("search_out_seqs", self.out_search_seqs_file.get_path())

  @classmethod
  def create_returnn_config(cls, search_data, model_checkpoint, returnn_config, output_mode, log_verbosity, device,
          **kwargs, ):
    """
    ...
    config = {
      "load": model_checkpoint.ckpt_path, "search_output_file_format": output_mode, "need_data": False,
      "search_do_eval": 0, }
    ...
    if "search_data" in original_config:
      config["search_data"] = {
        **original_config["search_data"].copy(), **search_data, }
    else:
      config["search_data"] = search_data

    post_config = {
      "device": device, "log": ["./returnn.log"], "log_verbosity": log_verbosity, }
  ...
  def hash(cls, kwargs):
    d = {
      "returnn_config": ReturnnDumpSearchJob.create_returnn_config(**kwargs),
      "returnn_python_exe": kwargs["returnn_python_exe"], "returnn_root": kwargs["returnn_root"], }
    return super().hash(d)