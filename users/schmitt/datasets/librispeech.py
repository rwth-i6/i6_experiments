...
from i6_experiments.users.schmitt.util.generic_job_output import generic_job_output
from i6_experiments.users.schmitt import tools_paths
from i6_experiments.users.schmitt.util.basic import make_hashable
from i6_experiments.users.schmitt.speed_pert.librosa_09_10_11_kaiser_fast import (
...
from i6_experiments.users.schmitt.datasets.utils.spm import SentencePieceModel
from i6_experiments.users.schmitt.datasets.utils.bytes import Utf8BytesVocab
from i6_experiments.users.schmitt.datasets.utils.char import get_char_vocab
from i6_experiments.users.schmitt.datasets.utils.lexicon import get_phon_vocab

if TYPE_CHECKING:
  from returnn.tensor import Tensor, Dim

from .utils.pseudo_labels import CorpusReplaceOrthFromPyDictJob, get_ogg_zip_dict_pseudo_labels, MetaDataset, \
  GetScoresDummy
...
@cache
def _get_librispeech_ogg_zip_dict_pseudo_labels(pseudo_labels_path: tk.Path, part: str, return_scores: bool) -> Dict[
  str, tk.Path]:
...
def get_vocab_by_str(vocab: str, train_small: bool = False) -> Union[
  ...
  if re.match("^spm[0-9]+.*$", vocab):
    ...
    return get_phon_vocab(num_classes=29, extra_labels=("\x00",), eos_label="\x00")
  else:
...
class LibrispeechOggZip(DatasetConfig):
  ...
  def get_dataset(self, key: str, *, training: bool = False, subset: Optional[int] = None,
    ...
    if training and self.pseudo_label_path:
      ...
      d = MetaDataset({"data": ("zip_dataset", "data"), "classes": ("zip_dataset", "classes"),
                       "weights": ("weights_datasets", "data")}, {"zip_dataset": d, "weights_datasets": d_weights},
                      "zip_dataset").as_returnn_opts()
    return d
...
def get_librispeech_task_raw_v2(
  ...
  if save_pseudo_labels is not None:
    ds_ls = ["train-clean-360", "train-other-500"] if save_pseudo_labels == TrainDatasetSel.train_860h else [
...
from i6_experiments.users.mueller.datasets.librispeech import \
  get_librispeech_task_raw_v2 as get_librispeech_task_raw_v2_mueller
...
def get_librispeech_task_raw_v2_schmitt(
  ...
  vocab = kwargs["vocab"]
  if vocab == "phon":
    # in Marten's setup, the "phon" option does not exist, therefore we change it to "char" there.
    # this is then overridden in the get_vocab_by_str call below.
    kwargs["vocab"] = "char"
