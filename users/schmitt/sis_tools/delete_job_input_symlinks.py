"""
In gs.JOB_INPUT ("input") directory, Sisyphus keeps symlinks to input files.
Those are purely for the user and not really necessary.
Also, the same info can be found in the ``info`` file,
so the symlinks are purely redundant.

Copy of :mod:`i6_experiments.users.zeyer.sis_tools.delete_job_input_symlinks`,
which does not follow symlinked job dirs:
a job dir can be a symlink into another user's setup (shared dataset jobs),
and ``os.walk`` lists such dirs, while ``os.path.isfile``/``os.path.isdir``
in ``_is_job_dir`` resolve through them. The original then calls ``shutil.rmtree``
on the *other* setup's ``input`` dir. Here such job dirs are skipped, jobs whose
``input`` dir is not owned by us are skipped as well, and a failing ``rmtree``
no longer aborts the whole run.
"""

import sys
import os
import argparse
import time
import shutil
import stat
from collections import defaultdict
from functools import reduce


_my_dir = os.path.dirname(__file__)
_base_dir = reduce(lambda p, _: os.path.dirname(p), range(4), _my_dir)
_setup_dir = os.path.dirname(_base_dir)


def _setup():
    # In case the user started this script directly.
    if not globals().get("__package__"):
        globals()["__package__"] = "i6_experiments.users.schmitt.sis_tools"
        if _base_dir not in sys.path:
            sys.path.append(_base_dir)
        # Sisyphus lives in <setup>/tools/sisyphus in some setups, in <setup>/sisyphus in others.
        for sis_dir in [_setup_dir + "/tools/sisyphus", _setup_dir + "/sisyphus"]:
            if os.path.isdir(sis_dir + "/sisyphus") and sis_dir not in sys.path:
                sys.path.append(sis_dir)
                break


_setup()


def main():
    from sisyphus import gs

    arg_parser = argparse.ArgumentParser(description=f"{__doc__}", formatter_class=argparse.RawDescriptionHelpFormatter)
    arg_parser.add_argument("work_dir", default=gs.WORK_DIR)
    arg_parser.add_argument("--mode", default="dryrun", help="dryrun (default), remove")
    args = arg_parser.parse_args()

    work_dir = args.work_dir
    my_uid = os.getuid()

    prev_report_time = time.monotonic()
    count_all_jobs = 0
    count_cleaned_jobs = 0
    count_skipped_links = 0
    count_skipped_foreign = 0
    count_errors = 0
    total_counts = defaultdict(int)  # file-type -> count, summed over all cleaned input dirs

    for root, dirs, files in os.walk(work_dir):
        if time.monotonic() - prev_report_time > 10:
            prev_report_time = time.monotonic()
            print(f"Jobs found so far: {count_all_jobs}, cleaned: {count_cleaned_jobs}, current dir: {root!r}")

        exclude_recurse_dirs = set()
        for dir_name in dirs:
            job_dir = root + "/" + dir_name

            if _is_job_dir(job_dir):
                count_all_jobs += 1
                exclude_recurse_dirs.add(dir_name)

                if os.path.islink(job_dir):
                    # Symlink into some other setup (e.g. a shared dataset job). Not ours to clean.
                    count_skipped_links += 1
                    continue

                job_input_dir = job_dir + "/input"
                if os.path.isdir(job_input_dir):
                    if os.stat(job_input_dir).st_uid != my_uid:
                        count_skipped_foreign += 1
                        continue

                    # Count contents in both modes so the grand total at the end is accurate.
                    counts_per_type = defaultdict(int)
                    with os.scandir(job_input_dir) as it:
                        for entry in it:
                            file_type = stat.S_IFMT(entry.stat(follow_symlinks=False).st_mode)
                            file_type_str = {
                                stat.S_IFLNK: "links",
                                stat.S_IFREG: "files",
                                stat.S_IFDIR: "dirs",
                            }.get(file_type, f"other({file_type})")
                            counts_per_type[file_type_str] += 1
                    counts_str = ", ".join(f"{k}={v}" for k, v in counts_per_type.items())

                    if args.mode == "dryrun":
                        print(f"[dryrun] would remove input dir in job {job_dir!r} contents: {counts_str}")
                    elif args.mode == "remove":
                        try:
                            shutil.rmtree(job_input_dir)
                        except OSError as exc:
                            # Keep going: one unremovable dir should not abort the whole run.
                            print(f"ERROR: could not remove input dir in job {job_dir!r}: {exc}")
                            count_errors += 1
                            continue
                        print(f"Removed input dir in job {job_dir!r} contents: {counts_str}")
                    else:
                        raise ValueError(f"Unknown mode: {args.mode!r}")

                    for k, v in counts_per_type.items():
                        total_counts[k] += v
                    count_cleaned_jobs += 1

            elif "." in dir_name:
                # exclude directories with dots in their names (probably not job dirs)
                exclude_recurse_dirs.add(dir_name)

        if exclude_recurse_dirs:
            # don't visit those directories
            dirs[:] = [d for d in dirs if d not in exclude_recurse_dirs]

    # Each cleaned job frees its input/ contents plus the input/ dir itself.
    total_entries = sum(total_counts.values())
    total_inodes = total_entries + count_cleaned_jobs
    counts_str = ", ".join(f"{k}={v}" for k, v in sorted(total_counts.items())) or "(none)"
    print(f"Total jobs found: {count_all_jobs}, input dirs cleaned: {count_cleaned_jobs}")
    print(f"Skipped symlinked job dirs: {count_skipped_links}, foreign-owned input dirs: {count_skipped_foreign}")
    if count_errors:
        print(f"Errors: {count_errors}")
    print(f"Total input contents: {counts_str}")
    verb = "freed" if args.mode == "remove" else "that would be freed"
    print(f"Total inodes {verb}: {total_inodes} (contents {total_entries} + {count_cleaned_jobs} input dirs)")


def _is_job_dir(job_dir: str) -> bool:
    """
    :param job_dir: absolute or relative path to job directory
    """
    base_name = os.path.basename(job_dir)
    if "." not in base_name:
        return False
    if not os.path.isfile(job_dir + "/info"):
        return False
    if not os.path.isdir(job_dir + "/output"):
        return False
    return True


if __name__ == "__main__":
    main()
