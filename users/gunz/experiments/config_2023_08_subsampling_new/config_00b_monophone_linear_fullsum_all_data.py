...
class Results:
    phmm_30ms: typing.Tuple[Experiment, fh_system.FactoredHybridSystem]
    phmm_40ms: typing.Tuple[Experiment, fh_system.FactoredHybridSystem]
    phmms_30ms: typing.Tuple[Experiment, fh_system.FactoredHybridSystem]
    phmms_40ms: typing.Tuple[Experiment, fh_system.FactoredHybridSystem]


def run(returnn_root: tk.Path) -> Results:
    # ******************** Settings ********************
    ...
    def r(exp: Experiment) -> fh_system.FactoredHybridSystem:
        return run_single(
...
def run_single(
    *,
    ...
    name = f"mlpa-1-lr:{lr}-n:{n_states_per_phone}-ss:{subsampling_approach}-dx:{output_time_step/(10/1000)}-d:{model_dim}-bwl:{bw_label_scale}-bwt:{bw_transition_scale}"
    print(f"fh {name}")
    ...
    s.set_rasr_returnn_input_datas(
        is_cv_separate_from_train=False,  # done via num segs in CV
