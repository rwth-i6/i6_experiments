...
def run_single(
    *,
    ...
    for ep, crp_k in itertools.product([300, 550, max(keep_epochs)], ["dev-other"]):
        ...
        if own_priors:
            ...
            s.experiments["fh"]["priors"] = PriorInfo.from_triphone_job(
                tk.Path(
                    "/work/asr3/raissi/shared_workspaces/gunz/kept-experiments/2023-05--subsampling-tf2/priors/conf-tri-30ms-scratch-ff-e550"
                )
            )
    ...
    if run_performance_study:
        ...
        for cfg in [
            dataclasses.replace(best_config, altas=a, beam=b, lm_scale=2.4)
            ...
            jobs.search.rqmt.update({"sbatch_args": ["-p", "rescale_amd", "--nice=500"]})
