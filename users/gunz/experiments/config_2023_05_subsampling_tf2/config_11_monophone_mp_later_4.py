...
def run(returnn_root: tk.Path):
    ...
    for exp in configs:
        run_single(
            ...
            lr=exp.lr,
            max_pooling_after_layer=exp.max_pooling_after_layer,
...
def run_single(
    *,
    ...
    name = f"conf-1-lr:{lr}-ss:{subsampling_factor}-mp:{'x'.join(str(s) for s in reduction_factor)}+{max_pooling_after_layer}"
    print(f"fh {name}")
