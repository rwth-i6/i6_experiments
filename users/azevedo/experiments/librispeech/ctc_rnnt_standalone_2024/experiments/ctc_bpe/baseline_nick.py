...
def bpe_ls960_1023_low_vocab_test():
    ...
    def tune_and_evaluate_helper(training_name, dev_dataset_tuples, test_dataset_tuples, asr_model, base_decoder_config,
                                 lm_scales, prior_scales, decoder_module="ctc.decoder.flashlight_ctc_v1", debug=False,
                                 rescale_mode=False, run_test=True, use_gpu=False):
        tune_parameters = []
        tune_values_clean = []
        tune_values_other = []
        report_values = {}
        ...
        if run_test:
            for key, tune_values in [("test-clean", tune_values_clean), ("test-other", tune_values_other)]:
                pick_optimal_params_job = GetOptimalParametersAsVariableJob(parameters=tune_parameters,
                                                                            values=tune_values, mode="minimize")
    ...
    def lexical_free_search(
            ...
            debug=False,
            decoder_module: str = "ctc.decoder.flashlight_ctc_lexical_free_v1"
    ):
        # remove prior if exists
        asr_model = copy.deepcopy(asr_model)
        asr_model.prior_file = None
    ...
    for BPE_SIZE in [0, 128, 256, 512, 1024]:
        ...
        training_name = prefix_name + "/" + str(
            BPE_SIZE) + "/" + network_module_conv_first + ".512dim_sub4_24gbgpu_50eps"
            ...
            for beam_size in [256, 512, 1024]:
                for beam_size_token in [4, 8, 12, 16]:
                    for beam_threshold in [8, 10, 12, 14]:
                        ...
                        search_name = training_name + "/cpu_fast_search/search_lm_1.8_prior_0.2_bs_%i_bst_%i_bth_%i" % (
                        beam_size, beam_size_token, beam_threshold)
                        search_jobs, wers = search(
                            search_name,
                        ...
                        search_name = training_name + "/rescale_amd_search/search_lm_1.8_prior_0.2_bs_%i_bst_%i_bth_%i" % (
                        beam_size, beam_size_token, beam_threshold)
                        search_jobs, wers = search(
                            search_name,
        ...
        if BPE_SIZE == 0 or BPE_SIZE == 128 or BPE_SIZE == 512:
            ...
            train_args_conv_first_ep100_sp_late_peak["config"]["learning_rates"] = list(
                np.linspace(7e-6, 5e-4, 480)) + list(
            ...
            train_args_conv_first_ep100_sp_late_peak_full_reg["net_args"] = {
                "model_config_dict": asdict(model_config_full_spec)}

            train_args_conv_first_ep100_sp_late_peak_full_reg_gradnorm = copy.deepcopy(
                train_args_conv_first_ep100_sp_late_peak_full_reg)
            ...
            # Radam and plateau
            train_args_conv_first_ep100_sp_radam_plateau_full_reg = copy.deepcopy(
                train_args_conv_first_ep100_sp_late_peak_full_reg)
            train_args_conv_first_ep100_sp_radam_plateau_full_reg["config"]["optimizer"]["class"] = "radam"
            train_args_conv_first_ep100_sp_radam_plateau_full_reg["config"]["optimizer"][
                "decoupled_weight_decay"] = True
            train_args_conv_first_ep100_sp_radam_plateau_full_reg["config"]["learning_rates"] = list(
                np.linspace(1e-4, 5e-4, 240)) + list(
            ...
            if BPE_SIZE == 128:
                train_args_pairs += [
                    ...
                    (".512dim_sub4_24gbgpu_100eps_sp_lp_fullspec_gradnorm",
                     train_args_conv_first_ep100_sp_late_peak_full_reg_gradnorm),
                    (".512dim_sub4_24gbgpu_100eps_sp_lp_fullspec_radam_plateau",
                     train_args_conv_first_ep100_sp_radam_plateau_full_reg)
                ]
            ...
            if BPE_SIZE == 128:
                ...
                train_job = training(training_name, train_data_bpe_no_peak_norm, train_args, num_epochs=1000,
                                     **default_returnn)
                train_job.rqmt["gpu_mem"] = 24
                asr_model = prepare_asr_model(
                    training_name, train_job, train_args, with_prior=True, datasets=train_data_bpe,
                ...
                tune_and_evaluate_helper(training_name, dev_dataset_tuples_no_peaknorm, test_dataset_tuples_no_peaknorm,
                                         asr_model,
                                         default_decoder_config_bpe, lm_scales=[1.6, 1.8, 2.0],
                                         prior_scales=[0.2, 0.3, 0.4])
                greedy_decoder_config = GreedyDecoderConfig(
                ...
                train_job = training(training_name, train_data_bpe_no_preemph, train_args, num_epochs=1000,
                                     **default_returnn)
                train_job.rqmt["gpu_mem"] = 24
                asr_model = prepare_asr_model(
                    training_name, train_job, train_args, with_prior=True, datasets=train_data_bpe,
                ...
                tune_and_evaluate_helper(training_name, dev_dataset_tuples_no_preemph, test_dataset_tuples_no_preemph,
                                         asr_model,
                                         default_decoder_config_bpe, lm_scales=[1.6, 1.8, 2.0],
                                         prior_scales=[0.2, 0.3, 0.4])
                greedy_decoder_config = GreedyDecoderConfig(
                ...
                lexical_free_search(training_name + "/th50", asr_model=asr_model, decoder_config=lexical_free_config,
                                    debug=True)
                """
                # Neural LM test
                from ...pytorch_networks.ctc.decoder.flashlight_ctc_v2_neural_lm import \
                    DecoderConfig as DecoderConfigNeuralLM
                ...
                from ...pytorch_networks.ctc.decoder.flashlight_ctc_v2_neural_lm_rescoring import \
                    DecoderConfig as DecoderConfigRescoringNeuralLM
                neural_lm = get_lm_model("bpe5k_2x2024_kazuki_lstmlm_3ep")
