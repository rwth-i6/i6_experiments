...
def get_train_config(model_config, keep, module, accum_grads=1,  **kwargs):
    ...
    # train_config_24gbgpu["learning_rates"] = [float(i) for i in train_config_24gbgpu["learning_rates"]]
...
def run_experiments(**kwargs):
    prefix_name = "experiments/librispeech/ctc_rnnt_standalone_2024/ls960_lah_co_node_speeds"
        ...
        for param_combi in param_combinations:
            ...
            for node in NODES:
                ...
                training_name = (
                    ...
                    ".512dim_sub6_1eps_from_scratch_radamv1_lah0_cs0_co0_specaug%d" % model_config.specauc_start_epoch + "/" +
                    node_key
                )
                ...
                #     "host": node_key,
                # }
                train_args["config"]["cleanup_old_models"]["keep"] = KEEP + [node]
                train_job = training(training_name, train_data_bpe, train_args,
                                    num_epochs=1, **default_returnn)
                train_job.rqmt["gpu_mem"] = 24 if node < 506 else 48
                train_job.rqmt["sbatch_args"] = ["-w", node_key]
                ...
                asr_model = prepare_asr_model(
                    ...
                    datasets=train_data_bpe, get_specific_checkpoint=1
                )
                evaluate_helper(
                    training_name + "/keep_1",
                    asr_model,
                    decoder_config_v2,
                    use_gpu=False,
                    beam_size=12,
...
def full_streaming_node_time_stats():
    experiment_configs = {
        "large_chunk_exps": {
            "model_params": {
                "chunk_size": [2.4],
