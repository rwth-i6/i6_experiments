...
def unified_ct_ls960_1023_low_bpe_from_scratch():
    prefix_name = "experiments/librispeech/ctc_rnnt_standalone_2024/ls960_unified_ct_low_bpe_from_scratch"
    ...
    def evaluate_helper(
        ...
        beam_size: int = 1,
        use_gpu=False,
        decoder_module: str = "rnnt.decoder.experimental_rnnt_decoder_v2",
    ):
        """
    ...
    from ...pytorch_networks.rnnt.streaming_conformer.unified_conformer_transducer_cfg import ModelConfig
    ...
    for CHUNK_SIZE, BPE_SIZE in product([0.5, 1.0, 2.72], [128, ]):   # [128, 512]
        ...
        network_module = "rnnt.streaming_conformer.unified_conformer_transducer"
        ...
        for KERNEL_SIZE in [7, 11, 31]:

            accum_grads = 2
            if CHUNK_SIZE == 2.72 or (CHUNK_SIZE == 1.0 and KERNEL_SIZE == 7):
                accum_grads = 3
            
            train_args["config"]["accum_grad_multiple_step"] = accum_grads
            train_args["config"]["batch_size"] = 200 * 16000 // accum_grads
            ...
            training_name = (
                ...
                "/" + network_module + ".512dim_sub4_11gbgpu_25eps_from_scratch_radamv1_unified" +
            ...
            train_job.rqmt["gpu_mem"] = 11 #if CHUNK_SIZE >= 1.0 else 11
