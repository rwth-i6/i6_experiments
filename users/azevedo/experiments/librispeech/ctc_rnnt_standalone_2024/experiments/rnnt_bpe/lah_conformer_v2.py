...
def run_experiments(**kwargs):
    ...
    from ...pytorch_networks.rnnt.conformer_0924.uni_lah_carryover_cfg import ModelConfigV2
    ...
    for experiment in experiments_config:
        ...
        for param_combi in param_combinations:
            ...
            training_name = (
                ...
                "%deps_" % (num_epochs//10) +
                "from_scratch_radamv1_streaming_lah%s_specaug%d" % (alias, model_config.specauc_start_epoch) + "/" +
            ...
            #
            # use newer network module to decode old model
            #
            ...
            model_config_dict.update({
                "online_model_scale": 0,
                "carry_over_size": 1,
                "training_strategy": str(TrainingStrategy.STREAMING)
            })
            model_config_v2 = ModelConfigV2(**model_config_dict)

            asr_model_lah_carryover = prepare_asr_model(
                training_name, train_job, train_args, with_prior=False,
                datasets=train_data_bpe, get_specific_checkpoint=num_epochs
            )
            asr_model_lah_carryover.network_module = "rnnt.conformer_0924.model_streaming_lah_carryover_v3"
            asr_model_lah_carryover.net_args = {"model_config_dict": asdict(model_config_v2)}

            evaluate_helper(
                training_name + "/keepv3_%i" % num_epochs,
                asr_model_lah_carryover,
                decoder_config_v2,
                use_gpu=False,
                beam_size=12,
...
def streaming_lookahead_ls960_1023_low_bpe_from_scratch_updated_configs():
    experiment_configs = {
        10: {
            "model_params": {
                "chunk_size": [2.4],
        ...
        # NOTE: old experiment config, requires keep_best_n = 2
        # 10: {
        #     "model_params": {
        #         "chunk_size": [2.4],
        #         "lookahead_size": [8],
