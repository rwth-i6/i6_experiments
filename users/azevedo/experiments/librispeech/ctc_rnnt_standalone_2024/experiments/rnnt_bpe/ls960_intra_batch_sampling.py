...
def get_train_config(model_config, keep, module, accum_grads=1, **kwargs):
    ...
    network_module = "rnnt.conformer_1124.%s" % module
...
def run_experiments(**kwargs):
    prefix_name = "experiments/librispeech/ctc_rnnt_standalone_2024/ls960_intra_batch_sampling"
    ...
    for experiment in experiments_config:
        ...
        for param_combi in param_combinations:
            ...
            training_name = (
                    ...
                    "%deps_" % (num_epochs // 10) +
                    "from_scratch_radamv1_intra_sampling_specaug%d" % model_config.specauc_start_epoch + "/" +
            ...
            for keep in [num_epochs]:  # + KEEP
                ...
                evaluate_helper(
                    ...
                    use_gpu=True,
                    beam_size=12,
                    decoder_module="rnnt.decoder.carryover_decoder_v1"  # default: v1
                )
                evaluate_helper(
...
def ls960_1124_intra_batch_sampling():
    experiment_configs = {
        10: {
            "model_params": {
                "chunk_size": [2.4 * 16e3],
            ...
            "network_module": "model_intra_batch_sampling",
            "accum_grads": 1,
            "gpu_mem": 48,
            "num_epochs": 1000,
