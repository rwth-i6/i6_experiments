...
def run_experiments(**kwargs):
    prefix_name = "experiments/librispeech/ctc_rnnt_standalone_2024/ls960_relpos_streaming_0525_ffnnpred_low_bpe_from_scratch"
    ...
    from ...pytorch_networks.rnnt.decoder.streaming_0525_decoder import DecoderConfig, ExtraConfig
    ...
    from ...pytorch_networks.rnnt.conformer_0525.model_ffnn_predictor_cfg import ModelConfig
    from ...pytorch_networks.rnnt.conformer_0525.ffnn_prediction_net import FFNNPredictorConfig
    ...
    for experiment in experiments_config:
        ...
        for param_combi in param_combinations:
            predictor_config = FFNNPredictorConfig(
                symbol_embedding_dim=256,
                emebdding_dropout=0.2,
                num_layers=1,
                hidden_dim=512,
                prediction_dropout=0.1,
                context_size=param_combi["predictor_context_size"],
                prediction_act="relu",
            )
            model_config = ModelConfig(
                ...
                conv_kernel_size=31,
                final_dropout=0.1,
                specauc_start_epoch=11,  # TODO: change to 21
                joiner_dim=640,
                ...
                ctc_output_loss=0.3,  # TODO: change to 0.7
            ...
            training_name = (
                ...
                "%deps_" % (num_epochs//10) +
                "from_scratch_radamv1_%s_specaug%d" % (train_strat, model_config.specauc_start_epoch) + "/" +
            ...
            for keep in KEEP + [num_epochs]:
                ...
                evaluate_helper(
                    ...
                    use_gpu=True,
                    beam_size=12,
                    decoder_module="rnnt.decoder.streaming_0525_decoder"
                )
                evaluate_helper(
...
def relpos_streaming_ffnn_ls960_0525_low_bpe_from_scratch():
    experiment_configs = {
        "baseline": {
            "model_params": {
                "chunk_size": [2.39],
                ...
                "dual_mode": [False],
                "predictor_context_size": [1],
            },

            "network_module": "rnnt.conformer_0525.model_ffnn_predictor",
            "accum_grads": 1,
            "gpu_mem": 48,
            "num_epochs": 1000,
