...
def get_train_config(model_config, keep, accum_grads=1,  **kwargs):
    ...
    network_module = "rnnt.streaming_conformer.unified_lookahead_transducer_v2"
...
def run_experiments(**kwargs):
    prefix_name = "experiments/librispeech/ctc_rnnt_standalone_2024/ls960_uni_lah_v2_low_bpe_from_scratch"
    ...
    def evaluate_helper(
        ...
        beam_size: int = 1,
        use_gpu=False,
        decoder_module: str = "rnnt.decoder.uni_lah_v3_decoder"
    ):
        """
    ...
    from ...pytorch_networks.rnnt.streaming_conformer.unified_lookahead_transducer_cfg import ModelConfigV2 as ModelConfig
    ...
    for experiment in experiments_config:
        ...
        for param_combi in param_combinations:
            model_config = ModelConfig(
                ...
                final_dropout=0.1,
                specauc_start_epoch=param_combi['specauc_start_epoch'],  # 21
                joiner_dim=640,
                ...
                chunk_size=param_combi["chunk_size"],
                lookahead_size=param_combi["lookahead_size"],
                online_model_scale=0.5,
                
                num_split_blocks=param_combi["num_split_blocks"],
                causal_conv_kernel_size=param_combi["causal_conv_kernel_size"],
                causal_conv_dropout=0.1,
                causal_scale=param_combi["causal_scale"],
            )
            ...
            decoder_config = DecoderConfig(
                ...
                right_size=int(model_config.lookahead_size),
                keep_states=True,
                keep_hyps=True,
                test_version=0.2,   # current: 0.1
            )
            ...
            training_name = (
                ...
                ".512dim_sub6_24gbgpu_" + 
                "%deps_" % (num_epochs//10) +
                "from_scratch_radamv1_uni_lah_specaug%d" % model_config.specauc_start_epoch + "/" +
                "%.2f" % (param_combi["chunk_size"]/16e3) + "/" +
                "conv%i" % model_config.conv_kernel_size + "/" + 
                "lah%.2f" % (model_config.lookahead_size/16e3) + "/" + 
                "nsplits%i_cs%.2f" % (model_config.num_split_blocks, model_config.causal_scale)
            )
            ...
            # FIXME: only for nsplits 12
            if model_config.num_split_blocks == 12:
                train_job.rqmt["sbatch_args"] = ["-w", "cn-290"]
...
def uni_lah_v2_ls960_1023_low_bpe_from_scratch_updated_configs():
    experiment_configs = {
        10: {
            "model_params": {
                "chunk_size": [2.4 * 16e3],
                ...
                "num_split_blocks": [1, 6], #12],
                "causal_conv_kernel_size": [15],
                "causal_scale": [0.0],
            },
            "accum_grads": 1,
            "gpu_mem": 24,
