...
def run_experiments(**kwargs):
    prefix_name = "experiments/librispeech/ctc_rnnt_standalone_2024/ls960_nsplits_lah_co_low_bpe_from_scratch"
    ...
    from ...pytorch_networks.rnnt.conformer_0924.uni_lah_carryover_cfg import ModelConfigNSplits as ModelConfig
    ...
    for experiment in experiments_config:
        ...
        for param_combi in param_combinations:
            model_config = ModelConfig(
                ...
                num_splits=param_combi["num_splits"],
            )
            ...
            decoder_config = DecoderConfig(
                ...
                keep_states=True,
                keep_hyps=True,
                test_version=0.0,  # default 0.3
            )
            decoder_config_offline_v2 = DecoderConfigV2(
            ...
            training_name = (
                ...
                "%deps_" % (num_epochs//10) +
                "from_scratch_radamv1_nsplits_lah_co_specaug%d" % model_config.specauc_start_epoch + "/" +
                str(param_combi["chunk_size"]) + "/" +
                "nsplits%i" % model_config.num_splits
            )
            ...
            for keep in KEEP + [num_epochs]:
                ...
                evaluate_helper(
                    ...
                    asr_model,
                    decoder_config_offline_v2,
                    use_gpu=True,
                    beam_size=12,
                    decoder_module="rnnt.decoder.carryover_decoder_v3_nsplits",
                )
            
            decoder_config_offline_v2.test_version = 0.1
            evaluate_helper(
            ...
            if False: # model_config.num_splits == 6:
                # model_config_nochk = copy.deepcopy(model_config)
                train_args_nochk = copy.deepcopy(train_args)
                train_args_nochk["net_args"] = {
                    "model_config_dict": asdict(model_config),
                    "checkpoint_joiner": False
                }
                train_args_nochk["debug"] = True
                training_name_nochk = (
                    prefix_name + "/" + str(bpe_size) + "/" + 
                    train_args["network_module"] +
                    ".512dim_sub6_%dgbgpu_" % gpu_mem + 
                    "%deps_" % (num_epochs//10) +
                    "from_scratch_radamv1_nsplits_nochk_lah_co_specaug%d" % model_config.specauc_start_epoch + "/" +
                    str(param_combi["chunk_size"]) + "/" +
                    "nsplits%i" % model_config.num_splits
                )
                train_job = training(training_name_nochk, train_data_bpe, train_args_nochk,
                                    num_epochs=num_epochs, **default_returnn)
                train_job.rqmt["gpu_mem"] = gpu_mem
                train_job.set_env("PYTORCH_CUDA_ALLOC_CONF", "expandable_segments:True")


def nsplits_lah_carryover_ls960_1023_low_bpe_from_scratch():
    experiment_configs = {
        10: {
            "model_params": {
                "chunk_size": [2.4],
            ...
            "network_module": "model_nsplits_lah_carryover_v2",
            "accum_grads": 1,
            "gpu_mem": 24,
            "num_epochs": 1000,
