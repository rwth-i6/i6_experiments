...
def rnnt_bpe_ls960_0924_relposencoder():
    ...
    for BPE_SIZE in [128, 512]:
        ...
        lookahead_size = 8
        carry_over_size = 2
        for chunk_size, carry_over_size in [(0.6, 4), (2.4, 2)]:
            for training_strategy in [str(TrainingStrategy.STREAMING), str(TrainingStrategy.UNIFIED)]:

                if training_strategy == str(TrainingStrategy.UNIFIED) and chunk_size == 0.6: continue
                ...
                training_name = (
                    ...
                    network_module +
                    ".512dim_sub6_48gbgpu_100eps_from_scratch_radamv1_%s_lah_co" % train_strat_str + "/" +
                    str(chunk_size) + "/" +
