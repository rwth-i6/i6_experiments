...
from .mrnnt_decoder_profiler import MonotonicRNNTBeamSearchProfiling, Hypothesis
...
def forward_init_hook(run_ctx, **kwargs):
    ...
    run_ctx.rnnt_decoder = MonotonicRNNTBeamSearchProfiling(
        model=rnnt_model,
...
def forward_finish_hook(run_ctx, **kwargs):
    ...
    dev_df = pd.concat(run_ctx.profiler_data, ignore_index=True)
    dev_df.to_pickle("profiler.pkl")

    als = pd.Series(run_ctx.rnnt_decoder.beam_alignments)
    als.to_pickle("als.pkl")
...
def forward_step(*, model, data, run_ctx, **kwargs):
    ...
    for i in range(raw_audio.shape[0]):
        if config.mode == Mode.OFFLINE:
            hyp, p_data = process_offline_sample(raw_audio=raw_audio[i], raw_audio_len=raw_audio_len[i], run_ctx=run_ctx)
        else:
            hyp, p_data = process_streaming_sample(raw_audio=raw_audio[i], raw_audio_len=raw_audio_len[i], config=config, run_ctx=run_ctx)
    ...
    for hyp, seq_df, tag in zip(hyps, p_datas, data["seq_tag"]):
        ...
        seq_df["tag"] = tag
        run_ctx.profiler_data.append(seq_df)
        # run_ctx.profiler_data[tag] = hyp_p_data
...
def process_offline_sample(
    ...
    (hypothesis, p_data), _ = run_ctx.rnnt_decoder.infer(
        input=raw_audio,
        length=raw_audio_len,
        beam_width=run_ctx.beam_size,
    )

    df_seq = pd.DataFrame(p_data)

    return hypothesis[0].tokens[:-1], df_seq  # remove <S> token
...
def process_streaming_sample(
    ...
    hypothesis: List[Hypothesis] = None
    states = []
    profiler_data: List[pd.DataFrame] = []
    ...
    for chunk, eff_chunk_sz in chunk_streamer:
        (hypothesis, p_data), state = run_ctx.rnnt_decoder.infer(
            input=chunk,
        ...
        if profiler_data:
            num_frames = profiler_data[-1]["t"].max()
            p_data["t"] = [t+num_frames+1 for t in p_data["t"]]
        df_chunk = pd.DataFrame(p_data)
        profiler_data.append(df_chunk)

    seq_df = pd.concat(profiler_data, ignore_index=True)
    return hypothesis[0].tokens[:-1], seq_df