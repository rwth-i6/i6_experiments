...
from .chunk_handler import AudioStreamerV2 as AudioStreamer
    ...
    def infer(
        ...
        with torch.no_grad():
            ...
            chunk_size_frames = num_samples_to_frames(
                ...
                num_samples=int(self.chunk_size)
            )
...
def forward_step(*, model, data, run_ctx, **kwargs):
    ...
    if config.chunk_size is None or config.chunk_size <= 0:
        ...
        for i in range(raw_audio.shape[0]):
            ...
            for chunk, eff_chunk_len, time_step in chunk_streamer:

                run_ctx.rnnt_decoder.model.transcriber.set_time_step(val=time_step)
