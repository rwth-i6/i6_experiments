...
from .rnnt_decoder_profiler import RNNTBeamSearch, Hypothesis
...
def forward_finish_hook(run_ctx, **kwargs):
    ...
    als = pd.Series(run_ctx.als)
    als.to_pickle("als.pkl")
    # with open("profiler.json", "w") as f:
    #     json.dump(run_ctx.profiler_data, f, indent=4)
...
def process_streaming_sample(
    ...
    for chunk, eff_chunk_sz in chunk_streamer:
        ...
        run_ctx.als.extend(p_data["als"])
        without_als: Dict = copy.deepcopy(p_data)
        without_als.pop("als")
        df_chunk = pd.DataFrame(without_als)
