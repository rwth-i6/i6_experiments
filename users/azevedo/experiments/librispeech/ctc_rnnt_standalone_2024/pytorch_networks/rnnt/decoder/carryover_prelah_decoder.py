...
class Transcriber(nn.Module):
    ...
    def infer(
        ...
        with torch.no_grad():
            conformer_in, audio_features_len = self.feature_extraction(squeezed_features, lengths)
            lah_frames = self.lookahead_size // self.feature_extraction.hop_length

            encoder_out, out_mask, state = self.encoder.infer(conformer_in, audio_features_len, 
                                                              states, lookahead_size=lah_frames)
...
def forward_init_hook(run_ctx, **kwargs):
    ...
    rnnt_model = RNNT(              # copies Model structure from ...conformer_* to be in torch's RNNT-wrapper
        transcriber=Transcriber(
            ...
            lookahead_size=int(model.cfg.lookahead_size),
        ),
...
def forward_step(*, model, data, run_ctx, **kwargs):
    ...
    if config.left_size is None or config.left_size <= 0:
        # non-streaming mode
        # TODO: this is still wrong, requires to add FAC inbetween chunks still (like nsplit?)
