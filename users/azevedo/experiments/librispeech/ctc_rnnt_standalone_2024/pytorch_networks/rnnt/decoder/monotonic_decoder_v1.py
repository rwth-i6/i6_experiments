...
from .monotonic_rnnt_beam_search_v1 import MonotonicRNNTBeamSearch, Hypothesis
...
def forward_step(*, model, data, run_ctx, **kwargs):
    ...
    for i in range(raw_audio.shape[0]):
        if config.mode == Mode.OFFLINE:
            ...
            hyp = process_streaming_sample(raw_audio=raw_audio[i], raw_audio_len=raw_audio_len[i], config=config,
                                           run_ctx=run_ctx)
