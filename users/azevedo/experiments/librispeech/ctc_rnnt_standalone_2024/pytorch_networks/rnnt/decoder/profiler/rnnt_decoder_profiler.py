...
def get_sequence(parent: int, token: int, arr: List[Tuple[int, int]]) -> List[int]:
    seq = [token]
    while (parent is not None) and (0 <= parent <= len(arr)):
        parent, token = arr[parent]
        seq.append(token)
...
class RNNTBeamSearch(torch.nn.Module):
    ...
    def _search(
        ...
        p_data: Dict[str, List[any]] = {"len_inner": [], "time_inner": [], "t": [], "als": []}
        total_steps = 0

        # al_to_idx: Dict[List[int], int] = {[self.blank]: 0}
        # if hypo is not None:
        #     # init al_to_idx:
        #     for i, (parent, token) in enumerate(p_data["als"][-beam_width:]):
        #         seq = get_sequence(parent, token, p_data["als"])
        #         al_to_idx[seq] = len(p_data["als"]) - beam_width + i

        inner_hypos: List[Hypothesis] = []
        outer_hypos = self._init_outer_hypos(hypo, device) if hypo is None else hypo
        for t in range(n_time_steps):
            ...
            while len(inner_hypos) > 0:
                p_data["len_inner"].append(len(inner_hypos))
                p_data["t"].append(t)
                p_data["als"].append(tuple([tuple(ih.alignment) for ih in inner_hypos]))

                # # for each hypo: append parent hypo and current token (save mem)
                # TODO: Wrong like this, need to save list of lists in "als"
                # for ih in inner_hypos:
                #     parent = al_to_idx.get(ih.alignment[:-1], None)
                #     al_to_idx[ih.alignment] = len(p_data["als"])
                #     p_data["als"].append((parent, ih.alignment[-1]))
                ...
                # convert all inner hypothesis to outer hypothesis by extending with blank score
                # from the second inner loop call onwards we might have also recombination happening
                old_hypos = outer_hypos.copy()
                ...
                p_data["time_inner"].append(time.perf_counter() - t_start)
                total_steps += 1
            ...
            # # remove grandparent nodes (dont need them if we save parents) + saves mem
            # for al in al_to_idx:
            #     # at end of t=0 loop: alignment with single blank added
            #     if al.count(self.blank)-1 < t:
            #         al_to_idx.pop(al, None)

        return outer_hypos, p_data
