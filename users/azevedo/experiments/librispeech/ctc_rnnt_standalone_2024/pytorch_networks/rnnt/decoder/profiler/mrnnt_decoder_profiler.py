...
class MonotonicRNNTBeamSearchProfiling(torch.nn.Module):
    ...
    def _search(
        ...
        p_data = {"len_hypos": [], "time": [], "t": [], "num_blankend": []}

        hypos: List[Hypothesis] = self._init_hypos(hypo, device) if hypo is None else hypo
        for t in range(n_time_steps):

            p_data["t"].append(t)
            p_data["len_hypos"].append(len(hypos))
            num_blankend_hyps = len([h for h in hypos if h.alignment[-1] == self.blank])
            p_data["num_blankend"].append(num_blankend_hyps)
            self.beam_alignments.append(tuple([tuple(h.alignment) for h in hypos]))
            ...
            p_data["time"].append(time.perf_counter() - t_start)
