...
def subframes_to_samples(
    chunk_size: int, num_subframes: int, sub_factor: float, samples_to_frames: Callable, max_it=10e5
    ) -> int:
    # (sub + R)*sub_factor = ((num_samples - nfft) // hop_length) + 1
    # num_samples = ((sub + R)*sub_factor)*hop_length + nfft

    #error = 1
    #num_samples = (num_subframes*sub_factor + chunk_size + error)*hop_length + nfft

    # NOTE:  - honestly just adjust the sequence mask, i.e. give it an extra chunk and adjust seq. mask
    #        - would also correspond to how model is trained 
    ...
    for i in range(raw_audio.shape[0]):
        ...
        for chunk, eff_chunk_len in chunk_streamer:
            if config.right_size is not None:
                # FIXME: has to be config.right_size*0.06*16e3
                lah_samples = int(config.right_size*1.1 * 16e3)   # e.g. 8 sub. frames = 0.48s (+ leeway)
