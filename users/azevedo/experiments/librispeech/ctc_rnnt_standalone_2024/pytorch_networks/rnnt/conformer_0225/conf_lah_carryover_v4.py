...
class StreamableConformerConvolutionV1(StreamableModule):
    ...
    def infer(self, x: torch.Tensor, states: Optional[List[torch.Tensor]], chunk_sz: int, lookahead_sz: int) -> torch.Tensor:
        kernel_size = self.depthwise_conv.kernel_size[0]

        if states is not None:
            states_no_fac = [layer_out[:-lookahead_sz] for layer_out in states]  # TODO: verify this
            y = torch.cat((*states_no_fac, x), dim=0)
            
            before_prev_fac = chunk_sz+lookahead_sz
            conv_in = torch.cat(
                (y[-before_prev_fac-kernel_size//2: -before_prev_fac], y[-chunk_sz:]),
                dim=0
            ).unsqueeze(0)
            y = self.forward_offline(conv_in)[:, -chunk_sz:]  # [1, C+R, F]
            x = x + y[0, -chunk_sz:]  # [C+R, F]
        else:
            x = x.unsqueeze(0)
            x = self.forward_offline(x) + x
            x = x.squeeze(0)
