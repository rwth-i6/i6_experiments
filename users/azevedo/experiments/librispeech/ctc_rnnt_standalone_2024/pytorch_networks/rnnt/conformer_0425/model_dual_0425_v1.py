...
class Model(StreamableModule):
    def __init__(self, model_config_dict: ModelConfig, **kwargs):
        ...
        self.conformer = StreamableConformerEncoderRelPosV3(cfg=conformer_config)
        ...
        self.joiner = StreamableJoinerV2(
            input_dim=self.cfg.conformer_size + self.cfg.joiner_dim,  # NOTE: this is different
    ...
    def forward_offline(
        ...
        """
        conformer_out, out_mask = self.conformer(raw_audio, raw_audio_len)
        conformer_out_lengths = torch.sum(out_mask, dim=1)
        ...
        if self.cfg.ctc_output_loss > 0:
            ctc_logprobs = torch.log_softmax(self.encoder_ctc_off(conformer_out), dim=-1)
        else:
            ctc_logprobs = None
    ...
    def forward_streaming(
        ...
        if self.cfg.ctc_output_loss > 0:
            ctc_logprobs = torch.log_softmax(self.encoder_ctc_on(conformer_out), dim=-1)
        else:
            ctc_logprobs = None
    ...
    def infer(
        ...
        if self._mode == TrainingStrategy.OFFLINE:
