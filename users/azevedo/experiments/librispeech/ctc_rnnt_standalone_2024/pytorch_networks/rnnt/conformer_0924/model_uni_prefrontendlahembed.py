...
# TODO: different encoder, config etc...
...
from .conf_uni_lah_co_prefrontendlah_v1 import (
    ConformerBlockV1Config,
    ConformerEncoderV1Config,
    ConformerEncoderCOPreLAHV1
)
...
from .uni_lah_carryover_cfg import ModelConfigPrefrontendLAH as ModelConfig
...
from ..auxil.functional import get_future_ac_ctx, Mode
...
class Model(nn.Module):
    def __init__(self, model_config_dict, **kwargs):
        ...
        self.conformer = ConformerEncoderCOPreLAHV1(cfg=conformer_config)
        ...
        self.fac_encoding = torch.nn.Parameter(
            torch.randn(1, self.cfg.feature_extraction_config.num_filters)
        )  # [1, F]

    @staticmethod
    def prep_streaming_input(
        raw_audio: torch.Tensor, raw_audio_len: torch.Tensor, chunk_size: int
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """Chunks raw audio samples.

        Args:
            raw_audio (torch.Tensor): (B, T) raw audio samples
            raw_audio_len (torch.Tensor): (B) length of audio
    ...
    def streaming_feature_extractor(
        ...
        :param chunk_size: number of frames in a chunk        
        """
        ...
        future_ac_ctx, added_lengths = get_future_ac_ctx(raw_audio_chunked, raw_audio_len, self.lookahead_size)
        raw_audio_chunked_fac = torch.cat((raw_audio_chunked, future_ac_ctx), dim=2)  # (B, N, C+R)
        # add extra length due to future acoustic context
        raw_audio_len_chunked_fac = raw_audio_len_chunked
        raw_audio_len_chunked_fac = raw_audio_len_chunked_fac + added_lengths
        # reshape to (B*N, C+R), (B*N, 1)
        ...
        lah_size_frames = self.lookahead_size // self.feature_extraction.hop_length  # lah size in #frames

        # NOTE: adding encoding to future acoustic context
        conformer_in[:, -lah_size_frames:] += self.fac_encoding
    ...
    def forward(
        ...
        conformer_out, out_mask = self.conformer(conformer_in, mask,
                                                 lookahead_size=lah_size_frames,
