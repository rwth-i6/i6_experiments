...
# from ....pytorch_networks.rnnt.search.beam_search import RNNTSearchConfig
from ....pytorch_networks.rnnt.search import RNNTSearchConfig
# from ....pytorch_networks.mrnnt.search.beam_search import MonotonicRNNTSearchConfig
from ....pytorch_networks.mrnnt.search import MonotonicRNNTSearchConfig
...
def run_experiments(**kwargs):
    prefix_name = "experiments/librispeech/ctc_rnnt_standalone_2025/ls960_streamable_rnnt_bpe_refactored"
    ...
    def tune_and_evaluate_helper(
        ...
        for lm_scale in lm_scales:
            for ilm_scale in ilm_scales:
                ...
                search_config: Union[RNNTSearchConfig, MonotonicRNNTSearchConfig] = decoder_config_lm.search_config
                search_config.lm_model_args = lstm_2x1024.net_args
                search_config.lm_checkpoint = lstm_2x1024.checkpoint
                search_config.lm_module = "pytorch_networks.lm.lstm.kazuki_lstm_zijian_variant_v2.Model"
                search_config.lm_package = PACKAGE
                search_config.lm_scale = lm_scale
                search_config.zero_ilm_scale = ilm_scale

                search_name = training_name + f"/{decoder_config_lm.mode.lower()}/lm{lm_scale:.2f}_ilm{ilm_scale:.2f}"
    ...
    from ....pytorch_networks.rnnt._base_streamable_rnnt import StreamableRNNTConfig 
    ...
    for experiment in experiments_config:
        ...
        for param_combi in param_combinations:
            ...
            model_config = StreamableRNNTConfig(
                ...
                ctc_output_loss=0.3,  # NOTE: 0.7 for ffnn predictor (see below)
            )

            # search config stuff
            search_config = RNNTSearchConfig()
            base_decoder_config_streaming = DecoderConfig(
                beam_size=12,
            ...
            base_decoder_config_offline = DecoderConfig(
                beam_size=12,
                returnn_vocab=label_datastream_bpe.vocab,
                search_config=search_config,
            ...
            for keep in [num_epochs]: # + KEEP
                ...
                evaluate_helper(
                    ...
                    asr_model,
                    base_decoder_config_streaming,
                    use_gpu=True,
                    beam_size=base_decoder_config_streaming.beam_size,
                    debug=True
                )
                evaluate_helper(
                    training_name + "/offline/keep_%i" % keep,
                    asr_model,
                    base_decoder_config_offline,
                    use_gpu=True,
                    beam_size=base_decoder_config_offline.beam_size,
                    debug=True,
                )
            ...
            optimal_params = tune_and_evaluate_helper(
                ...
                asr_model=asr_model,
                base_decoder_config=base_decoder_config_streaming,
                ilm_scales=[0, 0.05, 0.1],
                lm_scales=[0.0, 0.25, 0.3, 0.5, 0.55, 0.6, 0.7],
            )
            decoder_config_opt = copy.deepcopy(base_decoder_config_streaming)
            opt_search_config: RNNTSearchConfig = decoder_config_opt.search_config
            opt_search_config.lm_scale = optimal_params[0]
            opt_search_config.zero_ilm_scale = optimal_params[1]
            ...
            optimal_params = tune_and_evaluate_helper(
                training_name=training_name,
                asr_model=asr_model,
                base_decoder_config=base_decoder_config_offline,
                ilm_scales=[0, 0.05, 0.1, 0.2, 0.3, 0.4],
                lm_scales=[0.0, 0.25, 0.5, 0.6, 0.8, 0.9],
            )
            decoder_config_opt = copy.deepcopy(base_decoder_config_offline)
            opt_search_config: RNNTSearchConfig = decoder_config_opt.search_config
            opt_search_config.lm_scale = optimal_params[0]
            opt_search_config.zero_ilm_scale = optimal_params[1]
            decoder_config_opt.beam_size = 4
            ...
            training_name = (
                prefix_name + "/" + str(bpe_size) + "/" + 
                network_module + ".512dim_sub6_%dgbgpu_" % gpu_mem + 
                "%deps_radamv1_%s_specaug21" % (num_epochs//10, train_strat)
            )
            if param_combi["training_strategy"] != TrainMode.OFFLINE:
            ...
            # checkpoint decodings
            # TODO: compare [rnnt, mrnnt] search configs and ensure that identical
            mrnnt_search_config = MonotonicRNNTSearchConfig(**asdict(search_config))
            monotonic_decoder_config_streaming = copy.deepcopy(base_decoder_config_streaming)
            monotonic_decoder_config_offline = copy.deepcopy(base_decoder_config_offline)
            monotonic_decoder_config_streaming.search_config = mrnnt_search_config
            monotonic_decoder_config_offline.search_config = mrnnt_search_config
            ...
            for keep in [num_epochs]:  # KEEP + 
                ...
                evaluate_helper(
                    ...
                    use_gpu=True,
                    beam_size=monotonic_decoder_config_streaming.beam_size,
                )
                evaluate_helper(
                    training_name + "/offline/keep_%i" % keep,
                    asr_model,
                    ...
                    use_gpu=True,
                    beam_size=monotonic_decoder_config_offline.beam_size,
                )
            ...
            optimal_params = tune_and_evaluate_helper(
                training_name=training_name,
                asr_model=asr_model,
                base_decoder_config=monotonic_decoder_config_streaming,
                ilm_scales=[0, 0.05, 0.1, 0.2],
                lm_scales=[0.0, 0.25, 0.5, 0.6, 0.8, 0.9],
            )
            decoder_config_opt = copy.deepcopy(monotonic_decoder_config_streaming)
            opt_search_config: MonotonicRNNTSearchConfig = decoder_config_opt.search_config
            opt_search_config.lm_scale = optimal_params[0]
            opt_search_config.zero_ilm_scale = optimal_params[1]
            decoder_config_opt.beam_size = 4
            evaluate_helper(
            ...
            optimal_params = tune_and_evaluate_helper(
                training_name=training_name,
                asr_model=asr_model,
                base_decoder_config=monotonic_decoder_config_offline,
                ilm_scales=[0, 0.05, 0.1, 0.2],
                lm_scales=[0.0, 0.25, 0.5, 0.6, 0.8, 0.9],
            )
            decoder_config_opt = copy.deepcopy(monotonic_decoder_config_offline)
            opt_search_config: MonotonicRNNTSearchConfig = decoder_config_opt.search_config
            opt_search_config.lm_scale = optimal_params[0]
            opt_search_config.zero_ilm_scale = optimal_params[1]
            decoder_config_opt.beam_size = 4
...
def ls960_streamable_rnnt_refactored():
    experiment_configs = {
        ...
        #     },
        #
        #     "network_module": "rnnt.models.model",
        #     "accum_grads": 1,
        #     "gpu_mem": 48,
        #     "num_epochs": 1000,
        ...
        "rnnt.streaming": {
            "model_params": {
                ...
                "kernel_size": [31],
                "specauc_start_epoch": [11],
                "training_strategy": [TrainMode.STREAMING, TrainMode.OFFLINE],
            },
            "network_module": "rnnt.models.model",
            "accum_grads": 1,
            "gpu_mem": 48,
