...
from .full_qat_v1_modules_shared_observer_streamable import LinearQuant, QuantizedMultiheadAttentionStreamable, Conv1dQuant, \
    ActivationQuantizer
...
class ConformerMHSAQuantStreamable(StreamableModule):
    ...
    def __init__(self, cfg: QuantizedMultiheadAttentionV4Config, mhsa_in_quant, mhsa_out_quant):
        ...
        self.mhsa = QuantizedMultiheadAttentionStreamable(cfg=cfg, soft_in_quant=mhsa_in_quant, soft_out_quant=mhsa_out_quant)
        self.dropout = cfg.dropout
        self.layer_norm_in_quant = ActivationQuantizer(
            bit_precision=cfg.activation_bit_prec,
            dtype=cfg.activation_quant_dtype,
    ...
    def forward_offline(self, input_tensor: torch.Tensor, sequence_mask: torch.Tensor,
        ...
        output_tensor = input_tensor / (
                torch.sum(torch.abs(input_tensor), dim=-1, keepdim=True) / input_tensor.size(-1) + torch.tensor(
            1e-5))
        output_tensor = output_tensor * self.layer_norm_scale + self.layer_norm_bias
        output_tensor = self.layer_norm_out_quant(
            output_tensor)  # [B,T,F] or [B,N,C,F] (but we only do layernorm across last dim so its fine)
...
class ConformerBlockQuantStreamable(StreamableModule):
    ...
    def __init__(self, cfg: ConformerBlockQuantV1Config, mhsa_in_quant, mhsa_out_quant, conv_glu_in_quant, conv_glu_out_quant):
        """
        ...
        self.mhsa = ConformerMHSAQuantStreamable(cfg=cfg.mhsa_cfg, mhsa_in_quant=mhsa_in_quant, mhsa_out_quant=mhsa_out_quant)
        self.conv = ConformerConvolutionQuantStreamable(model_cfg=cfg.conv_cfg, glu_in_quant=conv_glu_in_quant, glu_out_quant=conv_glu_out_quant)
...
class ConformerEncoderQuantStreamable(StreamableModule):
    ...
    def __init__(self, cfg: ConformerEncoderQuantV1Config):
        ...
        mhsa_in_quant = ActivationQuantizer(
            bit_precision=cfg.block_cfg.mhsa_cfg.activation_bit_prec,
            dtype=cfg.block_cfg.mhsa_cfg.activation_quant_dtype,
            method=cfg.block_cfg.mhsa_cfg.activation_quant_method,
            channel_axis=1,
            moving_avrg=cfg.block_cfg.mhsa_cfg.moving_average,
            observer_only_in_train=cfg.block_cfg.mhsa_cfg.observer_only_in_train,
        )
        ...
        conv_glu_in_quant = ActivationQuantizer(
            bit_precision=cfg.block_cfg.conv_cfg.activation_bit_prec,
            dtype=cfg.block_cfg.conv_cfg.activation_quant_dtype,
            method=cfg.block_cfg.conv_cfg.activation_quant_method,
            channel_axis=1,
            moving_avrg=cfg.block_cfg.conv_cfg.moving_average,
            observer_only_in_train=cfg.block_cfg.conv_cfg.observer_only_in_train,
        )
        ...
        self.module_list = torch.nn.ModuleList([ConformerBlockQuantStreamable(
            cfg.block_cfg,
            mhsa_in_quant=mhsa_in_quant,
            mhsa_out_quant=mhsa_out_quant,
            conv_glu_in_quant=conv_glu_in_quant,
            conv_glu_out_quant=conv_glu_out_quant) for _ in range(cfg.num_layers)])
