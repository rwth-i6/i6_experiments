...
from .greedy_bpe_ctc import forward_step as forward_offline
...
def forward_streaming(model, raw_audio, raw_audio_len, run_ctx):
...
def forward_step(*, model: StreamableModule, data, run_ctx, **kwargs):
    ...
    if run_ctx.config.chunk_size is None or run_ctx.config.chunk_size <= 0:
        ...
        batch_indices = forward_streaming(model, raw_audio, raw_audio_len, run_ctx)
