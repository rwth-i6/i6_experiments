...
from .generative_ctc_v1_blank_pen_cfg import ModelConfig
...
class Model(torch.nn.Module):
    def __init__(self, model_config_dict, **kwargs):
        ...
        self.num_mixtures = getattr(self.cfg, "num_mixtures", 2)
        self.gaussian_mixture_weight = nn.Linear(conformer_size, self.num_mixtures, bias =True)
        ...
        self.blank_penalty = self.cfg.blank_pen
    ...
    def forward(
        ...
        for i, (out_layer, scale) in enumerate(zip(conformer_out_layers, self.scales)):
            ...
            if not self.norm_vector:
                log_mixture_list = []
                # blank prob is forced to be 0
                blank_mask = torch.zeros_like(prod_term)
                blank_mask[:,: -1] = self.blank_penalty# last index is for blank
                prod_term = prod_term + blank_mask

                gaussian_log_prob_tmp = torch.nn.functional.log_softmax(prod_term, dim=-1)
                log_mixture_list.append(gaussian_log_prob_tmp)
                # if len(self.extra_gaussian)> 0:
                ...
                #         log_mixture_list.append(gaussian_log_prob_tmp)
                #     log_mixture_weight = torch.nn.functional.log_softmax(self.gaussian_mixture_weight(conformer_out), dim=-1)
                #     log_gaussian_mixtures = torch.stack(log_mixture_list, dim=2)
                #     gaussian_log_prob = torch.logsumexp(log_gaussian_mixtures + log_mixture_weight.unsqueeze(-1), dim=2)
                # else:
                gaussian_log_prob = log_mixture_list[-1]
                ...
                gaussian_log_prob = prod_term - 1.0
        ...
        return gaussian_log_probs_list, torch.sum(out_mask, dim=1)
