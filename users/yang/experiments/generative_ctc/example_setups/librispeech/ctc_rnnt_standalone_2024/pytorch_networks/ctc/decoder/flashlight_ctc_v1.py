...
def forward_step(*, model, data, run_ctx, **kwargs):
    ...
    if len(model_output) == 2:
        logprobs, audio_features_len = model_output
    elif len(model_output) == 3:
        logprobs, logpriors, audio_features_len = model_output # this prior is the per-batch, prior, should not be used in recognition
    else:
        assert False, "not supporeted model output"
    if isinstance(logprobs, list):
        if hasattr(model, "get_log_probs_by_layer"):
            logprobs = model.get_log_probs_by_layer(logprobs, run_ctx.decode_layer_index)
        else:
            logprobs = logprobs[-1]
