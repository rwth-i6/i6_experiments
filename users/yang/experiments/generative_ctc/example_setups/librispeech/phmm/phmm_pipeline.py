...
def compute_prior(
    ...
    for output_file, out_file in search_job.out_files.items():
...
def compute_feature_variance(
    ...
    """
    Run a forward-only job that computes feature statistics.
    """
...
def prepare_asr_model(
    ...
    if with_prior:
        ...
        prior_layers = model_config_dict.get("aux_loss_layers", None) or [-1]
