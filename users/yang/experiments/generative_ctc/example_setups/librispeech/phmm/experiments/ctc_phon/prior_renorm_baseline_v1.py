...
def eow_phon_ls960_relposencoder_0924_base():
    prefix_name = "prior_renorm/librispeech/ctc_rnnt_standalone_2024/"
    ...
    network_module = "ctc.conformer_0924.prior_renorm_ctc_v1"
    ...
    for train_prior_scale in [0.5]:
        for prior_match_loss_scale in [100.01, 50]:
            for blank_ratio in [0.7]:
                name = f".prior_renorm_scale{train_prior_scale}_prior_batch_match_scale_{prior_match_loss_scale}_blank_ratio_{blank_ratio}"
                train_args_work2 = copy.deepcopy(train_args)
                train_args_work2["post_config"] = {"num_workers_per_gpu": 2}
                train_args_work2["net_args"]["model_config_dict"]["train_prior_scale"] = train_prior_scale
                train_args_work2["net_args"]["model_config_dict"]["prior_match_loss"] = "batch"
                train_args_work2["net_args"]["model_config_dict"]["empirical_blank_ratio"] = blank_ratio
                train_args_work2["net_args"]["model_config_dict"]["prior_match_loss_scale"] = prior_match_loss_scale

                training_name = prefix_name + "/" + network_module + name + "_v2_smoothing0.02"
                ...
                tune_and_evaluate_helper(training_name, asr_model, default_decoder_config, lm_scales=[1.6,1.4,1.8], prior_scales=[0.2,0.4,0.6,0.9])
    ...
    for train_prior_scale in [0.5]:
        for prior_match_loss_scale in [1, 10, 100]:
            for blank_ratio in [0.6, 0.5,0.8]:
                name = f".prior_renorm_scale{train_prior_scale}_prior_batch_match_scale_{prior_match_loss_scale}_blank_ratio_{blank_ratio}"
                train_args_work2 = copy.deepcopy(train_args)
                train_args_work2["post_config"] = {"num_workers_per_gpu": 2}
                train_args_work2["net_args"]["model_config_dict"]["train_prior_scale"] = train_prior_scale
                ...
                tune_and_evaluate_helper(training_name, asr_model, default_decoder_config, lm_scales=[1.6,1.4,1.8], prior_scales=[0.2,0.4,0.6,0.5, 0.7])
                asr_model.returnn_vocab = label_datastream.vocab
                asr_model.settings = train_settings
                asr_model.label_datastream = label_datastream
                add_ctc_model(network_module + name, asr_model)
...
py = eow_phon_ls960_relposencoder_0924_base