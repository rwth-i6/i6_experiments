...
def bpe128_ls960_0924_base():
    prefix_name = "generative/librispeech/ctc_rnnt_standalone_2024/preload_fixed_ctc_bpe_128"
    ...
    print("#########################vocab size:", vocab_size_without_blank)
    ...
    from ...pytorch_networks.ctc.decoder.greedy_bpe_ctc_v4 import DecoderConfig as GreedyDecoderConfig
    ...
    def greedy_search_helper(training_name: str, asr_model: ASRModel, decoder_config: GreedyDecoderConfig):
        ...
        search_jobs, wers = search(
            ...
            forward_config={},
            asr_model=asr_model,
            decoder_module="ctc.decoder.greedy_bpe_ctc_v4",
            decoder_args={"config": asdict(decoder_config)},
            test_dataset_tuples=dev_dataset_tuples,
            **default_returnn,
        )
    ...
    from ...pytorch_networks.ctc.conformer_0924.generative_ctc_v1_cfg import (
        SpecaugConfig,
        VGG4LayerActFrontendV1Config_mod,
        ModelConfig,
        LogMelFeatureExtractionV1Config,
        ConformerPosEmbConfig,
        convert_conformer_ctc_to_model_config
    )

    from ...pytorch_networks.ctc.conformer_0924.generative_ctc_preload import get_default_config_v1
    network_args= {
        "aux_losses": {"11": 1},
        "d_model": 768,
    ...
    default_config = get_default_config_v1(num_inputs=80, num_outputs=184,network_args=network_args) # hard coded

    default_model_config = convert_conformer_ctc_to_model_config(default_config)
    ...
    train_config_24gbgpu_amp_radam = {
        ...
        "learning_rates": list(np.linspace(1e-5, 1e-4, 100))
        + list(np.linspace(1e-4, 1e-5, 90))
        + list(np.linspace(1e-5, 1e-6, 10)),
        #############
        "batch_size": 240 * 16000,
        "max_seq_length": {"audio_features": 32 * 16000},
        ...
        "preload_from_files": {
            "codebook": {
                #"filename": "/u/jxu/setups/pretraining/2025-02-28--best-rq-pretraining/alias/experiments/ctc/conformer_baseline/train/epochs_600_peak_lr_0_0003_batch_20k_mask_0.6_16_PCA_update_steps_18000_d_model_768/output/models/epoch.600.pt",
                "filename": "/u/jxu/setups/pretraining/2025-02-28--best-rq-pretraining/alias/experiments/ctc/conformer_baseline/train/epochs_600_peak_lr_0_0004_batch_20k_cb_distance_L2_norm_mask_0.6_16_no_aux_loss_d_model_768/output/models/epoch.600.pt",
                "init_for_train": True,
                "checkpoint_key": "model",
                "ignore_missing": True,
                "ignore_params_prefixes": ["output_linears"],
            },
        }
    }

    best_rq_path = "/u/jxu/setups/pretraining/2025-02-28--best-rq-pretraining/alias/experiments/ctc/conformer_baseline/train/epochs_600_peak_lr_0_0004_batch_20k_cb_distance_L2_norm_mask_0.6_16_no_aux_loss_d_model_768/output/models/epoch.600.pt"
    ctc_path = "/work/asr4/zyang/mini/work/i6_core/returnn/training/ReturnnTrainingJob.ePvQOf2CzHjj/output/models/epoch.1000.pt"
    preload ={
        #'best_rq': best_rq_path,
        'ctc': ctc_path,
    }

    network_module = "ctc.conformer_0924.generative_ctc_v1"
    train_args_radam = {
        "config": train_config_24gbgpu_amp_radam,
        "network_module": network_module,
        "net_args": {"model_config_dict": asdict(default_model_config)},
        "use_speed_perturbation": True,
        "debug": False,
    }

    for name, file_path in preload.items():
        for _norm_vector in [False]:
            train_config = copy.deepcopy(train_args_radam)
            train_config['config']['preload_from_files']['codebook']['filename'] = file_path
            if name == 'ctc':
                train_config['net_args']["model_config_dict"] = asdict(model_config)
            
            train_config["net_args"]['model_config_dict']["freeze_encoder"] = False
            train_config["net_args"]['model_config_dict']["norm_vector"] = _norm_vector
            if _norm_vector:
                suffix = "-norm_vector"
            else:
                suffix = ""
            ...
            training_name = (
                ...
                + "/"
                + network_module
                + f"encoder_trainable-init-{name}-ep20"+ suffix
            )
            train_job = training(training_name, train_data_bpe, train_config, num_epochs=200, **default_returnn)
