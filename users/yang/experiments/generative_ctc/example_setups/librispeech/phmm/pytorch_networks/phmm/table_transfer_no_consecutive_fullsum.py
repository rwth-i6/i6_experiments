...
class Model(nn.Module):
    """
    Table-only generative transfer model using the no-consecutive full-sum loss.

    The trainable parameter is `emission_table` with shape
    [input_vocab_size, output_vocab_size]. After log-softmax over the input
    dimension, it represents log p(input_id | output_id).
    """
    ...
    def forward(self, input_ids: torch.Tensor) -> torch.Tensor:
        ...
        return log_p_input_given_output[input_ids]
...
def train_step(*, model: Model, data, run_ctx, **kwargs):
    ...
    log_likelihood = no_concecutive_full_sum(
        log_probs=log_probs,
        seq_len=seq_len,
