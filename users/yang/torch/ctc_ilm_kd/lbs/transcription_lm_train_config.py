...
def get_dataset(data, order="random"):
...
lbs_bpe10k_trans_lm_adam_config = dict(
    backend="torch",
    behavior_version=21,
    task="train",
    ...
    learning_rate_control_error_measure = "dev_loss_log_ppl",
    train_order="laplace:.1000",
    **base_dataloader_config,
)
...
def train_lbs_bpe10k_transcription_lm(
            ...
            serialization.Collection(
                [   
                    ...
                    serialization.NonhashedCode(
                        textwrap.dedent(
                            """
                            sys.path.insert(0, "/u/zyang/setups/rf/recipe")
                ...
                [
                    ...
                    serialization.NonhashedCode(
                        textwrap.dedent(
                            """
                            dev = get_dataset("cv")
                            train = get_dataset("train", order=train_order)
                            """
                        )
                    ),
                ]
    ...
    returnn_train_config = get_serializable_config(returnn_train_config, serialize_dim_tags=False,)
