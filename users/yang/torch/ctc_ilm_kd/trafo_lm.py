...
# https://github.com/rwth-i6/i6_experiments/blob/main/users/gaudino/models/asr/rf/nn_lm/lm_import_2023_11_09.py
...
from i6_experiments.users.phan.rf_models.attention_modified_luca import sinusoidal_positional_encoding_v2, CausalSelfAttention
...
def get_model(*, epoch: int, **_kwargs_unused):
    ...
    assert lm_cls == "Trafo_LM_Model", "Only trafo lm is supported"
    extern_data_dict = config.typed_value("extern_data")
    data = Tensor(name="data", **extern_data_dict["data"])
    return Trafo_LM_Model(
        in_dim=data.sparse_dim,
        target_dim=data.sparse_dim,
        **lm_cfg,
    )
...
def train_step(*, model: Trafo_LM_Model, extern_data: TensorDict, **_kwargs_unused):
    ...
    batch_dims = delayed.remaining_dims(spatial_dim)
    init_state = model.default_initial_state(batch_dims=batch_dims)
    out = model(delayed,state=init_state, spatial_dim=spatial_dim)
    logits: Tensor = out["output"]
    logits_raw = logits.raw_tensor # (T,  V)
    targets_raw = targets.raw_tensor.long() # (B, T,V)
    ce = torch.nn.functional.cross_entropy(
        input = logits_raw.permute(0,2,1),
        target = targets_raw,
        reduction="none",
    )
...
default_trafo_transcription_config = { # should be used for transcription LM as well
