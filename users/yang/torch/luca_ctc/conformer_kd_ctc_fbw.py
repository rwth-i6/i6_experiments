...
def sis_run_with_prefix(prefix_name: Optional[str] = None):
    ...
    recog_epoch = list(map(int, list(recog_epoch)[1:]))
    extra_epochs = [118, 122,124,116,114,108]
    bszs = [(10_000,100), (15_000,100)]
    peak_lrs = [1e-5, 5e-6, 1e-6]
    kd_layers = [8,12]
    for i in extra_epochs:
        recog_epoch.append(i)
    for i in extra_epochs:
    ...
    for peak_lr in peak_lrs:
        for kd_layer in kd_layers:
            for bsz in bszs:
                for kd_scale in kd_scales:
                    for train_lm_scale in train_lm_scales:
                        k = 10
                        train_exp(
                            f"fwbw_kd_layer-{kd_layer}-topk{k}-kd_scale-{kd_scale}-lm_scale-{train_lm_scale}-bsz{bsz[0]}-epoch-{bsz[1]}-lr{peak_lr}",
                            config_11gb_v6_f32_accgrad1_gpu1_wd1e_4,
                            config_updates={
                                **_fine_tune_get_cfg_lrlin_oclr_by_bs_nep(bsz[0], bsz[1], peak_lr=peak_lr),
                                #"dynamic_learning_rate": dyn_lr_piecewise_linear,
                                # total steps after 2000 epochs: 982.312
                                # "learning_rate_piecewise_steps": [600_000, 900_000, 982_000],
                                # "learning_rate_piecewise_values": [1e-5, 1e-3, 1e-5, 1e-6],
            ...
            for bsz in bszs:
                for kd_scale in kd_scales:
                    for train_lm_scale in train_lm_scales:
                        k = 10
                        train_exp(
                            f"fwbw_kd_layer-{kd_layer}-topk{k}-kd_scale-{kd_scale}-lm_scale-{train_lm_scale}-bsz{bsz[0]}-epoch-{bsz[1]}-lr{peak_lr}-4gpu",
                            config_11gb_v6_f32_accgrad1_mgpu4_pavg100_wd1e_4,
                            config_updates={
                                **_fine_tune_get_cfg_lrlin_oclr_by_bs_nep(bsz[0], bsz[1], peak_lr=peak_lr),
                                #"dynamic_learning_rate": dyn_lr_piecewise_linear,
