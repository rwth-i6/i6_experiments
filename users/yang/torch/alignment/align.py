...
_v2_forward_out_filename = 'alignment.hdf'
...
def align_forward(
  *,
  ...
  align_def,
  align_post_config: Optional[Dict[str, Any]] = None,
    align_mem_rqmt: Union[int, float] = 6,
    ...
    python_epilog=[
      serialization.Collection(
        [
          ...
          serialization.Import(align_def, import_as="_align_def", ignore_import_as_for_hash=True),
...
def _returnn_v2_get_model(*, epoch: int, **_kwargs_unused):
  ...
  model = model_def(epoch=epoch, in_dim=data.feature_dim, target_dim=targets.sparse_dim, model_args=model_args,
                    search_args=search_args)
  return model
...
def _returnn_v2_forward_step(*, model, extern_data: TensorDict, **_kwargs_unused):
  ...
  realign_def = config.typed_value("_align_def")
  ...
  if len(realign_out) == 2:
    ...
    raise ValueError(f"unexpected num outputs {len(realign_out)} from align_def")
