...
from i6_experiments.users.yang.torch.decoding.recog import recog_training_exp as ctc_recog_train_exp
...
def train_exp(
    ...
    *,
    ...
    if ctc_greedy_layer is not None:
        from i6_experiments.users.yang.torch.decoding.ctc_greedy import model_recog_greedy
        assert isinstance(ctc_greedy_layer, int), "ctc output layer should be int"
        ## default values, to be comparable
        bsf = 11
        search_args = {
            "bsf": bsf,
            "beam_size": 1,
        ...
        model_args = {
            "ctc_output_args": {
                "ctc_enc_layer_id": ctc_greedy_layer,
            },
            "feature_batch_norm": True,
        }
        ctc_recog_train_exp(prefix+ f'-ctc-{ctc_greedy_layer}-greedy', task, model_with_checkpoint, recog_def=model_recog_greedy,
