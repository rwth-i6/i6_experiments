...
from i6_experiments.users.yang.torch.lm.network import transformer_lm_v2 as transformer_lm
...
from i6_experiments.users.yang.torch.lm.dataloader.lm_dataloader_word_200k import get_librispeech_lm_data_word_200k
...
def returnn_config_generator(
    ...
    **kwargs
    ...
    train_data, dev_data = get_librispeech_lm_data_word_200k(train_epoch_split=train_epoch_split)
    ...
    num_outputs = kwargs.get('num_outputs', 200002)
    embed_dim = kwargs.get('embed_dim', 128)
    hidden_dim = kwargs.get('hidden_dim', 512)
    num_layers = kwargs.get('num_layers',96)
    ...
    kwargs.update(lr)
    model_preload = kwargs.get('preload_from_files', None)
    if model_preload:
        extra_config.update({
...
def lbs960_train_word_200k_trafo() -> SummaryReport:
    prefix = "experiments/kazuki_word_200k"
