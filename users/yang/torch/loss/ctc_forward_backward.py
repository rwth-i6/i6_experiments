...
def ctc_forward(
    ...
    if backward:
        ...
        reverse_diff_mask = target_label_shift_diff_mask.flip(1)
...
# shape (T,B,2,S)
    def _fw_step(t, log_probs_target, log_probs_blank, log_gamma, label_mask, ):
        prev_gamma_0 = log_gamma[t-1,:,0,:].clone()
        prev_gamma_1 = log_gamma[t-1,:,1,:].clone()
        log_probs_t = log_probs_target[t]
        log_label_sum_target_masked = torch.logaddexp(prev_gamma_1,
                                                      prev_gamma_0 + label_mask)  # (B, S+1) repeated labels masked out
        ...
        new_gamma_1_label = log_probs_blank[t].unsqueeze(-1) + torch.logaddexp(
            prev_gamma_0[:, 1:], prev_gamma_1[:, 1:])
        log_gamma[t, :, 0,1:] = new_gamma_0_label
        log_gamma[t, :, 1,1:] = new_gamma_1_label
    ...
    if top_k_list is not None and backward:
    ...
    elif top_k_list is not None:
        # one dimension maybe faster and less mem usage?
        fw_label_mask = (targets_w_bos[:, :-1].unsqueeze(-1) == top_k_list).float() * log_zero # (B,S,K)
        fw_scores = torch.logaddexp(log_gamma_fw[:,:,1,:-1].unsqueeze(-1), log_gamma_fw[:,:,0,:-1].unsqueeze(-1) + fw_label_mask.unsqueeze(0)) # T,B,S,K
        time_mask = get_seq_mask(input_lengths, input_time_size, device=device).transpose(0,1)
        time_mask = (1-time_mask) * log_zero
        top_k = top_k_list.shape[2]
        top_k_list_flat = top_k_list.view(batch_size, max_label_seq_len*top_k)
        top_k_list_idx = top_k_list_flat.unsqueeze(0).expand(input_time_size,-1,-1).long()
        log_probs_select_k = log_probs.gather(-1, top_k_list_idx).view(input_time_size, batch_size, max_label_seq_len,
                                                                       top_k)  # (T,B,S,K)

        # a time shift of fw_scores
        init_state = torch.full((batch_size, max_label_seq_len, top_k), log_zero, dtype=torch.float32,
                            device=device)
        init_state[:,0,:] = 0
        fw_scores = torch.cat([init_state.unsqueeze(0), fw_scores], dim=0)[:-1,:,:,:] # (T,B,S,K) but shifted to left
        combined_score = time_mask.unsqueeze(-1).unsqueeze(-1) + fw_scores + log_probs_select_k
        fw_top_k = combined_score.logsumexp(dim=0) # sum over
    ...
    if top_k_list is not None and backward:
        ...
        return log_gamma_fw, (0, fw_top_k)
    if backward:
