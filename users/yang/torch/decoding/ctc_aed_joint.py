...
def model_recog(
    *,
    ...
    if model.search_args.get("encoder_ctc", False):
        collected_outputs = {}
        enc_args_ctc, enc_spatial_dim = model.encode(
            data, in_spatial_dim=data_spatial_dim, collected_outputs=collected_outputs
        )
        ctc_logit = model.enc_aux_logits_12(collected_outputs[str(11)])
        log_ctc_prob = torch.nn.functional.log_softmax(ctc_logit, dim=-1)
    ...
    if model.search_args.get("use_ctc", False):
        # need to check the ctc output dims manually
        if model.search_args.get("encoder_ctc", False):
