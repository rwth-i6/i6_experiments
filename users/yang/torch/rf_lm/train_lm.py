"""
This is for when the training uses an OggZipDataset in text-only mode
"""
...
def train_text_only(
    ...
    **kwargs,
    ...
    extern_data_raw = instanciate_delayed(extern_data_dict)
            ...
            serialization.Collection(
                [
                    ...
                    *serialize_model_def(model_def),
                    ...
                    serialization.Import(lm_train_step, import_as="train_step"),
    ...
    for k, v in dict(
        ...
        if k not in kwargs or kwargs[k] is None:
            kwargs[k] = v
    reserve_code = kwargs.pop("reserve_code", None)
    returnn_train_job = ReturnnTrainingJob(returnn_train_config, **kwargs)
    if reserve_code is not None:
        returnn_train_job.rqmt["sbatch_args"] = ["--reservation", reserve_code]
...
def _returnn_v2_get_model(*, epoch: int, **_kwargs_unused):
    ...
    sparse_dim = extern_data_dict[default_target_key]['sparse_dim']

    model_def = config.typed_value("_model_def")
    model = model_def(epoch=epoch, in_dim=sparse_dim, target_dim=sparse_dim)
    return model




def lm_train_step(*, model, extern_data: TensorDict, **_kwargs_unused):
    from returnn.config import get_global_config
    import torch.nn
    ...
    targets_raw = targets.raw_tensor.long()
    delayed_targets = extern_data["delayed"]
    targets_spatial_dim = targets.get_time_dim_tag()
    delayed_targets_spatial_dim = delayed_targets.get_time_dim_tag()
    out = model(delayed_targets, spatial_dim=delayed_targets_spatial_dim)
    out_logits_raw = out["output"].raw_tensor
    ce = torch.nn.functional.cross_entropy(
        input = out_logits_raw.permute(1, 2, 0),
        target = targets_raw,
        reduction="none",
    )
    targets_len_raw = targets_spatial_dim.dyn_size_ext.raw_tensor
    seq_mask = get_seq_mask(
        seq_lens=targets_len_raw,
        max_seq_len=targets_len_raw.max(),
        device=out_logits_raw.device
        )
    loss = (ce*seq_mask).sum()
    fake_ppl = torch.exp(loss/ targets_len_raw.float().sum())
    rf.get_run_ctx().mark_as_loss(
        name="log_ppl", loss=loss,
    )
    rf.get_run_ctx().mark_as_loss(
        name="fake_ppl", loss=fake_ppl, as_error=True,
    )
