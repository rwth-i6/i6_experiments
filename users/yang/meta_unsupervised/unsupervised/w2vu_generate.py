...
class UnsupGenerateConfig(FairseqDataclass):
    ...
    lexicon: Optional[str] = field(
        default=None,
        metadata={"help": "path to lexicon. This is also used to 'phonemize' for unsupvised param tuning"},
    )
    ...
    blank_mode: str = field(
        default="set",
        metadata={"help": "can be add or set, how to modify blank emission with blank weight"},
    )
    ...
    unsupervised_tuning: bool = field(
        default=False,
        metadata={"help": "if true, returns a score based on unsupervised param selection metric instead of UER"},
    )
    is_ax: bool = field(
        default=False,
        metadata={"help": "if true, assumes we are using ax for tuning and returns a tuple for ax to consume"},
    )
...
def generate(cfg: UnsupGenerateConfig, models, saved_cfg, use_cuda):
    ...
    logging.info(f"lm_score_sum is {lm_score_sum}")
