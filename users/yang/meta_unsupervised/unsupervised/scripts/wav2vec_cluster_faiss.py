...
def main():
    ...
    if osp.exists(feat_path + ".npy"):
        logging.info("Loading features from %s", feat_path)
        print("Loading features from %s", feat_path)
        feats = np.load(feat_path + ".npy", mmap_mode="r")
    else:
        ...
        # #######
        # # Alternatively, we can save the features in batches to avoid memory issues
        # batch_size = 1000
        # feature_batch = []
        # batch_idx = 0
        # feature_count_in_batch = 0

        # print(f"Saving features in batches of {batch_size}...")

        # for f in tqdm.tqdm(iterator, total=num):
        #     feature_batch.append(f)
        #     feature_count_in_batch += 1

        #     # Check if the batch is full
        #     if feature_count_in_batch == batch_size:
        #         # Convert the list of features to a numpy array
        #         batch_array = np.array(feature_batch)
        #         # Save the batch
        #         np.save(os.path.join(os.path.join(args.save_dir, "feat_batches"), f"feat_batch_{batch_idx}.npy"), batch_array)
        ...
        # # Save any remaining features in the last batch
        # if feature_batch:
        #     batch_array = np.array(feature_batch)
        #     np.save(os.path.join(feat_path, f"feat_batch_{batch_idx}.npy"), batch_array)
        #     print(f"Saved final batch with {len(feature_batch)} features.")
