...
class Wav2vec_UConfig(FairseqDataclass):
    ...
    discriminator_frame_output: bool = False
...
class Discriminator(nn.Module):
    def __init__(self, dim, cfg: Wav2vec_UConfig):
        ...
        self.frame_output = cfg.discriminator_frame_output
    ...
    def forward(self, x, padding_mask):
        ...
        if self.frame_output:
            if padding_mask is not None and padding_mask.any():
                return x[~padding_mask].squeeze(-1)
            else:
                return x.squeeze(-1)
...
class Wav2vec_U(BaseFairseqModel):
    def calc_gradient_penalty(self, real_data, fake_data):
        ...
        if self.discriminator_frame_out:
            gradients = gradients / disc_interpolates.shape[0] # normalize over frames
    ...
    def __init__(self, cfg: Wav2vec_UConfig, target_dict):
        ...
        self.discriminator_frame_out = cfg.discriminator_frame_output
    ...
    def forward(
        ...
        if d_step:
            ...
            if self.discriminator_frame_out:
                loss_dense = loss_dense/ dense_y.shape[0] * bsz
            ...
            if self.discriminator_frame_out:
                loss_token = loss_token/token_y.shape[0] * bsz
