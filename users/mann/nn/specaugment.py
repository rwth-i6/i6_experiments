...
def transform_wrapper(self, source, **kwargs):
    return transform(
...
def set_config(config):
    ...
    config.maybe_add_dependencies(_mask, random_mask, transform, transform_wrapper)
    net = config.config["network"]
    # substitute current source
    for layer in net.values():
        if ("from" not in layer and layer["class"] != "constant")\
            or any(k in layer.get("from", []) for k in ["data", "data:data"]):
            layer["from"] = "specaug"
    # add augmentation layer
    net["specaug"] = {
        "class": "eval",
        "eval": returnn.CodeWrapper(transform_wrapper.__name__), 
    }
