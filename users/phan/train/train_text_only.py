...
def train_text_only(
    ...
    **kwargs,
    ...
    if train_dataset_key is not None:
        ...
        # for text-only dataset we want dev and test
        eval_datasets = {key: dataset.get_main_dataset() for key, dataset in task.eval_datasets.items()}

    # Turn off the oggzip audio
    train.update({"audio": None})
    for key in eval_datasets:
        eval_datasets[key].update({"audio": None})
    ...
    # print(extern_data_raw)
    extern_data_raw.pop("data", None)
...
def _returnn_v2_get_model(*, epoch: int, **_kwargs_unused):
    ...
    model = model_def(epoch=epoch, in_dim=targets.sparse_dim, target_dim=targets.sparse_dim)
    return model
