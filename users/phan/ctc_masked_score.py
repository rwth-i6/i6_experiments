...
def ctc_masked_score(
    ...
    :param target_lengths: Lengths target sequences (B,) not(?) including EOS
    :param blank_idx: Index of blank in log_probs. Should only be either 0 or F-1 (last)
    :param eos_idx: If not None, then there is EOS in the vocab. EOS log posterior will be masked out.
        Rest probabilities are then renormalized.
    ...
    as if the <mask> was replaced by a true label w.
    """
    # Mask out EOS if needed to
    if eos_idx is not None:
        log_probs[:, :, eos_idx] = log_zero
    ...
    # is blank first or last?
    if blank_idx == 0:
        BLANK_IS_FIRST = True
    elif blank_idx == n_out-1:
        BLANK_IS_FIRST = False
    else:
        raise NotImplementedError("Index of blank must either be 0 or last!")
    ...
    if mask[0] == 0:
        ...
        forward_masked[0, :, 0, :] = log_probs[0, :, out_idx_no_blank].clone()
    ...
    all_w_diff_prev = torch.arange(start=vocab_start, end=vocab_end).unsqueeze(0).unsqueeze(0).expand(batch_size, len(mask_prev_not_mask_idx), -1).to(device) != \
    ...
    for t in range(1, input_time_size):
        ...
        if BLANK_IS_FIRST:
            targets_select_idx = targets[:, idx] - 1 # Because blank is 0
            targets_select_idx[targets_select_idx < 0] = 0
        else:
            targets_select_idx = targets[:, idx]
        forward[t, :, idx+1, 0] = log_probs_targets_t[:, idx] + logsubstractexp(
                ...
                forward_masked[t-1, :, midx, :].gather(-1, targets_select_idx.unsqueeze(-1)).squeeze(-1),
                log_zero,
            )
    ...
    all_w_diff_next = torch.arange(start=vocab_start, end=vocab_end).unsqueeze(0).unsqueeze(0).expand(batch_size, len(mask_next_not_mask_idx), -1).to(device) != \
    ...
    for t in range(input_time_size-2, -1, -1):
        ...
        res = log_probs_targets_t[:, idx] + logsubstractexp(
                ...
                backward_masked[t+1, :, midx, :].gather(-1, targets_select_idx.unsqueeze(-1)).squeeze(-1),
                log_zero,
            )
        backward[t, :, idx, 0] = torch.where(update_mask, res, backward[t, :, idx, 0])
