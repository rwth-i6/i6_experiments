...
def sis_run_with_prefix(prefix_name: Optional[str] = None):
    ...
    lr_list = [1e-6]
    ep_list = [20]
    recog_epoch = [2, 4, 6, 8, 12, 16, 20, 40] # [20, 40, 80, 100]
    # # ---------- double softmax experiments NO SPECAUGMENT ---------
    # # am_scales = [1.0, 1.3, 2.0] 
    # am_scales = [2.0] # loss convregence happens here
    ...
    #     train_exp( 
    #         f"conformer_double_softmax_final_noSpecAug_am-{am_scale}_lm-{lm_scale}_lr_{lr}_ep_{epoch}",
    #         config_24gb,
    ...
    for lr, epoch, am_scale, rel_scale in itertools.product(lr_list, ep_list, am_scales, rel_scales):
        lm_scale = round(am_scale*rel_scale, 2)
        train_exp( 
            f"conformer_double_softmax_final_wSpecAug_am-{am_scale}_lm-{lm_scale}_lr_{lr}_ep_{epoch}",
            config_24gb,
            from_scratch_training_double_softmax,
            gpu_mem=24,
            config_updates={
                ...
                "freeze_encoder": False,
                "freeze_ilm": True,
                "use_specaugment": True, # VERY IMPORTANT!!!
            },
            time_rqmt=50,
            post_config_updates={
                "cleanup_old_models": {"keep": recog_epoch},
...
def train_exp(
    ...
    *,
    ...
    recog_config_update_lstm_lm = {
        ...
        "external_language_model": default_tedlium2_extern_lm_hardcoded_layers_config, # dont be fooled by the name
    }
    ...
    # # --------------- decoding with LSTM LM from LibriSpeech -----------------
    ...
    #         "ctc_log_prior": False,
    #     }
    #     recog_config_update_extra = copy.deepcopy(recog_config_update_lstm_lm)
    ...
    #         prior_config["hash_override"] = "17/11/2024"
    #     recog_training_exp(
    #         prefix + f"_timeSyncRecombFirstV2_lstmlm_beam-{beam_size}_lm-{lm_scale}_ilm-{ilm_scale}_lenNorm-{length_norm_scale}_prior-{prior_scale}",
    #         task,
