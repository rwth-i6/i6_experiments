...
from i6_experiments.users.phan.rf_models.default_model_configs import default_extern_lm_config, default_ilm_config, default_bigram_config
from i6_experiments.users.phan.rf_models.bigram import get_model, train_step
...
def sis_run_with_prefix(prefix_name: Optional[str] = None):
    ...
    from i6_experiments.users.phan.lbs_transcription_bpe10k.train_config import train_lbs_bpe10k_transcription_lm

    # maybe first try same LR as lstm????
    train_job = train_lbs_bpe10k_transcription_lm(
        get_model,
        train_step,
        num_epochs=30,
        hashed_config={
            "lm_cfg": default_bigram_config,
        }
    )
    train_job.add_alias(_sis_prefix + "/train")
    tk.register_output(_sis_prefix + "/bpe10k_transcription_bigram/learning_rates", train_job.out_learning_rates)
