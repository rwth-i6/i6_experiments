...
from i6_experiments.users.phan.rf_models.model_conformer_with_lm import from_scratch_model_def, from_scratch_training_kldiv_sample_batch
    ...
    for weight in ground_truth_weights:
        for lrs in lr_list:
            for epochs in ep_list:
                ...
                train_exp( 
                    f"conformer_ilm_kldiv_sampling_weight_{weight}_lr1_{lr_1}_lr2_{lr_2}_ep1_{ep1}_ep2_{ep2}",
                    config_11gb,
                    gpu_mem=11,
                    config_updates={
                        "batch_size": 1200000,
...
def train_exp(
    ...
    *,
    ...
    beam_sizes = [32]
    lm_scales = [0.55, 0.6, 0.65]
    ilm_scales = [0.0]
    length_norm_scales = [1.0]
    for beam_size, lm_scale, ilm_scale, length_norm_scale in itertools.product(beam_sizes, lm_scales, ilm_scales, length_norm_scales):                
        search_args = {
            "beam_size": beam_size,
        ...
        recog_training_exp(
            prefix + f"_beam-{beam_size}_lm-{lm_scale}_ilm-{ilm_scale}_lenNorm-{length_norm_scale}",
            task,
            ...
            model_avg=False,
            train_exp_name=name,
            exclude_epochs=[20, 40, 60, 80, 100, 120],
        )
