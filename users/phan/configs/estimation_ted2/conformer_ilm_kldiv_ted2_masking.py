...
def sis_run_with_prefix(prefix_name: Optional[str] = None):
    """run the exp"""
    lr_list = [1e-3, 1e-3, 1e-3] # [1e-4, 1e-4, 1e-3, 1e-3, 1e-3, 1e-3]
    ep_list = [16]
    recog_epoch = [4, 8, 12, 16]
    masking_rates = [0.2, 0.4, 0.6] # 1.0 to try out zero encoder
    for lr, masking_rate in zip(lr_list, masking_rates):
        for epoch in ep_list:
            lrs = [lr]*epoch
            train_exp( 
                f"conformer_ilm_kldiv_ted2_masking_maskRate_{masking_rate}_lr_{lr}_ep_{epoch}",
                config_11gb,
...
def train_exp(
    ...
    *,
    ...
    beam_sizes = [32]
    length_norm_scales = [0.0] # we don't need 1.0 for time sync search!!!
    # lm_scales = [1.2, 1.3, 1.4, 1.5, 1.6]
    # ilm_scales = [0.8, 0.9, 1.0, 1.1, 1.2] 
    # prior_scales = [0.0]
...
def _get_ted2_task():
    ...
    alignments = {
        "train": ["/work/asr3/zyang/share/mnphan/alignment_data/tedlium2/ted2_train_gmm_pseudo_word_alignments.hdf"],
    }
    _ted2_task = get_tedlium2_task_libri_bpe10k_raw(with_eos_postfix=False, alignments=alignments, train_epoch_wise_filter=None)
    return _ted2_task
