...
from i6_experiments.users.phan.rf_models.model_conformer_with_ilm_v2 import from_scratch_model_def, \
    from_scratch_training_kldiv, from_scratch_training_kldiv_sample_batch
...
def sis_run_with_prefix(prefix_name: Optional[str] = None):
    """run the exp"""
    lr_list = [1e-3]
    ep_list = [8] # 4 is 1 full epoch
    recog_epoch = [4, 8]
    am_scales = [0.3, 0.5, 0.7]
    # Standard KLDiv ILM
    for lr in lr_list:
        for epoch in ep_list:
            for teacher_scale in am_scales:
                lrs = [lr]*epoch
                train_exp( 
                    f"conformer_ilm_kldiv_ted2_scale-{teacher_scale}_lr_{lr}_ep_{epoch}",
                    config_11gb,
...
def train_exp(
    ...
    *,
    ...
    # prior_scales = [0.0]
    lm_scales = [1.0, 1.2, 1.4, 1.6]
    ilm_scales = [0.4, 0.6, 0.8, 1.0] 
    prior_scales = [0.0]
    for beam_size, lm_scale, ilm_scale, length_norm_scale, prior_scale in itertools.product(beam_sizes, lm_scales, ilm_scales, length_norm_scales, prior_scales):                
        if ilm_scale >= lm_scale:
            continue
