...
def train_exp(
    ...
    *,
    ...
    print(model_with_checkpoint.fixed_epochs)
    import functools
    for beam_size in [32]:
        for lm_scale in [0.55, 0.6, 0.65]: # baseline without ILM: {"dev-clean": 2.23, "dev-other": 4.79, "test-clean": 2.4, "test-other": 5.21}
            for ilm_scale in [0.0]:
        # for lm_scale in [0.0]:
        #     for ilm_scale in [0.0]:
                search_args = {
                    "beam_size": beam_size,
                ...
                recog_training_exp(
                    prefix + f"_beam-{beam_size}_lm-{lm_scale}_ilm-{ilm_scale}_lenNorm-{1.0}",
                    task,
                    model_with_checkpoint,
                    search_config=recog_config_update_extra,
                    recog_def=model_recog,
                    model_avg=False,
                    exclude_epochs=[40, 59, 60, 80, 100, 120],
                )
