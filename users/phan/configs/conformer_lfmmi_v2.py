...
def sis_run_with_prefix(prefix_name: Optional[str] = None):
    ...
    # rel_scales = [0.2, 0.35] # 0.35 from Willi's paper
    # top_ks = [160]
    # for lrs, epochs, am_scale, rel_scale, top_k in itertools.product(lr_list, ep_list, am_scales, rel_scales, top_ks):
    #     lm_scale = round(am_scale*rel_scale, 3)
    #     lr_1, lr_2 = lrs
    #     ep1, ep2 = epochs
    #     lrs = const_linear_learning_rates(lr_1,lr_2,ep1,ep2)
    #     train_exp( 
    #         f"conformer_lfmmi_context1_v2_am-{am_scale}_lm-{lm_scale}_topk-{top_k}_lr1_{lr_1}_lr2_{lr_2}_ep1_{ep1}_ep2_{ep2}",
    #         config_24gb,
    ...
    # # ---------- lf mmi experiments ---------
    # am_scales = [1.0] # 1.5, 0.1 will diverge
    # rel_scales = [0.35] # 0.35 from Willi's paper
    # top_ks = [300, 450]
    # for lr, epoch, am_scale, rel_scale, top_k in itertools.product(lr_list, ep_list, am_scales, rel_scales, top_ks):
    #     lm_scale = round(am_scale*rel_scale, 3)
    #     train_exp( 
    #         f"conformer_lfmmi_context1_v2_am-{am_scale}_lm-{lm_scale}_topk-{top_k}_lr_{lr}_ep_{epoch}",
    #         config_24gb,
    #         from_scratch_training_lfmmi_context_1,
    #         gpu_mem=48,
    #         config_updates={
    ...
    # ---------- lf mmi experiments ---------
    am_scales = [1.0] # 1.5, 0.1 will diverge
    rel_scales = [0.2] # 0.35 from Willi's paper ????
    top_ks = [450]
    for lr, epoch, am_scale, rel_scale, top_k in itertools.product(lr_list, ep_list, am_scales, rel_scales, top_ks):
        lm_scale = round(am_scale*rel_scale, 3)
        train_exp( 
