...
def sis_run_with_prefix(prefix_name: Optional[str] = None):
    ...
    for lrs in lr_list:
        for epochs in ep_list:
            ...
            lrs = [lr_1]*ep1
            train_exp( 
                f"conformer_ilm_kldiv_lr1_{lr_1}_lr2_{lr_2}_ep1_{ep1}_ep2_{ep2}_fix_eos",
                config_11gb,
    ...
    # Try higher LR for the standard KLDiv
    lr = 1e-4
    ep = 40
    lrs = [lr]*ep
    train_exp( 
        f"conformer_ilm_kldiv_lr_{lr}_ep_{ep}_fix_eos",
        config_11gb,
        from_scratch_training_kldiv,
        gpu_mem=11,
        config_updates={
    ...
    for weight in ground_truth_weights:
        train_exp( 
            f"conformer_ilm_kldiv_sampling_weight_{weight}_lr_{lr}_ep_{ep}_fix_eos",
            config_11gb,
...
def train_exp(
    ...
    *,
    ...
    lm_scales = [0.75, 0.85, 0.95, 1.05, 1.15]
