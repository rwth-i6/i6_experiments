"""
Config for doing recognition with transcription LM, will all contexts (1, 6, 10, full)
The respective transcription LM training jobs are hardcoded due to hash issues.
"""
...
def train_exp(
    ...
    *,
    ...
    from i6_experiments.users.phan.configs.ilm_archs.ffnn_context1_layers2_hiddendim1000.ilm_config import default_ilm_config as ffnn_context1_config
    from i6_experiments.users.phan.configs.ilm_archs.ffnn_context6_layers2_hiddendim1000.ilm_config import default_ilm_config as ffnn_context6_config
    from i6_experiments.users.phan.configs.ilm_archs.ffnn_context10_layers2_hiddendim1000.ilm_config import default_ilm_config as ffnn_context10_config
    trans_lms = {
    ...
    for ilm_arch in trans_lms:
        for epoch in trans_lms[ilm_arch]["recog_epochs"]:
            ...
            # ---------------- Compute ILM statistics on librispeech -----------------
            ...
            forward_extra_config["preload_from_files"].update({
                ...
                "02_ilm": { # Import transcription LM checkpoint
                    "prefix": "ilm.",
                    "filename": ilm_ckpt,
                }
            })
            ...
            # for the 1-layer LSTM (standard ILM architecture),
            # additionally perform LibriSpeech recognition with no frame-level prior and ted2 recognition with frame-level prior
            if ilm_arch == "lstm_layers1_hiddendim1000":
                from i6_experiments.users.phan.recog.ctc_time_sync_recomb_first_v2 import model_recog_time_sync_recomb_first_v2

                # ----------- LibriSpeech recognition no frame-level prior ------------
                beam_sizes = [32]
                ...
                # --------------------- Cross-domain recognition on Tedlium 2 with frame-level prior ---------------------
            ...
            # # --------------------- Tedlium 2 recognitions with no frame-level prior (all transcription LM contexts) ---------------------
            from i6_experiments.users.phan.rf_models.default_checkpoints import default_ted2_lstm_extern_lm_checkpoint
            from i6_experiments.users.phan.recog.ctc_time_sync_recomb_first_v2 import model_recog_time_sync_recomb_first_v2
            beam_sizes = [32]
            length_norm_scales = [0.0] # we don't need 1.0 for time sync search!!!
            lm_scales = [1.3, 1.4, 1.5]
            ilm_scales = [0.8, 0.9, 1.0] 
            prior_scales = [0.0]
            for beam_size, lm_scale, ilm_scale, length_norm_scale, prior_scale in itertools.product(beam_sizes, lm_scales, ilm_scales, length_norm_scales, prior_scales):
                if ilm_scale >= lm_scale:
                    continue
                scales = (lm_scale, ilm_scale)
                if ilm_arch == "lstm_layers1_hiddendim1000" and scales != (1.5, 0.9):
                    continue
                elif ilm_arch == "ffnn_context1_layers2_hiddendim1000" and scales != (1.4, 1.0):
                    continue
                elif ilm_arch == "ffnn_context6_layers2_hiddendim1000" and scales != (1.3, 0.8):
                    continue
                elif ilm_arch == "ffnn_context10_layers2_hiddendim1000" and scales != (1.4, 1.0):
                    continue
                search_args = {
                    "beam_size": beam_size,
                    "length_normalization_exponent": length_norm_scale, # by default len norm
