"""
Config for training and recognition for FFNN ILM with the smoothing method
"""
...
from i6_experiments.users.phan.configs.ilm_archs.ffnn_context10_layers2_hiddendim1000.ilm_config import default_ilm_config as \
    default_ffnn_context10_layers2_hiddendim1000_ilm_config
all_context_ilm_configs = {
    1: default_ffnn_context1_layers2_hiddendim1000_ilm_config,
    6: default_ffnn_context6_layers2_hiddendim1000_ilm_config,
    10: default_ffnn_context10_layers2_hiddendim1000_ilm_config
}
...
def sis_run_with_prefix(prefix_name: Optional[str] = None):
    ...
    for lr in lr_list:
        for context_length in all_context_ilm_configs:
            if context_length == 1 and lr != 1e-4:
                continue
            if context_length == 6 and lr != 1e-3:
                continue
            if context_length == 10 and lr != 1e-3:
                continue
            for ep in ep_list:
                lrs = [lr]*ep
                # Smoothing method with weight 0.5
                ...
                for ground_truth_weight in [0.5]:
                    config_11gb.update({"internal_language_model": all_context_ilm_configs[context_length]})
                    train_exp( 
                        f"conformer_ilm_kldiv_ffnn-context-{context_length}_smoothing_weight_{ground_truth_weight}_lr_{lr}_ep_{ep}",
                        config_11gb,
...
def train_exp(
    ...
    *,
    ...
    # ------------------------ ted2 recognitions with no frame-level prior -------------------------
    ted2_prefix = "lbs_cross_domain_ted2/" + prefix
    ted2_task = _get_ted2_task()
    ilm_config = config["internal_language_model"]
    ...
    ted2_recog_config_update = {
        ...
        "internal_language_model": ilm_config,
    ...
    for beam_size, lm_scale, ilm_scale, length_norm_scale, prior_scale in itertools.product(beam_sizes, lm_scales, ilm_scales, length_norm_scales, prior_scales):                
        ...
        if ilm_config == all_context_ilm_configs[1]:
            if scales != (1.4, 1.2):
                continue
            exclude_epochs = [20, 60, 80, 100]
        elif ilm_config == all_context_ilm_configs[6]:
            if scales != (1.6, 1.3):
                continue
            exclude_epochs = [20, 40, 60, 100]
        elif ilm_config == all_context_ilm_configs[10]:
            if scales != (1.7, 1.3):
                continue
