...
def sis_run_with_prefix(prefix_name: Optional[str] = None):
    ...
    recog_epoch = list(range(20, 101, 20))
    # Standard KLDiv ILM
    for lrs in lr_list:
    ...
    # KLDiv with sampling from batch
    ...
    train_exp( 
        f"conformer_ilm_kldiv_lr1_{lr}_ep_{40}",
        config_11gb,
        from_scratch_training_kldiv,
        gpu_mem=11,
        config_updates={
    ...
    for weight in ground_truth_weights:
        train_exp( 
            f"conformer_ilm_kldiv_sampling_weight_{weight}_lr_{lr}_ep_{ep}",
            config_11gb,
            from_scratch_training_kldiv_sample_batch,
            gpu_mem=11,
            config_updates={
    ...
    # -------- experiments with shorter epochs and lower learning rates ---------
    # to verify whether higher PPL is really better
    # standard KL Div
    lr = 1e-6
    ep = 20
...
def train_exp(
    ...
    *,
    ...
    lm_scales = [0.75, 0.85, 0.95] # TO ADD: 0.95
    ilm_scales = [0.4, 0.5, 0.6] # TO ADD: 0.7 maybe try 0.3 0.4 ???
