...
def forward_compute_kldiv(
    ...
    extern_lm_out_raw = extern_lm_out["output"].raw_tensor # (B, S+1, V)
    ...
    ce = (ce*seq_mask).sum(-1)/(torch_target_lengths+1).to(ce.device).float()
    rf.get_run_ctx().mark_as_output(
        ce,
        "ce",
    )
...
class Compute_ILM_KLDiv_Stats_Forward_Callback(ForwardCallbackIface):
    ...
    def process_seq(self, *, seq_tag: str, outputs: TensorDict):
        ...
        self.n_seqs += 1
        self.avg_ppl += (np.exp(ce_tensor) - self.avg_ppl) / self.n_seqs
    ...
    def finish(self):
        res = {
            ...
            "extern_lm_kldiv": self.avg_extern_lm_kldiv,
            "ppl": self.avg_ppl,
