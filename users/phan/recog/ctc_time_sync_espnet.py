"""
https://github.com/rwth-i6/i6_experiments/blob/main/users/gaudino/experiments/rf_conformer_att_2023/librispeech_960/model_recogs/model_recog_ts_espnet.py

This does not do batch decoding!
"""
...
from i6_experiments.users.phan.recog.ctc_espnet.beam_search_time_sync_espnet import BeamSearchTimeSync
from i6_experiments.users.phan.recog.ctc_espnet.lm_ilm_scorer import LM_ILM_Scorer
from i6_experiments.users.phan.recog.search_functions import remove_eos_from_start_and_end
...
def model_recog_ts_espnet(
    *,
    ...
    enc_ctc = rf.log_softmax(enc_ctc, axis=model.target_dim_w_blank) # please give espnet log probs
    ...
    ctc_out = (
        ...
        .raw_tensor[0] # This does not do batch decoding!!!
    )
    ...
    if search_args.get("prior_scale", 0.0) > 0.0:
        ...
        if not search_args.get("ctc_log_prior", True):
    ...
    t2 = time.time_ns()
    print(f"ESPnet time sync beam search loop time (sec): {(t2 - t1)/10**9}")
