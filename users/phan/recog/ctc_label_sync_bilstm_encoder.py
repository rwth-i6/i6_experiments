...
def model_recog_label_sync(
    *,
    ...
    enc_ctc, enc_spatial_dim = model(data, in_spatial_dim=data_spatial_dim)
    ...
    if search_args.get("prior_corr", False):
        ctc_prior = numpy.loadtxt(search_args.get("prior_file", search_args.get("ctc_prior_file", "")),
                                      dtype="float32")
        if search_args.get("prior_log_space"):
    ...
    seq_backrefs = []
    while True:
        # if model.search_args.get("use_ctc", True): We only care about CTC
            # add ctc espnet
        ...
        if search_args.get("use_ctc", True):
            best_ids = target
