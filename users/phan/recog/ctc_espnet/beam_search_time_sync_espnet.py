...
class BeamSearchTimeSync(torch.nn.Module):
    ...
    def time_step(self, p_ctc: Any, ctc_score_dp: Any, hyps: Any) -> Any:
        ...
        for hyp_l in hyps:
            ...
            cands_with_last = cands.copy()
            cands_with_last.add(hyp_l[-1]) # ensuring that the last label is looked at to keep the cache updated
            for c in cands_with_last:
                if c == self.blank:
    ...
    def forward(
        ...
        for t in range(lpz.shape[0]):
            ...
            # # clean up cache by removing key, of which no hypothesis is a prefix
            # for key in list(self.lm_cache.keys()):
            #     key_str = "@".join(map(str, key))
            #     pop = True
            #     for h in hyps:
            #         h_str = "@".join(map(str, h[:-1]))
            #         if key_str.startswith(h_str):
            #             pop = False
            #             break
            #     if pop:
            ...
            # import pprint
            # print(t)
            # pprint.pprint(list(self.lm_cache.keys()))
            # print(hyps)
