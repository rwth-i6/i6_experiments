...
Reference note (2026-09-20, user question on the mathematical core; made self-contained 2026-09-21).
...
- **Lexicon term (lexlat phase).** The prior inside the lattice is extended so that a path is also
  ...
  introduced; its cost is a state-space question, currently under an efficiency gate.
...
## 7. The content-free point is stationary (added 2026-09-20, design review of `SAE_4A_infomax.md`)
...
`+ lambda H(Y | X)` of `SAE_4A_infomax.md` is such a term only through the network's residual input
...
All under `recipe/2025-10-speech-llm/src/speech_llm/`: lattice term and its DP, `sae/emc/lattice.py`
(module docstring); blank-free topology and the loss assembly, `sae/emc/blankfree.py` and
`prefix_lm/model/definitions/sae_blankfree.py`; reverse model, `sae/emc/reverse.py`; text prior,
`sae/emc/prior.py`; rate term, `sae/emc/rate_term.py`; corpus-statistics term, `sae/emc/agg.py`;
back-translation, `sae/emc/bt_blankfree.py`; temperature and learning-rate schedules,
`sae/emc/blankfree_budget_jobs.py`; strong-scorer terms, `sae/emc/soft_scorer.py` and
`sae/emc/sf_scorer.py`; lexicon term, `sae/emc/lexlat.py`. Bed constants and their provenance:
`SAE_4A_blankfree.md` "Registered first model"; the prior's text and SIL convention:
`SAE_4A_prior.md` "Bed and constants".
