...
deduplicated union (after the pool's exact text normalization: lowercase, ASCII fold) of the D7
...
shards. Score every candidate y in the loop's own per-unit shaped currency, with no new scorer,
...
(`base_lm_logprob_sum`, adapters disabled). The user's alpha and beta are therefore not free:
...
infeasible candidate (text minimum feasible frames > T_i) or empty text gets weight exactly 0
before normalization and is counted; a group whose whole support is infeasible collapses to
one-hot greedy and is counted (the preflight asserts greedy feasibility bed-wide). tau_star comes
...
1. **D8.0 feasibility read (CPU only, frozen artifact):** on the existing frozen fork-epoch tc100
   group-12 rollout dump (the artifact the D6 fixed-dump diagnostics read; recon column scored by
   `psi_g_tc100`; all stored temperatures reported, the T=0.7 slice decides), compute per group:
...
beyond D8.0: one group-12 sampled dump pass over the 960 h bed (the measured d6periodic group-12
dump envelope, ten shards), one CPU weight job, and up to two D7.1-sized fixed-final trainings;
