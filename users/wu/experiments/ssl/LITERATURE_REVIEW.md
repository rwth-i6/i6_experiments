...
### 4.0 Which embedding? (the answers below are embedding-TYPE-dependent)

The §4.1/§4.2 verdicts are **not embedding-agnostic** — "frequency strongly encoded, n-gram only first-order" is literally a statement about **static count-derived embeddings** (word2vec/SGNS, GloVe, PPMI+SVD): the frequency results are Schakel 2015 / Oyama 2023 / Mu & Viswanath 2018, and the n-gram result is Levy & Goldberg 2014 (SGNS ≈ shifted-PMI factorization). The frequency claim additionally holds for *contextual* models (Gao 2019, Rajaee 2021) but via a different geometry. What each embedding object actually carries:

| Embedding | Frequency | N-gram / order | Caveat |
|---|---|---|---|
| **Static SGNS/GloVe** (window=1, **separate** in/out) | strong: norm + top 1-3 PCs | **bigram-PMI** in ⟨in_a,out_b⟩; **higher-order absent** | direction needs separate in/out + directional contexts; a **tied** table loses direction |
| **Static SGNS** (window>1, tied) | same | order-*agnostic* proximity-PMI ("near", not "precedes") | no transition structure |
| **Contextual LM hidden states** | strong (anisotropic cone) | higher-order *is* modeled, but per-position — **not in a static token vector** | a sequence function, not an "embedding" in the §4.1/4.2 sense |
| **LM input table (wte)** | strong (freq cone) | mostly identity/substitutability; **little explicit transition** | this is what "predict into the LM embedding space" gives you |
| **LM output/unembedding head** | — | **the directional "what comes next" object** — phonotactics lives here | usable only as a *scorer* over sequences, not as a static codebook |
| **Phoneme/syllable emb.** (small vocab) | trivial: 1-3 dims | bigram-PMI cleanly recoverable (tiny matrix) | identity↔articulatory only soft, r≈0.2-0.4 (Silfverberg 2018) |

**The consequence for §4.3's winner:** the embedding we'd actually use as a target is an **LM input table (wte)**, which carries **identity + frequency but NOT the phonotactic/n-gram structure** — that lives in the LM's *function and output head*. So the two jobs must use two different objects: (a) **target geometry / identity** (fix the non-discriminative target) → classify into a frequency-whitened phoneme/syllable embedding table; (b) **phonotactic / n-gram prior** → run the frozen LM *over the unit-embedding sequence* and use its next-token likelihood (the LAST lever) and/or PUSM skipgram on the discrete codes. Matching per-token embeddings never imposes order. (A static phoneme2vec with separate in/out gives bigram transition in its output matrix for free, but nothing higher-order — the trade-off is "bigram-in-the-table" vs "higher-order-in-the-LM-weights".)
