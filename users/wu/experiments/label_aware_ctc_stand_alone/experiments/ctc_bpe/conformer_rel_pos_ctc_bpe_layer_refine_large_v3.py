...
def bpe128_ls960_0924_base():
    prefix_name = "experiments/ctc/conformer_bpe_layer_refine"
    ...
    for peak_audio_mask, init_audio_mask in [(0, 0), (0.3, 0)]:
        for audio_mask_rate in [0.1, 0.2, 0.3]:
            for mask_low_confident_audio in [True, False]:
                if not peak_audio_mask and not init_audio_mask:
                    ...
                    audio_mask_rate = 0.15  # default
                    mask_low_confident_audio = False
                else:
                    audio_mask_prob_schedule = list(np.linspace(init_audio_mask, peak_audio_mask, 480)) + list(np.linspace(peak_audio_mask, init_audio_mask, 480))
