...
def bpe128_ls960_0924_base():
    ...
    from ...pytorch_networks.ctc.conformer_rel_pos_ctc_layer_refinement_v4_cfg import (
        SpecaugConfig,
    ...
    for mask_t2a_attn in [True, False]:
        for gt_prob_start, gt_prob_end in [(0, 0), (0.1, 0.1)]:
            if gt_prob_start == gt_prob_end:
                gt_prob_schedule = [gt_prob_start]
            else:  # try OCLR
                gt_prob_schedule = list(np.linspace(gt_prob_start, gt_prob_end, 250)) + list(np.linspace(gt_prob_end, gt_prob_start, 250))
            ...
            model_config = ModelConfig(
                ...
                mask_t2a_attn=mask_t2a_attn,
                min_gt_noise_prob=0.15,
            )
            ...
            network_module = "ctc.conformer_rel_pos_ctc_layer_refinement_v4"
            train_args_radam = {
            ...
            training_name = (
                ...
                + (f"_noT2AMask" if not mask_t2a_attn else "")
            )
