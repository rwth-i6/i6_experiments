...
def bpe128_ls960_0924_base():
    ...
    from ...pytorch_networks.ctc.conformer_rel_pos_ctc_layer_refinement_v3_cfg import (
        SpecaugConfig,
    ...
    for mask_a2t_attn_for_gt in [True]:
        for gt_prob_start, gt_prob_end in [(1.0, 0), (0.2, 0.2)]:
            for mlm_span_length in [1, 3]:
                gt_prob_schedule = list(np.linspace(gt_prob_start, gt_prob_end, 475))  # last 25 ep w/o gt
                ...
                model_config = ModelConfig(
                    ...
                    aux_ctc_loss_layers=[2, 5, 8, 11],
                    aux_ctc_loss_scales=[0.17, 0.17, 0.17, 0.5],  # self-cond CTC style
                    gt_prob_schedule=gt_prob_schedule,
                    audio_mask_prob_schedule=[0.0],
                    mlm_mask_rate=0.15,  # standard BERT
                    mlm_loss_scale=0.15,
                    mlm_span_length=mlm_span_length,
                    mask_a2t_attn_for_gt=mask_a2t_attn_for_gt,
                )
                ...
                network_module = "ctc.conformer_rel_pos_ctc_layer_refinement_v3"
                train_args_radam = {
                ...
                training_name = (
                    ...
                    + (f"_gtEndProb{gt_prob_end}" if gt_prob_end else "")
                    + (f"_mlmSpan{mlm_span_length}"if gt_prob_start else "")
                    + ("_maskA2T" if mask_a2t_attn_for_gt else "")
                )
