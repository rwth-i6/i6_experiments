...
from .conformer_rel_pos_ctc_layer_refinement_v3_cfg import ModelConfig
                        ...
                        for b_idx in range(B):
                            if do_audio_mask[b_idx]:
                                ...
                                if cur_len > span_len:
                                    ...
                                    if use_confidence:
                                        # Sample start indices based on confidence weights
                                        # We only consider valid start positions [0, cur_len - span_len]
                                        valid_w = weights[b_idx, :cur_len - span_len]
                                        
                                        # Check if valid_w is valid (non-empty)
                                        if valid_w.numel() > 0:
                                            # Sample 'num_spans' indices
                                            # replacement=True allows picking same area, but simpler to implement
