...
class LearnableEmbeddingBias(torch.nn.Module):
    """
    Project CTC blank logits into 1D vectors [B, T, 1, D] for Attention Bias.
    """
    ...
    def __init__(self, model_config_dict, **kwargs):
        ...
        if self.cfg.share_bias_compute:
            ...
            if self.cfg.enable_attn_bias:
                ...
                self.attn_bias_computes = LearnableEmbeddingBias(embed_dim=head_dim, start_epoch=bias_start_epoch)
        else:
    ...
    def forward(
        ...
        for i, encoder in enumerate(self.encoder_blocks):
            ...
            if i < len(self.encoder_blocks) - 1:
                ...
                if self.cfg.enable_self_cond:
                    # detach for now: seems more consistent
                    probs = torch.softmax(logits, dim=2)
                    cond_feat = self.self_cond_compute(probs.detach())
                ...
                if self.cfg.enable_attn_bias:
                    ...
                    current_attn_bias = attention_bias_compute(logits=logits.detach(), epoch=epoch)
                else:
                    current_attn_bias = None
