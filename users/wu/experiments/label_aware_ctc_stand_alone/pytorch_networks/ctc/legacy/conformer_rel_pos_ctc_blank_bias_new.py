...
class Model(torch.nn.Module):
    def __init__(self, model_config_dict, **kwargs):
        ...
        # --- Bias Initialization ---
        if self.cfg.bias_layer_index is None:
    ...
    def forward(
        ...
        if self.cfg.bias_layer_index is None:
            ...
            last_bottom_idx = self.cfg.bias_layer_index - 1
            ret_layers_bottom = [l for l in self.return_layers if l < self.cfg.bias_layer_index]
            ...
            try:
                ...
                raise ValueError(f"Bias layer {self.cfg.bias_layer_index} (idx {last_bottom_idx}) must be in aux_ctc_loss_layers {self.return_layers}")
            ...
            bias = self.compute_bias(logits=guide_logits.detach(), epoch=epoch)
            ...
            # 3. Run Top
            ret_layers_top = [l - self.cfg.bias_layer_index for l in self.return_layers if l >= self.cfg.bias_layer_index]
            if ret_layers_top:
                ...
                for i, layer_idx in enumerate(ret_layers_top):
                    real_idx = layer_idx + self.cfg.bias_layer_index
