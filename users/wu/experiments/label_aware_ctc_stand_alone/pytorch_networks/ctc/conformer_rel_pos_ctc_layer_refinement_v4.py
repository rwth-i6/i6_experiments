...
from .conformer_rel_pos_ctc_layer_refinement_v4_cfg import ModelConfig
...
class Model(torch.nn.Module):
    def __init__(self, model_config_dict, **kwargs):
        ...
        self.text_norm = nn.LayerNorm(conformer_size)
        
        self.denoise_head = nn.Linear(conformer_size, self.vocab_size)
    ...
    def prepare_noisy_gt(self, labels: torch.Tensor, labels_len: torch.Tensor, p_sub: torch.Tensor):
        B, L = labels.shape
        device = labels.device
        labels_len = labels_len.to(device=device)
        
        # p_sub is [B], broadcasting to [B, L]
        rand_vals = torch.rand(B, L, device=device)
        should_substitute = rand_vals < p_sub.unsqueeze(1)
        
        valid_len_mask = torch.arange(L, device=device)[None, :] < labels_len[:, None]
        loss_mask = should_substitute & valid_len_mask
        
        # Substitute with random tokens from the vocab
        random_tokens = torch.randint(0, self.vocab_size, (B, L), device=device, dtype=labels.dtype)
        
        input_ids = labels.clone()
        input_ids[loss_mask] = random_tokens[loss_mask]
    ...
    def forward(
        ...
        log_probs_list = []
        next_block_denoise_target = None
        denoise_outputs_list = [] 
        current_gt_mask = None  # True where ground truth is used

        for i, encoder in enumerate(self.encoder_blocks):
            # conformer_in is unpacked(padding only in the end, not in between) -> the mask_tensor logic works
            mask = mask_tensor(conformer_in, current_lens)
            
            attention_bias = None
            mask_t2a_attn = self.cfg.mask_t2a_attn
            mask_a2t_attn = self.cfg.mask_a2t_attn_for_gt and (current_gt_mask is not None and current_gt_mask.any())

            if (mask_t2a_attn or mask_a2t_attn) and (current_lens > audio_portion_len).any():
                ...
                # Basic masks: [B, T, 1]
                is_text_pos = (idx.view(1, T, 1) >= a_len)
                is_audio_pos = ~is_text_pos
                
                attention_bias = torch.zeros(B, 1, T, T, device=device)
                
                if mask_t2a_attn:
                    mask_t2a = is_text_pos & is_audio_pos.transpose(1, 2)  # text query to audio key, [B, T, T]
                    attention_bias.masked_fill_(mask_t2a.unsqueeze(1), -float('inf'))

                if mask_a2t_attn:
                    mask_base = is_audio_pos & is_text_pos.transpose(1, 2)  # audio query to text key, [B, T, T]
                    mask_a2t = mask_base.unsqueeze(1) & current_gt_mask.view(B, 1, 1, 1)  # only mask for ground truth
                    attention_bias.masked_fill_(mask_a2t, -float('inf')) 
            ...
            audio_feat = feat[:, :max_audio_len, :].clone()
            
            # Apply mask to ensure padding after audio_len is clean
            audio_mask = mask_tensor(audio_feat, audio_portion_len)
            audio_feat = audio_feat * audio_mask.unsqueeze(-1)
            ...
            # unpacking text first
            if next_block_denoise_target is not None:
                targets_dict = next_block_denoise_target["targets"]
                max_text_len = targets_dict["labels"].shape[1]
                sep_len = self.cfg.num_sep_tokens
                
                grid = torch.arange(max_text_len, device=feat.device).unsqueeze(0) # [1, L]
                start_offsets = (audio_portion_len + sep_len).unsqueeze(1) # [B, 1]
                
                # Gather indices: start + grid
                gather_indices = grid + start_offsets # [B, L]
                
                # Clamp to avoid index errors on padding (masked out anyway)
                gather_indices = torch.clamp(gather_indices, max=feat.shape[1]-1)
                
                # Expand dims for gather: [B, L, D]
                gather_indices_expanded = gather_indices.unsqueeze(-1).expand(-1, -1, feat.shape[-1])
                text_feat_packed = torch.gather(feat, 1, gather_indices_expanded)
                
                denoise_logits = self.denoise_head(text_feat_packed)
                
                denoise_outputs_list.append({
            ...
            if i < len(self.encoder_blocks) - 1:
                ...
                # adaptive substitution probability based on confidence
                B = feat.size(0)
                p_sub = torch.zeros(B, device=feat.device)
                
                if self.training:
                    # Calculate avg confidence of greedy output
                    with torch.no_grad():
                        probs = torch.exp(log_probs)
                        # Remove blank from consideration for confidence stats
                        probs_nb = probs[:, :, :self.vocab_size]
                        p_max_nb, _ = probs_nb.max(dim=-1) # [B, T]
                        
                        mask_audio = torch.arange(max_audio_len, device=feat.device)[None, :] < audio_portion_len[:, None]
                        is_non_blank = (log_probs.argmax(dim=-1) != self.vocab_size) & mask_audio

                        sum_conf = (p_max_nb * is_non_blank).sum(dim=1)
                        avg_conf = sum_conf / (is_non_blank.sum(dim=1) + 1e-6) # [B]
                        
                        min_prob = self.cfg.min_gt_noise_prob
                        max_prob = self.cfg.max_gt_noise_prob
                        
                        p_sub = torch.clamp(1.0 - avg_conf, min_prob, max_prob)

                if self.training and labels is not None:
                    gt_emb, gt_mask_target = self.prepare_noisy_gt(labels, labels_len, p_sub)
                ...
                if self.training and labels is not None:
                    gt_schedule = self.cfg.gt_prob_schedule
                    if epoch < len(gt_schedule):
                        gt_prob = gt_schedule[epoch]
                    else:
                        gt_prob = gt_schedule[-1]
                ...
                final_emb = self.text_norm(final_emb)
                sep_emb = self.text_norm(sep_emb)
                ...
                if self.training:
                    mask_schedule = self.cfg.audio_mask_prob_schedule
                    if epoch < len(mask_schedule):
                        selection_prob = mask_schedule[epoch]
                    else:
                        selection_prob = mask_schedule[-1]
                    ...
                    if do_audio_mask.any():
                        ...
                        use_confidence = self.cfg.mask_low_confident_audio
                        
                        if use_confidence:
                            # Calculate sampling weights based on uncertainty
                            # Prob(mask) ~ (1 - P_blank) * (1 - P_max_non_blank)
                            # Shape: [B, T]
                            probs = torch.exp(log_probs)
                            blank_idx = self.cfg.label_target_size
                            ...
                            probs_nb = probs.clone()
                            probs_nb[:, :, blank_idx] = 0.0
                            p_max_nb, _ = probs_nb.max(dim=-1)
                            
                            weights = (1.0 - p_blank) * (1.0 - p_max_nb)
                            
                            # Mask out padding in weights (set to 0 so they aren't sampled)
                            T_audio = weights.shape[1]
                            # Using audio_portion_len to mask weights
                            seq_mask = torch.arange(T_audio, device=weights.device)[None, :] < audio_portion_len[:, None]
                            weights = weights * seq_mask.float()
                            
                            # Ensure no negative or zero sum rows if possible (add epsilon)
                        ...
                        for b_idx in range(B):
                            if do_audio_mask[b_idx]:
                                ...
                                if cur_len > span_len:
                                    ...
                                    if use_confidence:
                                        # Only sample from valid start positions [0, cur_len - span_len]
                                        valid_w = weights[b_idx, :cur_len - span_len]
                                        
                                        # Check if valid_w is valid (non-empty)
                                        if valid_w.numel() > 0:
                                            # Sample 'num_spans' indices
                                            sampled_indices = torch.multinomial(valid_w, num_spans, replacement=True)
                ...
                # remove the padding between the seqs, make sure padding only after text tokens
                raw_concat = torch.cat([next_audio_feat, sep_emb, final_emb], dim=1)
                
                # We need to shift the non-audio parts left to skip the audio padding.
                # Since Sep and Text are contiguous in the SOURCE (after the gap), 
                # the shift amount is constant for both: (Max_Audio - Actual_Audio_Len).
                
                max_audio_src_len = next_audio_feat.shape[1]
                shift_amount = (max_audio_src_len - audio_portion_len).unsqueeze(1) # [B, 1]
                
                max_packed_len = (audio_portion_len + self.cfg.num_sep_tokens + final_lens).max().item()
                
                # create grid of destination indices
                grid = torch.arange(max_packed_len, device=feat.device).unsqueeze(0) # [1, L_packed]
                
                is_non_audio = grid >= audio_portion_len.unsqueeze(1) # [B, L_packed]
                
                # If non-audio: idx = grid + shift (jump over the gap)
                gather_indices = grid + (shift_amount * is_non_audio.long())
                gather_indices = torch.clamp(gather_indices, max=raw_concat.shape[1]-1)
                gather_indices_expanded = gather_indices.unsqueeze(-1).expand(-1, -1, raw_concat.shape[-1])
                conformer_in = torch.gather(raw_concat, 1, gather_indices_expanded)
                
                # zero out padding at the end
                valid_mask = mask_tensor(conformer_in, audio_portion_len + self.cfg.num_sep_tokens + final_lens)
                conformer_in = conformer_in * valid_mask.unsqueeze(-1)
                
                current_lens = audio_portion_len + self.cfg.num_sep_tokens + final_lens

        return log_probs_list, audio_portion_len, denoise_outputs_list
...
def train_step(*, model: Model, data, run_ctx, **kwargs):
    ...
    logprobs_list, audio_features_len, denoise_outputs_list = model(
        raw_audio=raw_audio,
    ...
    for i, denoise_data in enumerate(denoise_outputs_list):
        logits = denoise_data["logits"]
        targets = denoise_data["targets"]
        mask = denoise_data["mask"]
        active_mask = denoise_data["active_mask"]
        
        # Only compute loss at corrupted position in GT sequence
        ...
        if combined_mask.any():
            ...
            denoise_loss = nn.functional.cross_entropy(active_logits, active_targets, reduction='mean')  # mean reduction is same as using inv_norm_factor
            
            run_ctx.mark_as_loss(
                name=f"denoise_loss_block_{i}", 
                loss=denoise_loss, 
                scale=model.cfg.denoise_loss_scale
            )
