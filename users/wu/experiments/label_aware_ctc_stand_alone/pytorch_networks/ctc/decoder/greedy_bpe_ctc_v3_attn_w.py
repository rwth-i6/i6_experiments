...
def forward_finish_hook(run_ctx, **kwargs):
    ...
    torch.save(run_ctx.attn_list, 'all_attn_scores.pt')
...
def forward_step(*, model, data, run_ctx, **kwargs):
    ...
    logprobs, audio_features_len, _, attn_weights = model(
        raw_audio=raw_audio,
        raw_audio_len=raw_audio_len,
    )
    torch.save(attn_weights, "attn_scores.pt")  # running score for simple debug
    run_ctx.attn_list.append(attn_weights)
