"""
Like normal v1, but has added state handling for LM decoding
"""
import math
import torch
...
class TransformerMHSA(nn.Module):
    ...
    def forward(self, input, key_padding_mask, attn_mask, cache=None, cache_length=None):
        ...
        output, _ = self.mhsa(
            q, k, v, key_padding_mask=inv_key_padding_mask, need_weights=False, attn_mask=inv_attn_mask
        )
    ...
    def forward(self, input, labels_len, cache=None, cache_length=None):
        ...
        for i, block in enumerate(self.transformer_blocks):
            x, out_cache = block(
                ...
                seq_mask,
                causal_mask,
                cache=padded_cache[:, i] if cache is not None else None,
