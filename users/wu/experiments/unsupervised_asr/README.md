...
1. Create a setup dir with `recipe/` holding `i6_core`, `i6_experiments` and `returnn`, and with
   ...
   sisyphus command from the setup dir.
2. Set the server's paths in `settings.py`. The package reads these keys:
   ...
   - `SAE_PYTHON` (optional): the main conda env's python.
   - `K2_PYTHON` (optional): the k2 env's python.
   ...
   Without `SAE_PYTHON`, `K2_PYTHON` or `HF_HOME`, the reference cluster's path in `default_tools.py`
   is used. Every one of these paths carries a fixed `hash_overwrite`, so setting them moves no job
   hash. `FFMPEG_PIN_ACCEPT` is the exception: it moves the hashes on purpose. `RETURNN_PYTHON_EXE`
...
The main env is a conda env (`speech_llm` on the reference cluster), on aarch64 with GH200 GPUs. It
contains:
- python 3.11, torch 2.7.1, transformers 5.9.0, datasets 4.8.5 and huggingface_hub 1.16.1;
...
- sequitur-g2p 1.0.1668.30, built against SWIG 3.0.12 with `pip install --no-build-isolation`. SWIG
  4.2 and later produce broken wrappers.
...
The k2 env is a conda clone of the main env, with k2 built from source at
`ec31d2c96e04665eadb276ebcfd436892f74b0aa` for CUDA 12, sm_90 and torch 2.7.1. The installed version
is `1.24.4.dev20260921+cuda12.6.torch2.7.1`.

These are the steps on the reference cluster (JUPITER). The build environment is the sourced
`k2_env.sh`. The final `setup.py install` step is reconstructed from the build log.

```bash
conda create -p $K2_ENV --clone <main conda env>
git clone https://github.com/k2-fsa/k2 k2_src && git -C k2_src checkout ec31d2c96e04665eadb276ebcfd436892f74b0aa
# build environment (k2_env.sh)
export K2_ENV=<k2 env path>
module --force purge
module load Stages/2025 GCC/13.3.0 CUDA/12 CMake/3.30.3
export CC="$(command -v gcc)" CXX="$(command -v g++)"
export CUDACXX="$CUDA_HOME/bin/nvcc" CMAKE_CUDA_COMPILER="$CUDA_HOME/bin/nvcc"
export CMAKE_CUDA_ARCHITECTURES=90 TORCH_CUDA_ARCH_LIST=9.0 MAKEFLAGS=-j8 K2_MAKE_ARGS=-j8
...
export K2_CMAKE_ARGS="-UTORCH_LIBRARY -DCMAKE_BUILD_TYPE=Release \
 ...
 -DCUDA_GPU_DETECT_OUTPUT=9.0 -DCMAKE_CUDA_ARCHITECTURES=90 \
...
cd k2_src && python setup.py install
```

The k2 training arms run RETURNN under this interpreter. The HLG builds run their child scripts under
it too. Because of those child scripts, the package `__init__.py` files must stay import-free.
