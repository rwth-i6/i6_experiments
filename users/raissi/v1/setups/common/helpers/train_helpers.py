...
from i6_experiments.users.raissi.setups.common.helpers.network_architectures import make_config
...
def get_monophone_returnn_config(
    num_classes,
    ph_emb_size,
    st_emb_size,
    use_multi_task=True,
    ...
    **returnn_args,
    ...
    mono_returnn_config = make_config(
    ...
    mono_returnn_config.config["num_outputs"] = {"data": [returnn_args["num_input"], 2], "classes": [num_classes, 1]}

    return mono_returnn_config


def get_diphone_returnn_config(
    num_classes,
    ph_emb_size,
    st_emb_size,
    mlp_l2=0.01,
    ...
    **returnn_args,
    ...
    diphone_returnn_config = make_config(
        context_type=contextType,
        add_mlps=True,
        use_multi_task=use_multi_task,
        final_context_type=final_context_type,
    ...
    diphone_returnn_config.config["num_outputs"] = {"data": [returnn_args["num_input"], 2], "classes": [num_classes, 1]}

    return diphone_returnn_config


def get_monophone_for_bw_returnn_config(
    num_classes, addMLPs=False, mlpL2=0.0001, finalContextType=None, **returnn_args
):
    ctxMapper = ContextMapper()
    contextType = ContextEnum(ctxMapper.get_enum(1))
    finalContextType = finalContextType if finalContextType is not None else ContextEnum(ctxMapper.get_enum(4))

    monoCrnnConfig = make_config(
        contextType,
        ctxMapper,
        addMLPs=addMLPs,
    ...
    monoCrnnConfig.config["num_outputs"] = {"data": [returnn_args["num_input"], 2], "classes": [num_classes, 1]}
