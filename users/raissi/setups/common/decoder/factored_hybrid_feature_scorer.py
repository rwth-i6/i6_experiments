__all__ = ["FactoredHybridFeatureScorer", "FactoredHybridFeatureScorerV2"]
...
class FactoredHybridFeatureScorerV2(rasr.FeatureScorer):
    ...
    def __init__(self, fs_tf_config, context_type, transition_type,
                 prior_mixtures, prior_info,
                 num_states_per_phone, num_label_contexts, silence_id, num_encoder_output=1024, posterior_scales=None,
        ...
        """
        if posterior_scales is None:
            posterior_scales = dict(zip([f'{k}-scale' for k in ['left-context', 'center-state', 'right-context', ]], [1.0]*3))
