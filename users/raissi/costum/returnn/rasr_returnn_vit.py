__all__ = ["ReturnnRasrTrainingVITJob"]
...
class ReturnnRasrTrainingVITJob(ReturnnTrainingJob):
    """
    ...
    def __init__(
        ...
        *,  # args below are keyword-only args
        ...
        :param rasr.FlowNetwork feature_flow: dictionary of RASR flow file for feature extraction or feature cache for train and dev
        param rasr.FlowNetwork alignment: dictionary of RASR flow file for alignments for train and dev
        ...
        del kwargs["dev_crp"]
        kwargs["crps"] = {"train": train_crp, "dev": dev_crp}

        self.train_feature_flow = ReturnnRasrTrainingVITJob.create_flow(key="train", **kwargs)
        (
            self.rasr_train_config,
            self.rasr_train_post_config,
        ) = ReturnnRasrTrainingVITJob.create_config(key="train", **kwargs)

        self.dev_feature_flow = ReturnnRasrTrainingVITJob.create_flow(key="dev", **kwargs)
    ...
    def create_files(self):
        ...
        self.train_feature_flow.write_to_file("train.feature.flow")
        self.dev_feature_flow.write_to_file("dev.feature.flow")
    ...
    def create_config(
        ...
        **kwargs,
        ...
        if use_python_control:
            ...
            config.neural_network_trainer.feature_extraction.file = f"{key}.feature.flow"
            ...
            config.neural_network_trainer.extract_alignments = True
            ...
            config.neural_network_trainer.aligning_feature_extractor.feature_extraction.file = f"{key}.feature.flow"
    ...
    @classmethod
    def create_flow(cls, key, feature_flow, alignment, **kwargs):
        if alignment is not None:
            flow = mm.cached_alignment_flow(feature_flow[key], alignment[key])
        else:
            flow = copy.deepcopy(feature_flow[key])
        flow.flags["cache_mode"] = "bundle"
        return flow

    @classmethod
    def hash(cls, kwargs):
        train_flow = cls.create_flow(key="train", **kwargs)
        dev_flow = cls.create_flow(key="dev", **kwargs)
        ...
        del kwargs["dev_crp"]
        kwargs["crps"] = {"train": train_crp, "dev": dev_crp}
        train_config, train_post_config = cls.create_config(key="train", **kwargs)
        dev_config, dev_post_config = cls.create_config(key="dev", **kwargs)
        ...
        d = {
            ...
            "alignment_flow": [train_flow, dev_flow],
            "returnn_config": returnn_config,
