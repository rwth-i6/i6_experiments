__all__ = ["ReturnnRasrTrainingBWJob"]
...
class ReturnnRasrTrainingBWJob(ReturnnTrainingJob):
    ...
    def __init__(
        ...
        *,  # args below are keyword-only args
        ...
        :param rasr.FlowNetwork feature_flows: dictionary of RASR flow file for feature extraction or feature cache for train and dev
        ...
        self.train_feature_flow = ReturnnRasrTrainingBWJob.create_flow(key="train", **kwargs)
        (
            self.rasr_train_config,
            self.rasr_train_post_config,
        ) = ReturnnRasrTrainingBWJob.create_config(key="train", **kwargs)

        self.dev_feature_flow = ReturnnRasrTrainingBWJob.create_flow(key="dev", **kwargs)

        (
            self.rasr_dev_config,
            self.rasr_dev_post_config,
        ) = ReturnnRasrTrainingBWJob.create_config(key="dev", **kwargs)
    ...
    def create_config(
        ...
        **kwargs,
        ...
        if use_python_control:
            ...
            config.neural_network_trainer.extract_alignments = False
    ...
    @classmethod
    def create_flow(cls, key, feature_flows, **kwargs):
        flow = copy.deepcopy(feature_flows[key])
