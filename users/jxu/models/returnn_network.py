class _NetworkMakerHelper:
  ...
  def add_causal_conv_layer(self, name, source, filter_size, n_out, l2, activation=None, with_bias=True,**kwargs):
      d = {
        'class': 'causal_conv_1d', 'from': source, 'padding': 'causal', 'filter_size': filter_size, 'n_out': n_out,
        'activation': activation, 'with_bias': with_bias
      }
      if l2:
        d['L2'] = l2
  ...
  def add_drop_layer_combine(self, name, source, kind, n_out, **kwargs):
    self._net[name] = {'class': 'layer_drop', 'from': source, 'n_out': n_out}
    self._net[name].update(kwargs)
    return name
  ...
  def add_rms_norm_layer(self, name, source, **kwargs):
    self._net[name] = {'class': 'rms_norm', 'from': source, **kwargs}
    return name
  ...
  def add_linformer_att_layer(self, name, source, n_out, num_heads, total_key_dim, att_dropout=0., key_shift=None,
                        forward_weights_init=None, **kwargs):
    d = {
      'class': 'linformer_attention', 'from': source, 'n_out': n_out, 'num_heads': num_heads, 'total_key_dim': total_key_dim
    }
    if att_dropout:
      d['attention_dropout'] = att_dropout
    if key_shift:
  ...
  def add_talking_heads_self_layer(self, name, source, n_out, num_heads, total_key_dim, att_dropout=0., key_shift=None,
                        forward_weights_init=None, projection_weights_init=None, **kwargs):
    d = {
      'class': 'talking_heads_self_attention', 'from': source, 'n_out': n_out, 'num_heads': num_heads, 'total_key_dim': total_key_dim
    }
    if att_dropout:
      d['attention_dropout'] = att_dropout
    if key_shift:
    ...
    if forward_weights_init:
      d['forward_weights_init'] = forward_weights_init
    if projection_weights_init:
      d['projection_weights_init'] = projection_weights_init
    d.update(kwargs)
    self._net[name] = d
    return name

  def add_linear_att_layer(self, name, source, n_out, num_heads, total_key_dim, att_dropout=0., key_shift=None,
                      forward_weights_init=None, **kwargs):
    d = {
      'class': 'linear_attention', 'from': source, 'n_out': n_out, 'num_heads': num_heads, 'total_key_dim': total_key_dim
    }
    if att_dropout:
      d['attention_dropout'] = att_dropout
    if key_shift:
  ...
  def add_self_local_att_layer(self, name, source, n_out, num_heads, total_key_dim, att_dropout=0., key_shift=None,
                        forward_weights_init=None, local_window_only=True, left_window_size=5, right_window_size=5, stride=1,**kwargs):
    d = {
      'class': 'self_local_attention', 'from': source, 'n_out': n_out, 'num_heads': num_heads, 'total_key_dim': total_key_dim
    }
    if att_dropout:
      d['attention_dropout'] = att_dropout
    if key_shift:
    ...
    if forward_weights_init:
      d['forward_weights_init'] = forward_weights_init
    if local_window_only:
      d['left_window_size'] = left_window_size
      d['right_window_size'] = right_window_size
      d['stride'] = stride
    d.update(kwargs)
    self._net[name] = d
    return name

  def add_self_chunk_att_layer(self, name, source, n_out, num_heads, total_key_dim, att_dropout=0., key_shift=None,
                        forward_weights_init=None, local_window_only=True, chunk_size=10,**kwargs):
    d = {
      'class': 'self_chunk_attention', 'from': source, 'n_out': n_out, 'num_heads': num_heads, 'total_key_dim': total_key_dim
    }
    if att_dropout:
      d['attention_dropout'] = att_dropout
    if key_shift:
  ...
  def add_self_chunk_with_mem_att_layer(self, name, source, n_out, num_heads, total_key_dim, att_dropout=0., key_shift=None,
                        forward_weights_init=None, local_window_only=True, chunk_size=10,**kwargs):
    d = {
      'class': 'self_chunk_with_mem_attention', 'from': source, 'n_out': n_out, 'num_heads': num_heads, 'total_key_dim': total_key_dim
    }
    if att_dropout:
      d['attention_dropout'] = att_dropout
    if key_shift:
