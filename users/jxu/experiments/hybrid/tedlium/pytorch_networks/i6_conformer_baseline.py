...
from i6_models.parts.conformer import ConformerMHSAV1, ConformerMHSAV1Config, ConformerConvolutionV1, ConformerConvolutionV1Config, \
    ConformerPositionwiseFeedForwardV1Config
from i6_models.parts.conformer import ConformerPositionwiseFeedForwardV1
...
class ConformerBlockV1(nn.Module):
    ...
    def forward(self, tensor: torch.Tensor, key_padding_mask: Optional[torch.Tensor]):
        ...
        x = self.mhsa(residual, key_padding_mask)  # [B, T, F]
        residual = x + residual  # [B, T, F]
...
class VGG4LayerPoolFrontendV1(nn.Module):
    ...
    def __init__(self, model_cfg: VGG4LayerPoolFrontendV1Config):
        ...
        self.frontend_linear = nn.Linear(
            model_cfg.input_size // model_cfg.pool_kernel_size[1] * model_cfg.conv4_channels, 384)
...
class Model(torch.nn.Module):
    def __init__(self, **net_kwargs):
        ...
        target_size = 9001
        num_layers = 12
        if "num_layers" in net_kwargs:
            num_layers = net_kwargs["num_layers"]
        kernel_size =  9 if "kernel_size" not in net_kwargs else 31

        cfg_front_end = VGG4LayerPoolFrontendV1Config(input_size=80, conv1_channels=32, conv2_channels=64,
                                                      conv3_channels=64, conv4_channels=32, conv_kernel_size=3,
        ...
        conv_cfg = ConformerConvolutionV1Config(channels=conformer_size, kernel_size=kernel_size, dropout=0.2,
                                                activation=nn.SiLU(),
                                                norm=LayerNormNC(channels=conformer_size))
        mhsa_cfg = ConformerMHSAV1Config(input_dim=conformer_size, num_att_heads=6, att_weights_dropout=0.2,
                                         dropout=0.2)
