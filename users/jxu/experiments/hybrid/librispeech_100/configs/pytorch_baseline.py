...
def get_nn_args(num_outputs: int = 12001, num_epochs: int = 250, use_rasr_returnn_training=True, debug=False,
                **net_kwargs):
    # evaluation_epochs = list(range(num_epochs, num_epochs + 1, 10)) + [122]
    evaluation_epochs = [124]
...
def get_pytorch_returnn_configs(
    ...
    smaller_lr_config = copy.deepcopy(medium_lchunk_config)
    smaller_lr_config["learning_rates"] = list(np.linspace(1e-7, 8e-5, 10)) + list(np.linspace(8e-5, 8e-4, 50)) + list(np.linspace(8e-4, 8e-5, 50))

    no_gradient_clip_config = copy.deepcopy(smaller_lr_config)
    del no_gradient_clip_config['gradient_clip']

    no_gradient_noise_config = copy.deepcopy(smaller_lr_config)
    no_gradient_noise_config["gradient_noise"] = 0.0

    gradient_noise_01_config = copy.deepcopy(smaller_lr_config)
    gradient_noise_01_config["gradient_noise"] = 0.1
    ...
    return {
        # "torchaudio_conformer_v2_subx2_lchunk": construct_from_net_kwargs(medium_lchunk_config, {
        #     "model_type": "torchaudio_conformer_v2_subup_large"}, use_tracing=True),
        # "i6_conformer_v1": construct_from_net_kwargs(medium_lchunk_config, {"model_type": "i6_conformer"},
        #                                              use_tracing=True),
        # "i6_conformer_with_vgg_debug_smaller_lr": construct_from_net_kwargs(smaller_lr_config,
        #                                                          {"model_type": "i6_conformer_v2"},
        #                                                          use_tracing=True),
        # "i6_conformer_with_vgg_debug_no_gradient_clip": construct_from_net_kwargs(no_gradient_clip_config,
        #                                                                     {"model_type": "i6_conformer_v2"},
        #                                                                     use_tracing=True),
        # "i6_conformer_with_vgg_debug_no_gradient_noise": construct_from_net_kwargs(no_gradient_noise_config,
        #                                                                     {"model_type": "i6_conformer_v2"},
        #                                                                     use_tracing=True),
        # "i6_conformer_with_vgg_debug_gradient_noise_01": construct_from_net_kwargs(gradient_noise_01_config,
        #                                                                            {"model_type": "i6_conformer_v2"},
        #                                                                            use_tracing=True),
        # "nick_i6_conformer_v1": construct_from_net_kwargs(medium_lchunk_config, {"model_type": "nick_i6_conformer"},
        #                                                   use_tracing=True),
        "i6_ebranchformer_test": construct_from_net_kwargs(gradient_noise_01_config,
                                                                                   {"model_type": "i6_e_branchformer"},
                                                                                   use_tracing=True),
    }
