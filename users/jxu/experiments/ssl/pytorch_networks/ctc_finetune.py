...
class ConformerCTCModel(torch.nn.Module):
    def __init__(self, step: int, cfg: ConformerCTCConfig, **kwargs):
        ...
        # # Freeze all conformer parameters
        # for p in self.conformer.parameters():
        #     p.requires_grad = False
        # # Freeze all logmel_feat_extraction parameters
        # for p in self.logmel_feat_extraction.parameters():
        #     p.requires_grad = False
...
def train_step(*, model: torch.nn.Module, extern_data: TensorDict, **kwargs):
    ...
    for i in range(len(log_probs_list)):
        ...
        rf.get_run_ctx().mark_as_loss(
            name=f"CTC_{loss_layers[i]}", loss=loss, custom_inv_norm_factor=rf.reduce_sum(targets_len_rf, axis=batch_dim), scale=loss_scales[i]
        )
