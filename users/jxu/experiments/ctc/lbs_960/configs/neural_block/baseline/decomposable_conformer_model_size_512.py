...
def get_serializer(model_config, variant: ConfigVariant, in_dim: int = 1) -> Collection:
    from i6_experiments.users.jxu.experiments.ctc.tedlium2.pytorch_networks.neural_block.from_stratch.decomposable_conformer_no_input_normalize import \
...
def returnn_config_generator(train_data_config: dict, dev_data_config: dict, peak_lr: float) -> dict:
    ...
    network_args = {"specaug_args": {"time_min_num_masks": 2,
                    ...
                    "final_dropout": 0.1,
                    "model_size": 512, }
    lst_ff1_h_dims = [4, 5, 4, 4, 4, 4, 4, 4, 4, 4, 6, 8]
    lst_ff2_h_dims = [0, 4, 4, 4, 0, 0, 0, 2, 4, 4, 4, 4]
    lst_att_heads = [14, 16, 13, 8, 8, 8, 8, 8, 8, 8, 8, 0]
    lst_channels = [2, 4, 4, 6, 6, 6, 6, 6, 7, 8, 8, 8]
    network_args["lst_ff1_h_dims"] = [i * 512 for i in lst_ff1_h_dims]
    network_args["lst_ff2_h_dims"] = [i * 512 for i in lst_ff2_h_dims]
    network_args["lst_channels"] = [i * 128 for i in lst_channels]
    network_args["lst_att_heads"] = lst_att_heads

    config = get_default_config_v1(num_inputs=80, num_outputs=num_outputs,
                                   network_args=network_args)
    num_layers_12_experiment_name = f"double_prune_total_num_components_240_epochs_60_pct_0.15_iters_1_adapt_dropout_False_{peak_lr}"
    ...
    network_args["lst_channels"] = [i * 128 for i in lst_channels]
    network_args["lst_att_heads"] = lst_att_heads
    network_args["lst_ff1_dropouts"] = [(i / 4) * 0.1 for i in lst_ff1_h_dims]
    network_args["lst_ff2_dropouts"] = [(i / 4) * 0.1 for i in lst_ff2_h_dims]
    network_args["lst_conv_dropouts"] = [(i / 4) * 0.1 for i in lst_channels]
    network_args["lst_att_weights_dropout"] = [(i / 8) * 0.1 for i in lst_att_heads]
    network_args["lst_mhsa_dropouts"] = [(i / 8) * 0.1 for i in lst_att_heads]

    config = get_default_config_v1(num_inputs=80, num_outputs=num_outputs,
                                   network_args=network_args)
    num_layers_12_experiment_name = f"double_prune_total_num_components_240_epochs_60_pct_0.15_iters_1_neural_block_adapt_dropout_True_{peak_lr}"
    experiments[num_layers_12_experiment_name] = get_returnn_configs(config, config)
    ...
    config = get_default_config_v1(num_inputs=80, num_outputs=num_outputs,
                                   network_args=network_args)
    num_layers_12_experiment_name = f"decomposable_baseline_{peak_lr}"
    experiments[num_layers_12_experiment_name] = get_returnn_configs(config, config)
