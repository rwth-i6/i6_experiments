    ...
    for peak_lr in [3e-4, 4e-4, 5e-4]:
        ...
        for aux_losses  in [{"12":1, "8":0.3, "4":0.3}]:
            for vgg_act in ["relu"]:
                ...
                system.add_experiment_configs(
                    f"epochs_600_recog_num_layers_8_peak_lr_{str_peak_lr}_num_aux_losses_{len(aux_losses)}_batch_15000_wei_hyper_new_frontend",
                    get_returnn_config_collection(data.train_data_config, data.cv_data_config, lr=peak_lr_dict,
                                                  batch_size=15000 * 160, network_args=network_args)
                )
                network_args["recog_num_layer"] = 4
                system.add_experiment_configs(
                    f"epochs_600_recog_num_layers_4_peak_lr_{str_peak_lr}_num_aux_losses_{len(aux_losses)}_batch_15000_wei_hyper_new_frontend",
                    get_returnn_config_collection(data.train_data_config, data.cv_data_config, lr=peak_lr_dict,
                                                  batch_size=15000 * 160, network_args=network_args)
                )
                ...
                str_peak_lr = str(peak_lr).replace("-", "_").replace(".", "_")
                system.add_experiment_configs(
                    f"wei_lr_epochs_600_recog_num_layers_12_peak_lr_{str_peak_lr}_num_aux_losses_{len(aux_losses)}_batch_15000_wei_hyper_new_frontend",
                    get_returnn_config_collection(data.train_data_config, data.cv_data_config, lr=peak_lr_dict,
                                                  batch_size=15000 * 160, network_args=network_args)
                )
                network_args["recog_num_layer"] = 8
                system.add_experiment_configs(
                    f"wei_lr_epochs_600_recog_num_layers_8_peak_lr_{str_peak_lr}_num_aux_losses_{len(aux_losses)}_batch_15000_wei_hyper_new_frontend",
                    get_returnn_config_collection(data.train_data_config, data.cv_data_config, lr=peak_lr_dict,
                                                  batch_size=15000 * 160, network_args=network_args)
                )
                network_args["recog_num_layer"] = 4
                system.add_experiment_configs(
                    f"wei_lr_epochs_600_recog_num_layers_4_peak_lr_{str_peak_lr}_num_aux_losses_{len(aux_losses)}_batch_15000_wei_hyper_new_frontend",
                    get_returnn_config_collection(data.train_data_config, data.cv_data_config, lr=peak_lr_dict,
                                                  batch_size=15000 * 160, network_args=network_args)
                )
