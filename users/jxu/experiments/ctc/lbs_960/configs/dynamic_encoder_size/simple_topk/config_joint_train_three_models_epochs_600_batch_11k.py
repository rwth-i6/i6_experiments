...
def run_lbs_960_torch_conformer_raw_wave_wei_hyper() -> SummaryReport:
    ...
    for peak_lr in [5e-4]:
        for k_annealing_step, k_reduction_per_iter in [(200,16), (100,8), (50,4)]:
            for layer_dropout_mod_select in [0]:
                for layer_dropout_fix_mod in [0.3]:
                    ...
                    system.add_experiment_configs(
                        f"bach_12k_thee_model_subepochs_600_peak_lr_{str_peak_lr}_k_annealing_{k_annealing_step}_{k_reduction_per_iter}_mod_48",
                        get_returnn_config_collection(data.train_data_config, data.cv_data_config, lr=peak_lr_dict,
                                                      batch_size=11000 * 160, network_args=network_args, kwargs={})
                        )

                    network_args["recog_num_mods"] = 32
                    system.add_experiment_configs(
                        f"bach_12k_thee_model_subepochs_600_peak_lr_{str_peak_lr}_k_annealing_{k_annealing_step}_{k_reduction_per_iter}_mod_32",
                        get_returnn_config_collection(data.train_data_config, data.cv_data_config, lr=peak_lr_dict,
                                                      batch_size=11000 * 160, network_args=network_args, kwargs={})
                        )
                    ...
                    system.add_experiment_configs(
                        f"bach_12k_thee_model_subepochs_600_peak_lr_{str_peak_lr}_k_annealing_{k_annealing_step}_{k_reduction_per_iter}_mod_16",
                        get_returnn_config_collection(data.train_data_config, data.cv_data_config, lr=peak_lr_dict,
                                                      batch_size=11000 * 160, network_args=network_args, kwargs={})
                        )


        for peak_lr in [5e-4, 4e-4]:
            for k_annealing_step, k_reduction_per_iter in [(200, 16), (100, 8), (50, 4)]:
                for layer_dropout_mod_select in [0]:
                    for layer_dropout_fix_mod in [0.3]:
                        layer_dropout = {"layer_dropout_mod_select": layer_dropout_mod_select,
