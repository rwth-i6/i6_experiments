...
def run_lbs_960_torch_conformer_raw_wave_wei_hyper() -> SummaryReport:
    ...
    for peak_lr in [4e-4]:
        for k_annealing_step, k_reduction_per_iter in [(50,4)]:
            for layer_dropout_mod_select in [0, 0.1, 0.3]:
                for layer_dropout_fix_mod in [0, 0.1, 0.3, 0.5]:
                    ...
                    str_layer_dropout = str(layer_dropout_mod_select).replace("-", "_").replace(".", "_")+"_"+str(layer_dropout_fix_mod).replace("-", "_").replace(".", "_")
