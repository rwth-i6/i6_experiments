    ...
    for peak_lr in [4e-4]:
        for stage_1_layer_dropout_on_large in [False]:
            for layer_dropout in [0.3, 0.4]:
                for stage_1_num_steps_per_iter, stage_1_num_zero_per_iter in [([400],[32]), ([200,200], [16,32]), ([100]*4,[8,16,24,32]), ([50]*8,[4,8,12,16,20,24,28,32]), ([25]*16,list(np.arange(2, 33, 2))), ([12.5]*32, list(np.arange(1, 33)))]:
                    for zeroout_val in [-5]:
                        stage_1_expected_sparsity_per_iter = [n/48 for n in stage_1_num_zero_per_iter]
                        stage_1_num_steps_per_iter = [n*1220 for n in stage_1_num_steps_per_iter]
                        for sparsity_loss_scale in [5]:
                            for stage_2_small_model_loss_scale in [0.3]:
                                ...
                                system.add_experiment_configs(
                                    f"zeroout_val_{abs(zeroout_val)}_joint_train_after_selection_iter_{len(stage_1_num_zero_per_iter)}_stage_2_small_model_loss_scale_{stage_2_small_model_loss_scale}_loss_scale_{sparsity_loss_scale}_layer_dropout_{layer_dropout}_num_recog_mods_48_peak_lr_{peak_lr}",
                                ...
                                system.add_experiment_configs(
                                    f"zeroout_val_{abs(zeroout_val)}_joint_train_after_selection_iter_{len(stage_1_num_zero_per_iter)}_stage_2_small_model_loss_scale_{stage_2_small_model_loss_scale}_loss_scale_{sparsity_loss_scale}_layer_dropout_{layer_dropout}_num_recog_mods_32_peak_lr_{peak_lr}",
                                    get_returnn_config_collection(data.train_data_config, data.cv_data_config, lr=peak_lr_dict,
                                                                  batch_size=15000 * 160, network_args=network_args, kwargs={})
                                    )
                                ...
                                system.add_experiment_configs(
                                    f"zeroout_val_{abs(zeroout_val)}_joint_train_after_selection_iter_{len(stage_1_num_zero_per_iter)}_stage_2_small_model_loss_scale_{stage_2_small_model_loss_scale}_loss_scale_{sparsity_loss_scale}_layer_dropout_{layer_dropout}_num_recog_mods_16_peak_lr_{peak_lr}",
                                    get_returnn_config_collection(data.train_data_config, data.cv_data_config, lr=peak_lr_dict,
                                                                  batch_size=15000 * 160, network_args=network_args, kwargs={})
                                    )
    ...
    for peak_lr in [4e-4]:
        for stage_1_layer_dropout_on_large in [False]:
            for layer_dropout in [0.3]:
                for stage_1_num_steps_per_iter, stage_1_num_zero_per_iter in [([360],[32]), ([180,180], [16,32]), ([90]*4,[8,16,24,32]), ([40]*8,[4,8,12,16,20,24,28,32]), ([22.5]*16,list(np.arange(2, 33, 2))), ([11.25]*32, list(np.arange(1, 33)))]:
                    for zeroout_val in [-5]:
                        stage_1_expected_sparsity_per_iter = [n/48 for n in stage_1_num_zero_per_iter]
                        stage_1_num_steps_per_iter = [n*1220 for n in stage_1_num_steps_per_iter]
                        for sparsity_loss_scale in [5]:
                            for stage_2_small_model_loss_scale in [0.3]:
                                ...
                                str_layer_dropout = str(layer_dropout).replace("-", "_").replace(".", "_")
                                system.add_experiment_configs(
                                    f"selection_subepochs_360_zeroout_val_{abs(zeroout_val)}_joint_train_after_selection_iter_{len(stage_1_num_zero_per_iter)}_stage_2_small_model_loss_scale_{stage_2_small_model_loss_scale}_loss_scale_{sparsity_loss_scale}_layer_dropout_{layer_dropout}_num_recog_mods_48_peak_lr_{peak_lr}",
                                    get_returnn_config_collection(data.train_data_config, data.cv_data_config, lr=peak_lr_dict,
                                                                  batch_size=15000 * 160, network_args=network_args, kwargs={})
                                    )
                                ...
                                network_args["recog_num_mods"] = 32
                                system.add_experiment_configs(
                                    f"selection_subepochs_360_zeroout_val_{abs(zeroout_val)}_joint_train_after_selection_iter_{len(stage_1_num_zero_per_iter)}_stage_2_small_model_loss_scale_{stage_2_small_model_loss_scale}_loss_scale_{sparsity_loss_scale}_layer_dropout_{layer_dropout}_num_recog_mods_32_peak_lr_{peak_lr}",
                                    get_returnn_config_collection(data.train_data_config, data.cv_data_config, lr=peak_lr_dict,
                                                                  batch_size=15000 * 160, network_args=network_args, kwargs={})
                                    )
                                ...
                                network_args["recog_num_mods"] = 16
                                system.add_experiment_configs(
                                    f"selection_subepochs_360_zeroout_val_{abs(zeroout_val)}_joint_train_after_selection_iter_{len(stage_1_num_zero_per_iter)}_stage_2_small_model_loss_scale_{stage_2_small_model_loss_scale}_loss_scale_{sparsity_loss_scale}_layer_dropout_{layer_dropout}_num_recog_mods_16_peak_lr_{peak_lr}",
                                    get_returnn_config_collection(data.train_data_config, data.cv_data_config, lr=peak_lr_dict,
                                                                  batch_size=15000 * 160, network_args=network_args, kwargs={})
                                    )
