...
def get_serializer(model_config, variant: ConfigVariant, in_dim: int = 1) -> Collection:
    from i6_experiments.users.jxu.experiments.ctc.swb.pytorch_networks.decomposable_conformer.decomposable_conformer import \
...
def returnn_config_generator(num_subepochs:int, train_data_config: dict, dev_data_config: dict, peak_lr: float) -> dict:
    ...
    network_args = {"specaug_args": {"time_min_num_masks": 2,
                    ...
                    "final_dropout": 0.1,
                    "model_size": 512, }
    lst_ff1_h_dims = [0, 4, 4, 4, 4, 4, 4, 4, 4, 4, 8, 8]
    lst_ff2_h_dims = [0, 2, 2, 0, 4, 4, 4, 4, 4, 4, 8, 4]
    lst_att_heads = [8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 11, 8]
    lst_channels = [0, 0, 0, 2, 3, 5, 6, 6, 8, 8, 8, 8]
