...
from i6_models.assemblies.dynamic_adaptable_conformer.conformer_v1_adapt_dropout import (
...
from i6_experiments.users.jxu.experiments.ctc.tedlium2.pytorch_networks.neural_block.dynamic_adaptable_conformer.gradient_based_score import *
...
class ConformerCTCModel(torch.nn.Module):
    def __init__(self, step: int, cfg: ConformerCTCConfig, **kwargs):
        ...
        print(self.lst_cmp_cost.size())
        # assert self.total_num_components == self.lst_cmp_cost.size()[0]
        # TODO: should save the
        self.final_linear = torch.nn.Linear(cfg.conformer_cfg.block_cfgs[-1].ff2_cfg.input_dim, cfg.target_size)
    ...
    def forward(
        ...
        outs, sequence_mask = self.conformer(x, sequence_mask)  # [B, T, F]
        x = self.final_dropout(outs[0])
...
def get_default_config_v1(num_inputs: int, num_outputs: int, network_args: dict) -> ConformerCTCConfig:
    ...
    lst_channels = [model_dim] * num_layers if "lst_channels" not in network_args else network_args["lst_channels"]
    lst_ff1_dropouts = (
    ...
    block_modules = (
        ["ff1", "conv", "mhsa", "ff2"] if "block_modules" not in network_args else network_args["block_modules"]
    )
    ...
    for i in range(num_layers):
        ...
        conv_cfg = ConformerConvolutionV2Config(
            input_dim=model_dim,
            channels=lst_channels[i],
            kernel_size=kernel_size,
            dropout=dropout,
            activation=torch.nn.SiLU(),
            norm=LayerNormNC(lst_channels[i]),
        )
...
def train_step(*, model: torch.nn.Module, extern_data: TensorDict, global_train_step, **_):
    ...
    if (
        global_train_step <= max(model.adaptation_opts["adaptation_global_step"])
        and global_train_step % model.grad_score_opts["grad_score_update_steps"] == 0
    ):
        if model.grad_score_opts["grad_score_metric"] == "first_taylor":
            update_gradient_score = grad_score_taylor(loss, model)
        elif model.grad_score_opts["grad_score_metric"] == "obd":
            update_gradient_score = obd_score(
                model, audio_features, audio_features_len, targets, sequence_lengths, targets_len
            )
        elif model.grad_score_opts["grad_score_metric"] == "gradient":
            update_gradient_score = grad_score_mag(loss, model)
        elif model.grad_score_opts["grad_score_metric"] == "magnitude":
            update_gradient_score = weight_magnitude_score(model)
        ...
        print("score_per_component", model.score_per_component)
    ...
    if global_train_step in model.adaptation_opts["adaptation_global_step"]:
        ...
        new_component_dist = {"ff1": [], "ff2": [], "conv": [], "mhsa": []}
        new_lst_cmp_cost = []
        block_modules = model.conformer.module_list[0].cfg.modules
        cur_indx = 0
        selected_idx = 0
        for block_idx in range(len(model.conformer.module_list)):
            for mod_idx in range(len(block_modules)):
                ...
                if module in ["ff1", "ff2"]:
                    if len(gates) > 0:
                        dim_per_chunk = model.conformer.module_list[block_idx].module_list[
                            mod_idx
                        ].linear_ff_weight.size()[0] // len(gates)
                        model.conformer.module_list[block_idx].module_list[mod_idx].double_and_prune_params(
                            dim_per_chunk * num_chunks,
                            [e - cur_indx for e in selected_indices[selected_idx : selected_idx + num_chunks]],
                        )
                elif module in ["conv"]:
                    if len(gates) > 0:
                        chunk_dim = model.conformer.module_list[block_idx].module_list[
                            mod_idx
                        ].pointwise_conv2_weights.size(1) // len(gates)
                        model.conformer.module_list[block_idx].module_list[mod_idx].double_and_prune_params(
                            chunk_dim * num_chunks,
                            [e - cur_indx for e in selected_indices[selected_idx : selected_idx + num_chunks]],
                        )
                elif module in ["mhsa"]:
                    model.conformer.module_list[block_idx].module_list[mod_idx].mhsa.double_and_prune_params(
                        num_chunks, [e - cur_indx for e in selected_indices[selected_idx : selected_idx + num_chunks]]
                    )
        ...
        model.to("cuda")
        ff1_diff = sum(new_component_dist["ff1"]) - sum(model.component_dist["ff1"])
        ff2_diff = sum(new_component_dist["ff2"]) - sum(model.component_dist["ff2"])
        conv_diff = sum(new_component_dist["conv"]) - sum(model.component_dist["conv"])
        mhsa_diff = sum(new_component_dist["mhsa"]) - sum(model.component_dist["mhsa"])
        print(
            "cost diff",
            ff1_diff * model.adaptation_opts["dict_module_cost"]["ff1"]
            + ff2_diff * model.adaptation_opts["dict_module_cost"]["ff2"]
            + conv_diff * model.adaptation_opts["dict_module_cost"]["conv"]
            + mhsa_diff * model.adaptation_opts["dict_module_cost"]["mhsa"],
        )
        print("tmp_add_cost - tmp_rm_cost", tmp_add_cost - tmp_rm_cost)
        ...
        print("new_component_dist", new_component_dist)
...
def get_recog_serializer(
    ...
    return get_basic_pt_network_serializer(
        ...
        additional_serializer_objects=[
            ...
            Import(
                "i6_models.assemblies.dynamic_adaptable_conformer.conformer_v1_adapt_dropout.ConformerBlockV1Config"
            ),
            Import("torch.nn.SiLU"),
            Import("i6_models.parts.dynamic_adaptable_conformer.ConformerConvolutionV2Config"),
            ...
            Import("i6_models_repo.i6_models.parts.conformer.norm.LayerNormNC"),
        ],
    )
