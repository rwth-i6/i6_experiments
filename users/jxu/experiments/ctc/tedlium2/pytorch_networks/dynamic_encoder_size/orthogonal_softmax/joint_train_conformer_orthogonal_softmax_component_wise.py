...
from i6_models.parts.conformer_structure_prune.convolution import ConformerConvolutionV1Config
from i6_models.parts.conformer_structure_prune.mhsa import ConformerMHSAV1Config
from i6_models.parts.conformer_structure_prune.feedforward import ConformerPositionwiseFeedForwardV1Config
...
from i6_models.assemblies.conformer_with_dynamic_model_size.selection_with_independent_softmax_based_on_num_params_random_pct import (
...
class ConformerCTCModel(torch.nn.Module):
    ...
    def forward(
        ...
        if self.conformer.softmax_kwargs["softmax_constraint_loss_scale"] == "lagrange_adaptive":
            ...
            if "softmax_lambda_regularization_scale" in self.conformer.softmax_kwargs:
                softmax_constraint -= self.conformer.softmax_kwargs["softmax_lambda_regularization_scale"] * torch.square(softmax_lambda)
...
def get_default_config_v1(num_inputs: int, num_outputs: int, network_args: dict) -> ConformerCTCConfig:
    ...
    apply_ff_adaptive_dropout = network_args["apply_ff_adaptive_dropout"]
...
def train_step(*, model: torch.nn.Module, extern_data: TensorDict, global_train_step, **kwargs):
    ...
    if global_train_step <= model.stage_1_global_steps:
        for i in [0, -1]:
            ...
            if isinstance(model.aux_loss_scales[str(loss_layers[i])], float) or isinstance(model.aux_loss_scales[str(loss_layers[i])], int):
