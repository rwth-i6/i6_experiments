...
class ConformerCTCModel(torch.nn.Module):
    def __init__(self, step: int, cfg: ConformerCTCConfig, **kwargs):
        ...
        self.conformer.recog_num_layers = self.recog_num_layers
        self.k_anneal_kwargs = cfg.k_anneal_kwargs
    ...
    def forward(
        ...
        conformer_out_list, sequence_mask = self.conformer(x, sequence_mask, global_train_step,
                                                           self.k_anneal_kwargs)  # [B, T, F]
...
def get_default_config_v1(num_inputs: int, num_outputs: int, network_args: dict) -> ConformerCTCConfig:
    ...
    k_anneal_kwargs = network_args["k_anneal_kwargs"]
    ...
    return ConformerCTCConfig(
        ...
        k_anneal_kwargs=k_anneal_kwargs
    )
...
def train_step(*, model: torch.nn.Module, extern_data: TensorDict, global_train_step, **kwargs):
    ...
    k_anneal_num_steps_per_iter = model.k_anneal_kwargs["k_anneal_num_steps_per_iter"]
    k_reduction_per_iter = model.k_anneal_kwargs["k_reduction_per_iter"]
    k_anneal_num_iters = (4 * len(model.conformer.module_list) - model.conformer.min_k) / k_reduction_per_iter
