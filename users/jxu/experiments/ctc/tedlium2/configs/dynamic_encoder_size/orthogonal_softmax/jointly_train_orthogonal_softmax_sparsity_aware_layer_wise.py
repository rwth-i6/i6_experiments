...
def returnn_config_generator(train_data_config: dict, dev_data_config: dict, peak_lr: float, batch_size: int) -> dict:
    ...
    for num_iters in [1]:
        for layer_dropout_stage_2 in [0.1]:
                ...
                for softmax_constraint_norm in ["L2_norm_sqrt"]:
                    for softmax_constraint_loss_scale in ["linear_increase"]:
                        for aux_loss_scale in ["focal_loss"]:
                            for dropout, final_dropout in [(0.2, 0)]:
                                for freq_mask_max_size in [5]:
                                    for max_softmax_constraint_loss_scale, softmax_constraint_warmup_steps in [(1, 150)]:
                                        ...
                                        aux_loss_scales = {'0.19': aux_loss_scale, '0.35': aux_loss_scale, '0.51': aux_loss_scale,
                                                           '0.76': aux_loss_scale, "1": 1}
                                        ...
                                        network_args = {"time_max_mask_per_n_frames": 25,
                                                        ...
                                                        "pct_params_set": [0.19, 0.35, 0.51, 0.76, 1],
                                        ...
                                        for pct in [0.19, 0.35, 0.51, 0.76, 1]:
                                            ...
                                            if softmax_constraint_loss_scale == "linear_increase":
                                                exp_name = f"layer_dropout_0_{layer_dropout_stage_2}_final_dropout_{final_dropout}_softmax_{softmax_constraint_norm}_scale_{softmax_constraint_loss_scale}_{max_softmax_constraint_loss_scale}_{softmax_constraint_warmup_steps}_aux_loss_scale_{aux_loss_scale}_{pct}_model"
                                            else:
                                                exp_name = f"layer_dropout_0_{layer_dropout_stage_2}_final_dropout_{final_dropout}_softmax_{softmax_constraint_norm}_scale_{softmax_constraint_loss_scale}_aux_loss_scale_{aux_loss_scale}_{pct}_model"
