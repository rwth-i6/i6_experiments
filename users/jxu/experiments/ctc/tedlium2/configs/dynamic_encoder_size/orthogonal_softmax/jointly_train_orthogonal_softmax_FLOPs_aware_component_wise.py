                    ...
                    for softmax_constraint_norm in ["L2_norm_sqrt"]:
                        for softmax_constraint_loss_scale in ["linear_increase"]:
                            for aux_loss_scale in ["focal_loss"]:
                                for dropout, final_dropout in [(0.2, 0), (0.2, 0.2)]:
                                    for max_softmax_constraint_loss_scale, softmax_constraint_warmup_steps in [(1, 150),
                                                                                                               (
                                                                                                               1, 100)]:
                                        aux_loss_scales = {'0.337': aux_loss_scale, '0.457': aux_loss_scale,'0.578': aux_loss_scale, '0.759': aux_loss_scale, "1": 1}
                                        ...
                                        params_kwargs = {"num_params": ([2.0675] * 4 + [3.21] + [0.6966666667] * 6 + [2.0675] * 4) * 12,
                                                            "rest_params": 110,
                                                            "total_params": 400}
                                        ...
                                        for pct in [0.337, 0.457, 0.578, 0.759, 1]:
                                            ...
                                            if softmax_constraint_loss_scale == "linear_increase":
                                                exp_name = f"pct_0_337_final_dropout_{final_dropout}_softmax_{softmax_constraint_norm}_scale_{softmax_constraint_loss_scale}_{max_softmax_constraint_loss_scale}_{softmax_constraint_warmup_steps}_aux_loss_scale_{aux_loss_scale}_{pct}_model"
                                            else:
