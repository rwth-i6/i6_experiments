...
from i6_experiments.users.jxu.experiments.multilingual.voxpopuli.data.get_data import get_voxpopuli_data_per_lang
...
def conformer_ctc_noreturnn(
    ...
    from i6_experiments.users.jxu.experiments.multilingual.voxpopuli.pytorch_networks.i6modelsV1_VGG4LayerActFrontendV1_v6_onnx_exportable import \
        get_model_config
    ...
    for specauc_start_epoch in [1, 30]:
        model_config = get_model_config(vocab_size_without_blank=vocab_size, network_args={"specauc_start_epoch": specauc_start_epoch})
        ...
        for peak_lr in learning_rates:
            ...
            train_args = {
                **copy.deepcopy(train_args_adamw03_accum2_jjlr),
                "network_module": "i6modelsV1_VGG4LayerActFrontendV1_v6_onnx_exportable",
            ...
            if len(lang_list) < 16:
                ...
                train_job = run_exp(
                    prefix_name + f"conformer_new/i6modelsV1_VGG4LayerActFrontendV1_v6/ctc/{vocab_name.split('.')[0] + ('_prefixed' if vocab_name.split('.')[-1] == 'prefixed' else '')}/{'_'.join(lang_list)}/{batch_size}_{bpe_size}_lr{peak_lr}",
                    datasets=train_data, train_args=train_args, search_args=search_args, recog_args=recog_args,
                    with_prior=False, evaluate_epoch=eval_epoch, vocab_name=vocab_name)

                train_job.rqmt["gpu_mem"] = gpu_mem
                tk.register_output(
                    f"output/{vocab_name}/{'_'.join(lang_list)}/ctc/{batch_size}_{bpe_size}_lr{peak_lr}/learning_rates",
                    train_job.out_learning_rates)
                tk.register_output(
                    f"output/{vocab_name}/{'_'.join(lang_list)}/ctc/{batch_size}_{bpe_size}_lr{peak_lr}/out_model_dir",
                ...
                train_job = run_exp(
                    prefix_name + f"conformer_new/i6modelsV1_VGG4LayerActFrontendV1_v6/ctc/{vocab_name.split('.')[0]}/{batch_size}_{bpe_size}_lr{peak_lr}",
                    datasets=train_data, train_args=train_args, search_args=search_args, recog_args=recog_args,
