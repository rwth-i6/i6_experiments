...
from i6_experiments.users.jxu.experiments.multilingual.voxpopuli.data.dataset_settings import build_test_dataset, TrainingDatasetSettings
from i6_experiments.users.jxu.experiments.multilingual.voxpopuli.data.get_data import get_voxpopuli_data_per_lang, get_voxpopuli_data_with_language_id
...
RETURNN_ROOT = tk.Path("/u/jxu/setups/voxpopuli/2026-01-14-multilingual-bpe-ctc/tools/20251117_returnn/returnn")
...
def lid_aware_ctc(
    ...
    from i6_experiments.users.jxu.experiments.multilingual.voxpopuli.configs.bpe_ctc.get_returnn_config import \
        get_training_config, get_search_config, get_prior_config
    from i6_experiments.users.jxu.experiments.multilingual.voxpopuli.pytorch_networks.language_aware_ctc import \
        get_model_config
    from i6_experiments.users.rossenbach.experiments.rescale.tedlium2_standalone_2023.pipeline import training
    # from i6_experiments.users.nikolov.experiments.voxpopuli.ctc_rnnt_standalone_2024.pipeline_flashlight import search, \
    #     prepare_asr_model
    ...
    for lang in lang_list:
        ...
        test_dataset_tuples[lang] = get_voxpopuli_data_per_lang(
            ...
            split="test",
            lang_list=[lang],
            partition_epoch=1), tk.Path(
            f"/work/asr3/jxu/hiwis/nikolov/multilang_0325/corpus/asr_{lang}.test.corpus.xml.gz")

    # ---------------------------------------------------------------------------------------------------------------- #
    def run_exp(ft_name, datasets, train_args, search_args=None, recog_args=None, num_epochs=600,
                decoder="ctc.decoder.flashlight_ctc_v1_onnx_v2", with_prior=False, evaluate_epoch=None,
        ...
        #     lexicon=get_text_lexicon(ft_name + "/text_lex", bpe_size, add_prefix),
        ...
        #     beam_threshold=14,
        # )
        # # recog_args["net_args"]["model_config_dict"]["label_target_size"] = 9323
        # # print(train_args["net_args"]["model_config_dict"]["label_target_size"])
    ...
    for specauc_start_epoch in [30]:
        model_config = get_model_config(vocab_size_without_blank=vocab_size, network_args={"specauc_start_epoch": specauc_start_epoch, "lid_sc_layer": 3})
        recog_model_config = get_model_config(vocab_size_without_blank=vocab_size, network_args={"lid_sc_layer": 3})
        ...
        for peak_lr in learning_rates:
            train_args_adamw03_accum2_jjlr = {
                "config": {
                    ...
                    "extern_data": {"data": {"dim": 1}, "targets": {"dim": vocab_size+1, "sparse": True}, "language": {"dim": 16, "sparse": True}}
                },
                "debug": True,
            }
            ...
            train_args = {
                **copy.deepcopy(train_args_adamw03_accum2_jjlr),
                "network_module": "language_aware_ctc",
            ...
            train_data = CTCTrainingDatasets(
                train=get_voxpopuli_data_with_language_id("/u/kaloyan.nikolov/experiments/multilang_0325/output/voxpopuli_asr",
                                         f"/u/jxu/setups/voxpopuli/2026-01-14-multilingual-bpe-ctc/output/target_hdf/target_hdf_bpe_size_{bpe_size}",
                                         split="train",
                                         partition_epoch=20),
                cv=get_voxpopuli_data_with_language_id("/u/kaloyan.nikolov/experiments/multilang_0325/output/voxpopuli_asr",
                                      f"/u/jxu/setups/voxpopuli/2026-01-14-multilingual-bpe-ctc/output/target_hdf/target_hdf_bpe_size_{bpe_size}",
                                      split="dev",
                                      partition_epoch=1),
            ...
            train_job = run_exp(
                prefix_name + f"conformer_new/lid_aware_ctc/ctc/{vocab_name.split('.')[0]}/{batch_size}_{bpe_size}_lr{peak_lr}",
                datasets=train_data, train_args=train_args, search_args=search_args, recog_args=recog_args,
                with_prior=False, evaluate_epoch=eval_epoch, recog_mem=recog_mem, vocab_name=vocab_name)

            train_job.rqmt["gpu_mem"] = gpu_mem
            tk.register_output(f"output/ctc/{vocab_name}/{batch_size}_{bpe_size}_lr{peak_lr}/learning_rates",
                               train_job.out_learning_rates)
            tk.register_output(f"output/ctc/{vocab_name}/{batch_size}_{bpe_size}_lr{peak_lr}/out_model_dir",
