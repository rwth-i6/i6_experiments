...
def train_step(*, model, **kwargs):
    if kwargs.get("extern_data", None):
        data = kwargs.get("extern_data")
        raw_audio = data["data"].raw_tensor
        raw_audio = raw_audio.squeeze(-1)
        raw_audio_len = data["data"].dims[1].dyn_size_ext.raw_tensor
        labels = data["targets"].raw_tensor.long()
        labels_len = data["targets"].dims[1].dyn_size_ext.raw_tensor
