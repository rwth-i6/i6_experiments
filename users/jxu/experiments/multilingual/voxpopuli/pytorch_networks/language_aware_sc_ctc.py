...
class Model(torch.nn.Module):
    def __init__(self, model_config_dict, **kwargs):
        ...
        self.sc_softmax_linear = nn.ModuleList([nn.Linear(conformer_size, self.cfg.label_target_size + 1) for _ in range(len(self.sc_layer))])  # + CTC blank
        self.sc_linear = nn.ModuleList([nn.Linear(self.cfg.label_target_size + 1, conformer_size) for _ in range(len(self.sc_layer))])  # + CTC blank
    ...
    def forward(
        ...
        for i in range(max(return_layers) + 1):
            ...
            if i+1 in self.sc_layer:
                index = self.sc_layer.index(i+1)
                sc_out_prob = self.sc_softmax_linear[index](x)
                sc_log_prob = torch.log_softmax(sc_out_prob, dim=2)
                sc_log_probs.append(sc_log_prob)
                x = x + self.sc_linear[index](torch.exp(sc_log_prob))
        ...
        return log_probs, lid_out_probs, sc_log_probs, torch.sum(out_mask, dim=1)
...
def train_step(*, model: torch.nn.Module, extern_data: TensorDict, global_train_step, **kwargs):
    ...
    logprobs, lid_out_probs, sc_log_probs, audio_features_len = model(
        raw_audio=raw_audio,
    ...
    aux_loss_scale = 1/(len(sc_log_probs)+1) * 0.3
    for i in range(len(sc_log_probs)):
        transposed_sc_log_prob = torch.permute(sc_log_probs[i], (1, 0, 2))  # CTC needs [T, B, F]
        ctc_loss = nn.functional.ctc_loss(
            transposed_sc_log_prob,
            labels,
            input_lengths=audio_features_len,
            target_lengths=labels_len,
            blank=model.cfg.label_target_size,
        ...
        rf.get_run_ctx().mark_as_loss(name=f"ctc_sc_{i}", loss=ctc_loss,
                                      custom_inv_norm_factor=rf.reduce_sum(labels_len_rf, axis=batch_dim),
                                      scale=aux_loss_scale)
    ...
    if lid_out_probs is not None:
        ...
        rf.get_run_ctx().mark_as_loss(name="lid_ctc_loss", loss=lid_ctc_loss,
                                      custom_inv_norm_factor=rf.reduce_sum(language_len_rf, axis=batch_dim), scale=aux_loss_scale)
...
def get_model_config(vocab_size_without_blank: int, network_args: dict) -> ModelConfig:
    ...
    sc_layer = network_args["sc_layer"]
