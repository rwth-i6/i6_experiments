...
def ls960_hmm_blstm_base():
    prefix_name = "ctc_fh_2024/ls960_hmm/blstm"
    ...
    from ...pytorch_networks.hmm.blstm.blstm_cfg import (
        SpecaugConfig,
        BlstmPoolingEncoderConfig,
        BlstmModelConfig,
    ...
    ModelConfigTemplate = partial(BlstmModelConfig,
    ...
    # Manually excluded since it already ran under a different hash
    #del train_configs["nick_AM0.3"]

    # Use single config for debugging
    train_configs = {"conze_debug_4": train_configs["nick_AM0.3"]}
    ...
    for config_name, (lr_config, model_config) in train_configs.items():
        ...
        network_module_blstm = "hmm.blstm.blstm"
        train_args_blstm = {
            "config": train_config_11gbgpu_amp,
            "network_module": network_module_blstm,
            "net_args": {"model_config_dict": asdict(model_config)},
            "debug": False,
            "include_native_ops": True,
        }

        name = f"11gbgpu_hmm_{config_name}"
        training_name = prefix_name + "/" + network_module_blstm + name
        train_job = training(training_name, train_data, train_args_blstm, num_epochs=250, **default_returnn)
