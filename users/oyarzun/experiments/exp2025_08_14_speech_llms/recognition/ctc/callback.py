...
class RecognitionToTextDictCallback(ForwardCallbackIface):
    ...
    def process_seq(self, *, seq_tag: str, outputs: TensorDict, **kwargs):
        ...
        # slice to the actual length
        best_hyp = best_hyp[:best_hyp_len].tolist()
        # remove repetitions
        best_hyp = [
            i for (i, j) in zip(best_hyp[:-1], best_hyp[1:]) if i != j
        ]
        # remove blanks
        # TODO: do not statically assume that blank_idx=vocab.num_labels
        best_hyp = [i for i in best_hyp if i != self.vocab.num_labels]
