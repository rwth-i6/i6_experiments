...
from ..text import get_phonemized_data, get_phonemized_data_with_text
...
def get_phonemized_text(
    ...
    if lexicon_file is None:
        lexicon_file = get_librispeech_lexicon()
...
def get_phonemized_text_corpus(
    data_name: str,
    lexicon_file: Optional[Path] = None,
    vocab_file: Optional[Path] = None,
    sil_prob: float = 0.25,
    ...
    return get_phonemized_data_with_text(
        dataset_name=data_name,
        corpus_name="librispeech",
        text_file=text_data,
        lexicon_file=lexicon_file,
