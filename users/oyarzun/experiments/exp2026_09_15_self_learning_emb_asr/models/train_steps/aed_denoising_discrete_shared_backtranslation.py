...
def backtranslation_step(
    ...
    if torch.any(src_indices_lens != 0).item():
        ...
        with torch.no_grad():
            ...
            # Use 1.5x length limit to prevent excessive loops, but not too restrictive.
            ...
            # slice off the last token (which is eos_idx)
            pseudo_target_indices = pseudo_target_indices[:, :-1]  # B, T
