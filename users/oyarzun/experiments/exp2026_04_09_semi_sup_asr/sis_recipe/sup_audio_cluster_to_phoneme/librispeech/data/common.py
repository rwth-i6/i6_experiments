...
def build_training_datasets_clean100(
    settings: DatasetSettings,
    sil_prob: float = 0.25,
    surround_w_sil: bool = True,
    include_lm_data: bool = False,
):
    _, clusters_960, pca_960, _ = audio.get_featurized_audio(
        librispeech_key="train-other-960",
        dump_hdf_concurrent=10,
    ...
    _, _, _, clusters_clean100_hdfs = audio.get_featurized_audio(
    ...
    phoneme_clean100_hdfs, _, _, train_seq_tags = text.get_phonemized_text(
        "train-clean-100",
        lexicon_file=lexicon_file,
        dump_hdf_concurrent=10,
        vocab_file=phoneme_vocab,
    ...
    datasets = {
        "acoustic": MetaDataset(
            datasets={
                "feature_clusters": HdfDataset(
                    files=clusters_clean100_hdfs,
                    segment_file=train_seq_tags,
                ),
                "phon_indices": {
                    **HdfDataset(
                        files=phoneme_clean100_hdfs,
                        segment_file=train_seq_tags,
                        partition_epoch=settings.train_partition_epoch,
                        seq_ordering=settings.train_seq_ordering,
                    ).as_returnn_opts(),
