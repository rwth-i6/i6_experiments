...
def train_step(
    *,
    ...
    cross_attention_warmup_steps: int = 0,
    ...
    **_kwargs,
    ...
    if not is_pretraining:
        asr_step = ctx.step - mlm_pretrain_steps
        if asr_step == 0:
            print(f"========== PRETRAINING OVER, TRANSLATION STARTED at global step {ctx.step} ==========", flush=True)
        
        # 1. Cross-attention warmup: freeze everything except the text_decoder's cross-attention
        if cross_attention_warmup_steps > 0:
            if asr_step < cross_attention_warmup_steps:
                for param in model.parameters():
                    param.requires_grad = False
                for module in model.text_decoder.modules():
                    if "CrossAttention" in type(module).__name__:
        ...
        # 2. Lower initial ASR LR: scale the loss down during warmup
