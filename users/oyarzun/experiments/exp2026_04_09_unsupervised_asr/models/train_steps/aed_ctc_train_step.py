    ...
    **_kwargs,
    ...
    if phon_indices is not None and phon_indices_lens.size(0) != 0:
        ...
        phon_mask_packed = pack_sequence(
            ...
            [
                torch.concat((mask, torch.ones(num_eos_symbols, dtype=torch.bool, device=phon_indices.device)), dim=-1)
    ...
    if audio_features is not None and audio_features_lens.size(0) != 0:
        ...
        out_features_packed = pack_padded_sequence(
            out_features, audio_features_lens, batch_first=True, enforce_sorted=False
        )
        out_eos_logits_packed = pack_padded_sequence(
            out_eos_logits, audio_features_lens, batch_first=True, enforce_sorted=False
        )
