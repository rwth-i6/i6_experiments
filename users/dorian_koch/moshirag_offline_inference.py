...
def _run_fork_inference(
    in_dir: Path, out_dir: Path, *, repo: str, device: str, batch_size: int, use_gt_reference: bool, env: dict
) -> None:
    ...
    subprocess.run(cmd, env={**env, "PYTHONUNBUFFERED": "1"}, check=True)
...
def main():
    ...
    try:
        ...
        # Map each produced <stem>.wav (+ trace .json) back to its requested output path.
        n_ok = 0
        for stem, out in out_for_stem.items():
            produced = fork_out / f"{stem}.wav"
            assert produced.exists() and produced.stat().st_size > 0, (
                f"MoshiRAG produced no audio for {stem} (expected {produced})"
            )
            out_path = Path(out)
            ...
            if trace.exists():
                shutil.copy(
                    trace, out_path.parent / ("output.json" if out_path.name == "output.wav" else f"{stem}.json")
                )
            n_ok += 1
        print(f"[moshirag] finished {n_ok}/{len(pairs)} clips", flush=True)
    finally:
