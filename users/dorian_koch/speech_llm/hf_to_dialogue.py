...
def make_dialogue_gen(llm_url, model_name):
    ...
    def make_dialogue(example):
        ...
        user_msg =  "Background information: " + cot + "\n" \
                    "Here is the question: " + question + "\n" \
                    "The possible answers are: " + options_text + \
                    "The correct answer is: " + answer_text + ".\n" # TODO do we really want to give the agent so many different answers?
        
        user_msg += "You are a helpful assistant for converting multiple choice questions into dialogues. " \
...
class HfToDialogue(Job):
    ...
    def run(self):
        with vllm_server(self.llm_name) as llm_url:
            ...
            dataset = dataset["validation"].map(make_dialogue_gen(llm_url), num_proc=4)
