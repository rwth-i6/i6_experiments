...
def train_exp(
    ...
    *,
    ...
    model_with_checkpoints = train_v3(
        prefix,
        task=task,
        config=config,
...
class BlstmEncoder(ISeqDownsamplingEncoder):
    ...
    def __call__(
        ...
        *,
        ...
        x_fwd_lstm, _ = self.input_forward_lstm(x, in_spatial_dim=out_spatial_dim)
        x_bwd_lstm, _ = self.input_backward_lstm(x_flipped, in_spatial_dim=out_spatial_dim)

        for fwd_lstm_layer, bwd_lstm_layer in zip(self.forward_lstm_layers, self.backward_lstm_layers):
            lstm_inp = torch.cat((x_fwd_lstm, x_bwd_lstm), dim=-1)
            lstm_inp_flipped = rf.flip(lstm_inp, axis=out_spatial_dim)  # reverse input
            x_fwd_lstm, _ = fwd_lstm_layer(lstm_inp, in_spatial_dim=out_spatial_dim)
            x_bwd_lstm, _ = bwd_lstm_layer(lstm_inp_flipped, in_spatial_dim=out_spatial_dim)

        return torch.cat((x_fwd_lstm, x_bwd_lstm), dim=-1), out_spatial_dim
