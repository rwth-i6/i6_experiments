...
swb_lstm_bpe1k_subnet = {
    "class": "subnetwork",
    "from": ["prev:output"],
    "load_on_init": swb_lstm_bpe1k_model,
    "subnetwork": swb_lstm_bpe1k_net,
    "n_out": 1030,
}
...
libri_lstm_bpe10k_subnet = {
    "class": "subnetwork",
    "from": ["prev:output"],
    "load_on_init": libri_lstm_bpe10k_model,
    "subnetwork": libri_lstm_bpe10k_net,
    "n_out": 10025,
}


def get_libri_lstm_net(
    *, num_layers: int, embed_dim: int, lstm_dim: int, out_dim: int, use_transposed_weights: bool
) -> dict:
    d = dict()
    d["input"] = {"class": "linear", "n_out": embed_dim, "activation": "identity"}
    for i in range(num_layers):
        d["lstm%i" % i] = {
            ...
            "n_out": lstm_dim,
            "unit_opts": {"forget_bias": 0.0},
            "from": ["input"] if i == 0 else ["lstm%i" % (i - 1)],
        }
    d["output"] = {
        "class": "linear",
        "from": ["lstm%i" % (num_layers - 1)],
        "activation": "identity",
        "dropout": 0.0,
...
libri_100_lstm_bpe2k_model = (
    "/work/asr4/zeineldeen/setups-data/librispeech/2021-02-21--lm-bpe/dependencies/lm_models/lstm/epoch.004"
)
