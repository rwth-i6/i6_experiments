...
def conformer_baseline():
    ...
    run_exp(exp_name="base_conf_12l_trafo_6l_conv6_LR3e-4", train_args=args)

    for grad_noise in [1e-3, 1e-4]:
        args = copy.deepcopy(trafo_dec_exp_args)
        args["gradient_noise"] = grad_noise
        args["encoder_args"].input_layer = "conv-6"
        run_exp(exp_name=f"base_conf_12l_trafo_6l_conv6_gradNoise{grad_noise}", train_args=args)

    args = copy.deepcopy(trafo_dec_exp_args)
    args["encoder_args"].native_ctc = False
    args["encoder_args"].input_layer = "conv-6"
    run_exp(exp_name="base_conf_12l_trafo_6l_conv6_ctcTF", train_args=args)
    ...
    args['encoder_args'].input_layer = 'conv-6'
    run_exp(
        "base_conf12l_trafo_conv6_currLRv2",
        train_args=args,
        epoch_wise_filter=[(1, 5, 1000), (5, 10, 2000), (10, 20, 3000)],
    )
    ...
    run_exp(
        "base_conf12l_trafo_conv6_currLRv2_sqrdReLU",
        train_args=args,
        epoch_wise_filter=[(1, 5, 1000), (5, 10, 2000), (10, 20, 3000)],
    )
    ...
    for lr_decay in [0.9, 0.95]:
        ...
        args["encoder_args"].input_layer = "conv-6"
        args['retrain_checkpoint'] = train_job_avg_ckpt['base_conf_12l_trafo_6l_conv6_relu']
        run_exp(
            exp_name=f"base_conf_12l_trafo_6l_conv6_relu_retrain1_lrDecay{lr_decay}",
            train_args=args, epoch_wise_filter=None)

    args = copy.deepcopy(trafo_dec_exp_args)
    args["encoder_args"].input_layer = "conv-6"
    args['encoder_args'].input_layer_conv_act = 'swish'
    run_exp(exp_name=f"base_conf_12l_trafo_6l_conv6_swish", train_args=args)

    args = copy.deepcopy(trafo_dec_exp_args)
    args["encoder_args"].input_layer = "conv-6"
    args['encoder_args'].use_sqrd_relu = True
    run_exp(exp_name=f"base_conf_12l_trafo_6l_conv6_sqrdReLU", train_args=args)
    ...
    args["encoder_args"].input_layer = "conv-6"
    run_exp("base_conf_12l_trafo6l_OCLR_conv6_relu", train_args=args, num_epochs=435)
    ...
    args["encoder_args"].input_layer = "conv-6"
    run_exp(
        "base_conf_12l_trafo6l_OCLR_conv6_relu_CurrLRv2_nopre",
    ...
    # Notes:
    # - Pretraining variant 4 does not converge
    ...
    args = copy.deepcopy(lstm_dec_exp_args)
    args['decoder_args'].add_lstm_lm = True
    args['decoder_args'].lstm_lm_dim = 1024
    args['decoder_args'].lstm_lm_proj_dim = 512
    args['decoder_args'].embed_dim = 512
    run_exp(exp_name="base_conf_12l_lstm_1l_lstmLM", train_args=args)
    ...
    args["decoder_args"].loc_conv_att_num_channels = 512
    args['batch_size'] = 14 * 1000 * 160
    run_exp(exp_name=f"base_conf_12l_lstm_1l_locConv5_ch512_bs14", train_args=args)
