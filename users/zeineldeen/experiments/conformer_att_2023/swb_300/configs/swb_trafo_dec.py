            ...
            for enc_factor in [0.7]:
                for drop in [0.1, 0.2]:
                    for softmax_drop in [drop]:
                        ...
                        args["accum_grad"] = 1
                        args["batch_size"] = 15_000 * 80 * 2
                        args["pretrain_opts"]["initial_batch_size"] = 22500 * 80 * 2

                        args["oclr_opts"]["cycle_ep"] = int(0.45 * ep)
                        args["oclr_opts"]["total_ep"] = ep
                        run_default_exp(name, train_args=args, num_epochs=ep, gpu_mem=24)

    # TODO: double batch size
    for embed_weight in [False]:
        for bs_inc_factor in [2]:
            for enc_model_dim in [384, 256]:
                for specaug_version in [1]:
                    for drop in [0.1, 0.2]:
                        args = copy.deepcopy(get_base2_args(enc_red_factor=1.0, drop=drop))
                        name = f"base3_conf_{12}l_trafo_6l_conv{6}_ep{600}_drop{drop}_encDim{enc_model_dim}_bsInc{bs_inc_factor}_specaug{specaug_version}"
                        args.update(lstm_training_args)  # use as lstm decoder batching
                        ...
                        run_default_exp(name, train_args=args, num_epochs=600, gpu_mem=24)
