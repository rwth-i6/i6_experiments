"""
Chunked AED Sisyphus Config for LibriSpeech 960 which contains experiments for this paper:
    Title: Chunked Attention-based Encoder-Decoder Model for Streaming Speech Recognition
    Link: https://arxiv.org/abs/2309.08436

see also: users/zeineldeen/experiments/conformer_att_2022/librispeech_960/configs/baseline_960h_v2.py
"""
            ...
            for chunk_step_factor in chunk_step_factors:
                for start_lr in start_lrs:
                    for decay_pt_factor in decay_pt_factors:
                        ...
                        if chunk_size is None or chunked_decoder is False:
                            run_exp(
                                prefix_name=prefix_name,
                                exp_name=exp_name,
...
def run_all_exps():
    # Experiments with global encoder and chunked decoder (table 1)
    run_chunkwise_train(
    ...
    for lr in [2e-4]:
        for left_context, center_context, right_context, conv_cache_size, mem_size in [
            ...
            # LM combination (table 6)
            if center_context == 30:
                for beam_size in [80]:
