...
from i6_experiments.users.zeineldeen.experiments.chunkwise_att_2023.default_tools import RETURNN_ROOT, RETURNN_CPU_EXE
...
class DumpAttentionWeightsJob(Job):
  def __init__(
    ...
    self.returnn_python_exe = (returnn_python_exe if returnn_python_exe else RETURNN_CPU_EXE)
...
class DumpReturnnLayerJob(Job):
  ...
  def __init__(self, returnn_config: ReturnnConfig, *, rqmt=None, returnn_python_exe=None, returnn_root=None):
    super().__init__()
  ...
  def run(self):
    ...
    subprocess.check_call([
      self.returnn_python_exe.get_path(), self.returnn_root.get_path() + "/rnn.py", "returnn.config"])
...
class ConvertAlignmentFromTimeSyncToChunkSync(Job):

    def __init__(self, time_sync_align, *, rqmt=None):
        self.time_sync_align = time_sync_align
        self.rqmt = rqmt

    def tasks(self):
        yield Task("create_config", mini_task=True)
        yield Task("run", rqmt=self.rqmt)
    ...
    def create_config(self):
        ...
        returnn_config.write("returnn.config")
