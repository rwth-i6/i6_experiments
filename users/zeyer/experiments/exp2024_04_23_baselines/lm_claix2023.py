...
def py():
    ...
    # Seed variance of the standard LM baseline:
    # same data and schedule, only the weight init differs.
    for _seed in [2, 3]:
        seed_exp = train(
            f"{name}-seed{_seed}",
            config=dict_update_deep(
    ...
    exp = train(  # 32.88 (!!)
        ...
        config=dict_update_deep(
            config_96gb_bf16_accgrad1,
            {
                **_get_cfg_lrlin_oclr_by_bs_nep_v3(15_000, 100, batch_size_factor=1),
                "max_seqs": 400,
                "optimizer.weight_decay": 1e-2,
                "calculate_exp_loss": True,
            },
    ...
    # (trafo-n24-d512-gelu-drop0-b2k_80k-laplaceNone-spm10k) -> 38.46 PPL.
    # train(  # 38.46
