...
class ParakeetCtc(BaseModelInterface):
    ...
    def ctc_posteriors_per_token(self, *, audio: torch.Tensor, sample_rate: int, words: List[str]):
        """Per-ref-token CTC posterior over time: for each reference subword token i and encoder frame t,
        ``P(class(token_i) | t)`` from the model's own CTC emission. Same axes as the per-token grad /
        attention scores, so it feeds the same figure + alignment pipeline.
        ...
        with torch.no_grad():
            ...
            enc, enc_len = self.model.encoder(audio_signal=proc, length=proc_len)  # [1, H, T_enc]
            log_probs = self.model.decoder(encoder_output=enc)[0].float()  # [T_enc, V] (already log_softmax)
