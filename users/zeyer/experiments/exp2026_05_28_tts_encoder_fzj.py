...
def py():
    ...
    # dur07-packed never got the step compensation the ASR side did:
    ...
    # and still costs ~88 h against its 102 h, so it asks whether the remaining gap is just step count.
    # nep stretches the LR schedule, as for asr-base-mgpu-logmel-muon-lr5e3-wdbl-nep38.
    _train_tts_encoder(
        "pseudo-enc-logmel-mfatable-realdur2-lerp-dur07-packed-single-gumbel-muon-nep76",
        prefix=prefix,
        text_train_epoch_split=75,
        batch_size_audio_frames=70_000,
        batch_size_phon=6_000,
    ...
    # It was not: final dev-other is 4.10 at 82, 4.10 at 70, 3.97 at 60,
    # so 60 already beats the padded baseline's 4.01, and 50 tests whether the trend continues.
    ...
    # Same two tables on subwords instead of phonemes, from our own CTC alignment,
...
def _ctc_subword_tables(*, fill_within_words: bool = False):
    ...
    from i6_experiments.users.zeyer.datasets.librispeech_ctc_subword_stats import ComputeCtcSubwordStatsJob
