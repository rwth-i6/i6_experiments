...
def eow_phon_ls960_1023_jingjing_hyp_test():
    ...
    model_config = ModelConfig(
        ...
        conv_dropout=0.2,  # This here is different to baseline
        ff_dropout=0.2,  # This here is different to baseline
        mhsa_dropout=0.1,
        conv_kernel_size=31,
        final_dropout=0.0,  # Jingjing has no final dropout
        specauc_start_epoch=1,
    )

    learning_rates_jj_simon_250 = (
        list(np.linspace(5e-6, 5e-4, 115, endpoint=False))[1:]
        + list(np.linspace(5e-4, 5e-6, 115, endpoint=False))
        + list(np.linspace(5e-6, 1e-8, 21, endpoint=True))
    )  # this is different to baseline

    train_config_11gbgpu_jj = {
        "optimizer": {"class": "adamw", "epsilon": 1e-16, "weight_decay": 1e-3},
        "learning_rates": learning_rates_jj_simon_250,
        #############
        "batch_size": 150 * 16000,  # this is different to baseline
    ...
    train_args = {
        "config": train_config_11gbgpu_jj,
        "network_module": network_module,
        "net_args": {"model_config_dict": asdict(model_config)},
        "debug": False,
    }

    training_name = prefix_name + "/" + network_module + ".512dim_sub4_11gbgpu_25eps_jj_hyps_only"
