...
class AEDLabelScorer(LabelScorerIntf):
    ...
    def __init__(self, model: "Model", encoder_outputs, audio_features_len):
        """
    ...
    def score_and_update_state(
        ...
        if prev_state is not None:
            lstm_state_1, lstm_state_2, att_context, accum_att_weights = tuple([state.squeeze(0) for state in prev_state])
            # pack lstm state
            prev_state = (lstm_state_1, lstm_state_2), att_context, accum_att_weights
        ...
        log_softmax = torch.nn.functional.log_softmax(decoder_logits, dim=-1)  # [beam, 1, #vocab]
        log_softmax = torch.transpose(log_softmax, 0, 1)  # [1, beam, #vocab]

        # first unpack lstm
        states = states[0][0], states[0][1], states[1], states[2]
