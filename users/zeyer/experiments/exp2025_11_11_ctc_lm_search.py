...
from i6_experiments.users.zeyer.datasets.loquacious import (
    get_vocab_by_str as get_loquacious_vocab_by_str,
...
def py():
    ...
    ctc_recog_recomb_labelwise_prior_auto_scale(
        prefix=f"{prefix}/aed/{am_name_20ep}/ctc+lm-delayed-v2/{lm_name}",
        task=task,
        ctc_model=am_20ep,
        extra_config={"aux_loss_layers": [aux_ctc_layer_20ep]},
        lm=lm,
    ...
    for interval in [1, 5, 10, 20]:
        ...
        ctc_recog_recomb_labelwise_prior_auto_scale(
            prefix=f"{prefix}/aed/{am_name_20ep}/ctc+lm-delayed-v2-every{interval}/{lm_name}",
            task=task,
            ctc_model=am_20ep,
            extra_config={"aux_loss_layers": [aux_ctc_layer_20ep]},
            lm=lm,
    ...
    qwen2_1_5b_base_lm = get_qwen2_1_5b_lm_base()
    ...
    # Now the 7B LLM.
    ctc_recog_recomb_labelwise_prior_auto_scale(
        prefix=f"{prefix}/aed/{am_name_20ep}/ctc+lm-delayed-v2-always-beamSize8/qwen2-7b",
        task=task,
        ctc_model=am_20ep,
        extra_config={"aux_loss_layers": [aux_ctc_layer_20ep]},
        lm=qwen2_7b_lm,
