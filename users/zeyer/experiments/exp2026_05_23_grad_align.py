...
def py():
    ...
    for variant_suffix, variant_kwargs in [
        ...
        for smooth_kind, smooth_size in [
            ...
            for align_opts in _ALIGN_OPTS_GRID:
                ...
                align.add_alias(align_name)
                # CalcAlignmentMetricsJob aligns + scores in one (no separate boundaries HDF), so keep its inline WBE.
                reg(f"{align_name}-wbe.txt", align.out_wbe)
    ...
    def _wire_pertoken_with_sep(*, dl_ds, ds_name, split, phi4mm_cfg, attr, mgi, grad_alias, suffix):
        ...
        for align_opts in _ALIGN_OPTS_GRID:
            ...
            align.add_alias(align_name)
            reg(f"{align_name}-wbe.txt", align.out_wbe)  # WithSepGrads: no boundaries HDF, keep its inline WBE
    ...
    _pc_fa_test_m = CalcAlignmentMetricsFromWordBoundariesJob(
        word_boundaries_hdf=_pc_fa_test.out_word_boundaries_hdf,
        dataset_dir=dl_ds_timit.out_hub_cache_dir,
        dataset_key="test",
        dataset_offset_factors=_DATASET_OFFSET_FACTORS["timit"],
    )
    reg("baseline-parakeet-ctc-1.1b-timit-test-wbe.txt", _pc_fa_test_m.out_wbe)
    ...
    for _ol in [None, 6, 12, 15, 21]:
        ...
        _ofa_m = CalcAlignmentMetricsFromWordBoundariesJob(
            word_boundaries_hdf=_ofa.out_word_boundaries_hdf,
            dataset_dir=dl_ds_timit.out_hub_cache_dir,
            dataset_key="test",
            dataset_offset_factors=_DATASET_OFFSET_FACTORS["timit"],
        )
        reg(f"baseline-owsm-ctc-v4-1b-{_otag}timit-test-wbe.txt", _ofa_m.out_wbe)
    ...
    for _sv, _sv_kw in [
        ...
        if _sv != "A":
            ...
            _sv_pc_fa_m = CalcAlignmentMetricsFromWordBoundariesJob(
                word_boundaries_hdf=_sv_pc_fa.out_word_boundaries_hdf,
                dataset_dir=_sv_dir,
                dataset_key="test",
                dataset_offset_factors=_bk_off,
            )
            reg(f"baseline-parakeet-ctc-1.1b-{_sv_tag}-wbe.txt", _sv_pc_fa_m.out_wbe)
    ...
    for _fc_dir, _fc_key, _fc_off, _fc_tag in [
        ...
        reg(f"fastconformer-stream-ctc-{_fc_tag}-forced-boundaries.hdf", _fc_cfa.out_word_boundaries_hdf)
        _fc_cfa_m = CalcAlignmentMetricsFromWordBoundariesJob(
            word_boundaries_hdf=_fc_cfa.out_word_boundaries_hdf,
            dataset_dir=_fc_dir,
            dataset_key=_fc_key,
            dataset_offset_factors=_fc_off,
        )
        reg(f"baseline-fastconformer-stream-ctc-{_fc_tag}-wbe.txt", _fc_cfa_m.out_wbe)
    ...
    reg(f"parakeet-ctc-1.1b-{_xa_tag}-forced-boundaries.hdf", _xa_pc_fa.out_word_boundaries_hdf)
    _xa_pc_fa_m = CalcAlignmentMetricsFromWordBoundariesJob(
        word_boundaries_hdf=_xa_pc_fa.out_word_boundaries_hdf,
        dataset_dir=_xa_dir,
        dataset_key="test",
        dataset_offset_factors=_xa_off,
    )
    reg(f"baseline-parakeet-ctc-1.1b-{_xa_tag}-wbe.txt", _xa_pc_fa_m.out_wbe)
    ...
    _xa_pc_fa_char_m = CalcAlignmentMetricsFromWordBoundariesJob(
        word_boundaries_hdf=_xa_pc_fa_char.out_word_boundaries_hdf,
        dataset_dir=_xa_dir,
        dataset_key="test",
        dataset_offset_factors=_xa_off,
    )
    reg(f"baseline-parakeet-ctc-1.1b-char-{_xa_tag}-wbe.txt", _xa_pc_fa_char_m.out_wbe)
    ...
    for _ol in [None, 6, 12, 15, 21]:
        ...
        _xa_ofa_m = CalcAlignmentMetricsFromWordBoundariesJob(
            word_boundaries_hdf=_ofa.out_word_boundaries_hdf,
            dataset_dir=_xa_dir,
            dataset_key="test",
            dataset_offset_factors=_xa_off,
        )
        reg(f"baseline-owsm-ctc-v4-1b-{_otag}{_xa_tag}-wbe.txt", _xa_ofa_m.out_wbe)
