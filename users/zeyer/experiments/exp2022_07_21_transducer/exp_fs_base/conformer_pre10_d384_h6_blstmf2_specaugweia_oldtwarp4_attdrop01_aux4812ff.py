...
# Best results: {'best_scores': {'hub5e_00': 24.4, 'hub5e_01': 16.3, 'rt03s': 23.3}, 'best_epoch': 148}
_exclude_me = True
...
def specaugment_wei(
    ...
    *,
    ...
    with nn.Cond(nn.train_flag() | (not only_on_train)) as cond:
        x_masked = x
        x_masked = nn.make_layer({"class": "eval", "from": x_masked, "eval": random_warp}, name="random_warp")
...
def random_warp(*, source, self, **_kwargs):
    """
