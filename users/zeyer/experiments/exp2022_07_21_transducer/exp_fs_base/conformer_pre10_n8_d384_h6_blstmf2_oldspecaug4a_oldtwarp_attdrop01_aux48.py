...
# Best results: {'best_scores': {'hub5e_00': 21.6, 'hub5e_01': 15.7, 'rt03s': 20.9}, 'best_epoch': 148}
_exclude_me = True
...
config = dict(
    ...
    learning_rates=(
        ...
        list(numpy.linspace(0.0000001, 0.001, num=10)) * 2
...
def from_scratch_model_def(*, epoch: int, in_dim: nn.Dim, target_dim: nn.Dim) -> Model:
    ...
    num_enc_layers_ = sum(([i] * 10 for i in [2, 4, 8]), [])
...
def from_scratch_training(
    *, model: Model, data: nn.Tensor, data_spatial_dim: nn.Dim, targets: nn.Tensor, targets_spatial_dim: nn.Dim
    ...
    loss = nn.transducer_time_sync_full_sum_neg_log_prob(
        ...
        labels_spatial_dim=targets_spatial_dim,prev_labels_spatial_dim=prev_targets_spatial_dim,
