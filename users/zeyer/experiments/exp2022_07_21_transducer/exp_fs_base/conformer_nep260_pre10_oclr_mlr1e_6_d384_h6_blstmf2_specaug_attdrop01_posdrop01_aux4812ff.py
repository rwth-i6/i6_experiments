...
# Best results: {'best_scores': {'hub5e_00': 20.1, 'hub5e_01': 14.2, 'rt03s': 18.5}, 'best_epoch': 257}
_exclude_me = True
...
def sis_run_with_prefix(prefix_name: str):
    ...
    model = train(
        prefix_name, task=task, config=config, post_config=post_config, num_epochs=260,
...
config = dict(
    ...
    learning_rates=(
        # matching pretraining initially but then somewhat like one-cycle LR from Tina
        list(numpy.linspace(0.0000001, 0.001, num=10)) * 3 +
        list(numpy.linspace(0.0000001, 0.001, num=100)) +
        list(numpy.linspace(0.001, 0.0001, num=100)) +
        list(numpy.linspace(0.0001, 1e-6, num=30))
    ),
