...
_exclude_me = True  # bad, does not converge
    ...
    def encode(
        ...
        *,
        ...
        if self.training:
            rnd_scale = nn.where(
                nn.train_flag(),
                nn.random_uniform((), minval=0.8, maxval=nn.where(nn.dim_value(in_spatial_dim) <= 250, 1.2, 1.0)),
                nn.constant(1.0),
            )
        ...
        if self.training:
            ...
            stride = 50
            with nn.Cond(nn.train_flag() & (nn.dim_value(enc_spatial_dim) > win_dim.dimension)) as cond:
                # with chunking
                enc_chunked, chunk_spatial_dim = nn.window(
                    enc, spatial_dim=enc_spatial_dim, window_dim=win_dim, stride=stride
                )
                collected_outputs_ = {}
                ...
                collected_outputs_ = {
                    str(i - 1): nn.inverse_window(
                        ...
                        in_spatial_dim=chunk_spatial_dim,
                ...
                cond.true = enc_, collected_outputs_
...
def from_scratch_model_def(*, epoch: int, in_dim: nn.Dim, target_dim: nn.Dim, training: bool = False) -> Model:
    ...
    if num_enc_layers <= 2 and training:
