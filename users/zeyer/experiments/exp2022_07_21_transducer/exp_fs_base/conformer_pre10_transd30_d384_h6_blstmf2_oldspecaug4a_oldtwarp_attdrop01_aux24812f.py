...
# Best results: {'best_scores': {'hub5e_00': 19.8, 'hub5e_01': 15.6, 'rt03s': 20.0}, 'best_epoch': 146}
_exclude_me = True
...
def sis_run_with_prefix(prefix_name: str):
    ...
    recog_training_exp(
        prefix_name, task, model, recog_def=model_recog, exclude_epochs=list(range(1, transducer_start_epoch + 1))
    )
...
def from_scratch_training(
    *, model: Model, data: nn.Tensor, data_spatial_dim: nn.Dim, targets: nn.Tensor, targets_spatial_dim: nn.Dim
    ...
    """Function is run within RETURNN."""
    extra_net_dict = nn.NameCtx.top().root.extra_net_dict
    extra_config = extra_net_dict.setdefault("#config", {})
    collected_outputs = {}
    ...
    for i in aux_loss_layers:
        ...
        last_loss = f"ctc_{i}"
    if model.epoch < transducer_start_epoch:
        assert last_loss
        extra_config["learning_rate_control_error_measure"] = "dev_score_" + last_loss
    else:
...
def model_recog(
    *,
    ...
    if model.epoch >= transducer_start_epoch:
        ...
        loop.state.label = nn.constant(model.blank_idx, shape=batch_dims, sparse_dim=model.wb_target_dim)
    with loop:
        ...
        if model.epoch >= transducer_start_epoch:
            probs, loop.state.decoder = model.decode(
                **enc,
                ...
                prev_wb_target=loop.state.label,
