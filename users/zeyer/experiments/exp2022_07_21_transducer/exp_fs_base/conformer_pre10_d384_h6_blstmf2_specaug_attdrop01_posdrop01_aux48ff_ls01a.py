...
# Best results: {'best_scores': {'hub5e_00': 25.0, 'hub5e_01': 21.2, 'rt03s': 25.3}, 'best_epoch': 150}
_exclude_me = True
...
class Model(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        l2: float = 0.0001,
        grow_frac_enc: float = 1.0,
    ):
        super(Model, self).__init__()
        self.grow_frac_enc = grow_frac_enc
...
def from_scratch_training(
    *, model: Model, data: nn.Tensor, data_spatial_dim: nn.Dim, targets: nn.Tensor, targets_spatial_dim: nn.Dim
    ...
    out_log_prob = nn.label_smoothed_log_prob_gradient(out_log_prob, 0.1 * model.grow_frac_enc)
