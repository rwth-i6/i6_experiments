...
config = dict(
    ...
    learning_rates=(
        ...
        list(numpy.linspace(high_lr * 0.1, high_lr, num=10)) * 3 +
        list(numpy.linspace(high_lr * 0.1, default_lr, num=10))),
    min_learning_rate=high_lr / 50,
...
def from_scratch_model_def(*, epoch: int, in_dim: nn.Dim, target_dim: nn.Dim) -> Model:
    ...
    if epoch > len(num_enc_layers_):
        extra_net_dict["#config"]["use_last_best_model"] = dict(only_last_n=10)
