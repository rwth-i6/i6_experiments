...
# Best results: {'best_scores': {'hub5e_00': 20.5, 'hub5e_01': 17.2, 'rt03s': 21.0}, 'best_epoch': 149}
_exclude_me = True
    ...
    def __init__(
        ...
        *,
        ...
        self.encoder = nn.ConformerEncoder(
            ...
            enc_model_dim,
            ff_dim=enc_ff_dim,
            input_layer=ConvFrontend(in_dim),
...
class ConvFrontend(nn.ISeqDownsamplingEncoder):
    """
    Like Wei (and probably others)
    """
    ...
    def __init__(self, in_dim: nn.Dim):
        ...
        self.conv1 = nn.Conv2d(self._dummy_feat_dim, nn.FeatureDim("conv1", 32), filter_size=(3, 3), padding="same")
        self.conv2 = nn.Conv2d(
            self.conv1.out_dim, nn.FeatureDim("conv2", 64), filter_size=(3, 3), padding="same", strides=(2, 1)
        )
        self.conv3 = nn.Conv2d(
            self.conv2.out_dim, nn.FeatureDim("conv3", 64), filter_size=(3, 3), padding="same", strides=(3, 1)
        )
        self.out_dim = in_dim.ceildiv_right(2) * self.conv3.out_dim
    ...
    def __call__(self, x: nn.Tensor, *, in_spatial_dim: nn.Dim) -> Tuple[nn.Tensor, nn.Dim]:
        ...
        x, (in_spatial_dim, feat_dim) = self.conv3(x, in_spatial_dims=[in_spatial_dim, feat_dim])
        x, _ = nn.merge_dims(x, axes=[feat_dim, self.conv3.out_dim], out_dim=self.out_dim)
