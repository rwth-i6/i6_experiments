...
class ConformerEncoderLayerNoRes(nn.ConformerEncoderLayer):
    """no res connection"""
    ...
    def __call__(self, inp: nn.Tensor, *, spatial_dim: nn.Dim) -> nn.Tensor:
        ...
        x_ffn1_out = nn.dropout(x_ffn1, axis=inp.feature_dim, dropout=self.dropout)
        ...
        x_conv_out = nn.dropout(x_conv, axis=inp.feature_dim, dropout=self.dropout)

        # FFN
        x_ffn2_ln = self.ffn2_layer_norm(x_conv_out)
        x_ffn2 = self.ffn2(x_ffn2_ln)
        x_ffn2_out = nn.dropout(x_ffn2, axis=inp.feature_dim, dropout=self.dropout)

        # last LN layer (disabled)
        return x_ffn2_out
    ...
    def __init__(
        ...
        *,
        ...
        self.encoder = nn.ConformerEncoder(
            ...
            encoder_layer=ConformerEncoderLayerNoRes,
