...
# Best results: {'best_scores': {'hub5e_00': 21.4, 'hub5e_01': 15.5, 'rt03s': 19.8}, 'best_epoch': 149}
_exclude_me = True
...
class RelPosSelfAttentionWin(nn.RelPosSelfAttention):
    """
    Same as RelPosSelfAttention but with window in training
    """
    def __init__(self, *args, win_size: int, **kwargs):
        super(RelPosSelfAttentionWin, self).__init__(*args, **kwargs)
    ...
    def __call__(self, source: nn.Tensor, *, axis: nn.Dim, **_kwargs) -> nn.Tensor:
        ...
        win_start = nn.range_over_dim(hist_dim) - self.win_size // 2
        win_size = self.win_size
        with nn.Cond(nn.train_flag()) as cond:
            cond.true = nn.softmax(
                scores, axis=hist_dim, window_start=win_start, window_size=win_size, name="att_weights_train")
            cond.false = nn.softmax(scores, axis=hist_dim, name="att_weights_recog")
            att_weights = cond.result
...
def from_scratch_model_def(*, epoch: int, in_dim: nn.Dim, target_dim: nn.Dim) -> Model:
    ...
    return Model(
        ...
        enc_conformer_layer_opts=dict(
            self_att=RelPosSelfAttentionWin,
            self_att_opts=dict(
