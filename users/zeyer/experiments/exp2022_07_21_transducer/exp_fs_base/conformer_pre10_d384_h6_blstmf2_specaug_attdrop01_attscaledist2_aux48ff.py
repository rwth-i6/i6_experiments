...
# Best results: {'best_scores': {'hub5e_00': 23.7, 'hub5e_01': 17.6, 'rt03s': 22.6}, 'best_epoch': 149}
_exclude_me = True
...
class RelPosSelfAttentionGradScaleDist(nn.RelPosSelfAttention):
    """
    Same as RelPosSelfAttention but with window in training
    """
    def __init__(self, *args, grad_dist_scale: float, **kwargs):
        super(RelPosSelfAttentionGradScaleDist, self).__init__(*args, **kwargs)
        self.grad_dist_scale = grad_dist_scale
    ...
    def __call__(self, source: nn.Tensor, *, axis: nn.Dim, **_kwargs) -> nn.Tensor:
        ...
        score_grad_scale = nn.where(nn.random_uniform(dist.shape_ordered, maxval=1.) < 0.1, 1., score_grad_scale)
...
def from_scratch_model_def(*, epoch: int, in_dim: nn.Dim, target_dim: nn.Dim) -> Model:
    ...
    return Model(
        ...
        enc_conformer_layer_opts=dict(
            self_att=RelPosSelfAttentionGradScaleDist,
            self_att_opts=dict(
                grad_dist_scale=0.1,
            )
        ),
