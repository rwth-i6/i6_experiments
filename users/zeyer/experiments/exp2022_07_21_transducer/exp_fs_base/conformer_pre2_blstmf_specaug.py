...
config = dict(
    ...
    learning_rates=list(numpy.linspace(default_lr * 0.1, default_lr, num=10)) + [default_lr] * 30,
...
def from_scratch_model_def(*, epoch: int, in_dim: nn.Dim, target_dim: nn.Dim) -> Model:
    ...
    num_enc_layers = [2] * 12 + sum(([i, i] for i in range(3, 13)), [])
    return Model(
        in_dim,
        num_enc_layers=num_enc_layers[epoch - 1] if epoch <= len(num_enc_layers) else num_enc_layers[-1],
        nb_target_dim=target_dim,
        wb_target_dim=target_dim + 1,
        blank_idx=target_dim.dimension,
        bos_idx=_get_bos_idx(target_dim),
        enc_dropout=0.0 if epoch <= len(num_enc_layers) else 0.1,
        enc_att_dropout=0.0 if epoch <= len(num_enc_layers) else 0.1,
        l2=0.0 if epoch <= len(num_enc_layers) else 0.0001,
    )
