...
# Best results: {'best_scores': {'hub5e_00': 19.1, 'hub5e_01': 14.7, 'rt03s': 19.1}, 'best_epoch': 150}
_exclude_me = True
...
def transform(source, self, **_kwargs):
    ...
    def get_masked():
        ...
        x_masked = random_mask(x_masked, batch_axis=data.batch_dim_axis, axis=data.time_dim_axis,
                               min_num=0,
                               max_num=tf.maximum(tf.shape(x)[data.time_dim_axis] // int(1 / 0.70 * max_time),
                                                  max_time_num) // (1 + increase_flag),
                               max_dims=max_time
                               )
