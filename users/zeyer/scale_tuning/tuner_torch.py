...
def main():
    ...
    for fn in args.recog_output_dir:
        with gzip.GzipFile(fn + "/output.py.gz", "rt") as f:
            hyps = eval(f.read())
        with gzip.GzipFile(fn + "/output_ext.py.gz", "rt") as f:
            exts = eval(f.read())
    ...
    for step in range(args.num_steps):
        ...
        for entries_, num_errors_ in zip(entries):
            seq_scores = _logits(entries_)
            seq_probs = torch.nn.functional.softmax(seq_scores)
            loss += seq_probs * num_errors_
        loss.backward()
        opt.step()
