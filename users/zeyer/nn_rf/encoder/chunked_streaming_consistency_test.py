...
from .chunked_streaming_v2 import streaming_encode_and_get_ctc_log_probs_v2
...
def _streaming_consistency_forward_step_v2(*, model, extern_data, **_kwargs):
    """Forward step: compute offline + KV-cache-streaming log-probs, diff."""
    from returnn.config import get_global_config
    ...
    mean_diff = float(diff.mean().item())
    print(
        f"streaming-consistency-v2: T_off={t_off} T_str={t_str} "
...
def make_streaming_consistency_test_job(
    *,
    ...
    :param use_kv_cache_v2: if True, use the KV-cache streaming
        (``chunked_streaming_v2``) with overlap-and-trim instead of the v1
        history-re-encode path.
    """
