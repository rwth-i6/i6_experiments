...
def main():
    ...
    arg_parser.add_argument("--tmux-binary", type=str, help="path to the tmux binary")
    ...
    run(
        ...
        *sbatch_args,
        ...
        "--wrap=exec %s"
        % " ".join(
            [sys.executable, __file__, "--start-tmux"]
            + (["--group", args.group] if args.group else [])
            + (["--tmux-binary", args.tmux_binary] if args.tmux_binary else [])
        ),
    )
...
def _run_tmux(args):
    ...
    tmux_binary = args.tmux_binary if args.tmux_binary else "tmux"
    ...
    while True:
        run(tmux_binary, "-S", socket_filename, "new-session", "-d")
        print(f"Started tmux session with socket: {socket_filename}")
        print(f"To attach to the tmux session, run: ssh {hostname} -t {tmux_binary} -S {socket_filename} attach")
        ...
        sp.check_call([tmux_binary, "-S", socket_filename, "has-session"])
        ...
        try:
            ...
            run(tmux_binary, "-S", socket_filename, "kill-session")
            raise
