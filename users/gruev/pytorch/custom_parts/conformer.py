...
class ConformerEncoderConvFirstV1(nn.Module):
    ...
    def forward(
        ...
        :param sequence_mask: mask tensor where 0 defines positions within the sequence and 1 outside, shape: [B, T']
        :param aux_layers: list of layers for which an inter-CTC loss will be computed
        :return: torch.Tensor of shape [B, T, F']
        ...
        outputs = []
        aux_layers = [layer_num - 1 for layer_num in aux_layers]

        for i in range(len(self.module_list)):
            x = self.module_list[i](x, mask)  # [B, T, F']
            if i in aux_layers:
                outputs.append(x)
