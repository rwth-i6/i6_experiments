...
from i6_experiments.users.gruev.pytorch.default_tools import RETURNN_EXE, MINI_RETURNN_ROOT, SCTK_BINARY_PATH
...
def ctc_forward(checkpoint, config, returnn_exe, returnn_root, prefix):
    ...
    forward_out = forward_job.out_hdf_files["recognition.txt"]
    tk.register_output(prefix + "/recognition.txt", forward_out)
    return forward_out
...
def search(
    ...
    wers = {}
    for key, (test_dataset, _, test_dataset_reference) in test_dataset_tuples.items():
