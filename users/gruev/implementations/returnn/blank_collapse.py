...
def blank_collapse(network, softmax_layer, blank_threshold, blank_idx, apply_log=False):
    ...
    network, blank_axis = layers.add_gather_layer(
        network, "blank_axis", softmax_layer, axis="F", position=0
    )
    # computes a mask, softmax_layer_output[:, :, blank_idx] > blank_threshold, output_dims=[B, T]
    network, blank_mask = layers.add_compare_layer(
        network, "blank_mask", blank_axis, value=blank_threshold, kind="greater"
    )

    ### INTEGRATION OF AUDIO LENGTHS
    # computes audio lengths in batch, output_dims=[B,]
    network, audio_lengths = layers.add_length_layer(
        network, "audio_lengths", blank_mask, axis="T"
    )
    ...
    network, audio_lengths_range = layers.add_range_from_length_layer(
        network, "audio_lengths_range", audio_lengths
    )
    # computes a mask for individual audio lengths, output_dims=[B, T]
    network, audio_lengths_mask = layers.add_compare_layer(
        network,
        "audio_lengths_mask",
        [audio_lengths, audio_lengths_range],
        kind="less_equal",
    )
    ...
    network, mapping = layers.add_switch_layer(
        network,
        "mapping",
        condition=blank_mask_w_audio_lengths,
        true_from=int(1e4),
        false_from=audio_lengths_range,
    )
    # computes indices of first non-blank elements, output_dims=[B,]
    network, start_indices = layers.add_reduce_layer(
        network, "start_indices", mapping, axis="T", mode="argmin"
    )
    ...
    network, blank_mask_inverted = layers.add_combine_layer(
        network,
        "blank_mask_inverted",
        [blank_mask_w_shift, start_indices_mask],
        kind="logical_or",
    )
    ...
    network, blank_mask_final = layers.add_eval_layer(
        ...
        "blank_mask_final",
        [blank_mask_inverted],
        eval_str="tf.math.logical_not(source(0))",
    )

    ### BLANK COLLAPSE
    network, blank_collapse = layers.add_mask_layer(
        network, "blank_collapse", softmax_layer, mask=blank_mask_final, 
    )
    network, blank_collapse_reinterpret = layers.add_reinterpret_data_layer(
        network, "blank_collapse_reinterpret", blank_collapse, enforce_batch_major=True, is_output=True,
    )