...
def ls960_hmm_conformer_monophone():
    ...
    for config_name, (lr_config, model_config) in train_configs.items():
        ...
        # Same with conv first
        network_module_conv_first_onnx = "hmm.conformer_1023.i6modelsV1_VGG4LayerActFrontendV1_v6_conv_first_onnx"
        train_args_conv_first_onnx = {
            "config": train_config_11gbgpu_amp,
            "network_module": network_module_conv_first_onnx,
            "net_args": {"model_config_dict": asdict(model_config)},
            "debug": False,
            "include_native_ops": True,
        }
        ...
        decoding_returnn_config = get_decoding_config(training_datasets=train_data, **train_args_conv_first_onnx)
        ...
        p_info = PriorInfo.from_monophone_job(asr_model.prior_file)

        #crp_list = ["dev-clean", "dev-other"]
        ...
        if label_info.phoneme_state_classes.use_word_end():
            decoder.crp.acoustic_model_config.state_tying.use_word_end_classes = (
                label_info.phoneme_state_classes.use_word_end()
        )
        decoder.crp.acoustic_model_config.state_tying.use_boundary_classes = (
            label_info.phoneme_state_classes.use_boundary()
        )
        decoder.crp.acoustic_model_config.hmm.states_per_phone = label_info.n_states_per_phone
        decoder.crp.acoustic_model_config.state_tying.type = RasrStateTying.monophone


        search_parameters = SearchParameters.default_monophone(priors=prior_info)
        search_parameters = (
           search_parameters.with_prior_scale(center=0.2)
           .with_tdp_scale(0.1)
           .with_lm_scale(0.9)
           .with_tdp_silence((11.0, 0.0, "infinity", 20.0))
           .with_tdp_speech((8.0, 0.0, "infinity", 0.0))
           .with_beam_size(22.0)
           .with_altas(2.0)
        ) 
