__all__ = ["CheckErrorsJob2"]

from ast import literal_eval
from dataclasses import dataclass
import logging
from typing import Dict, Iterator, Tuple, Union, List
import json
...
class CheckErrorsJob2(tk.Job):
    def __init__(self, n, free_ctm: tk.Path, aed_ctms: List[tk.Path], reference_text: tk.Path, clip_lm_score: bool = True, dictionary: dict = None, alpha: float = 0.99, beta: float = 0.01):
    ...
    def run(self):
        ...
        constraint_segments = []
        for constraint_ctm in self.constraint_ctm:
            constraint_segments.append(self._parse_ctm(constraint_ctm.get_path()))
        ...
        with open(self.out_report, "wt") as out_report, open(self.out_search_error_free_ctm, "wt") as out_serr_free, open(self.out_search_error_only_ctm, "wt") as out_serr_only:
            for segment_name, reference_segment in reference_segments.items():
                aed_scores = self.dictionary[segment_name]
                free_segment = free_segments.get(segment_name)
                constraint_segment = None
                j = 0
                for segments in constraint_segments:
                    new_segment = segments.get(segment_name)
                    if new_segment is None:
                        continue
                    self.dictionary[segment_name][j] = (new_segment.am_score, self.dictionary[segment_name][j])
                    aed_score = aed_scores[j][0]
                    score = self.alpha * aed_score - self.beta * new_segment.total_score
                    if constraint_segment is None or score > best_score:
                        constraint_segment = new_segment
                        best_score = score
                    j = j + 1
                if (
        ...
        """Save a dictionary as a gzipped JSON file."""
        with gzip.open(f'/u/noureldin.bayoumi/work/mono_ctc_{self.n}.py.gz', "wt", encoding="utf-8") as f:  # "wt" = write text
...
def calculate_search_errors_trial(name: str, recog_jobs: DecodingJobs, aed_lattices, sctk_binary_path: Union[tk.Path, str], n: int, alpha: float, beta: float):
    """
    cheating_lattices is the output of a AdvancedTreeSearchJob that ran with LM of type cheating-segment
    """

    dict_gz_path = f'/work/asr3/raissi/shared_workspaces/bayoumi/work/thesis_work/log_linear/dependencies/mono_ctc_{n}.py.gz'
    
    with gzip.open(dict_gz_path, 'rt', encoding='utf-8') as f:  # Open in text mode
        hypotheses_dict = ast.literal_eval(f.read())
    ...
    filtered_corpus.out_corpus = tmp_crp.corpus_config.file
    #tmp_crp.concurrent = 1

    #tk.register_output("corpora/filtered.corpus" + str(n) + "_" + str(alpha) + ".xml.gz", filtered_corpus.out_corpus)
    ...
    i = 1
    aed_ctms = []
    for cheating_lattices in aed_lattices:
        test = RescoreLatticeCacheJob(crp=tmp_crp, lattice_cache=cheating_lattices)
        #tk.register_output(f"trafo_test_cheating/{name}/rescored_lattices" + str(n) + "_" + str(alpha) + "_" + str(i)  + ".cache.bundle", test.out_lattice_bundle)

        convert_cheating = LatticeToCtmJob(tmp_crp, test.out_lattice_bundle, extra_config=ec)
        #tk.register_output(f"trafo_test_cheating/{name}/cheating" + str(n) + "_" + str(alpha) + "_" + str(i)  + ".ctm", convert_cheating.out_ctm_file)
        
        aed_ctms.append(convert_cheating.out_ctm_file)
        i = i + 1
    ...
    #tk.register_output(f"trafo_test_cheating/{name}/free" + str(n) + "_" + str(alpha) + ".ctm", convert_full.out_ctm_file)

    reference_text = corpus_recipe.CorpusToTextDictJob(bliss_corpus=tmp_crp.corpus_config.file).out_dictionary
    check_errors = CheckErrorsJob2(n, convert_full.out_ctm_file, aed_ctms, reference_text,
                                        True, hypotheses_dict, alpha, beta)
    tk.register_output(f"junk/{name}/error" + str(n) + "_" + str(alpha) + ".report", check_errors.out_report)

    #scorer = filter_and_score(filtered_stm, convert_full.out_ctm_file, sctk_binary_path)
    #tk.register_output(f"trafo_test_cheating/{name}/filtered" + str(n) + "_" + str(alpha) + ".wer", scorer.out_report_dir)

    #scorer = filter_and_score(filtered_stm, check_errors.out_search_error_free_ctm, sctk_binary_path)
    #tk.register_output(f"trafo_test_cheating/{name}/no_search_error" + str(n) + "_" + str(alpha) + ".wer", scorer.out_report_dir)
    ...
    #tk.register_output(f"trafo_test_cheating/{name}/only_search_error" + str(n) + "_" + str(alpha) + ".wer", scorer.out_report_dir)
