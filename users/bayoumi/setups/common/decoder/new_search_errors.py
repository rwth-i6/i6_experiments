...
def calculate_search_errors(name: str, recog_jobs: DecodingJobs, aed_lattices, sctk_binary_path: Union[tk.Path, str], n: int, alpha: float, beta: float):
    ...
    dict_gz_path = f'/work/asr3/raissi/shared_workspaces/bayoumi/work/thesis_work/aed/aed_trafo_ilm_fh/results/new_trafo_test_{n}_processed.py.gz'
    ...
    for cheating_lattices in aed_lattices:
        ...
        tk.register_output(f"new_trafo_aed_4gram_fh_cheating/{name}/rescored_lattices" + str(n) + "_" + str(alpha) + "_" + str(i)  + ".cache.bundle", test.out_lattice_bundle)

        convert_cheating = LatticeToCtmJob(tmp_crp, test.out_lattice_bundle, extra_config=ec)
        tk.register_output(f"new_trafo_aed_4gram_fh_cheating/{name}/cheating" + str(n) + "_" + str(alpha) + "_" + str(i)  + ".ctm", convert_cheating.out_ctm_file)
    ...
    tk.register_output(f"new_trafo_aed_4gram_fh_cheating/{name}/free" + str(n) + "_" + str(alpha) + ".ctm", convert_full.out_ctm_file)
    ...
    tk.register_output(f"new_trafo_aed_4gram_fh_cheating/{name}/error" + str(n) + "_" + str(alpha) + ".report", check_errors.out_report)

    scorer = filter_and_score(filtered_stm, convert_full.out_ctm_file, sctk_binary_path)
    tk.register_output(f"new_trafo_aed_4gram_fh_cheating/{name}/filtered" + str(n) + "_" + str(alpha) + ".wer", scorer.out_report_dir)

    scorer = filter_and_score(filtered_stm, check_errors.out_search_error_free_ctm, sctk_binary_path)
    tk.register_output(f"new_trafo_aed_4gram_fh_cheating/{name}/no_search_error" + str(n) + "_" + str(alpha) + ".wer", scorer.out_report_dir)
    ...
    tk.register_output(f"new_trafo_aed_4gram_fh_cheating/{name}/only_search_error" + str(n) + "_" + str(alpha) + ".wer", scorer.out_report_dir)
