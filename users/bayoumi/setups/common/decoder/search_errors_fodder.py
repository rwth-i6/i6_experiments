...
class CheckJob2(tk.Job):
    def __init__(self, free_ctm: tk.Path, aed_ctms: List[tk.Path], reference_text: tk.Path, clip_lm_score: bool = True, dictionary: dict = None, n: int = 8, alpha: float = 0.99, beta: float = 0.01):
        ...
        with open(self.out_report, "wt") as out_report, open(self.out_search_error_free_ctm, "wt") as out_serr_free, open(self.out_search_error_only_ctm, "wt") as out_serr_only:
            ...
            for segment_name, reference_segment in reference_segments.items():
                ...
                for segments in constraint_segments:
                    ...
                    if constraint_segment == new_segment and flag is not None and (j >= self.n and flag):
                        other_flag = False
                ...
                if (
                    ...
                    out_report.write("\n")
                    out_serr_free.write("".join(free_segment.lines))
            out_report.write(f'{count / count_total}%')
            out_report.write("\n")
...
def calculate_search_errors(name: str, recog_jobs: DecodingJobs, aed_lattices, sctk_binary_path: Union[tk.Path, str], n: int, alpha: float, beta: float):
    """
    cheating_lattices is the output of a AdvancedTreeSearchJob that ran with LM of type cheating-segment
    """

    dict_gz_path = f'/work/asr3/raissi/shared_workspaces/bayoumi/work/thesis_work/aed/aed_trafo_ilm_fh/results/trafo_{n}_100_processed.py.gz'
    ...
    for cheating_lattices in aed_lattices:
        ...
        tk.register_output(f"trafo_aed_trafo_fh_cheating_test/{name}/rescored_lattices" + str(n) + "_" + str(alpha) + "_" + str(i)  + ".cache.bundle", test.out_lattice_bundle)

        convert_cheating = LatticeToCtmJob(tmp_crp, test.out_lattice_bundle, extra_config=ec)
        tk.register_output(f"trafo_aed_trafo_fh_cheating_test/{name}/cheating" + str(n) + "_" + str(alpha) + "_" + str(i)  + ".ctm", convert_cheating.out_ctm_file)
    ...
    tk.register_output(f"trafo_aed_trafo_fh_cheating_test/{name}/free" + str(n) + "_" + str(alpha) + ".ctm", convert_full.out_ctm_file)

    reference_text = corpus_recipe.CorpusToTextDictJob(bliss_corpus=tmp_crp.corpus_config.file).out_dictionary
    check_errors = CheckJob2(convert_full.out_ctm_file, aed_ctms, reference_text,
                                        True, hypotheses_dict, n, alpha, beta)
    tk.register_output(f"trafo_aed_trafo_fh_cheating_test/{name}/error" + str(n) + "_" + str(alpha) + ".report", check_errors.out_report)

    scorer = filter_and_score(filtered_stm, convert_full.out_ctm_file, sctk_binary_path)
    tk.register_output(f"trafo_aed_trafo_fh_cheating_test/{name}/filtered" + str(n) + "_" + str(alpha) + ".wer", scorer.out_report_dir)

    scorer = filter_and_score(filtered_stm, check_errors.out_search_error_free_ctm, sctk_binary_path)
    tk.register_output(f"trafo_aed_trafo_fh_cheating_test/{name}/no_search_error" + str(n) + "_" + str(alpha) + ".wer", scorer.out_report_dir)
    ...
    tk.register_output(f"trafo_aed_trafo_fh_cheating_test/{name}/only_search_error" + str(n) + "_" + str(alpha) + ".wer", scorer.out_report_dir)
