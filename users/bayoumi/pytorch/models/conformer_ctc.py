...
class ConformerCTCModel(torch.nn.Module):
    ...
    def forward(
        ...
        return log_probs, x, sequence_mask.sum(dim=1)


class ConformerCTCEncoder(torch.nn.Module):
    def __init__(self, step: int, cfg: ConformerCTCConfig, **kwargs):
        super().__init__()
        if cfg.feature_extraction is None:
            self.feature_extraction = None
    ...
    def forward(
        ...
        return x, sequence_mask.sum(dim=1)
...
class Softmax(torch.nn.Module):
    ...
    def forward(
        ...
        encoder_output_len: Optional[torch.Tensor] = None,
    ):

        logits = self.final_linear(encoder_output)  # [B, T, F]
        log_probs = torch.log_softmax(logits, dim=2)

        return log_probs, encoder_output_len, history
