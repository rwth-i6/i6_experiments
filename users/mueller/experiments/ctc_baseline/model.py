...
class Model(rf.Module):
    ...
    def __init__(
        ...
        *,
        ...
        module_selection = config.typed_value("module_selection", [])
        enc_layer_drop = config.float("enc_layer_drop", 0.0)
        assert not (module_selection and enc_layer_drop), "enc_layer_drop and module_selection are mutually exclusive"
        if enc_layer_drop:
            enc_sequential = functools.partial(SequentialLayerDrop, layer_drop=enc_layer_drop)
        elif module_selection:
            enc_sequential = functools.partial(SelectiveSequential, module_selection=module_selection)
        else:
        ...
        assert with_bias or output_bias_init is None
        bias_init = None
        ...
        for i in enc_aux_logits:
            enc_aux_logits_tmp = rf.Linear(self.encoder.out_dim, wb_target_dim, with_bias=with_bias)
            if bias_init is not None:
                enc_aux_logits_tmp.bias.initial = bias_init
            setattr(self, f"enc_aux_logits_{i}", enc_aux_logits_tmp)
        self.enc_logits = rf.Linear(self.encoder.out_dim, wb_target_dim, with_bias=with_bias)
        if bias_init is not None:
...
# Helpers -------------------------------------------------  
class SelectiveSequential(rf.ModuleList):
    """
    Sequential Module, takes callable of Modules which are then executed in sequence, but can be filtered based on the current epoch.
    """
    def __init__(self, *args, module_selection: Sequence[Tuple[int, int]]):
        super().__init__(*args)
    ...
    def __call__(self, inp, *, collected_outputs: Optional[Dict[str, Tensor]] = None, **kwargs) -> Tensor:
        ...
        Forward
        """
        curr_epoch = rf.get_run_ctx().epoch
        module_limit = -1
        for epoch_filter, module_filter in self.module_selection:
            if curr_epoch <= epoch_filter:
                assert module_filter >= 1
                module_limit = module_filter
                break
        modules = list(self.items())
        if module_limit >= 0:
            if module_limit > 2:
                modules = modules[:(module_limit - 2)] + modules[-2:]
            elif module_limit == 2:
                modules = modules[-2:]
            elif module_limit == 1:
                modules = modules[-1:]
