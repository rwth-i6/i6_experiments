...
def sum_loss(
    *,
    ...
    for t in range(1, max_audio_time):
        ...
        log_mass_skip = modified_log_matmul(log_q[-1][:, 0, :], log_lm_probs_scaled_wo_eos)
    ...
    sum_score = torch.logaddexp(sum_score, log_q_empty_seq) # (B,) # TODO do we need to add the empty seq? # TODO make logaddexp safe
...
# def safe_logaddexp(x: torch.Tensor, dim: int, *, keepdim: bool = False) -> torch.Tensor:
...
# def safe_add(a: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
#     """safe add, handles the case of -inf values."""
#     return torch.where(a.isfinite(), a + b, a)
...
#         torch.logical_not(torch.isfinite(a)), a == b
#     )
...
def safe_logsumexp2(x: torch.Tensor, dim: int, *, keepdim: bool = False) -> torch.Tensor:
    ...
    with torch.no_grad():
        ...
        max_x_ = max_x if keepdim else max_x.squeeze(dim=dim)
    print(max_x_)
    re = torch.where(max_x_.isneginf(), 0.0, (x - max_x).exp().sum(dim=dim, keepdim=keepdim))
    print(re)
    ret = max_x_ + re
    print(ret)
    return ret
...
def test2():
    ...
    t2 = torch.tensor([float("-inf"), float("-inf"), float("-inf")], requires_grad=True)
    # t = torch.tensor([float("0"), float("0"), float("0")], requires_grad=True)
    
    t = ag(t, "t")
    # sum_t = safe_logsumexp2(t, dim=0)
    # sum_t = torch.logsumexp(t, dim=0)
    # sum_t = torch.log(torch.sum(t.exp(), dim=0))
    
    # sum_t = torch.logaddexp(t, t2)
    sum_t = t.exp().log()
    sum_t = sum_t.sum()
