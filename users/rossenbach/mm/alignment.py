...
class LabelAlignmentJob(rasr.RasrCommand, Job):
    ...
    def run(self, task_id):
        ...
        shutil.move('alignment.cache.%d' % task_id, self.out_single_alignment_caches[task_id].get_path())
        if self.word_boundaries:
            shutil.move('word_boundary.cache.%d' % task_id, self.single_word_boundary_caches[task_id].get_path())
    ...
    def hash(cls, kwargs):
        ...
        return super().hash({'config'        : config,
