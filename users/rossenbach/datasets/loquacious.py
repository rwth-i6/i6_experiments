...
def get_loquacious_tts_segments(loq_corpus_key="train.small"):
    """
    ...
    segments = SegmentCorpusJob(bliss_corpus_dict[loq_corpus_key], 1).out_single_segment_files[1]
    shuffle_segment_file_job = ShuffleAndSplitSegmentsJob(
        segment_file=segments,
        split={"train": 0.99, "dev": 0.01},
        shuffle=True
    )
    return shuffle_segment_file_job.out_segments["train"], shuffle_segment_file_job.out_segments["dev"]
...
sub250_segments = None

def get_250_from_2250_segment_list():
    global sub250_segments
    if sub250_segments is None:
        asr_bliss = get_bliss_corpus_dict()["train.medium-wo-small"]

        from i6_core.corpus.segments import SegmentCorpusJob, ShuffleAndSplitSegmentsJob
        segments = SegmentCorpusJob(asr_bliss,1).out_single_segment_files[1]
        segment_split = ShuffleAndSplitSegmentsJob(
            segment_file=segments,
            split={"250-from-2250": 1.0/9.0, "rest": 8.0/9.0}
        )
        sub250_segments = segment_split.out_segments["250-from-2250"]
    assert sub250_segments is not None
    return sub250_segments
