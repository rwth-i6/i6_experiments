...
class ExtractDurationsFromRASRAlignmentJob(Job):
    ...
    def run(self):
        ...
        assert self.align.get_path().endswith(".bundle")
        files = open(self.align.get_path(), "rt")
        count_broken_rasr_alignments = 0
        for cache in files:
          ...
          for key in keys:
            ...
            for time, allophone in alignment:
              ...
              # we are at the end of our alignment
              elif time == len(alignment) - 1:
                  # gmm_seq.append(phoneme)
                  seq.append(time - start_time + 1)
            ...
            assert len(alignment) == sum(seq), (
              key,
              len(alignment),
            )
            if self.target_duration_hdf is not None:
                # take the difference between returnn feature extraction and rasr feature extraction in account
                target_length = numpy.sum(returnn_length_dict[key])
                given_length = len(alignment)
                difference = target_length - given_length
                if difference == 3:
                    # center true vs center false case
                    seq[0] += 2
                    seq[-1] += 1
                elif difference == 4:
                    seq[0] += 2
                    seq[-1] += 2
                # weird but somehow this happens
                elif difference == 5:
                    seq[0] += 3
                    seq[-1] += 2
                else:
                    count_broken_rasr_alignments += 1
                    #print(alignment)
                    #print(key)
                    #print(len(alignment))
                    #print(target_length)
                    seq[-1] += difference
                    #assert False, (
                    #    f"Invalid difference of {difference} between reference and target",
                    #)

                assert target_length == sum(seq)

            # make sure sequence starts and ends with a boundary token:
            if gmm_seq[0] != self.boundary_token:
                gmm_seq.insert(0, self.boundary_token)
                seq.insert(0, 0)
            if gmm_seq[-1] != self.boundary_token:
                gmm_seq.append(self.boundary_token)
                seq.append(0)
        ...
        offset = 0
        for tag, length in zip(tags, new_lengths):
          in_data = duration_sequence[offset : offset + length]
          in_data = numpy.expand_dims(in_data, axis=1)
          offset += length
          writer.insert_batch(numpy.asarray([in_data]), [in_data.shape[0]], [tag])
        writer.close()
