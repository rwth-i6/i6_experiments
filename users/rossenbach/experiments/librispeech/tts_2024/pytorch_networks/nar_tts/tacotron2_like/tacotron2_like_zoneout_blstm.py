...
from ...tts_shared.espnet_tacotron import Postnet, ZoneOutCell
...
class NarTacotronDecoder(torch.nn.Module):
    ...
    def __init__(self, cfg: NarTacotronDecoderConfig, encoder_hidden_size: int, speaker_embedding_size: int):
        ...
        for layer in range(cfg.num_lstm_layers):
            iunits = (encoder_hidden_size * cfg.reduction_factor) + speaker_embedding_size if layer == 0 else cfg.basic_dim
            lstm = torch.nn.LSTMCell(iunits, cfg.basic_dim)
    ...
    def forward(self, h_upsampled, h_lengths, speaker_embedding):
        ...
        input_time_first = torch.transpose(stacked_h_t, 0, 1)
        ...
        blstm_out = self.lstm(input_time_first).transpose(0, 1)
