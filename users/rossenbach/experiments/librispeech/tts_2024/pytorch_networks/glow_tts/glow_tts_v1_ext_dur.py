...
class Model(BaseTTSModelV1):
    ...
    def forward(
        self, x, x_lengths, raw_audio=None, raw_audio_lengths=None, durations=None, g=None, gen=False, noise_scale=1.0, length_scale=1.0
    ):
        if not gen:
        ...
        if gen:
            ...
            with torch.no_grad():
                attn = self.generate_path(durations, attn_mask.squeeze(1)).unsqueeze(1)
                attn = attn.detach()
