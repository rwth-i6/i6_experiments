...
class ConformerEncoderV1ConvFirst(nn.Module):
    ...
    def forward(self, data_tensor: torch.Tensor, sequence_mask: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if run_ctx.epoch < 15:
            ...
            for module in self.module_list[-1:]:
                x = module(x, sequence_mask)  # [B, T, F']
        elif run_ctx.epoch < 21:
            for module in self.module_list[:3]:
                x = module(x, sequence_mask)  # [B, T, F']
            for module in self.module_list[-1:]:
                x = module(x, sequence_mask)  # [B, T, F']
        elif run_ctx.epoch < 26:
            for module in self.module_list[:5]:
                x = module(x, sequence_mask)  # [B, T, F']
            for module in self.module_list[-1:]:
                x = module(x, sequence_mask)  # [B, T, F']
        elif run_ctx.epoch < 31:
            for module in self.module_list[:7]:
                x = module(x, sequence_mask)  # [B, T, F']
            for module in self.module_list[-1:]:
                x = module(x, sequence_mask)  # [B, T, F']
        elif run_ctx.epoch < 36:
            for module in self.module_list[:9]:
                x = module(x, sequence_mask)  # [B, T, F']
            for module in self.module_list[-1:]:
                x = module(x, sequence_mask)  # [B, T, F']
        else:
...
class ConformerAEDModel(nn.Module):
    ...
    def __init__(self, cfg: ConformerAEDModelConfig):
        ...
        self.ctc_linear = nn.Linear(in_features=cfg.encoder_cfg.block_cfg.ff_cfg.input_dim, out_features=self.vocab_size + 1)  # +1 for CTC
...
class Model(torch.nn.Module):
    def __init__(self, **net_kwargs):
        ...
        conformer_encoder_cfg = ConformerEncoderV1Config(
            ...
            block_cfg=ConformerBlockV1Config(
                ...
                conv_cfg=ConformerConvolutionV1Config(
                    ...
                    dropout=0.1,
                    activation=nn.SiLU(),
                    norm=nn.BatchNorm1d(num_features=512, affine=False),
                ),
            ),
        )
        ...
        decoder_cfg = AttentionLSTMDecoderV1Config(
            ...
            output_dropout=0.3,  # default 0.3
        )
...
def train_step(*, model: Model, data, run_ctx, **kwargs):
    ...
    ce_loss = nn.functional.cross_entropy(
        unnormalized_logits.transpose(1, 2), targets_masked.long(), reduction="sum", label_smoothing=0.1,
    )  # [B,N]
...
def search_step(*, model: Model, data, run_ctx, **kwargs):
    ...
    for hyp, tag in zip (label_stack, tags):
        res = numpy.nonzero(hyp == 0)
        end = res[0][0] if len(res[0] > 0) else len(hyp)  # first in tuple, first of tuple
        sequence = " ".join([run_ctx.labels[i] for i in hyp[:end]])
        print(tag)
