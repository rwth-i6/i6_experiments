...
@dataclass
class StackingFrontendConfig(ModelConfiguration):
    """
    Attributes:
...
class StackingFrontend(nn.Module):
    """
    Just stacking the frames and a linear
    """

    def __init__(self, model_cfg: StackingFrontendConfig):
        """
        ...
        self.linear = nn.Linear(model_cfg.in_features * model_cfg.stacks, model_cfg.out_features)
        self.pos_encoding = PositionalEncoding(model_cfg.out_features, 0.1)
    ...
    def forward(self, tensor: torch.Tensor, sequence_mask: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        """
        target_frames = ((tensor.size(1) - 1) // self.cfg.stacks) + 1
        target_size = target_frames * self.cfg.stacks
        pad_size = target_size - tensor.size(1)
        tensor = nn.functional.pad(tensor, (0, 0, 0, pad_size), mode="constant")
        tensor = torch.reshape(tensor, (tensor.size(0), target_frames, tensor.size(2) * self.cfg.stacks))

        tensor = self.linear(tensor)
        tensor = self.pos_encoding(tensor)

        return tensor, sequence_mask[:, ::6]
...
class Model(torch.nn.Module):
    def __init__(self, **net_kwargs):
        ...
        frontend_config = StackingFrontendConfig(
            in_features=80,
            stacks=6,
            out_features=512,
        )
        ...
        conformer_config = ConformerEncoderV1Config(
            num_layers=8,
            frontend=ModuleFactoryV1(module_class=StackingFrontend, cfg=frontend_config),
