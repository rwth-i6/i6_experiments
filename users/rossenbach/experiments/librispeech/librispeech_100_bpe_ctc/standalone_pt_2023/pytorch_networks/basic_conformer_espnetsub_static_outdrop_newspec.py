...
@dataclass
class Conv2dSubsampling6Config(ModelConfiguration):
    idim: int
    odim: int
    dropout_rate: float
...
class Conv2dSubsampling6(torch.nn.Module):
    ...
    def __init__(self, cfg: Conv2dSubsampling6Config):
        """Construct an Conv2dSubsampling6 object."""
        super(Conv2dSubsampling6, self).__init__()
        self.conv = torch.nn.Sequential(
            torch.nn.Conv2d(1, cfg.odim, 3, 2),
            torch.nn.ReLU(),
            torch.nn.Conv2d(cfg.odim, cfg.odim, 5, 3),
            torch.nn.ReLU(),
        )
        self.out = torch.nn.Sequential(
            torch.nn.Linear(cfg.odim * (((cfg.idim - 1) // 2 - 2) // 3), cfg.odim),
            PositionalEncoding(cfg.odim, cfg.dropout_rate),
        )
    ...
    def forward(self, x, x_mask):
        ...
        Args:
            ...
            x_mask (torch.Tensor): Input mask (#batch, time).

        Returns:
            torch.Tensor: Subsampled tensor (#batch, time', odim),
                where time' = time // 6.
            torch.Tensor: Subsampled mask (#batch, time'),
                where time' = time // 6.
...
class Model(torch.nn.Module):
    def __init__(self, **net_kwargs):
        ...
        frontend_cfg = Conv2dSubsampling6Config(
            idim=80,
            odim=512,
            dropout_rate=0.1,
        )
        ...
        conformer_config = ConformerEncoderV1Config(
            num_layers=8,
            frontend=ModuleFactoryV1(module_class=Conv2dSubsampling6, cfg=frontend_cfg),
