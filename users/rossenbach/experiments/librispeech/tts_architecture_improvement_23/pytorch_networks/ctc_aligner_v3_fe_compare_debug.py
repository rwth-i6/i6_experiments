"""
Like v1_fe, but with correctly transposed and broadcasted final dropout and xavier init also for the linear layer
...
class Model(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.final_dropout = nn.Dropout1d(p=self.dropout, inplace=True)
    ...
    def forward(
        ...
        blstm_out = self.final_dropout(blstm_out.transpose(1,2)).transpose(1, 2)
        logits = self.final_linear(blstm_out)  # [B, T, #PHONES]
        log_probs = torch.log_softmax(logits, dim=2)  # [B, T, #PHONES]
        probs = torch.softmax(logits, dim=2)  # [B, T, #PHONES]

        return log_probs, probs, audio_features_len
...
def forward_init_hook(run_ctx, **kwargs):
    ...
    input_data = h5py.File("/work/asr4/rossenbach/sisyphus_work_folders/tts_asr_2021_work/i6_experiments/users/rossenbach/tts/duration_extraction/ViterbiAlignmentToDurationsJob.AyAO6JWXTnVc/output/durations.hdf", 'r')
    inputs = input_data['inputs']
    seq_tags = input_data['seqTags']
    lengths = input_data['seqLengths']
    run_ctx.ext_durations = {}
    ...
    for tag, length in zip(seq_tags, lengths):
        ...
        offset += length[0]
        run_ctx.ext_durations[tag] = in_data
...
class DurationsHandler:

    @classmethod
    def get_durations(cls, alignment, mel_length=None, input_length=None):
        if mel_length is None:
            mel_length = np.shape(alignment)[0]

        if input_length is None:
            input_length = np.shape(alignment)[1]

        D = np.array([0 for _ in range(input_length)])

        prev_max_index = 0
        for i in range(mel_length):
            max_index = alignment[i].tolist().index(alignment[i].max())

            # enforce monotonicity and prevent attention jumps
            if prev_max_index > max_index or abs(max_index - prev_max_index) > 5 or max_index >= input_length:
                D[prev_max_index] = D[prev_max_index] + 1
            else:
    ...
    @classmethod
    def durations_to_alignments(cls, durations):
        att = np.zeros((durations.shape[0], np.sum(durations)), dtype=np.float32)
        j = 0
    ...
    @classmethod
    def average_by_duration(cls, x, durs):
        mel_len = durs.sum()
        ...
        for idx, start, end in zip(range(mel_len), durs_cum[:-1], durs_cum[1:]):
            ...
            if len(values) > 0:
                x_char[idx] = np.mean(values)
            else:
                x_char[idx] = 0.0 # np.mean([]) = nan.

        return x_char.astype(np.float32)

    @classmethod
    def expand_by_duration(cls, x, durs):
        return np.repeat(x, durs)

    @classmethod
    def get_aligner_durations_dijkstra(cls, tokens: np.array, pred: np.array) -> np.array:
        """
        Extracts durations from the attention matrix by finding the shortest monotonic path from
        top left to bottom right.
        """
...
def forward_step(*, model: Model, data, run_ctx, **kwargs):
    ...
    logprobs, probs, audio_features_len = model(
        audio_features=audio_features,
        audio_features_len=audio_features_len,
        speaker_labels=speaker_labels,
    )
    ...
    numpy_probs = logprobs.detach().cpu().numpy()
    numpy_ctc_loss = ctc_loss.detach().cpu().numpy()

    print("Shape")
    print(numpy_ctc_loss.shape)
    align_sequences = []

    for i, _ in enumerate(numpy_logprobs):
        # align_sequences.append(AlignSequence(single_logprobs[:feat_len], single_phonemes[:phon_len]))
        sequence = AlignSequence(numpy_logprobs[i, :audio_features_len[i]], numpy_phonemes[i, :phonemes_len[i]])
        durations = extract_durations_with_dijkstra(sequence)
        durations_alex = DurationsHandler.get_aligner_durations_dijkstra(numpy_phonemes[i, :phonemes_len[i]], numpy_probs[i, :audio_features_len[i]])
        loss = numpy_ctc_loss[i] / audio_features_len[i]
        print(loss)
        print(durations)
        print(durations_alex)
