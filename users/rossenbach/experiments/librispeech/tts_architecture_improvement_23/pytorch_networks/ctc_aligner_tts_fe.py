...
class TTSPredictor(torch.nn.Module):
    ...
    """
    def __init__(self, config: TTSPredictorConfig, input_size: int, target_size: int, speaker_embedding_size: int):
        """
        ...
        self.token_embedding = nn.Linear(input_size, config.token_embedding_size)
        self.speaker_embedding = nn.Embedding(251, speaker_embedding_size)
        self.lstm_stack = nn.LSTM(
            input_size=config.token_embedding_size + speaker_embedding_size,
    ...
    def forward(self, token_predictions, token_len, speaker_labels):
        ...
        :return:
        """
        token_embedding = self.token_embedding(token_predictions)
        ...
        blstm_in = torch.concat([speaker_embeddings, token_embedding], dim=2)  # [B, T, F]
...
def train_step(*, model: Model, data, run_ctx, **kwargs):
    ...
    if run_ctx.epoch > 20:
        ...
        run_ctx.mark_as_loss(name="rec_l1", loss=l1loss, inv_norm_factor=num_frames, scale=0.5)
