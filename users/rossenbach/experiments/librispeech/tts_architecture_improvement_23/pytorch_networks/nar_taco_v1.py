...
class GaussianUpsampling(nn.Module):
    ...
    def forward(
        ...
        :param variances: [B, N]
        :param b_n_op_mask: [B, N], 1-0 mask as float for multiplication
...
class ConvDurationSigmaPredictor(torch.nn.Module):
    ...
    def __init__(self, config: ConvDurationSigmaPredictorConfig, in_size: int):
        ...
        self.variance_convs = nn.Sequential(
            Conv1DBlock(in_size, config.hidden_size,
                        filter_size=config.filter_size, dropout=config.dropout),
            Conv1DBlock(config.hidden_size, config.hidden_size, filter_size=config.filter_size, dropout=config.dropout),
        )
    ...
    def forward(self, x: torch.Tensor):
        ...
        sigma_conv_out = self.sigmaation_convs(conv_in)  # [B, hidden_size, N]
...
class NarTacoEncoder(torch.nn.Module):
    ...
    def __init__(self, config: NarEncoderConfig):
        super().__init__()
        self.embedding = nn.Linear(config.label_in_dim, config.embedding_size)
    ...
    def forward(self, label_in, label_in_len):
        ...
        blstm_packed_in = nn.utils.rnn.pack_padded_sequence(blstm_in, label_in_len.to("cpu"), batch_first=True)
...
class NarTacoDecoder(torch.nn.Module):
    ...
    def forward(self, upsampling_output: torch.Tensor, masked_durations: torch.Tensor, speaker_embeddings: torch.Tensor):
        ...
        length = torch.cumsum(masked_durations, dim=1).int()
...
class Config:
    ...
    def from_dict(cls, d):
        ...
        d["decoder_config"] = NarEncoderConfig.from_dict(d["decoder_config"])
        d["duration_predictor_config"] = NarEncoderConfig.from_dict(d["duration_predictor_config"])
...
class Model(torch.nn.Module):
    ...
    def forward(
        ...
        predictor_in = torch.concat([encoder_out, speaker_embeddings], dim=2)
        ...
        upsampled_encoder = self.upsampler(encoder_out, upsampler_durations, sigmas, b_n_mask)  # [B, T, F]

        log_mel = self.decoder(
            upsampling_input=upsampled_encoder,
            masked_durations = masked_durations,
...
def train_step(*, model: Model, data, run_ctx, **kwargs):
    ...
    durations = data["durations"][indices, :]  # [B, ?]
    ...
    mel_loss = nn.functional.l1_loss(masked_log_mels, masked_audio_features, reduce=True, reduction="sum")

    run_ctx.mark_as_loss(name="log_mel_l1", loss=mel_loss, inv_norm_factor=num_frames)

    duration_loss = nn.functional.l1_loss(masked_durations, durations, reduce=True, reduction="sum")
