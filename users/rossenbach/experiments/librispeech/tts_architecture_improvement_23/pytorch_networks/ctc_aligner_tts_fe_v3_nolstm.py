...
class Model(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.final_linear = nn.Linear(config.conv_hidden_size, config.target_size)
    ...
    def forward(
        ...
        if self.final_dropout_layer is not None:
            conv_out = self.final_dropout_layer(conv_out).transpose(1, 2)
        else:
            conv_out = conv_out.transpose(1, 2)
        logits = self.final_linear(conv_out)  # [B, T, #PHONES]
