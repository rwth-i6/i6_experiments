...
def aed_bpe_ls960_1023_initial_tests():
    ...
    model_zeineldeen_config_v2_fromscratch = ModelConfigV2(
        feature_extraction_config=fe_config,
        frontend_config=frontend_config,
    ...
    train_config_24gbgpu_radam = {
        "optimizer": {"class": "radam", "epsilon": 1e-12, "weight_decay": 1e-2, "decoupled_weight_decay": True},
        "learning_rates": list(np.linspace(5e-5, 5e-4, 20)) + list(
        np.linspace(5e-4, 5e-4, 100)) + list(np.linspace(5e-4, 5e-5, 360)) + list(np.linspace(5e-5, 1e-7, 20)),
        #############
    ...
    network_module = "aed.conformer_zoneout_prototype_0624.aed_prototype_v2"
    train_args_from_scratch = {
        "config": train_config_24gbgpu_radam,
        "network_module": network_module,
        "net_args": {"model_config_dict": asdict(model_zeineldeen_config_v2_fromscratch)},
        "debug": True,
    }
    
    training_name = prefix_name + "/" + network_module + ".512dim_sub6_24gbgpu_50eps_amp_zeineldeen_decoder_baseline_v2_sp_from_scratch"
    train_job = training(training_name, train_data_bpe5000, train_args_from_scratch, num_epochs=500,
                         **default_returnn)
    train_job.rqmt["gpu_mem"] = 24
    train_job.set_env("PYTORCH_CUDA_ALLOC_CONF", "expandable_segments:True")
    asr_model = prepare_asr_model(
    ...
    # convergence test 11gb GPU
    local_train_args = copy.deepcopy(train_args_from_scratch)
    local_train_args["config"]["optimizer"] = {"class": "adam", "epsilon": 1e-12}
    local_train_args["config"]["batch_size"] = 150 * 16000
    local_train_args["config"]["accum_grad_multiple_step"] = 2
    local_train_args["config"]["gradient_clip_norm"] = 1.0
    local_train_args["config"].pop("use_speed_perturbation")
    local_train_args["config"].pop("torch_amp_options")

    # Does not work
    #training_name = prefix_name + "/" + network_module + ".512dim_sub6_24gbgpu_convergence_test/adam_nodecay_nosp"
    #train_job = training(training_name, train_data_bpe5000, local_train_args, num_epochs=500,
    #                     **default_returnn)
    #train_job.rqmt["gpu_mem"] = 11
    #train_job.set_env("PYTORCH_CUDA_ALLOC_CONF", "expandable_segments:True")

    local_train_args = copy.deepcopy(local_train_args)
    model_local = copy.deepcopy(model_zeineldeen_config_v2_fromscratch)
    model_local.specauc_start_epoch = 501
    model_local.ctc_loss_scale = 1.0
    local_train_args["net_args"] = {"model_config_dict": asdict(model_local)}

    # Does not work
    # training_name = prefix_name + "/" + network_module + ".512dim_sub6_24gbgpu_convergence_test/adam_nodecay_nosp_ctc1.0_nospec"
    # train_job = training(training_name, train_data_bpe5000, local_train_args, num_epochs=500,
    #                      **default_returnn)
    # train_job.rqmt["gpu_mem"] = 11
    # train_job.set_env("PYTORCH_CUDA_ALLOC_CONF", "expandable_segments:True")
    ...
    local_train_args = copy.deepcopy(local_train_args)
    local_train_args["config"]["optimizer"] = {"class": "radam", "epsilon": 1e-12}

    # Does not work
    # training_name = prefix_name + "/" + network_module + ".512dim_sub6_24gbgpu_convergence_test/radam_nodecay_nosp_ctc1.0_nospec"
    # train_job = training(training_name, train_data_bpe5000, local_train_args, num_epochs=500,
    #                      **default_returnn)
    # train_job.rqmt["gpu_mem"] = 11
    # train_job.set_env("PYTORCH_CUDA_ALLOC_CONF", "expandable_segments:True")
    ...
    local_train_args = copy.deepcopy(local_train_args)
    local_train_args["config"]["learning_rates"] = [5e-5]

    # Converged! But is not optimal
    # training_name = prefix_name + "/" + network_module + ".512dim_sub6_24gbgpu_convergence_test/radam_nodecay_nosp_ctc1.0_nospec_constlr_1e-5"
    # train_job = training(training_name, train_data_bpe5000, local_train_args, num_epochs=500,
    #                      **default_returnn)
    # train_job.rqmt["gpu_mem"] = 11
    # train_job.set_env("PYTORCH_CUDA_ALLOC_CONF", "expandable_segments:True")
    ...
    local_train_args = copy.deepcopy(local_train_args)
    local_train_args["use_speed_perturbation"] = True
    local_train_args["config"]["optimizer"] = {"class": "radam", "epsilon": 1e-12, "weight_decay": 1e-2, "decoupled_weight_decay": True}

    # Worked !
    # training_name = prefix_name + "/" + network_module + ".512dim_sub6_24gbgpu_convergence_test/radam_ctc1.0_nospec_constlr_1e-5"
    # train_job = training(training_name, train_data_bpe5000, local_train_args, num_epochs=500,
    #                      **default_returnn)
    # train_job.rqmt["gpu_mem"] = 11
    # train_job.set_env("PYTORCH_CUDA_ALLOC_CONF", "expandable_segments:True")
    ...
    local_train_args = copy.deepcopy(local_train_args)
    model_local = copy.deepcopy(model_zeineldeen_config_v2_fromscratch)
    model_local.specauc_start_epoch = 21
    model_local.ctc_loss_scale = 0.3
    local_train_args["net_args"] = {"model_config_dict": asdict(model_local)}

    # Did not work
    # training_name = prefix_name + "/" + network_module + ".512dim_sub6_24gbgpu_convergence_test/radam_ctc0.3_spec21_constlr_1e-5"
    # train_job = training(training_name, train_data_bpe5000, local_train_args, num_epochs=500,
    #                      **default_returnn)
    # train_job.rqmt["gpu_mem"] = 11
    # train_job.set_env("PYTORCH_CUDA_ALLOC_CONF", "expandable_segments:True")
    ...
    model_local = copy.deepcopy(model_zeineldeen_config_v2_fromscratch)
    model_local.specauc_start_epoch = 21
    model_local.ctc_loss_scale = 0.7
    local_train_args["net_args"] = {"model_config_dict": asdict(model_local)}

    # Worked !
    # training_name = prefix_name + "/" + network_module + ".512dim_sub6_24gbgpu_convergence_test/radam_ctc0.7_spec21_constlr_1e-5"
    # train_job = training(training_name, train_data_bpe5000, local_train_args, num_epochs=500,
    #                      **default_returnn)
    # train_job.rqmt["gpu_mem"] = 11
    # train_job.set_env("PYTORCH_CUDA_ALLOC_CONF", "expandable_segments:True")
    ...
    model_local = copy.deepcopy(model_zeineldeen_config_v2_fromscratch)
    model_local.specauc_start_epoch = 21
    model_local.ctc_loss_scale = 0.5
    local_train_args["net_args"] = {"model_config_dict": asdict(model_local)}

    # Diverged
    # training_name = prefix_name + "/" + network_module + ".512dim_sub6_24gbgpu_convergence_test/radam_ctc0.5_spec21_constlr_1e-5"
    # train_job = training(training_name, train_data_bpe5000, local_train_args, num_epochs=500,
    #                      **default_returnn)
    # train_job.rqmt["gpu_mem"] = 11
    # train_job.set_env("PYTORCH_CUDA_ALLOC_CONF", "expandable_segments:True")
    ...
    local_train_args = copy.deepcopy(train_args_from_scratch)
    local_train_args["config"]["optimizer"] = {"class": "adam", "epsilon": 1e-12}
    local_train_args["config"]["batch_size"] = 300 * 16000
    local_train_args["config"]["accum_grad_multiple_step"] = 1
    local_train_args["config"].pop("use_speed_perturbation")
    local_train_args["config"].pop("torch_amp_options")

    # training_name = prefix_name + "/" + network_module + ".512dim_sub6_24gbgpu_convergence_test/adam_nodecay_nosp_noaccum"
    # train_job = training(training_name, train_data_bpe5000, local_train_args, num_epochs=500,
    #                      **default_returnn)
    # train_job.rqmt["gpu_mem"] = 11
    # train_job.set_env("PYTORCH_CUDA_ALLOC_CONF", "expandable_segments:True")
    ...
    # training_name = prefix_name + "/" + network_module + ".512dim_sub6_24gbgpu_convergence_test/adam_nodecay_nosp_ctc1.0_nospec_noaccum"
    # train_job = training(training_name, train_data_bpe5000, local_train_args, num_epochs=500,
    #                      **default_returnn)
    # train_job.rqmt["gpu_mem"] = 11
    # train_job.set_env("PYTORCH_CUDA_ALLOC_CONF", "expandable_segments:True")
