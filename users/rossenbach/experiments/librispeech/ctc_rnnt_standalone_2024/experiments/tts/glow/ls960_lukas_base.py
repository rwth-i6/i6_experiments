...
def run_flow_tts_960h():
    ...
    prefix = "experiments/librispeech/ctc_rnnt_standalone_2024/tts/glow_tts_960h/"
    training_datasets = build_training_dataset(ls_corpus_key="train-other-960", partition_epoch=1)
    ...
    config = {
        ...
        "learning_rates": list(np.linspace(5e-5, 5e-4, 50)) + list(np.linspace(5e-4, 5e-7, 150)),
    ...
    train_job = run_exp(net_module + "_base256_200eps", train_args=train_args, num_epochs=200)
    train_job.rqmt["gpu_mem"] = 24
    tts_model = prepare_tts_model(net_module + "_base256_200eps", train_job, train_args, get_specific_checkpoint=200)
