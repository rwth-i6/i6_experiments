...
def bpe5k_kazuki_lstm():
    prefix_name = "experiments/librispeech/ctc_rnnt_standalone_2024/lm_lstm"
    ...
    for BPE_SIZE in [2000, 5000, 10000]:
        ...
        if BPE_SIZE == 5000:
            training_name = prefix_name + ("/bpe%ik/" % (BPE_SIZE // 1000)) + network_module + ".2x2024_1k_RAdam_1e-3_2ep_reduce_gcn1.0"
        ...
        training_name = prefix_name + (
                    "/bpe%ik/" % (BPE_SIZE // 1000)) + network_module + ".2x2048_1k_RAdam_1e-3_3ep_reduce_gcn1.0"
        train_job = training(training_name, train_data_bpe5k_part100, train_args, num_epochs=300, **lm_returnn)#

        from ...storage import add_lm
        from ...pipeline import NeuralLM
        add_lm("bpe%ik_2x2024_kazuki_lstmlm_3ep" % (BPE_SIZE // 1000), lm_model=NeuralLM(
