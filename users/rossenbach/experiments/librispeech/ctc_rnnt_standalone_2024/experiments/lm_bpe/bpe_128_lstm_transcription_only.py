...
def bpe128_kazuki_lstm_transcription_only():

    for BPE_SIZE in [128]:
        prefix_name = "experiments/librispeech/ctc_rnnt_standalone_2024/bpe_%i_lstm_transcript_only" % BPE_SIZE
        ...
        add_lm("bpe%i_2x2024_kazuki_lstmlm_200ep_transcription_only" % BPE_SIZE, lm_model=NeuralLM(
        ...
        # from ...storage import add_lm
        # from ...pipeline import NeuralLM
        # add_lm("bpe%i_2x2024_kazuki_lstmlm_300ep_transcription_only" % BPE_SIZE, lm_model=NeuralLM(
        ...
        # massively more regularized and shorter training
        train_config_modern_v1 = {
            "optimizer": {"class": "RAdam"},
            #############
            "batch_size": 1280,  # BPE tokens
            "accum_grad_multiple_step": 1,
            "gradient_clip_norm": 1.0,
            "learning_rates": list(np.linspace(1e-3, 1e-5, 50))
        }
        lstm_drop_config = copy.deepcopy(lstm_base_config)
        lstm_drop_config.dropout = 0.5
        network_module = "lm.lstm.kazuki_lstm_zijian_variant_v1"
        train_args = {
            "config": train_config_modern_v1,
            "post_config": {"num_workers_per_gpu": 1},
            "network_module": network_module,
            "net_args": {"model_config_dict": asdict(lstm_drop_config)},
            "debug": True,
            "add_cache_manager": True,
        }

        training_name = prefix_name + "/" + network_module + ".2x2024_1k_RAdam_1e-3_50ep_reduce_gcn1.0"
        train_job = training(training_name, train_data_bpe128_part100, train_args, num_epochs=50, **lm_returnn)

        from ...storage import add_lm
        from ...pipeline import NeuralLM
        add_lm("bpe%i_2x2024_kazuki_lstmlm_50ep_drop05_transcription_only" % BPE_SIZE, lm_model=NeuralLM(
            checkpoint=train_job.out_checkpoints[50],
            net_args=train_args["net_args"],
            network_module=network_module,
            prefix_name=training_name
        ))
