...
def _slice_state(states: List[List[torch.Tensor]], idx: int, device: torch.device, new_state_length=None, max_history=None) -> (List[List[torch.Tensor]], int):
    """
    ...
    if new_state_length is not None:
        assert new_state_length <= max_history + 1, "invalid new state length"
        offset = 0
        state_length = new_state_length
        if max_history is not None and new_state_length > max_history:
            offset = 1
            state_length = max_history
        shortened_states =  [[state.index_select(dim=0, index=idx_tensor)[:, offset:state_length] for state in state_tuple] for state_tuple in states]
        #print("shapes:")
        #print(state_length)
        #print([states[0][0].index_select(dim=0, index=idx_tensor).shape])
        #print([shortened_states[0][0].shape])
        return shortened_states, state_length - offset
    else:
        return [[state.index_select(dim=0, index=idx_tensor) for state in state_tuple] for state_tuple in states], 0
...
class RNNTBeamSearch(torch.nn.Module):
    ...
    def __init__(
        ...
        self.lm_max_state_length = lm_max_state_length

        self.default_init_hypo = None

        self.predictor_call_times = []
        self.lm_call_times = []
    ...
    def _init_outer_hypos(self, hypo: Optional[Hypothesis], device: torch.device) -> List[Hypothesis]:
        ...
        init_hypo = Hypothesis(
            ...
            lm_state_length=lm_state_length,
        )
    ...
    def _gen_next_token_probs(
        ...
        if self.lm_model and self.zero_ilm_scale != 0.0:
            ...
            logprobs_zero = torch.nn.functional.log_softmax(joined_out_zero / self.temperature, dim=3)
            blank_norm_prob = torch.logaddexp(zero_tensor, -logprobs_zero[:,:,:, self.blank])
            logprobs_zero_renormalized = logprobs_zero - blank_norm_prob
            logprobs_zero_renormalized[:,:,:, self.blank] = 0.0
            logprobs_joiner = logprobs_joiner - self.zero_ilm_scale * logprobs_zero_renormalized
    ...
    def _gen_outer_hypos(
        ...
        for i in range(len(a_hypos)):
            ...
            h_b = Hypothesis(
                ...
                lm_state_length=h_a.lm_state_length,
                score=score,
            )
            b_hypos.append(h_b)
    ...
    def _gen_new_hypos(
        ...
        self.predictor_call_times.append(time.time() - start)
        start = time.time()
        if self.lm_model:
            if self.lm_states_need_label_axis:
                ...
                #print("lm shapes:")
                #print(lm_states[0][0].shape)
                #for i, hyp in enumerate(base_hypos):
                #    print(hyp.lm_state_length)
                lm_out, pred_lm_state = self.lm_model(
                    tgt_tokens,
                    torch.tensor([1] * len(base_hypos), device=device),
                    cache=lm_states[0],
                    cache_length=torch.tensor([hyp.lm_state_length for hyp in base_hypos], device=device)
                )
            ...
            pred_lm_state = None
        torch.cuda.synchronize()
        self.lm_call_times.append(time.time() - start)

        new_hypos: List[Hypothesis] = []
        if self.lm_model:
            for i, h_a in enumerate(base_hypos):
                new_tokens = h_a.tokens + [tokens[i]]
                lm_state, lm_state_length = _slice_state(
                    [pred_lm_state],
                    i,
                    device,
                    new_state_length=h_a.lm_state_length + 1 if self.lm_states_need_label_axis else None,
                    max_history=self.lm_max_state_length if self.lm_states_need_label_axis else None,
                )
                new_hypo = Hypothesis(
                    tokens=new_tokens,
                    predictor_output=pred_out[i].detach(),
                    predictor_state=_slice_state(pred_states, i, device)[0],
                    score=scores[i],
    ...
    def _search(
        ...
        print(f"step_time_avg: {np.mean(step_times):.4f}")
        print(f"predictor_time_avg: {np.mean(self.predictor_call_times):.4f}")
        print(f"LM_time_avg: {np.mean(self.lm_call_times):.4f}")
        print()
        print(f"predictor total time: {np.sum(self.predictor_call_times):.2f}")
        print(f"LM total time: {np.sum(self.lm_call_times):.2f}")
        print(f"search_time: {time.time() - search_time_start:.2f}")
