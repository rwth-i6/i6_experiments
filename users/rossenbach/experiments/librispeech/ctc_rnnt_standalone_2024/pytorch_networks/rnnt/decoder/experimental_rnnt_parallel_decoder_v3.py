...
def parallel_search(decoder, raw_audio, raw_audio_len, beam_width):
    torch.set_num_threads(4)
    return decoder.forward_semi_batched(
        input=raw_audio,
...
def forward_step(*, model, data, run_ctx, **kwargs):
    ...
    if run_ctx.batched_encoder_decoding:
        num_seqs = raw_audio.shape[0]
        seqs_pro_proc = math.ceil(num_seqs / 8)
        args = []
        decoder_index = 0
        for i in range(0, num_seqs, seqs_pro_proc):
            if num_seqs - i < seqs_pro_proc:
                seqs_pro_proc = num_seqs - i
            raw_audio_slice = raw_audio[i:i+seqs_pro_proc]
            raw_audio_len_slice = raw_audio_len[i:i+seqs_pro_proc]
            args.append((run_ctx.rnnt_decoders[decoder_index], raw_audio_slice, raw_audio_len_slice, run_ctx.beam_size))
            decoder_index += 1

        stacked_hypothesis = run_ctx.pool.starmap(parallel_search, args)

        batched_hypotheses = [hyp for stacked_hyps in stacked_hypothesis for hyp in stacked_hyps]
