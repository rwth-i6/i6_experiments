...
def _mask(tensor: torch.Tensor, batch_axis: int, axis: int, pos: torch.Tensor, max_len: int, mask_value: Union[float, torch.Tensor]) -> torch.Tensor:
    """
...
def _random_mask(tensor: torch.Tensor, batch_axis: int, axis: int, min_num: int, max_num: int, max_len: int, mask_value: Union[float, torch.Tensor]):
    """
    ...
    for i in range(max_num_masks):
        tensor = torch.where(i < num_masks, _mask(tensor, batch_axis, axis, indices[:, i], max_len, mask_value), tensor)
...
def specaugment_v2(
    ...
    *,
    ...
    mean = torch.mean(audio_features)
    ...
    def forward(
        ...
        with torch.no_grad():
            ...
            if self.training and run_ctx.epoch >= self.specaug_start_epoch:
                audio_features_masked_2 = specaugment_v2_by_length(
                    audio_features,
