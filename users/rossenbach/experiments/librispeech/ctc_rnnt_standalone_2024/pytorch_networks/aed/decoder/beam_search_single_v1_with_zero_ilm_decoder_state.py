...
class AEDLabelScorer(LabelScorerIntf):
    ...
    def score_and_update_state(
        ...
        zero_ilm_logits, _ = self.model.decoder(
            torch.zeros_like(encoder_outputs), single_labels, audio_features_len, shift_embeddings=prev_state is None, state=decoder_state
        )  # [B,1,Vocab] and state of [B, *]
