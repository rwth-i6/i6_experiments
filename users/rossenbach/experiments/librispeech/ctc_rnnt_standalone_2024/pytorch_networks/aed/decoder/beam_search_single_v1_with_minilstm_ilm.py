...
if TYPE_CHECKING:
    from ..conformer_0924.i6models_relposV1_VGG4LayerActFrontendV1_LSTMDecoder_minilstm_v1 import Model
...
class AEDLabelScorer(LabelScorerIntf):
    ...
    def score_and_update_state(
        ...
        if prev_state is not None:
            lstm_state_1, lstm_state_2, att_context, accum_att_weights, lm_h, lm_c, ilm_lstm_state_1, ilm_lstm_state_2, ilm_att, minilstm_state1, minilstm_state2 = tuple([state.squeeze(0) if state is not None else None for state in prev_state])
            # pack lstm state
            decoder_state = (lstm_state_1, lstm_state_2), att_context, accum_att_weights
            ilm_in_state = (ilm_lstm_state_1, ilm_lstm_state_2), torch.zeros_like(att_context), ilm_att
            mini_lstm_state = (minilstm_state1.contiguous(), minilstm_state2.contiguous()) if minilstm_state1 is not None else None
        ...
        if single_labels.shape[0] != self.encoder_outputs.shape[0]:
            ...
            fake_context = encoder_zeros
            mini_lstm_out_state = (None, None)
        else:
            encoder_outputs = self.encoder_outputs
            audio_features_len = self.audio_features_len
            embeddings = self.model.decoder.target_embed(single_labels)
            if mini_lstm_state is not None:
                mini_lstm_state = (mini_lstm_state[0].unsqueeze(0), mini_lstm_state[1].unsqueeze(0))
            lstm_out, mini_lstm_out_state = self.model.mini_lstm(embeddings, mini_lstm_state)
            mini_lstm_out_state = (mini_lstm_out_state[0].squeeze(0), mini_lstm_out_state[0].squeeze(0))
            fake_context = self.model.mini_lstm_ff(lstm_out)
        ...
        ilm_logits, ilm_states = self.model.decoder(
            fake_context, single_labels, audio_features_len, shift_embeddings=prev_state is None, state=ilm_in_state, force_context=True,
        )  # [B,1,Vocab] and state of [B, *]
        ilm_log_softmax = torch.nn.functional.log_softmax(ilm_logits, dim=-1)  # [beam, 1, #vocab]
        ilm_log_softmax = torch.transpose(ilm_log_softmax, 0, 1)  # [1, beam, #vocab]
        ...
        log_softmax = decoder_log_softmax + (self.lm_scale * lm_log_softmax) - (self.zero_ilm_scale * ilm_log_softmax)

        # first unpack decoder lstm and lm lstm
        states = states[0][0], states[0][1], states[1], states[2], lm_out_state[0], lm_out_state[1], ilm_states[0][0], ilm_states[0][1], ilm_states[2], mini_lstm_out_state[0], mini_lstm_out_state[1]
        # [Batch, beam, ... required, so expand]
        expanded_states = [state.unsqueeze(0) if state is not None else None for state in states]
