...
from i6_models.decoder.attention import AttentionLSTMDecoderV1Config, ZoneoutLSTMCell, AdditiveAttention, AdditiveAttentionConfig
...
class AttentionLSTMDecoderZeroTrainV1(nn.Module):
    """
    ...
    def forward(
        ...
        for step in range(num_steps):
            ...
            if random.uniform(0.0, 1.0) < self.zero_prob:
                do_zero = True
                lstm_zero_state = self.s(torch.cat([target_embed, torch.zeros_like(att_context)], dim=-1), lstm_state)
                lstm_out = lstm_zero_state[0]
            ...
            if do_zero is True:
                att_context_list.append(torch.zeros_like(att_context))
            else:
