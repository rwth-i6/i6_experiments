...
from .kazuki_lstm_zijian_variant_v3_cfg import ModelConfig
...
class Model(nn.Module):
    ...
    def __init__(self, model_config_dict, **kwargs):
        ...
        if self.cfg.lstm_dropout > 0:
            self.lstm_dropout = nn.Dropout(p=self.cfg.lstm_dropout)
        else:
            self.lstm_dropout = None
        if self.cfg.output_dropout > 0:
            self.output_dropout = nn.Dropout(p=self.cfg.output_dropout)
        else:
