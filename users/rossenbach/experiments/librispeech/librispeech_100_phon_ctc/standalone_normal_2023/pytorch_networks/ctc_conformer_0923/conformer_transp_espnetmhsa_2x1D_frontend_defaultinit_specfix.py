"""
Jingjing relmhsa with default init
"""
...
from .espnet_relmhsa_v1 import RelPositionalEncoding, RelPositionMultiHeadedAttention, RelPositionMultiHeadedAttentionConfig
...
class TwoLayer1DFrontend(nn.Module):
    ...
    def __init__(self, model_cfg: TwoLayer1DFrontendConfig):
        ...
        self.pos_encoding = RelPositionalEncoding(model_cfg.conv2_channels, model_cfg.dropout)


    def forward(self, tensor: torch.Tensor, sequence_mask: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """
        ...
        tensor, pos_emb = self.pos_encoding(tensor)

        return tensor, pos_emb, sequence_mask
...
@dataclass
class RelConformerBlockV1Config(ModelConfiguration):
    """
    Attributes:
    ...
    mhsa_cfg: RelPositionMultiHeadedAttentionConfig
    conv_cfg: ConformerConvolutionV1Config

class RelPosConformerBlockV1(nn.Module):
    """
    Conformer block module modified to use ESPNet RelMHSA
    """

    def __init__(self, cfg: RelConformerBlockV1Config):
        """
        :param cfg: conformer block configuration with subunits for the different conformer parts
        """
        super().__init__()
        self.ff1 = ConformerPositionwiseFeedForwardV1(cfg=cfg.ff_cfg)
        self.mhsa = RelPositionMultiHeadedAttention(cfg=cfg.mhsa_cfg)
    ...
    def forward(self, x: torch.Tensor, /, pos_emb: torch.Tensor, sequence_mask: torch.Tensor) -> torch.Tensor:
        """
        ...
        # For ESPNet MHSA query, key and value are given separately
        # Also sequence_mask is [B,1,T] instead of [B, T]
        x = self.mhsa(x, x, x, pos_emb, sequence_mask.unsqueeze(1)) + x  #  [B, T, F]
...
@dataclass
class RelConformerEncoderV1Config(ModelConfiguration):
    """
    Attributes:
    ...
    block_cfg: RelConformerBlockV1Config


class RelTransparentConformerEncoderV1(nn.Module):
    """
    Implementation of the convolution-augmented Transformer (short Conformer), as in the original publication,
    modified with ESPNet relative positional encoding and transparent attention.

    The model consists of a frontend and a stack of N conformer blocks.
    C.f. https://arxiv.org/pdf/2005.08100.pdf
    """

    def __init__(self, cfg: RelConformerEncoderV1Config):
        """
        ...
        self.module_list = torch.nn.ModuleList([RelPosConformerBlockV1(cfg.block_cfg) for _ in range(cfg.num_layers)])
    ...
    def forward(self, data_tensor: torch.Tensor, sequence_mask: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        """
        x, pos_emb, sequence_mask = self.frontend(data_tensor, sequence_mask)  # [B, T, F']
        ...
        for i, module in enumerate(self.module_list):
            x = module(x, pos_emb, sequence_mask)  # [B, T, F']
...
class Model(torch.nn.Module):
    def __init__(self, model_config_dict, **kwargs):
        ...
        conformer_config = RelConformerEncoderV1Config(
            ...
            block_cfg=RelConformerBlockV1Config(
                ...
                mhsa_cfg=RelPositionMultiHeadedAttentionConfig(
                    ...
                    n_feat=conformer_size,
                    dropout_rate=self.cfg.mhsa_dropout,
                ),
                conv_cfg=ConformerConvolutionV1Config(
        ...
        self.conformer = RelTransparentConformerEncoderV1(cfg=conformer_config)
