...
def get_network_serializer(returnn_common_root: tk.Path, rc_extern_data: ExternData, training: bool, debug=False, **kwargs) -> Collection:
    ...
    rc_construction_code = Import(
        "i6_experiments.users.rossenbach.experiments.alignment_analysis_tts.rc_networks.%s.construct_network" % model_type
    )
    net_func_map = {
    ...
    if "audio_features" in [init_args.name for init_args in rc_extern_data.extern_data]:
        net_func_map["audio_data"] = "audio_features"
