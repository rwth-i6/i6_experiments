...
def train_small_bpe_kazuki_trafo():

    for BPE_SIZE in [128, 256, 512, 1000, 2000, 10000]:
        prefix_name = "experiments/loquacious/standalone_2025/lm_bpe/train_small_bpe_%i_trafo" % BPE_SIZE
        ...
        if BPE_SIZE == 128:
            ...
            train_args_no_regul = copy.deepcopy(train_args)
            train_args_no_regul["config"]["optimizer"]["weight_decay"] = 0.0
            training_name = prefix_name + "/" + network_module + ".24x768_2x3k_RAdam_3e-4_5ep_reduce_gcn2.0_no_weight_decay"
            train_job = training(training_name, train_data_bpe128_part100, train_args_no_regul, num_epochs=500, **lm_returnn)
            train_job.rqmt["gpu_mem"] = 24
