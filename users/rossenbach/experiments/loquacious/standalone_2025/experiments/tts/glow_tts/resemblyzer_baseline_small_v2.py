...
def run_resemblyzer_flow_tts_v2():
    """
    :return: durations_hdf
    """

    prefix = "experiments/loquacious/standalone_2025/tts/glow_tts_v2/"
    ...
    def run_exp(name, train_args, target_durations=None, num_epochs=100):
        if target_durations is not None:
            ...
            #                                                        loq_corpus_key="train-clean-100")
        else:
    ...
    def eval_exp(name, tts_model: TTSModel, decoder, decoder_options):
        ...
        # from i6_experiments.users.rossenbach.experiments.jaist_project.evaluation.swer import \
        #     run_evaluate_reference_swer
        ...
        #                                                  )
        # run_evaluate_reference_swer(prefix=prefix + "/" + tts_model.prefix_name,
        #                             bliss=realpath_corpus.out_merged_corpus)

    def local_extract_durations(name, tts_model: TTSModel, debug=False):
        forward_config = get_forward_config(
            network_module=tts_model.network_module,
            net_args=tts_model.net_args,
            decoder="glow_tts.duration_extraction_decoder_dynamic_speakers",
            decoder_args={},
            config={
        ...
        forward_config.config["max_seqs"] = 60
    ...
    from ....pytorch_networks.glow_tts.glow_tts_dynamic_speakers_v2 import (
    ...
    net_module = "glow_tts.glow_tts_dynamic_speakers_v2"
    ...
    train_name = "train.small_" + net_module + "_base256_400eps"
    ...
    eval_exp("base", tts_model=tts_model, decoder="glow_tts.simple_gl_decoder_dynamic_speakers", decoder_options=decoder_options)

    local_extract_durations("base", tts_model=tts_model, debug=True)
    ...
    for noise_scale in [0.5, 0.6, 0.7, 0.8, 0.9]:
        ...
        decoder_options_synthetic_["glowtts_noise_scale"] = noise_scale
        ...
        tk.register_output(prefix + "/" + train_name + "/" + f"generated_synthetic_noise_{noise_scale:.2f}/train-small-shuffled-speakers.xml.gz",
    ...
    tk.register_output(
        prefix + "/" + train_name + "/" + f"generated_synthetic_noise_{noise_scale:.2f}/train-small-static-speakers.xml.gz",
        merged_corpus_with_text)
    ...
    tk.register_output(
        prefix + "/" + train_name + "/" + f"generated_synthetic_noise_{noise_scale:.2f}/train-medium-wo-small-shuffled-speakers.xml.gz",
        merged_corpus_with_text)
