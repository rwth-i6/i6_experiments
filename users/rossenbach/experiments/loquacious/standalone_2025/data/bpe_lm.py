...
def build_lm_training_datasets(prefix, loquacious_key, bpe_size, settings: LMDatasetSettings):
    ...
    cv_bpe_data_job.add_alias(os.path.join(prefix, "apply_bpe_to_cv"))
...
def build_lm_perplexity_dataset(prefix, loquacious_key, dev_dataset_key, bpe_size):
    bpe_settings = get_subword_nmt_bpe(corpus_key=loquacious_key, bpe_size=bpe_size, unk_label='<unk>')

    if dev_dataset_key == "dev.short":
        bliss_corpus = get_dev_short_bliss()
    else:
        bliss_corpus = get_bliss_corpus_dict()[dev_dataset_key]
    dev_text = CorpusToTxtJob(bliss_corpus=bliss_corpus, gzip=False).out_txt
    cv_bpe_data_job = ApplyBPEToTextJob(
        text_file=dev_text,
        bpe_codes=bpe_settings.bpe_codes,
        bpe_vocab=bpe_settings.bpe_count_vocab,
    ...
    bpecount = PipelineJob(cv_bpe_data_job.out_bpe_text, ["wc -w"], mini_task=True).out
    tk.register_output(f"{prefix}/{dev_dataset_key}/BPE_{bpe_size}_{loquacious_key}/wordcount", wordcount)
    tk.register_output(f"{prefix}/{dev_dataset_key}/BPE_{bpe_size}_{loquacious_key}/bpecount", bpecount)
    ...
    return lm_cv_dataset, wordcount, bpecount
