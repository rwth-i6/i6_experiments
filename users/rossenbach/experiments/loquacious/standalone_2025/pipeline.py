...
from .data.common import DatasetSettings, get_devtest_all_stms
...
def search_single(
    ...
    if use_gpu:
        search_job.set_env("PYTORCH_CUDA_ALLOC_CONF", "expandable_segments:True")
...
def search(
    ...
    for key, (test_dataset, test_dataset_reference) in test_dataset_tuples.items():
        search_name = prefix_name + "/" + key
        wers[key], search_jobs[key], ctms[key] = search_single(
            search_name,
...
def compute_prior(
    ...
    returnn_exe: tk.Path,
    returnn_root: tk.Path,
    mem_rqmt: int = 60, # change back to 32
):
    """
    Run search for a specific test dataset
...
def calculate_perplexity(
        ...
        dataset: Dataset,
        config: Dict[str, Any],
        perplexity_module: str,
        perplexity_args: Dict[str, any],
        checkpoint: tk.Path,
        returnn_exe: tk.Path,
        returnn_root: tk.Path,
):
    """
    Calculate perplexity of an LM
    """
    config_ = {
        **config,
    ...
    returnn_config = get_forward_config(
        ...
        config=config_,
        net_args=train_args["net_args"],
        decoder=perplexity_module,
        decoder_args=perplexity_args,
        debug=True,
    ...
    output_files = [
            ...
            'word_ppl_no_eos',
            'word_ppl_no_unk',
            'word_ppl_no_unk_no_eos',
        ]
    perplexity_job=ReturnnForwardJobV2(
        model_checkpoint=checkpoint,
        returnn_config=returnn_config,
        log_verbosity=5,
        mem_rqmt=12,
    ...
    perplexity_job.add_alias(prefix_name + "/perplexity_job")
    for output_file in output_files:
        tk.register_output(f"{prefix_name}/{output_file}", perplexity_job.out_files[output_file])