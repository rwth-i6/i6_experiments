...
class FlowDecoder(nn.Module):
    def __init__(
        ...
        for b in range(cfg.num_blocks):
            ...
            self.flows.append(
                modules.InvConvNear(
                    channels=cfg.target_channels * cfg.num_squeeze,
                    n_split=cfg.num_splits
                )
            )
            self.flows.append(
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.decoder = FlowDecoder(
            cfg=config.flow_decoder_config,
            speaker_embedding_size=config.speaker_embedding_size
        )
...
def train_step(*, model: Model, data, run_ctx, **kwargs):
    ...
    (z, z_m, z_logs, logdet, z_mask), _, _, (attn, logw, logw_) = model(
        phonemes, phonemes_len, raw_samples, audio_features_len, speaker_embeddings
    )
