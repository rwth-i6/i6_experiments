...
from .i6models_relposV1_VGG4LayerActFrontendV1_LSTMDecoder_v2_cfg import ModelConfig
...
class Model(torch.nn.Module):
    def __init__(self, model_config_dict, **net_kwargs):
        ...
        self.ctc_output_dropout = BroadcastDropout(
            p=self.cfg.ctc_softmax_dropout,
            dropout_broadcast_axes=self.cfg.dropout_broadcast_axes
        )

        self.encoder_output_dropout = nn.Dropout(self.cfg.encoder_out_dropout)
    ...
    def forward(
        ...
        log_probs_list = []
        conformer_out = self.encoder_output_dropout(conformer_out_layers[-1])
        logits = None
        for i, (out_layer, scale) in enumerate(zip(conformer_out_layers, self.scales)):
            if scale == 0.0:
                continue
            ctc_out = self.ctc_output_dropout(out_layer)
            logits = self.output_linears[i](ctc_out)
        ...
        if encoder_only:
            ...
            encoder_outputs = conformer_out_layers[-1]
            # search here
            state = None
