...
def forward_step(*, model: Model, data, run_ctx, **kwargs):
    ...
    for linear, log_mel, length, tag in zip(linears, log_mels, feature_lengths, tags):
        ...
        # avoid filename too long issues, reserve 4 characters for .ogg extension
        if len(tag) >= 252:
            tag = tag[:251]
        ...
        if run_ctx.create_plots:
            plt.imshow(numpy.transpose(log_mel)[:, :length])
            plt.gca().invert_yaxis()
            plt.savefig(f"audio_files/{tag.replace('/', '_')}.png")

        if run_ctx.store_log_mels is True:
            run_ctx.hdf_writer.insert_batch(inputs=np.expand_dims(log_mel.numpy()[:length], 0), seq_len=[length], seq_tag=[tag])
