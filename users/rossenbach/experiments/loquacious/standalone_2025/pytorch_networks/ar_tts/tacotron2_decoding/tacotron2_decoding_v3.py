...
from ...tts_shared.encoder.transformer_v2 import TTSTransformerTextEncoderV2Config, ConvLayerNorm, DualConv
...
class Tacotron2Decoder(torch.nn.Module):
    ...
    def __init__(
        ...
        for layer in range(cfg.dlayers):
            iunits = (encoder_output_size * self.reduction_factor) + speaker_embedding_size + prenet_units if layer == 0 else cfg.dunits
        ...
        iunits = (encoder_output_size * cfg.reduction_factor) + speaker_embedding_size + cfg.dunits if cfg.use_concate else cfg.dunits
    ...
    def forward(self, h_t, h_t_lengths, target_audio_features, speaker_embedding):
        ...
        if self.reduction_factor > 1:
            ...
            padding = h_t.size(1) % self.reduction_factor
            h_t = torch.nn.functional.pad(h_t, [0, 0, 0, padding])  # pad axis -2 (= 1) right sided
            h_t = h_t.view(h_t.size(0), -1, h_t.size(2) * self.reduction_factor)
        ...
        missing_frame = h_t.size(1) -  target_audio_features.size(1)
        torch.nn.functional.pad(target_audio_features, [0, 0, 0, missing_frame])

        h_t_timefirst = h_t.transpose(0, 1)
        target_features_timefirst = target_audio_features.transpose(0, 1)

        # loop for an output sequence
        outs = []
        for y, h in zip(target_features_timefirst, h_t_timefirst):
    ...
    def inference(
        ...
        idx = 0
        outs, att_ws, probs = [], [], []
        for h in h_t.transpose(0, 1):  # [T, B, 2F]
            # updated index
        ...
        reduced_feature_lengths = ((h_t_lengths + (self.reduction_factor/2)) // self.reduction_factor) * self.reduction_factor
        t_mask = sequence_mask(reduced_feature_lengths).unsqueeze(1)

        outs = outs * t_mask  # (B, odim, L)
        if self.postnet is not None:
            outs = outs + self.postnet(outs)  # (B, odim, L)

        outs = outs * t_mask

        return outs.transpose(1, 2), reduced_feature_lengths  # return as normal B,T,F
...
class Model(BaseTTSModelV2):
    ...
    def forward(
        self, phon_labels, labels_lengths, speaker_embeddings, raw_audio=None, raw_audio_lengths=None, target_durations=None
    ):
        """

        :param phon_labels:
        :param labels_lengths:
        :param speaker_embeddings:
        :param raw_audio:
        ...
        if raw_audio is None:
            # inference
            output_features, reduced_feature_lengths = self.decoder.inference(h_t=upsampled_h, h_t_lengths=feature_lengths, speaker_embedding=spk.squeeze(-1))
            return output_features, reduced_feature_lengths, log_durations.squeeze(1)
        else:
            # Training
...
def train_step(*, model: Model, data, run_ctx, **kwargs):
    ...
    output_features_after, output_features_before, target_features, features_lengths, log_durations = model(
        phonemes, phonemes_len, speaker_embeddings, raw_samples, audio_features_len, durations,
    )
