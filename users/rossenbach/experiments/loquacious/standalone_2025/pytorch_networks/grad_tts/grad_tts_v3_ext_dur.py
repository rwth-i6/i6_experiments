...
from ..tts_shared.tts_base_model.base_model_v2 import BaseTTSModelV2
from ..tts_shared import DbMelFeatureExtractionConfig
from ..tts_shared.encoder.transformer import (
    GlowTTSMultiHeadAttentionV1Config,
    GlowTTSMultiHeadAttentionV1,
    ConditionedDualConv,
    ConvLayerNorm
)
from ..tts_shared.encoder.transformer_v2 import TTSTransformerTextEncoderV2Config
...
class MuNetConfig():
    ...
    def from_dict(cls, d):
        ...
        return MuNetConfig(**d)


class MuNet(nn.Module):
    """
    An extra network part to combine the encoder output with the speaker embedding
    to produce speaker-dependent means for the alignment process
    """
    ...
    def __init__(
        ...
        self.input_proj = nn.Conv1d(cfg.input_dim, cfg.basic_dim, kernel_size=1)
        ...
        for i in range(cfg.num_layers):
            ...
            self.ffn_layers.append(
                ConditionedDualConv(
                    cfg.basic_dim,
                    cfg.conv_dim,
                    condition_channels=speaker_embedding_size,
        ...
        self.out_proj_m = nn.Conv1d(cfg.basic_dim, target_size, 1)

    def forward(self, h, h_mask, spk_emb) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        :param h: [B, F, N]
        :param h_mask: [B, 1, N]
        :param spk_emb: [B, F', 1]
        :return Tuple of time axis last encoder states [B, target_size, N] and corresponding encoder mask as [B, 1, N]
        """
        h = self.input_proj(h)
        ...
        for i in range(self.cfg.num_layers):
            ...
            ff = self.ffn_layers[i](h, h_mask, spk_emb)
            ff = self.drop(ff)
            h = self.norm_layers_2[i](h + ff)
        h = h * h_mask

        return self.out_proj_m(h)
...
class Config:
    ...
    def from_dict(cls, d):
        ...
        d["encoder_config"] = TTSTransformerTextEncoderV2Config.from_dict(d["encoder_config"])
        d["duration_predictor_config"] = SimpleConvDurationPredictorV1Config(**d["duration_predictor_config"])
        d["mu_net_config"] = MuNetConfig.from_dict(d["mu_net_config"])
...
class Model(BaseTTSModelV2):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.mu_net = MuNet(config.mu_net_config, speaker_embedding_size=config.speaker_embedding_size, target_size=config.diffusion_decoder_config.n_feats)
    ...
    def forward(
        ...
        h, h_mask, spk_expanded, log_durations = self.forward_encoder(
            phon_labels=x,
            labels_lengths=x_lengths,
            speaker_embedding=g,
        )

        # apply speaker informed mu-net (the original model only contains a linear prediction here)
        h_mu = self.mu_net(h, h_mask, spk_expanded) * h_mask

        # speaker embedding is returned as [B, spk_emb, 1], but GradTTS does not need that last axis
        spk = spk_expanded.squeeze(-1)
        ...
        if gen:  # durations from dp only used during generation
            w = torch.exp(log_durations) * h_mask.squeeze(1) * length_scale  # durations
        ...
        if gen:
            ...
            z_m = torch.matmul(attn.transpose(1, 2), h_mu.transpose(1, 2)).transpose(1, 2)
            ...
            y = self.decoder(z, y_mask, z_m, n_timesteps=num_timesteps, spk=spk)
            return y, y_lengths, w_ceil
        else:
            with torch.no_grad():
...
def train_step(*, model: Model, data, run_ctx, **kwargs):
    ...
    speaker_embeddings = data["speaker_embeddings"].squeeze(axis=1)  # [B, 1, F] -> [B,F]
    durations = data["durations"]  # [B, N]

    duration_loss, prior_loss, diff_loss = model(
        phonemes, phonemes_len, raw_samples, audio_features_len, durations, speaker_embeddings
    )
