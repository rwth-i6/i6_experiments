...
from .documented_rnnt_beam_search_v7 import RNNTBeamSearch
...
class DecoderConfig:
    ...
    lm_max_state_length: Optional[int] = None

    max_token_per_frame: int = 100
...
def forward_init_hook(run_ctx, **kwargs):
    ...
    if config.lm_module is not None:
        ...
        lm_model.eval()
        run_ctx.lm_model = torch.quantization.quantize_dynamic(
            lm_model,  # the original model
    ...
    run_ctx.rnnt_decoder = RNNTBeamSearch(
        ...
        lm_max_state_length=config.lm_max_state_length,
        step_max_tokens=config.max_token_per_frame,
    )
    print("done!")
    ...
    run_ctx.engine._model = torch.quantization.quantize_dynamic(
        run_ctx.engine._model,  # the original model
        {nn.Linear},  # a set of layers to dynamically quantize
        dtype=torch.qint8,  # the target dtype for quantized weights
        inplace=True
    )
...
def forward_step(*, model, data, run_ctx, **kwargs):
    ...
    # sequence = [idx for idx in hyps[0] if idx < len(run_ctx.labels)]
    # lm_indices = torch.tensor([0] + sequence, device=run_ctx.device).unsqueeze(0)
    # print(torch.max(lm_indices))

    # run_ctx.lm_model.trace = True

    # length_tensor = torch.tensor([lm_indices.shape[1]], device=run_ctx.device)
    # print("lm_indices:", lm_indices.shape)
    # print("length_tensor:", length_tensor.shape)
    # print("length_tensor:", length_tensor)
    # lm_out, _ = run_ctx.lm_model(
    #     lm_indices,
    #     length_tensor,
    #     None
    # )
    # print("ground_truth out:")
    # print(lm_out[0][1])

    # one_tensor = torch.tensor([1], device=run_ctx.device)
    # lm_args = torch.tensor([[0]], device=run_ctx.device), one_tensor, None
    # lm_out_tmp, pred_lm_state = run_ctx.lm_model(*lm_args)
    # lm_outs = [lm_out_tmp]


    # for i, token in enumerate(sequence):
    #     in_token = torch.tensor([token], device=run_ctx.device).unsqueeze(0)
    #     state_length = torch.tensor([pred_lm_state[0].shape[0]], device=run_ctx.device)
    #     print("state_length", state_length)
    #     lm_out_tmp, pred_lm_state = run_ctx.lm_model(in_token, one_tensor, pred_lm_state, state_length)
    #     lm_outs.append(lm_out_tmp)
    #     print("should match out:")
    #     print(lm_out_tmp)
    #     assert False

    # result = torch.concatenate(lm_outs, dim=1)
    # print("finished:")
    # print(lm_out.shape)
    # print(lm_out[0][1])
    # print(result.shape)
