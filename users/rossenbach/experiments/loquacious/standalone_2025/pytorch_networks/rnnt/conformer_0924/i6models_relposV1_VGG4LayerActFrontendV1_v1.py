...
from .i6models_relposV1_VGG4LayerActFrontendV1_v1_cfg import ModelConfig, PredictorConfig
...
class Model(torch.nn.Module):
    ...
    def forward(
        ...
        conformer_joiner_out = self.encoder_out_linear(conformer_out_layers[-1])
        ...
        for i, (out_layer, scale) in enumerate(zip(conformer_out_layers, self.scales)):
            ...
            ctc_logprobs_list.append(log_probs)

        return output_logits, src_len, ctc_logprobs_list
...
def train_step(*, model: Model, data, run_ctx, **kwargs):
    ...
    logits, audio_features_len, ctc_logprobs_list = model(
        raw_audio=raw_audio,
    ...
    for logprobs, layer_index, scale in zip(ctc_logprobs_list, model.return_layers, model.scales):
