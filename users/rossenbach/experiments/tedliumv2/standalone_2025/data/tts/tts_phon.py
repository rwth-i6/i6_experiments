...
from i6_experiments.users.rossenbach.datasets.tedliumv2 import get_tedliumv2_tts_segments


from ..common import get_audio_raw_datastream, TrainingDatasets
from ...default_tools import RETURNN_EXE, MINI_RETURNN_ROOT

DATA_PREFIX = "experiments/tedliumv2/standalone_2025/tts_data/"
...
def get_tts_extended_bliss(ted_corpus_key: str) -> tk.Path:
    """
    ...
    :return:
    """
    ls_bliss = get_bliss_corpus_dict(audio_format="wav")[ted_corpus_key]
    lexicon = get_lexicon()
...
def get_tts_bliss_and_zip(ted_corpus_key, silence_preprocessed=False):
    """
    :param ted_corpus_key: e.g. "train", "dev", "test"
    ...
    bliss_dataset = get_tts_extended_bliss(ted_corpus_key=ted_corpus_key)

    zip_dataset = BlissToOggZipJob(
        bliss_corpus=bliss_dataset,
        no_conversion=False,  # explicitly for tedlium
...
def get_lexicon(with_blank: bool = False) -> tk.Path:
    ...
    """
    lexicon = get_g2p_augmented_bliss_lexicon(
        add_silence=False
    )
...
def get_tts_log_mel_datastream(silence_preprocessed=False) -> AudioFeatureDatastream:
    """
    ...
    _, train_ogg_zip = get_tts_bliss_and_zip(ted_corpus_key="train", silence_preprocessed=silence_preprocessed)
    train_segments, _ = get_tedliumv2_tts_segments()
...
def build_durationtts_training_dataset(
    ...
    train_segments, cv_segments = get_tedliumv2_tts_segments()
