...
from ..default_tools import RETURNN_CPU_EXE, RETURNN_ROOT, SCTK_BINARY_PATH
...
def search(prefix_name, returnn_config, checkpoint, test_dataset_tuples, returnn_exe, returnn_root):
    ...
    wers = {}
    for key, (test_dataset, swb_eval_dataset) in test_dataset_tuples.items():
        wers[key] = search_single(
            prefix_name + "/%s" % key, returnn_config, checkpoint, test_dataset,
            eval_bliss=swb_eval_dataset.bliss_corpus,
            eval_stm=swb_eval_dataset.stm,
            eval_glm=swb_eval_dataset.glm,
