...
def get_tts_data_from_ctc_align(alignment_hdf, ls_corpus_key="train-clean-100"):
    """
    ...
    raw_sample_datastream = get_audio_raw_datastream()
    ...
    viterbi_job = ViterbiAlignmentToDurationsJob(
        alignment_hdf, bliss_lexicon=get_lexicon(with_blank=True), returnn_root=RETURNN_ROOT, time_rqmt=4, mem_rqmt=16,
        blank_token=43,
    )
    ...
    datastreams = {
        "audio_samples": raw_sample_datastream,
    ...
    train_ogg_zip = OggZipDataset(
        path=zip_dataset,
        audio_options=raw_sample_datastream.as_returnn_audio_opts(),
