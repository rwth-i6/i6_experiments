...
class FeatureBroadcasting(nn.Module):

    def __init__(
            self,
            feature_in_dim: nn.Dim,
            feature_out_dim: nn.Dim,
            hidden_size: int,
    ):
        super().__init__()
        lstm_hidden = nn.FeatureDim("lstm_hidden", hidden_size)
        linear_out = nn.FeatureDim("linear_out", hidden_size)
        self.linear = nn.Linear(in_dim=feature_in_dim, out_dim=linear_out)
        self.lstm1fw = nn.LSTM(in_dim=feature_in_dim, out_dim=lstm_hidden)
        self.lstm1bw = nn.LSTM(in_dim=feature_in_dim, out_dim=lstm_hidden)
        self.lstm2fw = nn.LSTM(in_dim=2*lstm_hidden, out_dim=lstm_hidden)
        self.lstm2bw = nn.LSTM(in_dim=2*lstm_hidden, out_dim=lstm_hidden)
        self.out_linear = nn.Linear(in_dim=2*lstm_hidden+linear_out, out_dim=feature_out_dim)

    def __call__(self, inp: nn.Tensor, spatial_dim: nn.Dim):
        transformed = self.linear(inp)
        lstm1 = nn.concat_features(
            self.lstm1fw(inp, spatial_dim=spatial_dim, direction=1)[0],
            self.lstm1bw(inp, spatial_dim=spatial_dim, direction=-1)[0]
        )
        lstm2 = nn.concat_features(
            self.lstm2fw(lstm1, spatial_dim=spatial_dim, direction=1)[0],
            self.lstm2bw(lstm1, spatial_dim=spatial_dim, direction=-1)[0]
        )
        add = lstm1 + lstm2
        final_cat = nn.concat_features(add, transformed)
        out = self.out_linear(final_cat)
        return out
...
class NARTTSModel(nn.Module):
    ...
    def __init__(
        ...
        self.feature_broadcasting = FeatureBroadcasting(
            feature_in_dim=self.decoder.linear_dim,
            feature_out_dim=self.feature_extractor.out_linear_feature_dim,
            hidden_size=512,
        )
    ...
    def __call__(
        ...
        dec_log_lin = self.feature_broadcasting(inp=dec_mel, spatial_dim=rep_dim)
        ...
        if self.training:
            ...
            audio_linear_features, _ = nn.reinterpret_new_dim(
                audio_linear_features,
                in_dim=audio_linear_features.feature_dim,
                out_dim=dec_log_lin.feature_dim,
                name="reinterpret_loglin"
            )
            ...
            # loss on log 10 level
            dec_loglin_loss = nn.mean_absolute_difference(dec_log_lin, nn.safe_log(audio_linear_features, eps=1e-10)/np.log(10))
            dec_loglin_loss.mark_as_loss("decoder_loglin_mea_loss")

            dec_lin_loss = nn.mean_absolute_difference(10**dec_log_lin, audio_linear_features)
            dec_lin_loss.mark_as_loss("decoder_lin_mea_loss", scale=0.05)

        return 10**dec_log_lin
