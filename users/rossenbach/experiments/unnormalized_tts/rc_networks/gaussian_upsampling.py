...
class GaussianUpsampling(nn.Module):
    ...
    def __call__(
        ...
        # causes https://github.com/rwth-i6/returnn/issues/1252:w
        #t, out_time = nn.range_from_length(l, out_spatial_dim=out_time_axis)  # [B,T]

        t, temp_out_time_axis = nn.range_from_length(l)
        t, _ = nn.reinterpret_new_dim(t, in_dim=temp_out_time_axis, out_dim=out_time_axis)
