...
class Model(BaseTTSModelV1):
    ...
    def forward(
        ...
        if target_durations is not None:
            ...
            # subtract one as we added one during training targets
            masked_durations = torch.ceil(torch.exp(log_durations) - 1) * h_mask  # [B, 1, N]
...
def train_step(*, model: Model, data, run_ctx, **kwargs):
    ...
    # add one to allow for zero predictions
    log_duration_targets = torch.log(torch.clamp_min(durations + 1, 1))
