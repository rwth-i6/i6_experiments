...
class Config():
    ...
    def from_dict(cls, d):
        d = d.copy()
        d["fe_config"] = DbMelFeatureExtractionConfig(**d["fe_config"])
        return Config(**d)
...
class Model(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.n_layers_enc = config.n_layers_enc
        self.p_dropout = config.p_dropout
        self.n_blocks_dec = config.n_blocks_dec
        self.kernel_size_dec = config.kernel_size_dec
        self.dilation_rate = config.dilation_rate
        self.n_block_layers = config.n_block_layers
        self.p_dropout_dec = config.p_dropout_dec
        ...
        self.hidden_channels_enc = config.hidden_channels_enc
        self.hidden_channels_dec = config.hidden_channels_dec
        self.prenet = config.prenet

        self.feature_extraction = DbMelFeatureExtraction(config=config.fe_config)

        self.encoder = TextEncoder(
            config.n_vocab,
            config.out_channels,
            config.hidden_channels_enc or config.hidden_channels,
        ...
        self.decoder = FlowDecoder(
            config.out_channels,
            config.hidden_channels_dec or config.hidden_channels,
            config.kernel_size_dec,
            config.dilation_rate,
            config.n_blocks_dec,
            config.n_block_layers,
            p_dropout=config.p_dropout_dec,
    ...
    def forward(
        ...
        if gen:
            ...
            return (y, z_m, z_logs, logdet, z_mask, y_lengths), (x_m, x_logs, x_mask), (attn, logw, logw_, w_ceil)
        else:
            ...
            logw_ = torch.log(1e-8 + torch.sum(attn, -1)) * x_mask
            return (z, z_m, z_logs, logdet, z_mask), (x_m, x_logs, x_mask), (y_lengths, feature_lengths), (attn, logw, logw_)
...
def forward_init_hook(run_ctx, **kwargs):
    ...
    num_freq = 800
    run_ctx.iverse_mel = torchaudio.transforms.InverseMelScale(num_freq // 2 + 1, 80, f_min=0, f_max=7600, sample_rate=16000, driver="gelsd")
    run_ctx.griffin_lim = torchaudio.transforms.GriffinLim(num_freq, n_iter=32, win_length=int(0.05 * 16000),
...
def forward_step(*, model: Model, data, run_ctx, **kwargs):
    ...
    log_mels *= run_ctx.norm_std
    log_mels += run_ctx.norm_mean
    log_mels /= 20
    mels = torch.pow(10, log_mels)

    from matplotlib import pyplot as plt

    for mel, log_mel, length, tag in zip(mels, log_mels, y_lengths, tags):
        print(mel.shape)
        print(length)
        print(tag)
        power_spec = run_ctx.iverse_mel(mel[:,:length])
        wave = run_ctx.griffin_lim(power_spec).cpu().numpy()
