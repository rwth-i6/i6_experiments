...
from recipe.i6_experiments.users.enrique.jobs.fairseq.wav2vec.w2vu_generate_job import FairseqGenerateWav2VecUJob, ViterbiGenerateWav2VecUJob
...
def run_meta_experiments():
    ...
    #models_to_decode_with = [i for i in range(0, 40)] 
    #models_to_decode_with = [0, 9, 29, 6, 15 ,28, 19 ,38 ,37]
    models_to_decode_with = [9]

    decode_training_data = True
    models_to_decode_training_data_with = [0, 9, 29, 6, 15 ,28, 19 ,38 ,37] # 9, 29 best WER  6 15 28 best word per  19 38 37 best phn per
    models_to_decode_training_data_with = []
        ...
        for it in models_to_decode_with:
            ...
            for lm_job in prepare_text_job_decoding:
                ...
                job_alias = os.path.join(
                    ...
                    "decoding",
                    decoding_audio_name,
                    f"t_{it}", f"pruning_" + "_".join(map(str, lm_job.lm_pruning))
                )
                ...
                stats[decoding_audio_name][it]["_".join(map(str, lm_job.lm_pruning))] = {}
                if sclite_job.out_wer.get() is not None:
                    stats[decoding_audio_name][it]["_".join(map(str, lm_job.lm_pruning))]["wer"] = sclite_job.out_wer.get()
                ...
                tk.register_output(os.path.join(text_alias, "pruning_" + "_".join(map(str, lm_job.lm_pruning)), "perplexity"), calculate_word_perplexity_job.output_perplexity_var)
                ...
                if calculate_word_perplexity_job.output_perplexity_var.get() is not None:
                    stats[decoding_audio_name][it]["_".join(map(str, lm_job.lm_pruning))]["word_per"] = calculate_word_perplexity_job.output_perplexity_var.get()
                ...
                tk.register_output(os.path.join(text_alias, "pruning_" + "_".join(map(str, lm_job.lm_pruning)), "word_perplexity"), calculate_word_perplexity_job.output_perplexity_var)
                calculate_word_perplexity_job.add_alias(os.path.join(text_alias, "pruning_" + "_".join(map(str, lm_job.lm_pruning)), "word_perplexity_ground_truth"))
                ...
                tk.register_output(os.path.join(text_alias, "pruning_" + "_".join(map(str, lm_job.lm_pruning)), "phoneme_perplexity"), calculate_phoneme_perplexity_job.output_log2_prob_var)
                ...
                if calculate_phoneme_perplexity_job.output_log2_prob_var.get() is not None:
                    stats[decoding_audio_name][it]["_".join(map(str, lm_job.lm_pruning))]["log2_prob"] = calculate_phoneme_perplexity_job.output_log2_prob_var.get()
    ...
    best_models = UnsupervisedVocabUsageAndLMScoreMetric(transcripts_and_models=transcripts_and_models, vocab_path=prepare_text_job_training.processed_dict_phn_txt, kenlm_path=prepare_text_job_training.text_phones_lm_filtered_04_bin, sil="<SIL>", allow_margin=1.2).best_to_worst_models

    decode_with_n_best_unsup_metric_models = 1
    for i, model in enumerate(best_models[:decode_with_n_best_unsup_metric_models]):
        for dec_config in decoding_datasets:
            decoding_audio_name = dec_config["name"]
            decoding_audio_alias = os.path.join(audio_alias, decoding_audio_name)
            ...
            stats[decoding_audio_name][f"best_model_{i}"]={}
            for lm_job in prepare_text_job_decoding[:]:
                generate_job = FairseqGenerateWav2VecUJob(
                    decoding_audio_name=decoding_audio_name,
                    environment=environment,
                ...
                job_alias = os.path.join(
                    ...
                    "decoding",
                    decoding_audio_name,
                    f"best_unsup_metric_model_{i}", f"pruning_" + "_".join(map(str, lm_job.lm_pruning))
                )
                output_path = os.path.join(
                    alias, "decoding_results", decoding_audio_name, f"pruning_{lm_job.lm_pruning}"
                )
                ...
                sclite_job.add_alias(os.path.join(job_alias, "wer"))
                tk.register_output(os.path.join(output_path, "wer"), sclite_job.out_report_dir)
                stats[decoding_audio_name][f"best_model_{i}"]["_".join(map(str, lm_job.lm_pruning))] = {}
                if sclite_job.out_wer.get() is not None:
                    stats[decoding_audio_name][f"best_model_{i}"]["_".join(map(str, lm_job.lm_pruning))]["wer"] = sclite_job.out_wer.get()
                ...
                calculate_word_perplexity_job.add_alias(os.path.join(job_alias, "word_perplexity"))
                if calculate_word_perplexity_job.output_perplexity_var.get() is not None:
                    stats[decoding_audio_name][f"best_model_{i}"]["_".join(map(str, lm_job.lm_pruning))]["word_per"] = calculate_word_perplexity_job.output_perplexity_var.get()
                print(f"Best unsupervised metric model for {decoding_audio_name} registered with WER {stats[decoding_audio_name][f'best_model_{i}']['_'.join(map(str, lm_job.lm_pruning))].get('wer')}")
    ...
    for dec_config in stats.keys():
        ...
        for model_it in stats[dec_config].keys():
            for lm_prun in stats[dec_config][model_it].keys():
                # if "wer" not in stats[dec_config][model_it][lm_prun]:
                #     continue
                # if "word_per" not in stats[dec_config][model_it][lm_prun]:
                #     continue
                # if "log2_prob" not in stats[dec_config][model_it][lm_prun]:
                #     continue

                wer = stats[dec_config][model_it][lm_prun].get("wer", float('inf'))
                word_per = stats[dec_config][model_it][lm_prun].get("word_per", float('inf'))
                log2_prob = stats[dec_config][model_it][lm_prun].get("log2_prob", float('-inf'))
                results.append((model_it, lm_prun, wer, word_per, log2_prob))

        top_by_wer = sorted(results, key=lambda x: x[2])[:n_best]
        top_by_word_per = sorted(results, key=lambda x: x[3])[:n_best]
        top_by_log2_prob = sorted(results, key=lambda x: x[4])[-n_best:]

        print(f"\nTop {n_best} models for {dec_config} by WER:")
        for model_it, lm_prun, wer, word_per, log2_prob in top_by_wer:
            print(f"  Model {model_it}, LM pruning {lm_prun}: WER={wer}, Word Perplexity={word_per}, log_prob phoneme={log2_prob}")
    ...
    if decode_training_data:
        for t in models_to_decode_training_data_with:
            ...
            for lm_job in prepare_text_job_decoding[:1]:
                generate_job_valid = FairseqGenerateWav2VecUJob(
                ...
                generate_job_train = FairseqGenerateWav2VecUJob(
                    decoding_audio_name=training_audio,
                    environment=environment,
                    fairseq_root=fairseq_root_decoding,
                    task_data=featurize_training_audio_job.out_features_precompute_pca512_cls128_mean,
                ...
                divide_job = DivideLibriSpeech960hInto100h360h500hJob([generate_job_train.transcription_generated_formatted, generate_job_valid.transcription_generated_formatted])
                ...
                print(os.path.join(alias, "decoding", training_audio, "train_valid_divided", f"t_{t}", "pruning_" + "_".join(map(str, lm_job.lm_pruning))))
                print(divide_job.out_label_path_dict.get_path())
