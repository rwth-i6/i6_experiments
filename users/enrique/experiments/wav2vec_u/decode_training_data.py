...
def run_meta_experiments():
    ...
    models_ids_to_decode_with = [0, 24]
    ...
    delete_silences_job, featurize_training_audio_job, process_audio_manifest = process_audio(
        env=environment,
    ...
    for it in models_ids_to_decode_with:
        training_job = training_jobs[it]
        logging.info(training_job.out_dir.get_path())
        for lm_job in prepare_text_job_decoding:
            ...
            job_alias = os.path.join(
                ...
                "decoding",
                decoding_audio_name,
                f"t_{it}", "pruning_" + "_".join(map(str, lm_job.lm_pruning))
            )
