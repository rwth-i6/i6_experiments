...
gan_rasr_config = {
    ...
    'language_model': {
        'lexicon_path': tk.Path("/work/smt4/zeineldeen/enrique.leon.lozano/setups-data/ubuntu_22_setups/fairseq_2025_06_02/work/i6_experiments/users/enrique/jobs/fairseq/wav2vec/wav2vec_data_utils/PrepareWav2VecTextDataJob.RZfllsI3R2Pd/output/text/lexicon_filtered.lst"),
...
def run_rasr_experimets():
    ...
    for dec_config in decoding_datasets:
        ...
        for it in models_to_decode_with:
            for rasr_config in gan_rasr_init_configs:
                ...
                config_copy['audio_features_to_decode_folder'] = decoding_featurize_job.out_features_precompute_pca512_cls128_mean
    ...
    for gan_rasr_config in gan_rasr_configs:
        lexicon_xml_path = LexiconFromTextFileJob(text_file=gan_rasr_config["language_model"]["lexicon_path"], labels=get_default_phn_order()).out_bliss_lexicon
        ...
        rasr_config_file = GetTreeTimesyncRecogConfigJob(
            ...
            logfile_suffix="decoding", lm_path=gan_rasr_config["language_model"]["lm_path"],
    ...
    if decode_training_data:
        ...
        for t in models_to_decode_training_data_with:
            ...
            for rasr_config in gan_rasr_init_configs:
                ...
                gan_rasr_config['audio_features_to_decode_folder'] = featurize_training_audio_job.out_features_precompute_pca512_cls128_mean
                ...
                divide_job = DivideLibriSpeech960hInto100h360h500hJob([fairseq_rasr_decode_train_job.transcription_generated_formatted, fairseq_rasr_decode_valid_job.transcription_generated_formatted])
                divide_job.add_alias(os.path.join(alias,"rasr", "decoding", decoding_audio_name, "train_valid_divided", f"t_{t}"))
                tk.register_output(os.path.join(alias,"rasr", "decoding", decoding_audio_name, "train_valid_divided", f"t_{t}"), divide_job.out_label_path_dict)
