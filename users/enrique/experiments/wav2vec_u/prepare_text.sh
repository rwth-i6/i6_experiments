...
lid_path=$6
sil_prob=$7
# Accept the pruning thresholds as the 8th argument, for example: "0 0 0 3"
pruning_threshold_1=$8
pruning_threshold_2=$9
pruning_threshold_3=${10}
pruning_threshold_4=${11}
...
echo "pruning thresholds are $pruning_threshold_1 $pruning_threshold_2 $pruning_threshold_3 $pruning_threshold_4"
...
# Use the input pruning thresholds and change the output filename
$KENLM_ROOT/lmplz -o 4 < $target_dir/lm.upper.lid.txt --discount_fallback --prune $pruning_threshold_1 $pruning_threshold_2 $pruning_threshold_3 $pruning_threshold_4 >! $target_dir/kenlm.wrd.o4.arpa
# Update the filenames for the binary creation
$KENLM_ROOT/build_binary $target_dir/kenlm.wrd.o4.arpa $target_dir/kenlm.wrd.o4.bin

# Update the arpa file path for the kaldi initializer
lg=$lg python $FAIRSEQ_ROOT/examples/speech_recognition/kaldi/kaldi_initializer.py kaldi_root=$KALDI_ROOT fst_dir=$target_dir/fst/phn_to_words_sil lm_arpa=$target_dir/kenlm.wrd.o4.arpa wav2letter_lexicon=$target_dir/lexicon_filtered.lst data_dir=$target_dir/phones in_labels=phn "blank_symbol='<SIL>'"
lg=$lg python $FAIRSEQ_ROOT/examples/speech_recognition/kaldi/kaldi_initializer.py kaldi_root=$KALDI_ROOT fst_dir=$target_dir/fst/phn_to_words lm_arpa=$target_dir/kenlm.wrd.o4.arpa wav2letter_lexicon=$target_dir/lexicon_filtered.lst data_dir=$target_dir/phones in_labels=phn
