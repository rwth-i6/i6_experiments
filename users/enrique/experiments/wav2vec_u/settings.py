...
def engine():
    from sisyphus.sisyphus.engine import EngineSelector
    from sisyphus.sisyphus.localengine import LocalEngine

    return LocalEngine(cpus=4, gpus=0, mem=6)
    # Example how to use the engine selector, normally the 'long' engine would be a grid engine e.g. SGE
    # noinspection PyUnreachableCode
    return EngineSelector(
        engines={"short": LocalEngine(cpus=6, gpus=1), "long": LocalEngine(cpus=8, gpus=0)}, default_engine="long"
    )
