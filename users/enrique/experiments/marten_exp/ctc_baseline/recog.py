...
from i6_experiments.users.mueller.experiments.ctc_baseline.ctc import model_recog_lm, model_recog_flashlight, \
  model_recog_lm_albert, model_recog
from i6_experiments.users.enrique.experiments.marten_exp.ctc_baseline.ctc import model_recog as model_recog_schmitt
from i6_experiments.users.enrique.experiments.marten_exp.ctc_baseline.ctc import model_recog_gradients
from i6_experiments.users.enrique.experiments.marten_exp.language_models.n_gram import get_kenlm_n_gram, get_binary_lm, \
  ...
  def __call__(self, epoch_or_ckpt: Union[int, PtCheckpoint]) -> tuple[ScoreResultCollection, tk.Path, tk.Path]:
    ...
    if self.calculate_scores and isinstance(epoch_or_ckpt, int) and self.register_output:
      ...
      for dataset_name, score_out in score_outs.items():
        tk.register_output(self.prefix_name + f"/recog_results_per_epoch/{epoch_or_ckpt:03}/{dataset_name}_report",
                           score_out.report)
...
def recog_model(
  ...
  for dataset_name, dataset in task.eval_datasets.items():
    ...
    if save_pseudo_labels:
      if ("ps_nbest" in decoder_hyperparameters and decoder_hyperparameters[
        "ps_nbest"] > 1) or pseudo_label_alignment or recog_def is model_recog_gradients or return_beam:
        recog_paths[dataset_name] = beam_recog_out
      else:
        recog_paths[dataset_name] = recog_out.output
      prev_outs[dataset_name] = prev_out
  return task.collect_score_results_func(
    outputs) if calculate_scores else None, recog_paths if save_pseudo_labels else None, prev_outs if save_pseudo_labels else None, score_outs
  ...
  class _ReturnnRecogV2ForwardCallbackIface(ForwardCallbackIface):
    ...
    def init(self, *, model):
      ...
      if previous_hyps is not None:
        ...
        self.previous_hyps = eval(gzip.open(d["prev"][dataset_name], "rt").read(),
  ...
  def update(self):
    ...
    if not self._update_checked_relevant_epochs and self.exp.scores_and_learning_rates.available():
      ...
      with log_stream:
        ...
        if self.exp.get_training_job().returnn_config.post_config["num_epochs"] != 1 and "full-sum-sup-init" not in \
                self.exp.get_training_job().returnn_config.post_config["alias"]:
