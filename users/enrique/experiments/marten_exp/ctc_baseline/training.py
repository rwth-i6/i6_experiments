...
from i6_experiments.users.enrique.jobs.initializations.k_means.models import Wav2VecModel, KMeansModel, Wav2VecKMeansModel
from i6_experiments.users.enrique.experiments.marten_exp.ctc_baseline.decoding import recog_ffnn, \
...
def ctc_train(*, model: Union[Model, Wav2VecModel], data: rf.Tensor, data_spatial_dim: Dim, targets: rf.Tensor,
  ...
  if decode_every_step and rf.get_run_ctx().train_flag:
    ...
    with torch.no_grad():
      ...
      hyps, new_scores = recog_ffnn(model=model, label_log_prob=log_probs, enc_spatial_dim=enc_spatial_dim,
                                    hyperparameters=hyperparameters, batch_dims=batch_dims, prior_file=prior_file,
                                    train_lm=True)
      ...
      targets, targets_spatial_dim = _compare_targets(targets, targets_spatial_dim, new_targets,
                                                      new_targets_spatial_dim, keep_old, model, hyperparameters,
                                                      version=version)
  ...
  if nbest > 1 and rf.get_run_ctx().train_flag:
    ...
    for j in range(nbest):
      ...
      if norm_rescore:
        ...
        with torch.no_grad():
          lm_prior_score = _rescore(targets_s, targets_spatial_dim_s, model, hyperparameters,
                                    prior_file if not rescore_alignment_prior else None).raw_tensor
      ...
      if aux_loss_layers:
        for i, layer_idx in enumerate(aux_loss_layers):
          ...
          if j > 0:
            ...
            aux_loss_rescored = torch.where(targets_spatial_dim_s.dyn_size_ext.raw_tensor == 0, float("-inf"),
                                            aux_loss_rescored)
  ...
  sup_loss = config.typed_value("sup_loss", "ctc")
...
def full_sum_train(*, model: Model, data: rf.Tensor, data_spatial_dim: Dim, seq_tags: rf.Tensor = None,
                   targets: rf.Tensor, targets_spatial_dim: Dim):
  """Function is run within RETURNN."""
  from returnn.config import get_global_config
  from i6_experiments.users.enrique.experiments.marten_exp.ctc_baseline.sum_criterion import sum_loss_bigram, \
  ...
  fixed_seqs = ["train-other-500/5756-305214-0041/5756-305214-0041",
                "train-clean-360/2498-134786-0003/2498-134786-0003",
                "train-other-500/1643-138089-0068/1643-138089-0068"]  # MONICA DREW FRESH HOPE FROM HER SON'S WRITINGS THEY WERE FULL OF NOBLE THOUGHTS AND HIGH ASPIRATIONS, HERE IT IS
  print_for_idx = []
  ...
  if print_gradients and print_for_idx:
    ...
    for idx_t in print_for_idx:
      ...
      log_probs_raw = pg(log_probs_raw, "log_probs", alias_name, False, None, True, idx_t, [], ["Log Probs"],
  ...
  if use_prior:
    if empirical_prior is not None:
      ...
      if blank_prior:
        assert log_prior.size(0) == log_probs_raw.size(
          2), f"Empirical prior size does not match (full_sum) ({log_prior.size(0)} != {log_probs_raw.size(2)})!"
      else:
        assert log_prior.size(0) == log_probs_raw.size(
          2) - 1, f"Empirical prior size does not match (full_sum) ({log_prior.size(0)} != {log_probs_raw.size(2)} - 1)!"
  ...
  prior_penalty_opts = config.typed_value("prior_penalty_opts", {})
...
def ce_train(*, model: Model, data: rf.Tensor, data_spatial_dim: Dim, targets: rf.Tensor, targets_spatial_dim: Dim,
  ...
  logits = rf.replace_dim_v2(logits, in_dim=enc_spatial_dim, out_dim=targets_spatial_dim, allow_expand=False,
                             allow_shrink=False)
...
def seq_gamma_ctc_train(*, model: Model, data: rf.Tensor, data_spatial_dim: Dim, targets: rf.Tensor,
                        targets_spatial_dim: Dim, nbest_lengths: rf.Tensor = None, scores: rf.Tensor = None,
                        seq_tags: rf.Tensor = None):
  """Function is run within RETURNN."""
  from returnn.config import get_global_config
  ...
  if nbest > 1 and rf.get_run_ctx().train_flag:
    ...
    if aux_loss_layers:
      for i, layer_idx in enumerate(aux_loss_layers):
        ...
        with torch.no_grad():
          ...
          assert not (aux_nbest_scores_i_sum == float(
            "-inf")).any(), f"-inf in {aux_nbest_scores_i_sum.tolist()}\noriginal: {aux_nbest_scores_i.tolist()}"
          ...
          assert aux_nbest_scores_i_norm.sum(dim=-1).allclose(torch.tensor(1.0, device=aux_nbest_scores_i.device),
                                                              rtol=0.001), f"Prior probs do not sum to 1.0, but to {aux_nbest_scores_i_norm.sum(dim=-1)}"
          aux_targets = rtf.TorchBackend.convert_to_tensor(aux_nbest_scores_i_norm, dims=[batch_dim, nbest_dim],
                                                           dtype="float32", name=f"seq_gammas_{layer_idx}")

        aux_nbest_scores_i = torch.where(aux_nbest_scores_i == float("-inf"), 0.0, aux_nbest_scores_i)
        aux_nbest_scores_i = rtf.TorchBackend.convert_to_tensor(aux_nbest_scores_i, dims=[batch_dim, nbest_dim],
                                                                dtype="float32", name=f"am_lm_pr_scores_{layer_idx}")
        aux_loss = rf.cross_entropy(
    ...
    with torch.no_grad():
      ...
      assert not (nbest_scores_exp_sum == float(
        "-inf")).any(), f"-inf in {nbest_scores_exp_sum.tolist()}\noriginal: {nbest_scores.tolist()}"
      nbest_scores_norm_log = scores_scaled - nbest_scores_exp_sum
      nbest_scores_norm = nbest_scores_norm_log.exp()
      assert nbest_scores_norm.sum(dim=-1).allclose(torch.tensor(1.0, device=nbest_scores.device),
                                                    rtol=0.001), f"Prior probs do not sum to 1.0, but to {nbest_scores_norm.sum(dim=-1)}"
      targets = rtf.TorchBackend.convert_to_tensor(nbest_scores_norm, dims=[batch_dim, nbest_dim], dtype="float32",
                                                   name=f"seq_gammas")

    nbest_scores = torch.where(nbest_scores == float("-inf"), 0.0, nbest_scores)
    nbest_scores = rtf.TorchBackend.convert_to_tensor(nbest_scores, dims=[batch_dim, nbest_dim], dtype="float32",
                                                      name=f"am_lm_pr_scores")
    loss = rf.cross_entropy(
...
def _rescore(
  ...
  if prior_file and prior_weight > 0.0:
    ...
    if prior.size(0) == int(model.wb_target_dim.get_dim_value()):
      ...
      assert prior.size(0) == int(
        model.target_dim.get_dim_value()), f"Prior size does not match! {prior.size(0)} vs {int(model.target_dim.get_dim_value())}"
    prior *= prior_weight
...
def _blank_penalty(log_probs, model, blank_penalty_opts):
  import numpy as np
  from returnn.config import get_global_config
  config = get_global_config()  # noqa

  blank_log_probs = rf.gather(log_probs, indices=model.blank_idx, axis=model.wb_target_dim)
  mean_blank_log_prob = rf.reduce_mean(blank_log_probs, axis=blank_log_probs.dims)

  target_mean_blank_prob = blank_penalty_opts["target_mean_blank_prob"]
  target_mean_blank_log_prob = np.log(target_mean_blank_prob)

  blank_penalty = rf.pow(mean_blank_log_prob - target_mean_blank_log_prob, 2)

  blank_penalty.mark_as_loss(
    "blank_penalty",
    # custom_inv_norm_factor=enc_spatial_dim.get_size_tensor(),
  )
...
# TODO: implement correctly in log space
def _prior_penalty(log_probs, model, opts, log_prior):
  import numpy as np
  from returnn.config import get_global_config
  config = get_global_config()  # noqa

  assert model.blank_idx == model.wb_target_dim.dimension - 1

  mean_log_prob = rf.reduce_mean(log_probs, axis=log_probs.remaining_dims(model.wb_target_dim))

  log_prior = rf.Tensor(
      "log_prior",
      dims=[model.wb_target_dim],
      dtype=rf.get_default_float_dtype(),
      raw_tensor=log_prior,
    )

  prior_penalty = rf.pow(mean_log_prob - log_prior, 2) * opts["scale"]
