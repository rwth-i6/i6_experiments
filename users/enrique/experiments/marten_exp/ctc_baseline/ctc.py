...
from i6_experiments.users.enrique.experiments.marten_exp.language_models.n_gram import (
    get_count_based_n_gram,
    get_prior_from_unigram,
)
from i6_experiments.users.enrique.experiments.marten_exp.language_models.ffnn import (
    FeedForwardLm,
    get_ffnn_lm,
    FeedForwardLmV2,
)
from i6_experiments.users.enrique.experiments.marten_exp.ctc_baseline.configs import (
...
from i6_experiments.users.enrique.experiments.marten_exp.ctc_baseline.decoding import (
    recog_flashlight_ngram,
    recog_no_lm,
    recog_flashlight_ffnn,
    recog_ffnn,
    recog_gradients,
)
from i6_experiments.users.enrique.experiments.marten_exp.ctc_baseline.training import (
    ctc_train,
    full_sum_train,
    ce_train,
    seq_gamma_ctc_train,
...
from i6_experiments.users.enrique.experiments.marten_exp.ctc_baseline.rasr import (
    get_bpe_lexicon,
    get_librasr_fsa_config,
)

from i6_experiments.users.enrique.jobs.initializations.k_means.returnn_kmeans.k_means_training import (
    k_means_guided_by_LM_train,
)
from i6_experiments.users.enrique.jobs.initializations.k_means.models import Wav2VecModel, Wav2VecKMeansModel
from i6_experiments.users.enrique.jobs.initializations.k_means.pca import compute_PCA_matrixes
...
starting_config_id = 5
last_config_id = 6

returnn_root = tk.Path("/u/marten.mueller/dev/ctc_baseline/returnn")
returnn_python_exe = tk.Path(sys.executable)
executables = {
...
get_librasr_fsa_config(lexicon_path=bpe_lexicon, corpus_path=merged_corpus)
...
def py():
    ...
    k_means = (True,)
    num_clusters = (200,)
    pca_dim = 100  # Dimensionality of PCA, None for no PCA
    n_points_to_calculate_pca = 10_000  # Number of points to use for PCA calculation
    ...
    decoder_lm_config = {
        "class": "FeedForwardLm",
        "context_size": 8,
    }  # LM selection for decoding, empty for word-level 4-gram
    ...
    correction_in_final_score = (
        False  # Apply the correction in the final score as well instad of just using the corrections duing top-k
    )
    ...
    wav2vec_base_config_v2 = dict_update_deep(wav2vec_base_config_v1, {"epochs": 20, "bs_feat": 20_000})

    wav2vec_base_config_v3 = dict_update_deep(wav2vec_base_config_v1, {"epochs": 16, "bs_feat": 20_000})
    ...
    train_configs = [
        ...
        *[
            (
                dict_update_deep(wav2vec_base_config_v1, {"num_enc_layers": n}),
        ...
        *[
        ...
        # ----------------------- longer training -------------------------------
        (full_sum_config_v2_long, (f"full-sum-v2/nep-{80}_sgpu_am-{0.1}-lm-{1.5}-prior-{0.4}")),
        ...
        *[
            ...
            for n_epochs, am_scale, lm_scale, prior_scale, accum_grad, peak_lr, lr_schedule_type, blank_penalty_opts in [
                ...
                (
                    5,
                    {"steps": [500, 1_500], "values": [0.1, 0.1, 0.4]},
                    0.5,
                    {"steps": [500, 1_500], "values": [0.4, 0.4, 0.8]},
                    1,
                    1e-4,
                    "static",
                    None,
    ...
    for train_config, alias_name in train_configs[starting_config_id:last_config_id]:

        print(f"Config: {alias_name} ({len(train_config)})")
        print(train_config)
        ...
        if lr_schedule_type == "static":
            ...
            if lr_schedule_type == "relative_oclr":
                ...
                lr_config = _get_cfg_lrlin_oclr_by_bs_nep_v5(
                    epochs, const_lr=peak_lr, low_lr=1e-6, step_peak_fraction=0.1, step_const_fraction=0.5
                )
        ...
        if decoding_imp == "marten-greedy":
            decoder_hyperparameters = {"greedy": True}
            ...
            if decoder_lm_config:
                decoder_hyperparameters["lm_order"] = (
                    decoder_lm_config["order"]
                    if decoder_lm_config["class"] == "ngram"
            ...
            if (
                decode_every_step
                or pseudo_nbest > 1
                or decode_every_step_init
                or use_sum_criterion
                or init == "100h-unsupervised"
            ):
            ...
            if decoding_imp == "gradients":
                ...
                decoder_hyperparameters_grad["lm_order"] = (
                    train_lm_config["order"]
                    if train_lm_config["class"] == "ngram"
                    else f"ffnn{train_lm_config['context_size']}"
                )
            ...
            if alt_decoder:
                ...
                if decode_every_step:
                    ...
                    assert train_lm_config
                    every_step_hyperparameters["lm_order"] = (
                        train_lm_config["order"]
                        if train_lm_config["class"] == "ngram"
                        else f"ffnn{train_lm_config['context_size']}"
                    )
        ...
        if init.endswith("unsupervised"):
            if decode_every_step_init:
                ...
                every_step_hyperparameters_init["lm_order"] = (
                    train_lm_config["order"]
                    if train_lm_config["class"] == "ngram"
                    else f"ffnn{train_lm_config['context_size']}"
                )
        ...
        # config.pop("torch_distributed") #TODO: remove this line
            ...
            model_config.update(
                {
                    ...
                    "init_centroids": None,  # TODO: add init_centroids option
        ...
        if use_w2v_model:
            ...
            if train_config.get("w2v_model", "base") != "base":
                ...
                assert not (
                    w2v_config == "large" and "wav2vec-large-lv60-v2" not in alias_name
            ...
            if train_config.get("freeze_encoder_first_n_steps", 2_500) != 2_500:  # 4 GPUS, otherwise: 10_000
                ...
                pass
            if (
                train_config.get("unfrozen_encoder_layers", None) is not None
                and w2v_opts["freeze_encoder_first_n_steps"] > 0
            ):
            ...
            if pca_dim is not None:
                w2v_opts["pca_dim"] = pca_dim
                w2v_opts["n_points_to_calculate_pca"] = n_points_to_calculate_pca

            if k_means:
                w2v_opts["output_raw_features"] = True
        ...
        if self_training_rounds > 0:
            ...
            if not reset_steps:
                if True:
                    ...
                    peak_lr = 5e-4
                    config_updates_self_training["learning_rate_piecewise_values"] = [
                        peak_lr * 2e-2,
                        peak_lr,
                        peak_lr * 2e-2,
                        peak_lr * 2e-3,
            ...
            if use_sgd:
                config_updates_self_training["optimizer"] = {"class": "sgd"}
            if decoding_imp == "gradients":
...
def train_exp(
    ...
    *,
    ...
    from i6_experiments.users.enrique.experiments.marten_exp.ctc_baseline.recog import generate_pseudo_labels
    from i6_experiments.users.schmitt.datasets.librispeech import (
        get_librispeech_task_raw_v2_schmitt as get_librispeech_task_raw_v2,
    )
    ...
    if config_updates.get("decode_every_step", False):
        ...
        if config_updates.get("start_with_prior_gamma_steps", 0) > 0 and label_prior:
            config_updates["frame_prior"] = get_prior_from_unigram(
                task.prior_dataset.vocab, task.prior_dataset, vocab, False
            )
    ...
    if not model_def:
        if k_means:
            model_def = k_means_model_def
            model_def_self = k_means_model_def
        else:
    ...
    if model_config:
        ...
        if "output_bias_init" in mc and mc["output_bias_init"]:
            mc["output_bias_init"] = (
                "/u/marten.mueller/dev/ctc_baseline/work/i6_experiments/users/mueller/experiments/language_models/n_gram/ExtractPrior.Z4T3thKgQeci/output/prior.txt"
            )
    ...
    if tune_hyperparameters and tune_version >= 2:
        ...
        dec_params = (
            decoder_hyperparameters[0] if isinstance(decoder_hyperparameters, tuple) else decoder_hyperparameters
        )
        prior_tune, lm_tune = _tune_prior_and_lm(
            label_prior,
            task,
            emp_prior,
            search_config,
        ...
        if config_updates_self_training and "hyperparameters_decoder" in config_updates_self_training:
            ...
            if (
                config_updates_self_training["hyperparameters_decoder"]["lm_order"]
                != decoder_hyperparameters["lm_order"]
            ):
                assert train_lm is not None
                dec_params_train = config_updates_self_training["hyperparameters_decoder"]
                prior_tune_train, lm_tune_train = _tune_prior_and_lm(
                    label_prior,
                    task,
                    emp_prior,
                    search_config,
    ...
    recog_training_exp(
        ...
        exclude_epochs=(
            sorted(list(model_with_checkpoint[0].fixed_epochs))[:-1] if not decode_all_fixed_epochs_init else ()
        ),
    ...
    for i in range(self_training_rounds):
        ...
        if tune_hyperparameters:
            ...
            if "hyperparameters_decoder" in config_updates_self_training:
                ...
                if (
                    config_updates_self_training["hyperparameters_decoder"]["lm_order"]
                    != hyperparamters_self_training["lm_order"]
                ):
                    dec_params_train = config_updates_self_training["hyperparameters_decoder"]
                    prior_tune_train, lm_tune_train = _tune_prior_and_lm(
                        label_prior,
        ...
        pseudo_label_path_dict = recog_training_exp(
            ...
            save_pseudo_labels=(
                None
                if not calc_last_pseudo_labels and i + 1 == self_training_rounds
                else (pseudo_labels_ds, train_100_ds)
            ),
            pseudo_label_alignment=use_ce_loss,
            pseudo_nbest=pseudo_nbest,
            ...
            exclude_epochs=(
                sorted(list(model_with_checkpoint[i + 1].fixed_epochs))[:-1] if not decode_all_fixed_epochs else ()
            ),
...
def _tune_prior_and_lm(
    ...
    from i6_experiments.users.mueller.scale_tune import (
...
def k_means_model_def(*, epoch: int, in_dim: Dim, target_dim: Dim) -> Wav2VecKMeansModel:
    ...
    if train_language_model is not None:
        ...
        cls_name = train_language_model.pop("class")
        assert cls_name == "FeedForwardLm"
        train_lm = FeedForwardLmV2(vocab_dim=target_dim, **train_language_model)
    ...
    if recog_language_model is not None:
        ...
        cls_name = recog_language_model.pop("class")
        assert cls_name == "FeedForwardLm"
        recog_lm = FeedForwardLmV2(vocab_dim=target_dim, **recog_language_model)
    ...
    if rescore_language_model is not None:
        ...
        cls_name = rescore_language_model.pop("class")
        assert cls_name == "FeedForwardLm"
        rescore_lm = FeedForwardLmV2(vocab_dim=target_dim, **rescore_language_model)

    w2v_opts = config.typed_value("w2v_opts", {})
    num_clusters = config.int("num_clusters", 200)
    init_centroids = config.bool("init_centroids", None)
    normalize_centroids = config.bool("normalize_centroids", False)

    return Wav2VecKMeansModel(
        w2v_opts=w2v_opts,
...
k_means_model_def: ModelDef[Model]
k_means_model_def.behavior_version = 21
k_means_model_def.backend = "torch"
k_means_model_def.batch_size_factor = _batch_size_factor
...
def ctc_training(
    *,
    ...
    nbest_lengths: rf.Tensor = None,
    scores: rf.Tensor = None,
