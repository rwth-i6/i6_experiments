...
from i6_experiments.users.mueller.datasets.librispeech import get_librispeech_lm_combined_txt, \
  _extract_audio_seq_len_file, _extract_text_seq_len_file, _get_corpus_text_dict, TextDictToTextLinesJob
...
from i6_experiments.users.mueller.experiments.language_models.n_gram import ApplyBPEToTextJob, KenLMplzJob, ExtractPrior, ConvertARPAtoTensor
...
def get_kenlm_n_gram(vocab: Bpe, N_order: int) -> tk.Path:
  ...
  bpe_text = ApplyBPEToTextJob(
    ...
    # i need to add Marten's abs path here, otherwise the hashes do not match
    bpe_vocab=tk.Path(
      "/u/marten.mueller/dev/ctc_baseline/work/i6_core/text/label/subword_nmt/train/ReturnnTrainBpeJob.P1DXd9G7EdsU/output/bpe.dummy_count.vocab"),
...
def get_prior_from_unigram(vocab: Bpe, prior_dataset: Optional[DatasetConfig], vocab_name: str,
                           label_prior: bool) -> tk.Path:
  kenlm_repo = CloneGitRepositoryJob("https://github.com/kpu/kenlm").out_repository.copy()
  KENLM_BINARY_PATH = CompileKenLMJob(repository=kenlm_repo).out_binaries.copy()
  KENLM_BINARY_PATH.hash_overwrite = "LIBRISPEECH_DEFAULT_KENLM_BINARY_PATH"
  ...
  tk.register_output(f"datasets/LibriSpeech/lm/{'label' if label_prior else 'frame'}_prior_from_unigram",
                     prior_job.out_prior_tensor)
...
#       if self.N_order == 2:
#         s = tensor.sum(1)
#         assert s[0].allclose(torch.tensor(1.0),
#                              atol=atol), f"The next word probabilities for <s> do not sum to 1! {s[0]}"
...
#       else:
#         assert (tensor.sum(
#           -1) < 1.005).all(), f"The next word probabilities are not smaller than 1! {tensor.sum(-1).flatten()[tensor.sum(-1).flatten() >= 1.005]}"
#       assert tensor.sum(tuple(range(0, N - 1)))[1].allclose(
#         torch.tensor(0.0)), f"Prob of <unk> should be 0! (2) {tensor.sum(tuple(range(0, N - 1)))[1]}"
#
#       tensor = tensor.log()
#
...
#     assert tensor.exp().sum().allclose(torch.tensor(1.0),
#                                        atol=0.0001), f"The word probabilities do not sum to 1! {tensor.sum()}"
#     tensor = tensor.numpy()
#
