...
class FairseqHydraTrainJob(Job):
    ...
    def run(self):
        ...
        final_fairseq_hydra_config['task']['data'] = self.fairseq_hydra_config['task']['data'].get_path()
        final_fairseq_hydra_config['task']['text_data'] = self.fairseq_hydra_config['task']['text_data'].get_path()
        final_fairseq_hydra_config['task']['kenlm_path'] = self.fairseq_hydra_config['task']['kenlm_path'].get_path()
        final_fairseq_hydra_config['checkpoint']['save_dir'] = self.out_dir.get_path()
        final_fairseq_hydra_config['common']['user_dir'] = os.path.join(self.fairseq_root.get_path(), "examples/wav2vec/unsupervised")
