...
class TrainKenLMJob(Job):
    ...
    def run(self):
        sh_call = f"{self.kenlm_root.get_path()}/lmplz -o {self.lm_order} --prune {' '.join(map(str, self.pruning))} < {self.text_file_path.get_path()} --discount_fallback > {self.output_arpa.get_path()}"
