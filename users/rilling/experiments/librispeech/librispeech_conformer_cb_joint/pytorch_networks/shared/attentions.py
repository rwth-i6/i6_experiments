...
class CouplingBlock(nn.Module):
  def __init__(self, in_channels, hidden_channels, kernel_size, dilation_rate, n_layers, gin_channels=0, p_dropout=0, sigmoid_scale=False, logs_epsilon=False):
    super().__init__()
...
class ConformerCouplingBlock(nn.Module):
  def __init__(self, in_channels, hidden_channels, kernel_size, dilation_rate, n_layers, n_heads=2, gin_channels=0, p_dropout=0, sigmoid_scale=False, logs_epsilon=False):
    super().__init__()
    self.in_channels = in_channels
    self.hidden_channels = hidden_channels
    self.kernel_size = kernel_size
    ...
    # self.wn = modules.WN(in_channels, hidden_channels, kernel_size, dilation_rate, n_layers, gin_channels, p_dropout)
    self.conformer_net = modules.ConformerCouplingNet(hidden_channels, 2*hidden_channels, kernel_size, n_layers, n_heads, gin_channels, p_dropout)
