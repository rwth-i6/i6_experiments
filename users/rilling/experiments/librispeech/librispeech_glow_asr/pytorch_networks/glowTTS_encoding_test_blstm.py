...
class Model(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.linear = nn.Linear(2 * 512, self.n_vocab + 1)  # + CTC blank
    ...
    def forward(
        ...
        with torch.no_grad():
            x_m, x_logs, x_mask = self.encoder(x, x_lengths, g=None)  # mean, std logs, duration logs, mask

        # breakpoint()
        encoder_out = torch.cat((x_m, x_logs), dim=1).transpose(1, 2)  # [B, T, F]

        blstm_out = self.blstm(encoder_out, x_lengths)
...
def forward_step(*, model: Model, data, run_ctx, **kwargs):
    ...
    pred = torch.exp(logprobs).argmax(dim=2)
