...
from i6_experiments.users.rossenbach.common_setups.returnn.datastreams.audio import AudioFeatureDatastream, DBMelFilterbankOptions, ReturnnAudioFeatureOptions, FeatureType, ReturnnAudioRawOptions, AudioRawDatastream
...
DATA_PREFIX = "experiments/librispeech_100_phon_ctc/standalone_ttslike_2023_normal_ctc/data/"
...
def get_text_lexicon(corpus_key="train-clean-100") -> tk.Path:
    bliss_lex = get_librispeech_eow_lexicon(corpus_key)
...
def get_ctc_bliss(ls_corpus_key, remove_unk_seqs=False) -> tk.Path:
    """
...
def get_lexicon(corpus_key="train-clean-100", with_g2p=True) -> tk.Path:
    """
...
def get_vocab_datastream(with_blank: bool = False, corpus_key="train-clean-100") -> LabelDatastream:
    ...
    """
    lexicon = get_lexicon(corpus_key=corpus_key)
    blacklist = {"[SILENCE]"}
...
def get_train_bliss_and_zip(ls_corpus_key, remove_unk_seqs=False):
    ...
    bliss_dataset = get_ctc_bliss(ls_corpus_key=ls_corpus_key, remove_unk_seqs=remove_unk_seqs)
...
def get_log_mel_datastream() -> AudioFeatureDatastream:
    """
    ...
    ls100_bliss, ls100_ogg_zip = get_train_bliss_and_zip("train-clean-100")
...
def build_training_datasets_normal_ctc(
        librispeech_key: str,
    ...
    train_bliss, train_ogg = get_train_bliss_and_zip("train-clean-100")
    _, dev_clean_ogg = get_train_bliss_and_zip("dev-clean", remove_unk_seqs=True)
    _, dev_other_ogg = get_train_bliss_and_zip("dev-other", remove_unk_seqs=True)
...
@lru_cache()
def build_test_dataset_normal_ctc(librispeech_key:str, dataset_key: str, silence_preprocessing=False):
    """
