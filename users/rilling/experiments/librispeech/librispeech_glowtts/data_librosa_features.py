...
from .default_tools import RETURNN_PYTORCH_EXE, MINI_RETURNN_ROOT
...
def get_ls460_silence_preprocessed_bliss() -> tk.Path:
    ...
    # convert the corpus transcriptions into phoneme and marker representation
    sil_pp_train_clean_100_tts = process_corpus_text_with_extended_lexicon(
        bliss_corpus=spp_460_corpus, lexicon=get_librispeech_lexicon("train-clean-460")
    )
...
def get_ls960_silence_preprocessed_bliss() -> tk.Path:
    ...
    sil_pp_train_other_960_tts = process_corpus_text_with_extended_lexicon(
        bliss_corpus=spp_960_corpus, lexicon=get_librispeech_lexicon("train-other-960")
    )
...
def get_ls360_zip_for_synthesis_only() -> tk.Path:
    ...
    :return:
    """
    ls460_lexicon = extend_lexicon_with_tts_lemmas(
        get_g2p_augmented_bliss_lexicon_dict(use_stress_marker=False)["train-clean-460"]
    )
    corpus = get_bliss_corpus_dict()["train-clean-360"]  # original corpus as .flac
    tts_corpus = process_corpus_text_with_extended_lexicon(bliss_corpus=corpus, lexicon=ls460_lexicon)
