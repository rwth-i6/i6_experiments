    ...
    def forward(
        ...
        if not gen:
            with torch.no_grad():
                ...
                x_m, x_logs, _, x_mask = self.encoder(x, x_lengths, g=g)  # mean, std logs, duration logs, mask

        # breakpoint()
        encoder_out = torch.cat((x_m, x_logs), dim=1).transpose(1, 2)  # [B, T, F]

        linear_out = self.ffn(encoder_out)
...
def train_step(*, model: Model, data, run_ctx, **kwargs):
    ...
    mask = commons.sequence_mask(phonemes_len)
    ce_losses = nn.functional.cross_entropy(logits.transpose(1, 2), phonemes.long(), reduction="none")
...
def forward_step(*, model: Model, data, run_ctx, **kwargs):
    ...
    logits, audio_features_len = model(x=phonemes, x_lengths=phonemes_len, g=speaker_labels)
    ...
    accuracies = (((pred == phonemes) * mask).sum(dim=1) / phonemes_len).unsqueeze(-1).unsqueeze(-1)
