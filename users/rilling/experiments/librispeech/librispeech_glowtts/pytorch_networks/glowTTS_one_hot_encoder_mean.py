...
class TextEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.proj_w = DurationPredictor(self.out_channels + gin_channels, filter_channels_dp, kernel_size, p_dropout)

    def forward(self, x, x_lengths, g=None):
        # x_old = self.emb(x) * math.sqrt(self.hidden_channels)  # [b, t, h]
        x = nn.functional.one_hot(x.long(), self.n_vocab)
        x = nn.functional.pad(x, (0, self.out_channels - self.n_vocab))
