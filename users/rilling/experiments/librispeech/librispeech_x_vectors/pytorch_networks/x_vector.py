...
class Model(nn.Module):
    def __init__(self, input_dim = 40, num_classes=8, **kwargs):
        ...
        self.tdnn1 = TDNN(input_dim=input_dim, output_dim=512, context_size=5, dilation=1,dropout_p=0.5, batch_norm=kwargs["batch_norm"])
        self.tdnn2 = TDNN(input_dim=512, output_dim=512, context_size=3, dilation=2,dropout_p=0.5, batch_norm=kwargs["batch_norm"])
        self.tdnn3 = TDNN(input_dim=512, output_dim=512, context_size=2, dilation=3,dropout_p=0.5, batch_norm=kwargs["batch_norm"])
        self.tdnn4 = TDNN(input_dim=512, output_dim=512, context_size=1, dilation=1,dropout_p=0.5, batch_norm=kwargs["batch_norm"])
        self.tdnn5 = TDNN(input_dim=512, output_dim=512, context_size=1, dilation=1,dropout_p=0.5, batch_norm=kwargs["batch_norm"])
        #### Frame levelPooling
...
def train_step(*, model: Model, data, run_ctx, **kwargs):
    ...
    pred_logits, _ = model(audio_features, audio_features_len)
    # breakpoint()
    loss = F.cross_entropy(pred_logits, speaker_labels.squeeze(1))
