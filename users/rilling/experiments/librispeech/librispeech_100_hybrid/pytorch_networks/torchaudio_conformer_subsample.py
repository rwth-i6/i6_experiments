...
class Conformer(torch.nn.Module):
    ...
    *Conformer: Convolution-augmented Transformer for Speech Recognition*
    ...
    def forward(self, input: torch.Tensor, lengths: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        input = self.downsample_conv.forward(input.transpose(1,2)).transpose(1, 2)
...
def train_step(*, model: Model, data, run_ctx, **_kwargs):
    ...
    targets_packed = nn.utils.rnn.pack_padded_sequence(phonemes[:,::2], (phonemes_len+1)//2, batch_first=True, enforce_sorted=False)
