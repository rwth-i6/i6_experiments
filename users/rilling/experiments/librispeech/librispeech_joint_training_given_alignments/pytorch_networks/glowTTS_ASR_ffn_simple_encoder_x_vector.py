...
from .shared.feature_extraction import DbMelFeatureExtractionConfig

from .shared.configs import ModelConfigV2, EmbeddingTextEncoderConfig, FlowDecoderConfig
...
class TextEncoder(nn.Module):
    ...
    def __init__(
        self,
        config: EmbeddingTextEncoderConfig,
        n_vocab,
        out_channels,
        gin_channels=0,
    ):
        ...
        self.emb = nn.Embedding(n_vocab, self.hidden_channels)
        ...
        self.proj_w = DurationPredictor(
            self.hidden_channels + gin_channels, self.filter_channels_dp, self.kernel_size, self.p_dropout
        )
...
class FlowDecoder(nn.Module):
    def __init__(
        ...
        for b in range(self.n_blocks):
            self.flows.append(modules.ActNorm(channels=in_channels * self.n_sqz))
            self.flows.append(modules.InvConvNear(channels=in_channels * self.n_sqz, n_split=self.n_split))
            self.flows.append(
...
class Model(nn.Module):
    ...
    def forward(
        ...
        assert (not gen) or (
            gen and (g is not None)
        ), "Generating speech without given speaker embedding is not supported!"
        ...
        if gen:
            ...
            mask = mask_tensor(asr_in, y_lengths)
