...
from .shared.configs import DbMelFeatureExtractionConfig, ModelConfigV2, MultiscaleFlowDecoderConfig, TextEncoderConfig
...
class TextEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.emb = nn.Embedding(n_vocab, self.cfg.hidden_channels)
...
class FlowDecoder(nn.Module):
    def __init__(
        self,
        cfg: MultiscaleFlowDecoderConfig,
        in_channels,
        gin_channels=0,
    ):
        ...
        self.remaining_channels = [self.in_channels]
        for i in range(self.cfg.n_blocks):
            if i % self.cfg.n_early_every == 0 and i > 0:
                self.remaining_channels.append(self.remaining_channels[-1] // 2)

            self.flows.append(modules.ActNorm(channels=self.remaining_channels[-1] * self.cfg.n_sqz))
            self.flows.append(
                modules.InvConvNear(channels=self.remaining_channels[-1] * self.cfg.n_sqz, n_split=self.cfg.n_split)
            )
            self.flows.append(
                attentions.CouplingBlock(
                    self.remaining_channels[-1] * self.cfg.n_sqz,
                    self.cfg.hidden_channels,
    ...
    def forward(self, x, x_mask, g=None, reverse=False):
        ...
        if not reverse:
            output = torch.ones((x.shape[0], 0, x.shape[2])).to(x.device) # to slowly fill up the features with early outputs by concat
        else:
            input = x
            shift = (self.cfg.n_early_every - len(self.flows) % self.cfg.n_early_every) % self.cfg.n_early_every
            x = input[:, -1*self.remaining_channels[-1]*self.cfg.n_sqz :, :]
        for i, f in enumerate(flows):
            if not reverse:
                # since we are treating every type (InvConv, ActNorm and the actual CB) as a step in the flow we have to divide i by 3 to get the actual position
                if i % 3 == 0 and i > 0:
                    k = (i // 3) % self.cfg.n_early_every
                    if k == 0:
                        r = (i // 3) // self.cfg.n_early_every
                        output = torch.cat((output,x[:, self.remaining_channels[r] * self.cfg.n_sqz :]), dim=1)
                        x = x[:, : self.remaining_channels[r] * self.cfg.n_sqz]
                x, logdet = f(x, x_mask, g=g, reverse=reverse)
                logdet_tot += logdet # TODO: How does this work with multiscale?
            else:
                if i % 3 == 0 and i > 0:
                    k = ((i // 3) + shift) % self.cfg.n_early_every
                    if k == 0:
                        r = ((i // 3) + shift) // self.cfg.n_early_every
                        start = -1*self.remaining_channels[-r-1] * self.cfg.n_sqz
                        end = -1*self.remaining_channels[-r] * self.cfg.n_sqz
                        x = torch.cat((input[:, start:end, :], x), dim=1)
