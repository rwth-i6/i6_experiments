    ...
    def forward(
        ...
        with torch.no_grad():
            ...
            if recognition_input == "encoder":
                ...
                asr_in = (z_m + torch.exp(z_logs) * torch.randn_like(z_m) * noise_scale) * z_mask  # [B, F, T]
            else:
                asr_in, _ = self.decoder(y, z_mask, g=g, reverse=False) # [B, F, T]
        
        logits = self.asr_output(asr_in.transpose(1, 2))
