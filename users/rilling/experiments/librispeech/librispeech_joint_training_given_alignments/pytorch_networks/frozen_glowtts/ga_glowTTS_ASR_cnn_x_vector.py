...
class Model(nn.Module):
    ...
    def forward(
        self, x=None, x_lengths=None, raw_audio=None, raw_audio_lengths=None, g=None, recognition_input=None, given_attn=None, noise_scale=1.0, length_scale=1.0
    ):
        ...
        with torch.no_grad():
            ...
            if recognition_input == "encoder":
                ...
                asr_in = (z_m + torch.exp(z_logs) * torch.randn_like(z_m) * noise_scale) * z_mask  # [B, T, F]
            else:
...
def phoneme_prediction_step(*, model: Model, data, run_ctx, **kwargs):
    ...
    phonemes = data["phonemes"][indices, :]  # [B, T] (sparse)
    phonemes_len = data["phonemes:size1"][indices]  # [B, T]
    speaker_labels = None  # it is x-vector and during prediction the audio is available therefore g will be predicted
    durations = data["durations"][indices]
...
def encoder_phoneme_prediction_step(*, model: Model, data, run_ctx, **kwargs):
    ...
    (z, z_m, z_logs, logdet, z_mask), (x_m, x_logs, x_mask), y_lengths, (given_attn, logw, logw_),(logits, _) = model(
        x=phonemes, x_lengths=phonemes_len, raw_audio=audio_features, raw_audio_lengths=audio_features_len, g=speaker_labels, given_attn=given_attn, recognition=False
    )
    x_mask = torch.unsqueeze(commons.sequence_mask(phonemes_len, phonemes.size(1)), 1).to(phonemes.dtype)
