...
def train_step(*, model: nn.Module, data, run_ctx, **kwargs):
    ...
    ctc_loss = nn.functional.ctc_loss(
        ...
        blank=model.conf_cfg.label_target_size,
        reduction="sum",
        zero_infinity=True
    )
...
def forward_init_hook_prior(run_ctx, **kwargs):
    # we are storing durations, but call it output.hdf to match
    # the default output of the ReturnnForwardJob
...
def forward_finish_hook_prior(run_ctx, **kwargs):
...
def forward_step_prior(*, model, data, run_ctx, **kwargs):
    raw_audio = data["audio_features"]  # [B, T', F]
    raw_audio_len = data["audio_features:size1"]  # [B]
