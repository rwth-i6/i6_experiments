...
class DensityRatioILMDecoder(ILMDecoder):
    """
    Add ILM Decoder for the density ratio approach
    """
    def __init__(self, asr_decoder, prior_lm_opts):
        super(DensityRatioILMDecoder, self).__init__(asr_decoder)
        self.prior_lm_opts = prior_lm_opts
        self.target = asr_decoder.target

    def _create_prior_net(self, subnet_unit: ReturnnNetwork):
        ext_lm_subnet = self.prior_lm_opts["lm_subnet"]

        assert isinstance(ext_lm_subnet, dict)
        is_recurrent = self.prior_lm_opts.get(
            "is_recurrent", False
        )  # TODO: is this needed? we can always use subnet maybe
        if is_recurrent:
            lm_output_prob = self.prior_lm_opts["lm_output_prob_name"]
            ext_lm_subnet[lm_output_prob]["target"] = self.target
            subnet_unit.update(ext_lm_subnet)  # just append
        else:
            ext_lm_model = self.prior_lm_opts.get("lm_model", None)
            if ext_lm_model:
                load_on_init = ext_lm_model
            else:
                assert (
                    "load_on_init_opts" in self.prior_lm_opts
                ), "load_on_init opts or lm_model are missing for loading subnet."
                assert "filename" in self.prior_lm_opts["load_on_init_opts"], "Checkpoint missing for loading subnet."
                load_on_init = self.prior_lm_opts["load_on_init_opts"]
            subnet_unit.add_subnetwork(
                "prior_output", "prev:output", subnetwork_net=ext_lm_subnet, load_on_init=load_on_init
            )
            lm_output_prob = subnet_unit.add_activation_layer(
                "prior_output_prob", "prior_output", activation="softmax", target=self.target
            )
...
class ExternalLMDecoder:
    ...
    def _create_external_lm_net(self) -> dict:
        ...
        if self.prior_lm_opts:
            ...
            if self.prior_lm_opts["type"] == "density_ratio":
                ilm_decoder = DensityRatioILMDecoder(self.asr_decoder, self.prior_lm_opts)
