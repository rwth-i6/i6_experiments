...
def add_double_softmax(net, loss_scale, ce_scale, am_scale, lm_scale, ce_label_smoothing=0.0, lm_net=None):
    """
    ...
    subnet["lm_output"] = lm_net or get_trans_lstm_lm()
...
def add_min_wer(net, loss_scale, ce_scale, am_scale, lm_scale, beam_size, ce_label_smoothing=0.0, lm_net=None):
    """
...
def add_mmi(net, loss_scale, ce_scale, am_scale, lm_scale, beam_size, ce_label_smoothing=0.0, lm_net=None):
    """
    Add Maximum Mutual Information training criterion.
    """
    assert ce_label_smoothing==0.0, "label smoothing not implemented correctly"
    subnet = net["output"]["unit"]
    #print(f"lm_net is {lm_net}")
