...
lstm_10k_decoder_like_opts = {
    "lm_subnet": generic_lm.libri_decoder_like_willi_net,
    "lm_model": generic_lm.libri_decoder_like_willi_model,
    "name": "lstm",
}
...
def conformer_baseline():
    ...
    def run_lm_fusion(
        ...
        **kwargs,
        ...
        ext_lm_opts = copy.deepcopy(lstm_lm_opts_map[bpe_size]) if lm_type == "lstm" else copy.deepcopy(trafo_lm_opts_map[bpe_size])
            ...
            for scale in scales:
                ...
                if exp_name not in jobs_summary_reports:
                    jobs_summary_reports[exp_name] = SummaryReport(
                        col_names=["test_set", "prior_type", "am_scale", "lm_scale", "prior_scale", "beam_size", "wer"], col_sort_key="wer"
                    )
    ...
    # try different prior methods:
    # * Mini-LSTM
    # * zero encoder
    # * average encoder
    # * density ratio
    avg_ctx_baseline = CodeWrapper('__import__("numpy").loadtxt("/u/michel/setups/2023-08-09--librispeech_mohammad/dependencies/prior_estimations/init/mean.txt.mean.txt")')
    for prior_opts in [
        ...
        {"prior_type": "density_ratio", "ilm_train_opts": lstm_10k_decoder_like_opts},
    ]:
        for lm_scale in [0.2, 0.26, 0.3, 0.34, 0.36, 0.4, 0.42, 0.44, 0.46, 0.48, 0.5, 0.52, 0.54, 0.56, 0.58, 0.6, 0.62, 0.64, 0.66]:
            for prior_scale in [0.0, 0.16, 0.18, 0.2, 0.22, 0.24, 0.26, 0.28, 0.3, 0.32, 0.34, 0.36, 0.38, 0.4, 0.42, 0.44]:
                run_lm_fusion(
                    lm_type="trafo",
                    exp_name=name,
                    epoch="avg",
                    ...
                    **prior_opts,
                    ...
                    args=oclr_args,
                    beam_size=32,
                    batch_size=(1000 * 160) if 32 > 40 else (2000 * 160),
                    bpe_size=BPE_10K,
                )
        for lm_scale, prior_scale, test_set_name in [(0.34, 0.0, "test-clean"), (0.40, 0.30, "test-clean"), (0.34, 0.0, "test-other"), (0.60, 0.38, "test-other")]:
                run_lm_fusion(
                    lm_type="trafo",
                    exp_name=name,
                    epoch="avg",
    ...
    retrain_args["learning_rates_list"] = [1e-4] * 20 + list(numpy.linspace(1e-4, 1e-6, 180))
    retrain_args["lr_decay"] = 0.95
    retrain_exp_name = name + f"_retrain1_const20_linDecay180_{1e-4}"
    ...
    baseline_mini_lstm_j = mini_lstm_j

    for beam_size in [32]:
        for lm_scale in [0.4, 0.42, 0.44, 0.46, 0.48, 0.5, 0.52, 0.54, 0.56, 0.58, 0.6]:
            for prior_scale in [0.0, 0.3, 0.32, 0.34, 0.36, 0.38, 0.4]:
                run_lm_fusion(
                    lm_type="trafo",
                    exp_name=retrain_exp_name,
                    epoch="avg",
    ...
    for test_set, lm, prior, beam_size in [("test-other", 0.54, 0.38, 70), ("test-clean", 0.48, 0.37, 32)]:
        run_lm_fusion(
            lm_type="trafo",
            exp_name=retrain_exp_name,
            epoch="avg",
    ...
    # TODO: double softmax
    for total_ep, lr, const_ep in [(200, 1e-4, 40)]:
        for total_ep, lr, const_ep, abs_scale, rel_scale, ce_scale in [
            (200, 1e-4, 40, 1.0, 0.0, 0.0),
            (200, 1e-4, 40, 0.8, 0.35, 0.0),
            (200, 1e-4, 40, 1.0, 0.35, 0.0),
            (200, 1e-4, 40, 1.1, 0.35, 0.0),
            (200, 1e-4, 40, 1.2, 0.35, 0.0),
            (200, 1e-4, 40, 1.3, 0.35, 0.0),
            (200, 1e-4, 40, 1.5, 0.35, 0.0),
            (200, 1e-4, 40, 2.0, 0.35, 0.0),
            (200, 1e-4, 40, 3.0, 0.35, 0.0),
            (200, 1e-4, 40, 2.0, 0.4, 0.0),
            (200, 1e-4, 40, 2.0, 0.5, 0.0),
            (200, 1e-4, 40, 2.0, 0.6, 0.0),
            (200, 1e-4, 40, 2.0, 0.25, 0.0),
            # (200, 1e-4, 40, 3.0, 0.35, 0.1),
            (200, 1.6e-5, 40, 1.5, 0.35, 0.0),
            (200, 8e-4, 40, 1.5, 0.35, 0.0),
            (200, 1e-4, 20, 1.2, 0.30, 0.0),
            (200, 1e-4, 20, 1.2, 0.35, 0.0),  # same LR schedule as baseline (current best)
            (200, 1e-4, 20, 1.2, 0.40, 0.0),
            (200, 1e-4, 20, 1.2, 0.45, 0.0),
            (200, 1e-4, 20, 1.2, 0.35, 0.1),  
        ]:
            if not LOAD_ALL and (total_ep, lr, const_ep, abs_scale, rel_scale, ce_scale) != (200, 1e-4, 20, 1.2, 0.35, 0.0):
                continue
            am_scale = abs_scale
            ...
            for beam_size in [32]:
                search_lm_scales = [0.4, 0.42, 0.44, 0.46, 0.48, 0.5, 0.52, 0.54, 0.56, 0.58, 0.6]
                prior_scales = [0.0, 0.3, 0.32, 0.34, 0.36, 0.38, 0.4]
                test_sets = ["dev-other"]
                if abs_scale == 1.2:
                    search_lm_scales += [0.2, 0.26, 0.3, 0.34, 0.36, 0.62, 0.64, 0.66]
                    prior_scales += [0.12, 0.14, 0.16, 0.18, 0.2, 0.22, 0.24, 0.26, 0.28]
                if (total_ep, lr, const_ep, abs_scale, rel_scale, ce_scale) == (200, 1e-4, 20, 1.2, 0.35, 0.0):
                    test_sets = ["dev-clean", "dev-other"]
                for search_lm_scale in search_lm_scales:
                    for prior_scale in prior_scales:
                        run_lm_fusion(
                            lm_type="trafo",
                            exp_name=exp_name,
                            epoch="avg",
                            test_set_names=test_sets,
                ...
                if (total_ep, lr, const_ep, abs_scale, rel_scale, ce_scale) == (200, 1e-4, 20, 1.2, 0.35, 0.0):
                    for search_lm_scale, prior_scale, test_set_name in [(0.42, 0.0, "test-clean"), (0.42, 0.18, "test-clean"), (0.48, 0.0, "test-other"), (0.54, 0.12, "test-other")]:
                        run_lm_fusion(
                            lm_type="trafo",
                            exp_name=exp_name,
                            epoch="avg",
            ...
            # tune local norm for best training so far
            # --> worse than global norm
            # if (abs_scale, rel_scale, ce_scale) == (1.5, 0.35, 0.0):
            #     for beam_size in [32]:
            #         for search_lm_scale in [0.4, 0.42, 0.44, 0.46, 0.48, 0.5, 0.52, 0.54, 0.56, 0.58, 0.6]:
            #             for search_am_scale in [0.5, 0.8, 1.0, 1.2, 1.5, 2.0]:
            #                 run_lm_fusion(
            #                     lm_type="trafo",
            #                     exp_name=exp_name,
            #                     epoch="avg",
            #                     test_set_names=["dev-other"],
            #                     lm_scales=[search_lm_scale*search_am_scale],
            #                     am_scales=[search_am_scale],
            #                     prior_scales=None,
            #                     prior_type=None,
            #                     mini_lstm_ckpt=None,
            #                     local_norm=True,
            ...
            # # Try iLM correction with the iLM from the baseline
            # if (abs_scale, rel_scale, ce_scale) == (1.5, 0.35, 0.0):
            #     for beam_size in [32]:

            #         for search_lm_scale in [0.48, 0.5, 0.52, 0.54, 0.56, 0.58, 0.6]:
            #             for prior_scale in [0.28, 0.3, 0.32, 0.34, ]:
            #                 run_lm_fusion(
            #                     lm_type="trafo",
            #                     exp_name=exp_name,
            #                     epoch="avg",
            ...
            #                     prior_scales=[prior_scale],
            #                     prior_type="mini_lstm",
            #                     prior_type_name="baseline_mini_lstm",
            #                     mini_lstm_ckpt=get_best_checkpoint(baseline_mini_lstm_j, key="dev_score"),
            #                     train_job=train_j,
            #                     train_data=train_data,
            #                     feature_net=log10_net_10ms,
            #                     args=oclr_args,
    ...
    for total_ep, lr, const_ep in [(20, 2e-5, 20)]:
        if not LOAD_ALL:
            continue
        for abs_scale, rel_scale, ce_scale, beam in [
            ...
            args = copy.deepcopy(retrain_args)
            args["retrain_checkpoint"] = train_job_avg_ckpt[retrain_exp_name]
            args["accum_grad"] = 3
            args["max_seqs"] = 3

            args["learning_rates_list"] = [lr] * const_ep + list(numpy.linspace(lr, 1e-6, total_ep - const_ep))
            exp_name = f"att_retrain1_minWER_am{am_scale}_lm{lm_scale:.2f}_beam{beam}_transLM_ep{total_ep}_lr{lr}_const{const_ep}_ce{ce_scale}"
            train_j = run_seq_train(
                exp_name=exp_name,
                seq_train_opts=seq_train_opts,
                train_args=args,
    ...
    for total_ep, lr, const_ep in [(20, 2e-5, 20)]:
        ...
        for abs_scale, rel_scale, ce_scale, beam in [
            ...
            mini_lstm_j = train_mini_lstm(
                exp_name=exp_name,
                checkpoint=train_job_best_epoch[exp_name],  # or train_job_avg_ckpt
                args=oclr_args,
                num_epochs=40,
                w_drop=True,
            )
            ...
            if (abs_scale, rel_scale) != (0.3, 0.35):
                continue

            ctx = {
                0.5: CodeWrapper('__import__("numpy").loadtxt("/u/michel/setups/2023-08-09--librispeech_mohammad/dependencies/prior_estimations/init/mmi.att.mean.txt")'),
                0.0: CodeWrapper('__import__("numpy").loadtxt("/u/michel/setups/2023-08-09--librispeech_mohammad/dependencies/prior_estimations/init/mmi_ce00.att.mean.txt")'),
            }
            for prior_opts in [
                {"prior_type": "mini_lstm", "mini_lstm_ckpt": get_best_checkpoint(mini_lstm_j, key="dev_score")},
                {"prior_type": "zero"},
                {"prior_type": "train_avg_ctx", "ilm_train_opts": {"data": ctx[ce_scale]}},
                {"prior_type": "density_ratio", "ilm_train_opts": lstm_10k_decoder_like_opts},
            ]:
                for search_lm_scale in [0.4, 0.42, 0.44, 0.46, 0.48, 0.5, 0.52, 0.54, 0.56, 0.58, 0.6]:
                    for prior_scale in [0.0, 0.3, 0.32, 0.34, 0.36, 0.38, 0.4]:
    ...
    # MMI Model by Zijian and Minh-Nghia Phan
    train_checkpoint = Checkpoint(tk.Path("/work/asr3/zyang/share/mnphan/work_mohammad_attention/i6_core/returnn/training/ReturnnTrainingJob.MpfQMNAnvTTw/output/models/epoch.012.index"))
    ilm_checkpoint = Checkpoint(tk.Path("/work/asr3/zyang/share/mnphan/work_mohammad_attention/i6_core/returnn/training/GetBestTFCheckpointJob.ORoA1Hb5rtLR/output/model/epoch.029.index"))
    exp_name = "MMI_Zijian_Hiwi"
    train_job_avg_ckpt[exp_name] = train_checkpoint
    train_job_best_epoch[exp_name] = train_checkpoint
    avg_prior_file = CodeWrapper('__import__("numpy").loadtxt("/u/michel/setups/2023-08-09--librispeech_mohammad/dependencies/prior_estimations/init/mmi_zijian.att.mean.txt")')
    for prior_opts in [
        {"prior_type": "mini_lstm", "mini_lstm_ckpt": ilm_checkpoint},
        {"prior_type": "zero"},
        {"prior_type": "train_avg_ctx", "ilm_train_opts": {"data": avg_prior_file}},
        {"prior_type": "density_ratio", "ilm_train_opts": lstm_10k_decoder_like_opts},
    ]:
        for search_lm_scale in [0.34, 0.36, 0.38, 0.4, 0.42, 0.44, 0.46, 0.48, 0.5, 0.52, 0.54, 0.56, 0.58, 0.6]:
            for prior_scale in [0.0, 0.16, 0.18, 0.2, 0.22, 0.24, 0.26, 0.28, 0.3, 0.32, 0.34, 0.36, 0.38, 0.4, 0.42, 0.44, 0.46, 0.48]:
                run_lm_fusion(
                    lm_type="trafo",
                    exp_name=exp_name,
                    epoch="best",
        ...
        for search_lm_scale, prior_scale, test_set_name in [(0.44, 0.0, "test-clean"), (0.5, 0.18, "test-clean"), (0.44, 0.16, "test-clean"),(0.44, 0.0, "test-other"), (0.54, 0.24, "test-other")]:
            run_lm_fusion(
                lm_type="trafo",
                exp_name=exp_name,
                epoch="best",
