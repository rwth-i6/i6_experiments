...
from i6_experiments.users.zeineldeen.experiments.conformer_att_2022.librispeech_100.attention_asr_config import \
...
from i6_experiments.users.zeineldeen.experiments.conformer_att_2022.librispeech_100.data import \
...
def conformer_baseline():
    ...
    for testset in ["dev-clean", "dev-other", "test-clean", "test-other"]:
        test_dataset_tuples[testset] = build_test_dataset(
            testset, bpe_size=BPE_SIZE, use_raw_features=True,
        )
    ...
    oclr_args["oclr_opts"] = {
        ...
        "cycle_ep": 90,  # 45%
        "total_ep": 200,
        "n_step": 1350,  # TODO: check log -> set to 1400
    }
    ...
    for dropout_val in [0.1]:
        ...
        args['decoder_args'].dropout = dropout_val
        args['decoder_args'].att_dropout = dropout_val
        args['max_seq_length'] = None  # keep all seqs
        run_exp(
            f"base_conf_12l_lstm_1l_conv6_OCLR_sqrdReLU_drop-{dropout_val}_attDrop-{dropout_val}",
            train_args=args,
            num_epochs=200
        )

    # for libri 960, we use 75 and Dropped seqs: 87 (0.00031%)
    # Total seqs: 28539
    # 95: Dropped seqs: 8 (0.00028%)
    # 92: Dropped seqs: 19 (0.00067%)
    # 90: Dropped seqs: 27 (0.00095%)
    # 85: Dropped seqs: 101 (0.00354%)
    # 75: Dropped seqs: 947 (0.03318%)

    for dropout_val in [0.1, 0.2, 0.3]:
      args = copy.deepcopy(oclr_args)
      args['oclr_opts']['n_step'] = 1400
      args['decoder_args'].dropout = dropout_val
      args['decoder_args'].att_dropout = dropout_val
      args['max_seq_length'] = 90
      run_exp(
        f"base_conf_12l_lstm_1l_conv6_OCLR_sqrdReLU_cyc90_ep200_drop-{dropout_val}_attDrop-{dropout_val}",
        train_args=args,
        num_epochs=200
      )