...
def extract_results(prefix, names):
    global out
    ...
    #os.chdir(os.cwd() + "/..") # leave the tools dir
    ...
    print_loading = False
    for n in names:
        call = PY + " get_wer.py --prefix " + prefix + " --job_name " + n + " --print true"
        if print_loading:
            print(call)
        ...
        for data_set in ["dev-other", "dev-clean", "test-other", "test-clean"]:
            call = PY + " get_wer_for_set.py --prefix " + prefix + " --job_name " + n + " --print true --set %s" % data_set
            if print_loading:
                print(call)
            _call = call.split(" ") # We shall never have space in name!!!!
