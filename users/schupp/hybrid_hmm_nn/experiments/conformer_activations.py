...
# ------------------------------------------
gs.ALIAS_AND_OUTPUT_SUBDIR = "conformer/activations/"
type = 'conformer'
prefix = 'conformer_'
...
def run_exp(): #  'swish' -> is same as baseline
  for act in ["cyclemoid", 'relu', 'gelu']: # ['relu', 'gelu', 'swish', 
    train_activation(act)
    train_activation(act, False)


def train_activation(act_name, with_se=True):
  name = f"conf__act-{act_name}"
  ...
  _enc_half_args["tbs_version"] = "cyclemoid"

  _enc_half_args["ff_activation"] = act_name
  _enc_half_args["conv_activation"] = act_name
  ...
  system.nn_train(prefix + '_' + name, 200, network, # 200 is the 'long' train
                ...
                epoch_split = 20, add_cyclemoid=(act_name=="cyclemoid"),
                **_nn_train_args)
