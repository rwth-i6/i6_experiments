...
# ------------------------------------------
gs.ALIAS_AND_OUTPUT_SUBDIR = "conformer/label_smooth/"
type = 'conformer'
prefix = 'conformer_'
...
def run_exp(): #  'swish' -> is same as baseline
  train_lb_smooth(smoothing=0.0)
  train_lb_smooth(smoothing=0.05)
  train_lb_smooth(smoothing=0.1)
  train_lb_smooth(smoothing=0.15)
  train_lb_smooth(smoothing=0.2)

def train_lb_smooth(smoothing=0.1):
  name = f"conf__lbl-smoothing-{smoothing}"
  ...
  _nn_train_args["crnn_config"]["label_smoothing"] = smoothing


  _enc_half_args = deepcopy(enc_half_args)
  _enc_half_args["ff_dim"] = 1538 # Best from previous

  _enc_half_args["tbs_version"] = "stoch_depth"
  ...
  rec_epochs = [10, 20, 40, 80, 100] + [140, 180, 190, 200] # Extendet epochs for extendent train
