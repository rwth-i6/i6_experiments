...
# ------------------------------------------
gs.ALIAS_AND_OUTPUT_SUBDIR = "conformer/decoupled_constraints/"
type = 'conformer'
prefix = 'conformer_'
...
def run_exp(): #  'swish' -> is same as baseline
  #train_ff_gate()
  train_decoupled("nadam")
  train_decoupled("adam")

def train_decoupled(optim="nadam", fact=0.025):
  name = f"conf__decoupled_optim-{optim}_dec-fact-{fact}"
  ...
  _nn_train_args["crnn_config"]["optimizer"] = {"class": optim}
  ...
  _nn_train_args["crnn_config"]["decouple_constraints"] = True
  _nn_train_args["crnn_config"]["decouple_constraints_factor"] = fact
