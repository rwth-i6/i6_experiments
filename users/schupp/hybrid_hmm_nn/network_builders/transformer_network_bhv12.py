...
class attention_for_hybrid:
  def __init__(self, target, num_classes, num_enc_layers, enc_args,

               type = "transformer",

               second_encoder=False, num_sec_enc_layers=None,
               variant="same", share_par=True,

               normalized_loss=False, label_smoothing=0.0, focal_loss_factor=0.0,
               softmax_dropout=0.0,

               use_spec_augment=True,
               use_pos_encoding=False, add_to_input=True, src_embed_args = None,

               add_blstm_block=False, blstm_args=None, blstm_pooling_args=None,
               add_conv_block=False, conv_args=None,

               feature_stacking=False, feature_stacking_before_frontend=False,
               feature_stacking_window=None, feature_stacking_stride=None,
               ...
               att_weights_inspection=False, inspection_idx=None,
               window_limit_idx=None, window_size=None, gauss_window=False,
    ...
    if att_weights_inspection:
      assert isinstance(inspection_idx, list), \
    ...
    if second_encoder:
      assert variant in ['same', 'different'], \
    ...
    if use_pos_encoding:
      if not src_embed_args:
        src_embed_args = \
          dict(dim=enc_args['model_dim'], init=enc_args['initialization'],
               with_bias=True,
               weight=enc_args['model_dim'] ** 0.5, dropout=0.0)
  ...
  def _blstm_block(self, inp=None, prefix=""):
    ...
    # first blstm layer
    idx = 0
    layers.add_blstm_layer(self.network, idx=idx, dim=self.blstm_args['dims'][idx],
                           dropout=self.blstm_args.get('dropout', 0.0),
                           l2=self.blstm_args.get('l2', 0.0), from_layers=last_layers, prefix=prefix)
    ...
    if idx in pooling_layers:
      if self.reduction_factor and self.reduction_factor[0] >= 2:
        layers.add_pool_layer(self.network, idx=idx, mode='max',
                              padding='same',
                              pool_size=(self.reduction_factor[0],),
                              from_layers=last_layers, trainable=False)
  ...
  def _conv_block(self, inp=None, prefix=""):
    ...
    self.network["source0"] = {"class": "split_dims", "axis": "F", "dims": (-1, 1)}  # (T,50,1)
    ...
    ## first vgg block
    self.network[f"{prefix}conv0_0"] = {"class": "conv", "from": "source0", "padding": "same",
                                        "filter_size": (3, 3),
                                        "n_out": 32, "activation": None, "with_bias": True, "in_spatial_dims": ["T", "dim:50"]}  # (T,50,32)
    #, "in_spatial_dims": ["T", "dim:1"]
    #, "in_spacial_dim": ["dim:1"]

    self.network[f"{prefix}conv0_1"] = {"class": "conv", "from": f"{prefix}conv0_0", "padding": "same", "filter_size": (3, 3),
                               "n_out": 32, "activation": 'relu', "with_bias": True, "in_spatial_dims": ["T", "dim:50"]}  # (T,50,32)

    self.network[f"{prefix}conv0p"] = {"class": "pool", "mode": "max", "padding": "same", "pool_size": (1, 2), 'strides': (1, 2),
                              "from": f"{prefix}conv0_1", "in_spatial_dims": ["T", "dim:50"] }  # (T,25,32)
    ...
    self.network[f"{prefix}conv1_0"] = {"class": "conv", "from": f"{prefix}conv0p", "padding": "same", "filter_size": (3, 3), "n_out": 64,
                               "activation": None, "with_bias": True,"in_spatial_dims": ["T", "dim:25"] }  # (T,25,64)
    self.network[f"{prefix}conv1_1"] = {"class": "conv", "from": f"{prefix}conv1_0", "padding": "same", "filter_size": (3, 3),
                               "n_out": 64, "activation": 'relu', "with_bias": True, "in_spatial_dims": ["T", "dim:25"]}  # (T,25,64)
    self.network[f"{prefix}conv1p"] = {"class": "pool", "mode": "max", "padding": "same", "pool_size": (1, 1), 'strides': (1, 1), # strides': (1, 1)?
                              "from": f"{prefix}conv1_1", "in_spatial_dims": ["T", "dim:25"]}  # (T,25,64)
    ...
    self.network[f"{prefix}conv_merged"] = {"class": "merge_dims", "from": f"{prefix}conv1p", "axes": ["dim:25", "dim:64"]}  # (T,1600) 

    if self.conv_args:
      for name in ["conv0_0", "conv0_1", "conv0p", "conv1_0", "conv1_1", "conv1p"]:
  ...
  def _transformer_encoder(self, inp):
    ...
    if self.att_weights_inspection and 1 in self.inspection_idx: # layer 1 is inspected -> separate attention layer definition
      # with additional windowing
      separated = True
      if self.window_limit_idx and 1 in self.window_limit_idx:
        windowing = True
    layers.trafo_enc_layer_all_in_one(self.network, "enc_001", from_layers=inp,
      separated=separated, windowing = windowing, window_size = self.window_size, gauss_window = self.gauss_window,
      **self.enc_args)
    ...
    for n in range(2, self.num_enc_layers + 1):
      ...
      layers.trafo_enc_layer_all_in_one(
        self.network, f"enc_{n:03d}", from_layers=last_layer,
        separated=separated, windowing=windowing, window_size=self.window_size, gauss_window = self.gauss_window,
        **self.enc_args)
      ...
      if self.loss_layer_idx and n in self.loss_layer_idx:
        self._add_auxiliary_loss(idx=n)
  ...
  def _conformer_encoder(self, inp):
    ...
    if self.att_weights_inspection and 1 in self.inspection_idx:  # layer 1 is inspected -> separate attention layer definition
      ...
      if self.window_limit_idx and 1 in self.window_limit_idx:
        windowing = True
    layers.conformer_enc_layer_all_in_one(self.network, "enc_001", from_layers=inp,
                                      separated=separated, windowing=windowing, window_size=self.window_size, gauss_window = self.gauss_window,
                                      **self.enc_args)
  ...
  def _second_encoder(self, inp, ca_layer):
    ...
    layers.separated_trafo_ca_layer(
        self.network, "sec_enc_001", from_layers=inp, ca_layer=ca_layer, **self.enc_args)

    for n in range(2, self.num_sec_enc_layers + 1):
      prev_n = n - 1

      layers.separated_trafo_ca_layer(
        self.network, f"sec_enc_{n:03d}", from_layers=f"sec_enc_{prev_n:03d}",
        ca_layer=ca_layer, **self.enc_args)
  ...
  def _add_feature_representation(self, idx, from_layers):
    ...
    # concat along T
      ...
      self.network[f"enc_{idx:03d}_feature_repre"] = {
        "class": "eval", "from": self.network[f"enc_{idx:03d}_att_value0"]["from"] + from_layers,
  ...
  def _add_auxiliary_loss(self, idx):
    ...
    self.network[f'aux_{idx}_output_prob'] = {'class': "softmax",
  ...
  def _feature_stacking(self, from_layers):
    ...
    self.network['feature_stacking_window'] = {
      ...
      'window_size': self.feature_stacking_window[0] + \
                     self.feature_stacking_window[1] + 1,  # window_size == window_left + window_right + 1
  ...
  def _upsampling_by_transposed_conv(self, from_layers, prefix=None):
    ...
    self.network[f'{prefix}length_masked'] = {
      'class': 'reinterpret_data', 'from': last_layers,
  ...
  def _reduce_alignment(self):
    ...
    ## (B, T) -> (B, T, 1)
    self.network["expand_classes"] = {"class": "eval", "from": "data:classes",
                                 "eval": "tf.expand_dims(source(0, auto_convert=False), axis=-1)",
                                 "out_type": {'dim': 1, 'shape': (None, 1), 'sparse': False}}
    ## for convolution operation
    self.network["type_casting"] = {"class": "cast", "from": f"expand_classes", "dtype": "float32"}

    ## label downsampling with fixed conv. kernel [1, 0,..,0]
    last_pool_layer = "type_casting"

    if self.feature_stacking and self.feature_stacking_stride >= 2:
      self.network["classes_pool0"] = {"class": "conv", "from": last_pool_layer, "padding": "same",
                                       "filter_size": (self.feature_stacking_stride,),
                                       'strides': (self.feature_stacking_stride,),  # (T, 1)
                                       "n_out": 1, "activation": None, "with_bias": False,
                                       "forward_weights_init":
                                         f'numpy.reshape(numpy.array([1] + [0] * {self.feature_stacking_stride-1}, dtype="float32"), '\
    ...
    if self.reduction_factor:
      if self.reduction_factor[0] >= 2:
        self.network[f"classes_pool1"] = {"class": "conv", "from": last_pool_layer, "padding": "same",
                                   "filter_size": (self.reduction_factor[0],),
                                   'strides': (self.reduction_factor[0],),  # (T, 1)
                                   "n_out": 1, "activation": None, "with_bias": False,
                                   "forward_weights_init":
                                          f'numpy.reshape(numpy.array([1] + [0] * {self.reduction_factor[0]-1}, dtype="float32"), '
                                          f'newshape=({self.reduction_factor[0]}, 1, 1))',
                                   "trainable": False}
        last_pool_layer = "classes_pool1"
      if self.reduction_factor[1] >= 2:
        self.network["classes_pool2"] = {"class": "conv", "from": last_pool_layer, "padding": "same",
                                       "filter_size": (self.reduction_factor[1],),
                                       'strides': (self.reduction_factor[1],),  # (T, 1)
                                       "n_out": 1, "activation": None, "with_bias": False,
                                       "forward_weights_init":
                                         f'numpy.reshape(numpy.array([1] + [0] * {self.reduction_factor[1]-1}, dtype="float32"), '\
                                         f'newshape=({self.reduction_factor[1]}, 1, 1))',
                                       "trainable": False}
        last_pool_layer = "classes_pool2"

    self.network["type_casting_back"] = {"class": "cast", "from": last_pool_layer, "dtype": "int32"}
    ## squeeze and register the reduced class
    self.network["squeeze_classes"] = {"class": "eval", "from": "type_casting_back",
  ...
  def _build(self):
    ...
    self.network['output'] = {
      'class': "softmax", 'dropout': self.softmax_dropout,
      'from': last_layer,
      'loss': "ce", 'loss_opts': {
        'label_smoothing': self.label_smoothing,
        'use_normalized_loss': self.normalized_loss,
