...
def add_SE_block(network, in_layer, name_prefix):
  ...
  network[name_prefix + "_SE_elm_mul"] = {
    ...
    "from" : [name_prefix + "_SE_act2", in_layer]
  }

  return name_prefix + "_SE_elm_mul"


def conformer_conv_norm_swith(
  ...
  act_first_conv = conv_acts[0]
  act_sec_conv = conv_acts[1]

  conv_first_norm = conv_norm[0]
  conv_sec_norm = conv_norm[1]

  if overwrite_conv_dim is None:
    conv_dim = 2 * model_dim
  else:
    conv_dim = overwrite_conv_dim
  ...
  if ff_activation == "cyclemoid":
    network[f"{name}_ff1_conv1_pre"] = {
    ...
    network[f"{name}_ff1_conv1"] = {
      "class" : "eval",
      "eval" : "self.network.get_config().typed_value('cyclepy')(source(0))",
      "from" : f"{name}_ff1_conv1_pre"
    }
  else:
    network[f"{name}_ff1_conv1"] = {
  ...
  if conv_first_norm:
    ...
    network[f"{name}_conv_laynorm"] = {'class': "copy",
                                      'from': [f"{name}_self_att_out"]}
  ...
  network[f"{name}_conv_pointwise1"] = {
    'class': "linear", 'activation': None, 'with_bias': False,
    'from': [f"{name}_conv_laynorm"], 'n_out': conv_dim,
    'forward_weights_init': initialization
  }
  ## (batch, time, feature)
  ...
  if not act_first_conv:
    network[f"{name}_conv_pointwise2"] = {
      'class': "linear", 'activation': None, 'with_bias': False,
      'from': [f"{name}_conv_batchnorm"], 'n_out': model_dim,
      'forward_weights_init': initialization
    }
