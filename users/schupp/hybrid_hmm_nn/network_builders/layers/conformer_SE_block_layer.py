...
def add_SE_block(network, in_layer, name_prefix):
  ...
  network[name_prefix + "_SE_pool"] = {
    "class" : "pool",
    "mode" : "avg",
    "pool_size" : (1, 256), # TODO: use model_dim here
    "from" : in_layer
  }

  network[name_prefix + "_SE_linear1"] = {
    "class" : "linear",
    "from" : name_prefix + "_SE_pool",
    "n_out" : 32
  }
  ...
  network[name_prefix + "_SE_act2"] = {
    "class" : "activation",
    "activation" : "swish",
    "from" : name_prefix + "_SE_linear2"
  }

  network[name_prefix + "_SE_expand"] = {
    "class" : "expand_dims",
    "axis" : "T",
    "from" : name_prefix + "_SE_act2"
  }

  return name_prefix + "_SE_expand"
...
def conformer_enc_layer_all_in_one_SE(
  ...
  amount_ff_mod = 2,
  amount_mhsa_mod = 1, # TODO: implement
  amount_conv_mod = 1, # TODO: implement
  with_se=True # Has SE blocks
):
  ...
  _name = name
  _amount_ff_mod = int((_amount_ff_mod / 2) + 0.5) # +1 cause we would rather only want one in the front, than only in the back
  if _amount_ff_mod >= 1:

    ## first ffn layer
    i = 0
    ## TBS: removed switch code here
    while _amount_ff_mod > 0:
      _amount_ff_mod -= 1
      _name = name + "_id" + str(i)
      i += 1
      ff_name = "ff1"
      add_ff_mod(_name, ff_name, network, model_dim, ff_activation, initialization, ff_activation_dropout, ff_post_dropout, ff_dim, half_ratio)

  amount_ff_mod -= _amount_ff_mod # To know how much we add in the end
  ...
  network[f"{name}_self_att_out"] = {
    ...
    'from': [f"{_name}_ff1_out", f"{name}_self_att_drop"],
    'n_out': model_dim
  }
  ## convolution module
  ...
  if with_se:
    ...
    out_layer_name = add_SE_block(network, inpl, name)
...
def add_ff_mod(name, ff_name, network, model_dim, ff_activation, initialization, ff_activation_dropout, ff_post_dropout, ff_dim, half_ratio):
  network[f"{name}_{ff_name}_laynorm"] = {'class': "layer_norm",
                                    'from': [f"{name}_conv_output"]}
  network[f"{name}_{ff_name}_conv1"] = {
    'class': "linear", 'activation': ff_activation, 'with_bias': True,
    'from': [f"{name}_{ff_name}_laynorm"],
    'n_out': ff_dim, 'forward_weights_init': initialization
  }

  network[f"{name}_{ff_name}_conv2"] = {
    'class': "linear", 'activation': None, 'with_bias': True,
    'from': [f"{name}_{ff_name}_conv1"], 'dropout': ff_activation_dropout,
    'n_out': model_dim, 'forward_weights_init': initialization
  }
  network[f"{name}_{ff_name}_drop"] = {'class': "dropout",
                                 'dropout': ff_post_dropout,
                                 'from': [f"{name}_{ff_name}_conv2"]}

  network[f"{name}_{ff_name}_drop_half"] = {
    'class': "eval",
    'eval': f"{half_ratio} * source(0)",
    'from': [f"{name}_{ff_name}_drop"]
  }
  network[f"{name}_{ff_name}_out"] = {
    'class': "combine", 'kind': "add",
    'from': [f"{name}_conv_output", f"{name}_{ff_name}_drop_half"]
  }