...
class Model(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.blstm_stack = nn.LSTM(input_size=2 * lstm_size, hidden_size=lstm_size, bidirectional=True, num_layers=7,
            batch_first=False)
    ...
    def forward(
        ...
        if self.training:
            ...
            audio_features_masked = mask_along_axis(audio_features_time_masked, mask_param=10, mask_value=0.0, axis=2)
        else:
            audio_features_masked = audio_features
        blstm_in = torch.swapaxes(audio_features_masked, 0, 1)  # [B, T, F] -> [T, B, F]
        ...
        blstm_out, _ = nn.utils.rnn.pad_packed_sequence(blstm_packed_out, padding_value=0.0,
