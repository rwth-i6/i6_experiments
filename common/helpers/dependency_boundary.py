...
def dependency_boundary(func: Callable[[], T], *, hash: Optional[str]) -> T:
    ...
    if hash_via_user:
        cache_fn = get_cache_filename_for_func(func, hash=hash_via_user)
    ...
    if not hash_via_cache:
        cache_fn = get_cache_filename_for_func(func, hash=hash_via_func)
...
# noinspection PyShadowingBuiltins
def get_cache_filename_for_func(func: Callable[[], T], *, hash: str) -> str:
    """
    ...
    return f"{mod_dir}/_dependency_boundary_autogenerated_cache/cached.{_func_name(func)}.{hash}.py"
...
def _func_name(func: Callable[[], T]) -> str:
    if isinstance(func, functools.partial):
        return f"{_func_name(func.func)}.partial"
