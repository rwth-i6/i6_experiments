...
def _get_raw_bliss_lexicon(
    ...
    #)
    text_lexicon = tk.Path("/work/asr4/rossenbach/corpora/loquacious/LoquaciousAdditionalResources/loquacious-lexicon.txt", hash_overwrite="loquacious_temp_lex_v1")
...
def get_g2p_augmented_bliss_lexicon_dict(
    ...
    # make sure the order small -> medium -> large is correct during g2p augmentation
    train_stage = 0
    for corpus_name, bliss_corpus in bliss_corpus_dict.items():
        if corpus_name in ["train.medium-wo-small"]:
            continue
        if "train" in corpus_name:
            assert train_stage != 0 or corpus_name == "train.small"
            assert train_stage != 1 or corpus_name == "train.medium"
            if corpus_name in ["train.large"]:
                pass
            else:
    ...
    augmented_bliss_lexica["train.medium-wo-small"] = augmented_bliss_lexica["train.medium"]
