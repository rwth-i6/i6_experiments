...
def run(name: str = "default", config: Optional[PipelineConfig] = None) -> None:
    gs.ALIAS_AND_OUTPUT_SUBDIR = f"bpe_transducer/{name}"
    ...
    for idx, recog_config in enumerate(config.recog_configs, start=1):
        ...
        report.add_entry(col="3 Enc RTF", row=recog_result.descriptor, var=recog_result.enc_rtf)
    ...
    tk.register_report(f"bpe_transducer/{name}/report.txt", values=report, required=True)
