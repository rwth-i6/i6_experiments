...
from .pytorch_modules import (
    FFNNTransducerCTCConfig,
    FFNNTransducerCTCModel,
    FFNNTransducerCTCRecogConfig,
...
from .returnn_steps import (
    ComputeCTCPriorCallback,
    ExtractSearchRTFJob,
    SearchCallback,
    ctc_flashlight_recog_step,
    ctc_prior_step,
    rasr_recog_step,
    scorer_forward_step,
    train_step,
...
def train(config: TrainRoutineConfig, model_config: FFNNTransducerConfig) -> ReturnnTrainingJob:
    ...
    train_returnn_config = ReturnnConfig(
        config={
            ...
            "accum_grad_multiple_step": config.accum_grad_multiple_step,
            "cleanup_old_models": {
                "keep_last_n": 1,
                "keep_best_n": 0,
        ...
        python_epilog=[
            *model_serializers,
            PartialImport(
                ...
                hashed_arguments={"enc_loss_scales": config.enc_loss_scales},
                unhashed_arguments={},
...
def compute_ctc_priors(
    config: CTCPriorRoutineConfig, model_config: FFNNTransducerConfig, train_job: ReturnnTrainingJob, epoch: int
) -> tk.Path:
    prior_model_config = FFNNTransducerCTCConfig(
    ...
    model_serializers = get_model_serializers(model_class=FFNNTransducerCTCModel, model_config=prior_model_config)
    ...
    prior_returnn_config = ReturnnConfig(
        ...
        python_epilog=[
            *model_serializers,
            Import(
                f"{ctc_prior_step.__module__}.{ctc_prior_step.__name__}",
                import_as="forward_step",
                unhashed_package_root=__package__,
            ),
            Import(
                f"{ComputeCTCPriorCallback.__module__}.{ComputeCTCPriorCallback.__name__}",
    ...
    tk.register_output(
        f"prior_enc-{config.enc_layer}/returnn.config",
    ...
    prior_job.add_alias(f"prior_enc-{config.enc_layer}/prior.e-{epoch}")
    tk.register_output(f"prior_enc-{config.enc_layer}/prior.e-{epoch}.txt", prior_job.out_files["prior.txt"])

    return prior_job.out_files["prior.txt"]


def ctc_rasr_greedy_recog(
    config: CTCRasrGreedyRecogRoutineConfig, model_config: FFNNTransducerConfig, train_job: ReturnnTrainingJob
) -> ResultItem:
    prior_file = compute_ctc_priors(
        train_job=train_job, epoch=config.epoch, config=config.prior_config, model_config=model_config
    )

    recog_model_config = FFNNTransducerCTCRecogConfig(
        enc_layer=config.enc_layer,
        logmel_cfg=model_config.logmel_cfg,
        conformer_cfg=model_config.conformer_cfg,
        enc_dim=model_config.enc_dim,
    ...
    model_serializers = get_model_serializers(model_class=FFNNTransducerCTCRecogModel, model_config=recog_model_config)
...
def ctc_flashlight_recog(
    config: CTCFlashlightRecogRoutineConfig, model_config: FFNNTransducerConfig, train_job: ReturnnTrainingJob
) -> ResultItem:
    prior_file = compute_ctc_priors(
        train_job=train_job, epoch=config.epoch, config=config.prior_config, model_config=model_config
    )
    ...
    recog_returnn_config = ReturnnConfig(
        ...
        python_epilog=[
            *model_serializers,
            ...
            PartialImport(
                code_object_path=f"{ctc_flashlight_recog_step.__module__}.{ctc_flashlight_recog_step.__name__}",
                unhashed_package_root=__package__ or "",
                hashed_arguments={
...
def export_scorer(model_config: FFNNTransducerRecogConfig, train_job: ReturnnTrainingJob, epoch: int) -> tk.Path:
    ...
    export_job.add_alias(f"onnx_export/scorer.e-{epoch}")

    return export_job.out_onnx_model


def rasr_greedy_recog(
    config: RasrGreedyRecogRoutineConfig, model_config: FFNNTransducerConfig, train_job: ReturnnTrainingJob
) -> ResultItem:
    recog_model_config = FFNNTransducerRecogConfig(
        ...
        joiner_dim=model_config.joiner_dim,
        joiner_activation=model_config.joiner_activation,
        ilm_scale=config.ilm_scale,
        blank_penalty=config.blank_penalty,
    )

    encoder_serializers = get_model_serializers(model_class=FFNNTransducerEncoder, model_config=recog_model_config)
    scorer_onnx_checkpoint = export_scorer(model_config=recog_model_config, train_job=train_job, epoch=config.epoch)
    ...
    recog_rasr_config.lib_rasr.label_scorer.onnx_model.session.file = scorer_onnx_checkpoint
...
def rasr_beam_recog(
    config: RasrBeamRecogRoutineConfig, model_config: FFNNTransducerConfig, train_job: ReturnnTrainingJob
) -> ResultItem:
    recog_model_config = FFNNTransducerRecogConfig(
        logmel_cfg=model_config.logmel_cfg,
        conformer_cfg=model_config.conformer_cfg,
...
def recog(config: RecogRoutineConfig, model_config: FFNNTransducerConfig, train_job: ReturnnTrainingJob) -> ResultItem:
    if isinstance(config, CTCRasrGreedyRecogRoutineConfig):
        return ctc_rasr_greedy_recog(config=config, model_config=model_config, train_job=train_job)
    if isinstance(config, CTCFlashlightRecogRoutineConfig):
        return ctc_flashlight_recog(config=config, model_config=model_config, train_job=train_job)
