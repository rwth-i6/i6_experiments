...
from ...model_pipelines.bpe_ctc.pipeline import PipelineConfig, run_pipeline
from ...model_pipelines.bpe_ctc.pytorch_modules import ConformerCTCConfig, SpecaugmentByLengthConfig
from ...model_pipelines.bpe_ctc.subroutines.configs import (
...
def get_baseline_config() -> PipelineConfig:
    ...
    prior_routine_config = PriorRoutineConfig(
        prior_data_config=get_default_prior_data(),
        batch_frames=20_000,
    )

    def get_rasr_recog_routine_config(
        corpus_name: str,
        descriptor: str,
        ...
        return RasrRecogRoutineConfig(
            ...
            epoch=epoch,
            prior_config=prior_routine_config,
    ...
    def get_flashlight_recog_routine_config(
        corpus_name: str,
        descriptor: str,
        epoch: int = train_routine_config.save_epochs[-1],
        lexicon_file: Optional[tk.Path] = None,
        ...
        return FlashlightRecogRoutineConfig(
            descriptor=descriptor,
            corpus=ScorableCorpus(
                corpus_name=corpus_name,
                bliss_corpus_file=get_corpus_object_dict(audio_format="wav", output_prefix="corpora")[
    ...
    for corpus_name in ["dev", "test"]:
        recognitions.append(
            get_rasr_recog_routine_config(descriptor="rasr_greedy", corpus_name=corpus_name, max_beam_size=1)
        )

        recognitions.append(
            get_flashlight_recog_routine_config(descriptor="flashlight_greedy", corpus_name=corpus_name, beam_size=1)
        )

        recognitions.append(
            get_flashlight_recog_routine_config(
                descriptor="flashlight_beam-8_4gram",
                corpus_name=corpus_name,
                beam_size=8,
