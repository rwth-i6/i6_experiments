...
from ...data.tedlium2.bpe import bpe_to_vocab_size, get_bpe_vocab_file
...
from ...data.tedlium2.lexicon import get_bpe_bliss_lexicon
from ...data.tedlium2.lm import get_arpa_lm_config
...
def run_bpe_ctc_baseline(prefix: str = "tedlium2/bpe_ctc") -> List[RecogResult]:
    with ExperimentContext(prefix):
        ...
        tedlium_arpa_lm_config = get_arpa_lm_config(lm_name="4gram", lexicon_file=lexicon_file)
        tedlium_arpa_lm_config.scale = 0.6

        recog_data = {corpus_name: get_default_recog_data(corpus_name) for corpus_name in ["dev", "test"]}
        score_corpora = {corpus_name: get_default_score_corpus(corpus_name) for corpus_name in ["dev", "test"]}
