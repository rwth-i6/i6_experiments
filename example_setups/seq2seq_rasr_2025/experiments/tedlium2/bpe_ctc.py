...
from ...data.tedlium2 import lm as tedlium2_lm
from ...data.tedlium2.bpe import bpe_to_vocab_size, get_bpe_vocab_file, vocab_to_bpe_size
...
def get_train_options() -> TrainOptions:
    return TrainOptions(
        train_data_config=tedlium2_datasets.get_default_bpe_train_data(bpe_size=128),
        cv_data_config=tedlium2_datasets.get_default_bpe_cv_data(bpe_size=128),
...
def run_recognitions_offline_tree_4gram(
    ...
    prior_file = compute_priors(
        prior_data_config=tedlium2_datasets.get_default_prior_data(),
        model_config=model_config,
        checkpoint=checkpoint,
    )

    lexicon_file = get_bpe_bliss_lexicon(bpe_size=vocab_to_bpe_size(model_config.target_size - 1), add_blank=True)
    arpa_lm_config = tedlium2_lm.get_arpa_lm_config(lm_name="4gram", lexicon_file=lexicon_file, scale=lm_scale)
