...
def get_baseline_model_config() -> ConformerCTCMultiOutputConfig:
    ...
    return ConformerCTCMultiOutputConfig(
        ...
        dim=512,
        layer_idx_target_size_list=[(11, PHONEME_SIZE + 1), (11, vocab_size + 1)],
        dropout=0.1,
    )
...
def run_bpe_phoneme_ctc_baseline(prefix: str = "librispeech/bpe_phoneme_ctc") -> List[RecogResult]:
    for phoneme_layer in [5, 8, 11]:
        model_config = get_baseline_model_config()
        model_config.layer_idx_target_size_list[0] = (phoneme_layer, PHONEME_SIZE + 1)

        with ExperimentContext(f"{prefix}_phon-layer-{phoneme_layer}"):
            ...
            for output_idx, name in enumerate(["phoneme", "bpe"]):
                ...
                for trafo_layers in []:
                    ...
                    for recog_corpus in ["dev-clean", "dev-other", "test-clean", "test-other"]:
                        recog_results.append(
                            recog_rasr(
                                descriptor=f"bpe-phoneme-ctc_{name}-output_tree_trafo-{trafo_layers}l",
                                checkpoint=checkpoint,
                                recog_data_config=recog_data[recog_corpus],
                                recog_corpus=score_corpora[recog_corpus],
                                model_serializers=encoder_serializers,
