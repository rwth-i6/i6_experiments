...
class BaseRecogVariant:
    ...
    search_algorithm_params: Union[
        LexiconfreeLabelsyncRecogParams, LexiconfreeTimesyncRecogParams, LoquaciousTreeTimesyncRecogParams
    ]
...
def _run_single_variant(
    ...
    if isinstance(variant.search_algorithm_params, LexiconfreeLabelsyncRecogParams):
        ...
        align_config = None
    elif isinstance(variant.search_algorithm_params, LoquaciousTreeTimesyncRecogParams):
        assert lexicon_file is not None
        if variant.search_algorithm_params.word_lm_params is not None:
            lm_config = loquacious_lm.get_word_lm_config(
...
def run_single_bpe_variant(
    ...
    vocab_file = get_bpe_vocab_file(corpus_key=train_corpus_key, bpe_size=bpe_size, add_blank=use_blank)
    lexicon_file = get_bpe_bliss_lexicon(
        corpus_key=train_corpus_key, bpe_size=bpe_size, add_blank=use_blank, add_sentence_end_pron=use_sentence_end
    )
