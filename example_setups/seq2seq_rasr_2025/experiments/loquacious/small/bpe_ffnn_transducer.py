...
from ....data.loquacious.lexicon import get_bpe_bliss_lexicon
...
def get_train_options(
    bpe_size: int = 128,
    num_epochs: int = 100,
    batch_frames: int = 24_000,
    accum_grad: int = 1,
    speed_perturb: bool = True,
) -> FFNNTransducerTrainOptions:
    train_data_config = loquacious_datasets.get_small_bpe_train_data(bpe_size=bpe_size, speed_perturb=speed_perturb)
    cv_data_config = loquacious_datasets.get_small_bpe_cv_data(bpe_size=bpe_size)
    ...
    for variant in variants:
        ...
        if variant.tree_search:
            lexicon_file = get_bpe_bliss_lexicon(
                bpe_size=bpe_size, corpus_key="train.small", add_blank=True, add_sentence_end_pron=False
            )
            ...
            vocab_file = get_bpe_vocab_file(bpe_size=bpe_size, corpus_key="train.small", add_blank=True)
