__all__ = ["get_combine_label_scorer_config"]
...
def _get_ctc_label_scorer_config(
    ...
    rasr_config.onnx_model.io_map.outputs = "scores"

    return rasr_config


def _get_transducer_label_scorer_config(
    model_config: CombinationModelConfig,
    checkpoint: PtCheckpoint,
    ilm_scale: float = 0.0,
    blank_penalty: float = 0.0,
...
def _get_attention_label_scorer_config(
    model_config: CombinationModelConfig,
    checkpoint: PtCheckpoint,
) -> RasrConfig:
...
def get_combine_label_scorer_config(
    ...
    if ctc_score_scale != 0:
        ...
        ctc_scorer = _get_ctc_label_scorer_config(
    ...
    if transducer_score_scale != 0:
        transducer_scorer = _get_transducer_label_scorer_config(
            model_config=model_config,
            checkpoint=checkpoint,
    ...
    if attention_score_scale != 0:
        attention_scorer = _get_attention_label_scorer_config(
            model_config=model_config,
            checkpoint=checkpoint,
        )
