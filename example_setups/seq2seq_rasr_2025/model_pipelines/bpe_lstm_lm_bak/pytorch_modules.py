...
class LstmLm(torch.nn.Module):
    ...
    def forward(
        ...
        logits = self.final_linear(self.final_dropout(lstm_out))  # [B, S, V]

        return logits


class LstmLmScorerModel(LstmLm):
    def forward(
        self,
        ...
        scores = torch.nn.functional.pad(scores, (0, 1), value=0)  # [B, V+1]
        return scores


class LstmLmStateInitializerModel(LstmLm):
...
class LstmLmStateUpdaterModel(LstmLm):
    def forward(
        self,
