...
from apptek_asr.lib.pytorch.networks.lstm import LstmLmModelV1Config
...
from .i6_model_pytorch_modules_v2 import LstmLmScorer, LstmLmStateInitializer, LstmLmStateUpdater
...
def export_scorer(model_config: LstmLmModelV1Config, checkpoint: PtCheckpoint) -> tk.Path:
    model_serializers = get_model_serializers(model_class=LstmLmScorer, model_config=model_config)

    lstm_hidden_size = model_config.lstm_encoder_cfg.lstm_layers_cfg.hidden_dim
...
def export_state_initializer(model_config: LstmLmModelV1Config, checkpoint: PtCheckpoint) -> tk.Path:
    model_serializers = get_model_serializers(model_class=LstmLmStateInitializer, model_config=model_config)

    lstm_hidden_size = model_config.lstm_encoder_cfg.lstm_layers_cfg.hidden_dim
    lstm_layers = model_config.lstm_encoder_cfg.lstm_layers_cfg.num_layers
    ...
    return _export_model(
        ...
        returnn_config_dict={
            ...
            "model_outputs": {
                "lstm_c": {
                    "dim": lstm_hidden_size,
                    "shape": (lstm_layers, lstm_hidden_size),
                    "time_dim_axis": None,
                    "dtype": "float32",
                },
                "lstm_h": {
...
def export_state_updater(model_config: LstmLmModelV1Config, checkpoint: PtCheckpoint) -> tk.Path:
