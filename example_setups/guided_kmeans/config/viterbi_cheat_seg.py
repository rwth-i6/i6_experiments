"""Chunked guided k-means: Viterbi search, Euclidean model, cheating segmentation, random init."""
...
def run():
    ...
    decode_loop_prob = 0.4       # standard decode
    ...
    latex_report = LatexTableReport(
        ...
        caption=f"Chunked Viterbi k-means, cheating seg, random init: epoch 0 vs epoch {num_epochs - 1}.",
    )
    latex_report_seg = LatexTableReport(
        columns=["lm_scale", "epoch", "per", "del", "ins", "sub"],
        sort_by=["lm_scale"],
        epochs=(0, num_epochs - 1),
        caption=f"Chunked Viterbi k-means, cheating seg decode, random init: epoch 0 vs epoch {num_epochs - 1}.",
    )
    recog_results = []
    recog_results_seg = []

    for lm_scale in lm_scales:
        exp_name = f"lm-{lm_order}-{lm_scale}_loop-{loop_prob}"
        ...
        for decode_lm_scale in decode_lm_scales:
            ...
            for recog_epoch in range(num_epochs + 1):
                ...
                tk.register_output(f"guided_kmeans/{exp_dir}/eval/{decode_name}_epoch-{recog_epoch}_l1_dist", res.l1_dist)
                ...
                tk.register_output(f"guided_kmeans/{exp_dir}/eval/{decode_name}_seg_epoch-{recog_epoch}_l1_dist", res_seg.l1_dist)
