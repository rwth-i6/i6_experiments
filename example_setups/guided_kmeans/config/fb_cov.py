"""Chunked guided k-means: FB search, Gaussian model, standard segmentation, random init."""
...
def run():
    ...
    latex_report = LatexTableReport(
        columns=["lm_scale", "loop_prob", "am_scale", "dec_lm", "epoch", "per", "del", "ins", "sub"],
        sort_by=["lm_scale", "loop_prob", "am_scale", "dec_lm"],
        epochs=(0, num_epochs - 1),
        caption=f"Chunked FB k-means + cov, random init: epoch 0 vs epoch {num_epochs - 1}.",
    )
    recog_results = []

    for lm_scale, loop_prob, distance_scale in product(lm_scales, loop_probs, distance_scales):
        ...
        exp_name = f"seed-{seed}_lm-{lm_order}-{lm_scale}_ts-{_ts}_loop-{loop_prob}_am-{distance_scale}_beam-{_beam}"
        ...
        for decode_lm_scale in decode_lm_scales:
            ...
            for recog_epoch in range(num_epochs + 1):
                ...
                latex_report.add_row(
                    result=res,
                    params={"lm_scale": lm_scale, "loop_prob": loop_prob, "am_scale": distance_scale, "dec_lm": decode_lm_scale},
                    epoch=recog_epoch,
