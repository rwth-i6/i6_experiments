"""Chunked guided k-means: FB search, Gaussian model, cheating segmentation, cheating init."""
...
exp_dir = "fb_cov_cheat_seg_cheat_init"
...
def run():
    ...
    distance_scales = [0.0001, 1.0]  # AM weight applied to emission scores before FB search

    decode_lm_scales = [40.0]
    transition_scale_decode = None
    decode_loop_prob = 0.4
    decode_distance_scale = 1.0

    train_beam_size = 1000
    train_score_threshold = None
    decode_beam_size = None
    ...
    latex_report = LatexTableReport(
        ...
        caption=f"Chunked FB k-means + cov, cheating seg+init: epoch 0 vs epoch {num_epochs - 1}.",
    )
    latex_report_seg = LatexTableReport(
        columns=["lm_scale", "am_scale", "dec_lm", "epoch", "per", "del", "ins", "sub"],
        sort_by=["lm_scale", "am_scale", "dec_lm"],
        epochs=(0, num_epochs - 1),
        caption=f"Chunked FB k-means + cov, cheating seg+init, seg decode: epoch 0 vs epoch {num_epochs - 1}.",
    )
    recog_results = []
    recog_results_seg = []
