"""Chunked guided k-means: FB search, Gaussian model, cheating segmentation, random init."""
...
def run():
    ...
    latex_report = LatexTableReport(
        ...
        caption=f"Chunked FB k-means + cov, cheating seg, random init: epoch 0 vs epoch {num_epochs - 1}.",
    )
    latex_report_seg = LatexTableReport(
        columns=["lm_scale", "am_scale", "dec_lm", "epoch", "per", "del", "ins", "sub"],
        sort_by=["lm_scale", "am_scale", "dec_lm"],
        epochs=(0, num_epochs - 1),
        caption=f"Chunked FB k-means + cov, cheating seg decode, random init: epoch 0 vs epoch {num_epochs - 1}.",
    )
    recog_results = []
    recog_results_seg = []
