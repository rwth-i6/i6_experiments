...
from i6_experiments.example_setups.guided_kmeans.setup.dataset_config import DatasetConfig, RandomNumber
    ...
    for name, initializer_config in initializer_configs.items():
        for lm_scale, loop_probability, silence_loop_probability in parameters:
           ...
           # )
            ...
            res = decode_and_score(decode_config, dataset_config)

            tk.register_output(f"guided_kmeans/recognition/ls-100/{name}/{exp_name}_per", res.score.wer)
