...
# Full upstream RETURNN checkout (real RETURNN, not MiniReturnn). Used by the posterior-HMM
# training so the pipeline runs on the maintained engine. A distinct hash_overwrite is used so
# that switching to it intentionally changes the training hash (new experiment).
RETURNN_ROOT = tk.Path(
    "/u/hwu/setups/librispeech-960/2026-03-11-posterior-hmm/recipe/returnn",
    hash_overwrite="LIBRISPEECH_POSTERIOR_HMM_RETURNN_ROOT",
).copy()
...
RECIPE_ROOT = tk.Path(
    "/u/hwu/setups/librispeech-960/2026-03-11-posterior-hmm/recipe",
