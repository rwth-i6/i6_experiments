...
class SilenceModel:
    ...
    between_word: float = 0.0
    sentence_bounds: float = 0.0
    random_seed: int = 1307

class SilenceGenerator:
    def __init__(self, silence_model: SilenceModel | None):
        self._sm = silence_model or SilenceModel()
        self._rg = random.Random(self._sm.random_seed)
    
    def maybe_add_at_boundary(self, phons: list[str]) -> list[str]:
        if self._rg.random() < self._sm.sentence_bounds:
            return phons + [self._sm.silence_token]
        return phons
    
    def maybe_add_between_word(self, phons: list[str]) -> list[str]:
        if self._rg.random() < self._sm.between_word:
            return phons + [self._sm.silence_token]
        return phons
...
class TextToPhonemeJob(Job):
    ...
    def __init__(
        ...
        silence_model: SilenceModel | None = None
    ):
        super().__init__()
    ...
    def run(self):
        ...
        #     self.silence_model = SilenceModel()
        silence_gen = SilenceGenerator(self.silence_model)
        ...
        with uopen(self.text_file.get_path(), "rt") as inf, uopen(self.out_text.get_path(), "wt") as outf:
            for line in inf:
                ...
                # sentence begin silence
                phons = silence_gen.maybe_add_at_boundary(phons)
                ...
                for i, word in enumerate(words):
                    ...
                    phons.append(phon)
                    if i < len(words) - 1:
                        phons = silence_gen.maybe_add_between_word(phons)
                
                # maybe at silence at the end
                phons = silence_gen.maybe_add_at_boundary(phons)
                if not ok:
                    continue
...
def _train_g2p_model(prefix: str, bliss_lexicon: tk.Path, python_path: tk.Path | None = None) -> tk.Path:
    ...
    # g2p_python = tk.Path("/work/asr3/michel/mann/virtualenv/2025-04-23_tensorflow-2.17_onnx-1.20_v1/bin/python3")
    # g2p_path = tk.Path(
    #     "/work/asr3/michel/mann/virtualenv/2025-04-23_tensorflow-2.17_onnx-1.20_v1/bin"
    # ).join_right("g2p")
    # g2p_python = tk.Path("/work/asr4/hwu/sis_env/bin/python3")
    # g2p_path = tk.Path(
    #     "/work/asr4/hwu/sis_env/bin/g2p.py"
    # )
    g2p_train_job = TrainG2PModelJob(g2p_lexicon=g2p_train_lex) # , g2p_path=g2p_path, g2p_python=g2p_python)
...
def build_phon_lm_training_datasets(
    ...
    if skip_normalization:
        bliss_lexicon = get_bliss_lexicon(use_stress_marker=False, add_silence=False)
        g2p_augmenter = G2PBasedOovAugmenter(original_bliss_lexicon=bliss_lexicon, train_lexicon=bliss_lexicon)
        g2p_augmenter._train_and_set_g2p_model("phon_lm")
        g2p_model = g2p_augmenter.g2p_model_path
    else:
        g2p_model = _train_g2p_model(prefix=prefix, bliss_lexicon=bliss_lexicon, python_path=python_path)
